...
Phase 17 corrects ElevenLabs commercial-entitlement handling. Official sources prove that PAYG can coexist with a Free base plan, so `payg · active` is not paid-subscription evidence. `ELEVENLABS_COMMERCIAL_ENTITLEMENT_V1` replaces the former non-Free predicate with one explicit paid-tier evaluator shared by clean-fixture, Stage 10 and Golden-audio synthesis. PAYG alone, Free, inactive and unknown tiers fail closed. The current clean fixture remains immutable and rights-pending; Audio QA stays blocked until generation-time paid-plan evidence is appended or a new fixture is generated under an explicit paid base plan. The source correction adds zero provider request and zero spend; production deployment evidence is pending.
