CREATE TABLE `v7_youtube_golden_revision_17_receipts` (
  ...
  `revision_key` text NOT NULL CHECK (`revision_key` = 'AUDIENCE_GOLDEN_REVISION_17'),
...
CREATE TRIGGER `v7_youtube_golden_revision_17_no_update` BEFORE UPDATE ON `v7_youtube_golden_revision_17_receipts` BEGIN SELECT RAISE(ABORT, 'YOUTUBE_GOLDEN_REVISION_17_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `v7_youtube_golden_revision_17_no_delete` BEFORE DELETE ON `v7_youtube_golden_revision_17_receipts` BEGIN SELECT RAISE(ABORT, 'YOUTUBE_GOLDEN_REVISION_17_IMMUTABLE'); END;
