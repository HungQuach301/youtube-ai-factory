...
export async function POST(request: Request) {
  try {
    ...
    const providerPlan = { OPENAI: { stages: ["08"], maximumRequests: 2, maximumSpendUsd: 4.5 }, PEXELS: { stages: ["09"], maximumRequests: Math.max(1, maxRequests - 8), maximumSpendUsd: 0 }, PIXABAY: { stages: ["09"], fallbackOnly: true, maximumSpendUsd: 0 }, ELEVENLABS: { stages: ["10"], maximumRequests: 6, maximumSpendUsd: 5 }, contingencyUsd: Math.max(0, requested - 9.5), forbidden: ["LEGACY_ASSET_REUSE", "UNLICENSED_DOWNLOAD", "AUTO_PUBLISH"] };
    const rightsPlan = { SOURCE: { providers: ["PEXELS", "PIXABAY"], commercialUse: "VERIFY_PROVIDER_LICENSE_AND_STORE_SOURCE_URL" }, MAKE: { owner: "CHANNEL", commercialUse: "CHANNEL_OWNED_ORIGINAL" }, HYBRID: { requirement: "BOTH_SOURCE_LICENSE_AND_CHANNEL_OWNED_OVERLAY" }, ELEVENLABS: { requirement: "NON_FREE_COMMERCIAL_PLAN_VERIFIED_BEFORE_SYNTHESIS", voiceIdentity: "ONE_LOCKED_VOICE" }, audit: { storeBytes: true, sha256: true, providerResponseIds: true, licenseUrls: true } };
    ...
    await env.DB!.prepare("INSERT INTO v7_sequential_budget_plans (id,program_id,queue_id,version,lifecycle_state,stage_scope_json,max_spend_usd,max_provider_requests,provider_plan_json,rights_plan_json,actual_spend_usd,actual_provider_requests,approved_by,approval_evidence_hash,created_at,updated_at) VALUES (?,?,?,?,'APPROVED',?,?,?,?,?,0,0,?,?,?,?)")
      .bind(planId, program.id, queue.id, version, json(["08", "09", "10"]), requested, maxRequests, json(providerPlan), json(rightsPlan), user.email.toLowerCase(), approvalEvidenceHash, now(), now()).run();
    return Response.json({ outcome: "APPROVED", planId, version, stageScope: ["08", "09", "10"], maxSpendUsd: requested, maxProviderRequests: maxRequests, spentBeforePlanUsd: spentBeforePlan, totalHardCapUsd: PER_VIDEO_HARD_CAP_USD, providerPlan, rightsPlan, approvedBy: user.email, approvalEvidenceHash }, { status: 201, headers: NO_STORE });
  } catch (error) {
