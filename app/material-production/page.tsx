  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "READY_FOR_SEQUENCE_PROOF" && data.canary.version === "RELEASE_TRAIN_V1_SHOT_CONTRACT_QA" && <div className="reliabilityStart"><p>MP-001 is accepted and MP-002–MP-010 passed shot-contract and technical preflight. Release only MP-002 against the contract-specific rubric; overall must be at least 92 and every dimension at least 90. No retry or later-unit dispatch is allowed.</p><button onClick={() => void canaryAction("RELEASE_RELEASE_TRAIN_SEQUENCE_PROOF")} disabled={Boolean(working) || data.requestLedger.active > 0 || data.canary.currentIndex !== 1}>{working === "RELEASE_RELEASE_TRAIN_SEQUENCE_PROOF" ? "Releasing MP-002 proof…" : "Run MP-002 Sequence Proof · max $1"}</button></div>}
        {data.canary.status === "SEQUENCE_PROOF_PASS_REVIEW" && data.canary.version === "RELEASE_TRAIN_V1_SHOT_CONTRACT_QA" && <div className="reliabilityStart"><p>MP-002 passed the strict shot-contract rubric. Open the bounded MP-003–MP-010 batch; every unit hard-stops at terminal QA, no retry is permitted, and the concurrency ceiling remains 2.</p><button onClick={() => void canaryAction("START_RELEASE_TRAIN_BATCH")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "START_RELEASE_TRAIN_BATCH" ? "Opening bounded batch…" : "Run MP-003–MP-010 bounded batch"}</button></div>}
        {data.canary.status === "BATCH_UNIT_PASS_REVIEW" && data.canary.version === "RELEASE_TRAIN_V1_SHOT_CONTRACT_QA" && <p className="stateBanner">The current batch unit passed and reached a hard-stop. The next frozen unit is released under the same bounded authorization; no repair retry is allowed.</p>}
        {data.canary.status === "SEQUENCE_OR_BATCH_FAILED_PRESERVED" && <p className="stateBanner errorState">The failing unit and exact terminal gates are preserved. No retry or later-unit release occurred.</p>}
