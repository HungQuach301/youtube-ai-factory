...
| X27 | P0 | The clean audio control had no exact-lineage visual master, measured synchronization or independent browser evidence path | `HANDLED__OWNER_REVIEW_OPEN`: Sites v474 seals the exact-lineage master, Factory `LIKELY_CLEAN` 95 and Browser `LIKELY_CLEAN` with 100% playback, decoded audio, cross-cue motion, 4/4 mobile frames, focus/reflow PASS, zero page errors and exact R2 evidence. One failed Browser attempt is preserved without a receipt; attempt 2 consumes the sole receipt and opens only the non-delegable owner task. Dataset, assurance and release authority remain absent | Technical media/WP7 |
| X28 | P0 | Technical decode, contact-sheet and Browser pixel-motion evidence can be mistaken for competitive YouTube audience quality | `ACTIVE_REPAIR__R21_VISUAL_REJECTED`: Sites v504 materializes R21 exact master `3f968794…`. English/en-US audio passes 95, but Factory visual QA fails 67 with P1 unresolved settlement/repeated slide composition and P2 small labels/reused topology/premature ledger values. The local 32-frame source atlas therefore remains non-authoritative. Preserve R21 and design an append-only R22 only from its exact visual FAIL/audio PASS receipts; Browser, owner, freeze and release remain blocked | Video #1/Assurance |
| X29 | P0 | Canonical and compatibility frontends projected different production tables, hiding current materials and presenting stale state as authoritative | `HANDLED__PRODUCTION_ACTIVE_SITES_V502`: Video Engine now reconciles the current Audience Golden snapshot; canonical navigation targets `/video-engine`; Golden materials are directly discoverable; eight V7/V23 routes disclose compatibility-only authority; the mandatory Projection Integrity Gate and pure tests block route/source regression. R20 remains visual FAIL and release-ineligible | Projection integrity |
| X30 | P0 | Revision-specific renderers can repeat slide grammar without a Factory-wide visual operating model | `DESIGN_LOCKED__IMPLEMENTATION_REQUIRED`: `VISUAL_PRODUCTION_OPERATING_MODEL_V1` defines hierarchy, typed visual jobs, SOURCE/MAKE/HYBRID routing, Blueprint/Shot/Scene Graph contracts, anti-slide controls and R22 canary boundary | Visual architecture/R22 |
| X31 | P0 | One judge or static atlas can miss temporal, audio, Browser and final-payoff failures | `DESIGN_LOCKED__CALIBRATION_REQUIRED`: `AI_FIRST_PRODUCTION_ASSURANCE_V1` defines L0-L7, exact-artifact receipts, four outcomes and qualification thresholds; no AI auto-accept authority exists yet | Assurance |
| X32 | P0 | Stage-owned provider calls and storage would fragment secrets, rights, cost, lineage and retry behavior | `DESIGN_LOCKED__IMPLEMENTATION_REQUIRED`: `DATA_PROVIDER_CONTROL_PLANE_V1` centralizes the gateway, bindings, lifecycle, D1/R2/Drive authority and reconciliation | Data/Provider |
| X33 | P1 | Visual, audio, accessibility, localization, rights and runtime floors are split across standards | `DESIGN_LOCKED__IMPLEMENTATION_REQUIRED`: Video Production Quality Standard V3 is active for new production; V2 remains historical and no prior verdict is rewritten | Standard Registry |
| X34 | P1 | Channel visual language is not frozen independently of a renderer revision | `DESIGN_LOCKED__R22_CANARY_REQUIRED`: Hidden Systems Visual DNA V1 defines US/en-US identity, treatment palette, prohibitions, mobile/accessibility and R22 phase-specific assertions | Channel/R22 |
| X35 | P1 | Provider/model/prompt/rubric/sampler drift can leave stale qualification active | `DESIGN_LOCKED__IMPLEMENTATION_REQUIRED`: every binding and judge qualification is versioned and automatically stale on dependency change | Assurance/Provider |
| X36 | P1 | Git truth can diverge between ChatGPT Sites and the user's personal GitHub | `ACTIVE_CONTROL__CONNECTOR_ACCESS_REQUIRED`: owner reports the new repo exists, but the GitHub connection cannot enumerate it; migration blocks until access, fetch and exact local/origin/github SHA proof; divergence cannot be force-resolved automatically | Repository governance |
| X37 | P0 | Reusing `HungQuach301/youtube-ai-factory-v2` would merge an independent project boundary into the active Factory | `EXCLUDED_BY_POLICY`: V2 is not an upstream, mirror, fallback or recovery source; the target is the new private `HungQuach301/youtube-ai-factory` repository | Repository governance |
| X38 | P0 | Technical components could automate decisions without explicit business authority, economics or exception ownership | `DESIGN_LOCKED__IMPLEMENTATION_REQUIRED`: `FACTORY_BUSINESS_OPERATING_MODEL_V1` defines capabilities, decision rights, AI/owner/provider modes, one-root exception routing, accepted-output economics and portfolio arbitration | Business architecture |
| X39 | P0 | UI projections, workers and media timelines could diverge without one runtime/event/timebase architecture | `DESIGN_LOCKED__IMPLEMENTATION_REQUIRED`: `FACTORY_TECHNICAL_RUNTIME_ARCHITECTURE_V1` defines planes, typed events, canonical timebase, worker fencing, observability/replay, recovery and UI truth boundaries | Technical architecture |
| X40 | P0 | Final QA cannot repair unsupported or invalid upstream production inputs | `DESIGN_LOCKED__IMPLEMENTATION_REQUIRED`: `E2E_PRODUCTION_GATE_MODEL_V1` defines DoR, producer self-check, independent gate, owner and handoff from audience need through learning, with release/publication separated | Production gates |
| X41 | P1 | Visual techniques can degrade into generic templates without technique-specific semantic and motion contracts | `DESIGN_LOCKED__QUALIFICATION_REQUIRED`: `VISUAL_MOTION_TECHNIQUE_PLAYBOOK_V1` covers footage, diagrams, flows, maps, timelines, ledgers, transformations, charts, topology, transitions, mobile, accessibility and localization | Visual capability |
| X42 | P1 | Single-video or cross-channel outcomes can be promoted as causal learning without sufficient evidence | `DESIGN_LOCKED__IMPLEMENTATION_CALIBRATION_REQUIRED`: `MULTI_CHANNEL_SCALE_AND_LEARNING_V1` defines isolation, experiment validity, minimum evidence, champion/challenger, scale, revoke and rollback | Learning/Portfolio |
| X43 | P0 | Privacy, Content ID, retention, recovery, SLA and incident controls can fall between stage owners | `DESIGN_LOCKED__ENFORCEMENT_EXERCISES_REQUIRED`: `CROSS_CUTTING_CONTROL_STANDARD_V1` centralizes immutable versioned requirements and fail-closed dispositions | Governance/Operations |
