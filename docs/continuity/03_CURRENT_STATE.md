...
PHASE_45 = IN_PROGRESS__ASSURANCE_CALIBRATION_AND_QA_COCKPIT_SOURCE_QUALIFIED__LIVE_SCHEMA_AND_EXACT_DEPENDENCY_EVIDENCE_PENDING
PHASE_46 = GATED__ASSURANCE_CALIBRATION_AND_EXACT_DEPENDENCY_ADMISSION_REQUIRED__R22_BLOCKED
SOURCE_MIGRATION = 0115_FACTORY_ASSURANCE_CALIBRATION_AND_QA_COCKPIT__SOURCE_QUALIFIED
SOURCE_MIGRATION_LIVE_D1 = VERIFIED__0114_SEVEN_EVIDENCE_ASSURANCE_TABLES__ZERO_ROWS
...
CURRENT_SITE_VERSION = 538__FINAL_ASSURANCE_DOCUMENTATION_CHECKPOINT__ENV_REVISION_50__TEMPORARY_FLAGS_ABSENT
...
```text
1. Deploy migration `0115`, read back its four zero-row calibration tables and verify `/qa-cockpit` truthfully reports `CALIBRATION_REQUIRED` with no demo fallback or PASS authority.
2. Materialize and calibrate exact L0-L7 judge/sampler dependencies in `AI_SHADOW` from independent controls and holdouts; keep every result advisory until exact qualification registration passes.
