...
Batch 2 V10 audit evidence established a metadata-to-renderer disconnect: archetype, claim and provenance metadata passed deterministic checks, but the renderer continued to select hard-coded family layouts that did not make the contract entity, action, constraint and temporal delta visually operative. V11 introduces a contract semantic projection for each ENTRY/MIDPOINT/EXIT state. Each projection contains a focal entity, supporting entities, action, constraint, source evidence clause and delta from the prior state; the renderer consumes those fields directly. Activation is blocked until 50/50 projections, 50/50 grammar signatures and 150/150 frame hashes pass at zero provider spend. V8/V9/V10 products and audits remain immutable evidence; V11 reproduces all 50 products under a new activation and receives a new independent audit.
...
- **ADR-104 — Rejected engines retain a balanced evolution record.** V13–V18 remain immutable evidence with three distinct fields: capabilities proved by stored products/audits, the systemic reason the engine was retired, and the capabilities inherited by its successor. Audit rejection never erases demonstrated progress, and a successor may inherit a capability only when its production contract and regression gate preserve it. V19 therefore inherits contract-native scene lineage, domain primitives, evidence-bound motion and the pixel-failure taxonomy, while replacing logical-ID branches, sample-specific rendering, shared diagram shells and self-attested layout proof with semantic topology classification, constraint-solved geometry, a primitive-only renderer and independently recomputed mobile/layout measurements.
