...
async function snapshot() {
  ...
  const engine = clean(productionBatch?.engine_version), v12ControlVisible = engine === WAVE_BATCH_2_V11_ENGINE_VERSION, v11ControlVisible = engine === WAVE_BATCH_2_V10_ENGINE_VERSION;
  const expectedPreflightAction = v12ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V12_ACTIVATION" : v11ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" : "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION";
  const expectedActivationAction = v12ControlVisible ? "ADOPT_WAVE_BATCH_2_V12_ENGINE_ROOT_CORRECTION" : v11ControlVisible ? "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" : "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION";
...
async function newRequest(db: DB, authorization: Row, briefId: string, phase: string, provider: string, modelId = "none", reasoning = "none", expected = 0, maximum = 0, stableRequestId?: string) {
  ...
  if (phase === "WAVE_BATCH_2_PRODUCT_AUDIT" && briefId === "WAVE-09-BATCH-2") {
    ...
    const rejectedEngineForCurrent = clean(batch?.engine_version) === WAVE_BATCH_2_V12_ENGINE_VERSION
    ...
    batchAuditAuthorized = Boolean(batch)
      ...
      && (v8InitialAudit || v9QualifiedAudit || v10QualifiedAudit || v11QualifiedAudit || v12QualifiedAudit);
    if (!batchAuditAuthorized) throw new Error("BATCH_2_PRODUCT_AUDIT_FIREWALL · qualified V8 initial audit or lineage-bound V9/V10/V11/V12 durable audit intent is required");
  }
...
function waveProductionManifest(contract: Row, engineVersion = WAVE_PRODUCTION_ENGINE_VERSION, batchVersion = WAVE_BATCH_1_VERSION) {
  const structuredOntology = engineVersion === WAVE_BATCH_2_V12_ENGINE_VERSION;
  ...
  const visualOntology = structuredOntology ? structuredVisualOntology(contract, contractBindings, semanticActors, kind) : undefined;
  return {
    ...manifest,
    states,
...
function waveProductOracle(manifest: Row) {
  ...
  const structuredOntology=clean(semantic.version)==="STRUCTURED_VISUAL_ONTOLOGY_V12",semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11"||structuredOntology,archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),ontology=rec(semantic.visualOntology),ontologyStates=arr(ontology.states).map(rec),layoutProof=rec(ontology.layoutProof),motionPolicy=rec(ontology.motionPolicy),corpus=clean(semantic.corpusHashInput).toUpperCase(),generic=["$100 PURCHASE","REWARD ENTRY"],genericEscape=contractBound&&generic.some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy);
  const checks=[
    {id:"SEMANTIC_SCENE_GRAPH",status:["CONTRACT_SIGNATURE_SCENE_GRAPH_V5_SPATIAL_BOUND","CONTRACT_BOUND_SCENE_GRAPH_V9","ARCHETYPE_SEMANTIC_MANIFEST_V10","CONTRACT_SEMANTIC_PROJECTION_V11","STRUCTURED_VISUAL_ONTOLOGY_V12"].includes(clean(semantic.version))&&required.length>=3?"PASS":"FAIL",evidence:`${clean(semantic.kind)} · ${required.join(" · ")}`},
  ...
  return{version:structuredOntology?"WAVE_STRUCTURED_VISUAL_ONTOLOGY_PRODUCT_ORACLE_V12":semanticProjection?"WAVE_CONTRACT_SEMANTIC_PRODUCT_ORACLE_V11":archetypeBound?"WAVE_ARCHETYPE_SEMANTIC_PRODUCT_ORACLE_V10":contractBound?"WAVE_CONTRACT_BOUND_PRODUCT_ORACLE_V9":"WAVE_SPATIAL_RELATION_PRODUCT_ORACLE_V7",logicalId:clean(manifest.logicalId),frames,checks,passed:checks.every((item)=>item.status==="PASS")};
}

function waveManifestQualification(manifest: Row) {
  const states=arr(manifest.states).map(rec),semantic=rec(manifest.semanticModel),required=arr(semantic.requiredElements).map(clean).filter(Boolean),structuredOntology=clean(semantic.version)==="STRUCTURED_VISUAL_ONTOLOGY_V12",semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11"||structuredOntology,archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),ontology=rec(semantic.visualOntology),ontologyStates=arr(ontology.states).map(rec),layoutProof=rec(ontology.layoutProof),motionPolicy=rec(ontology.motionPolicy),corpus=clean(semantic.corpusHashInput).toUpperCase(),genericEscape=contractBound&&["$100 PURCHASE","REWARD ENTRY"].some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy),checks=[
    {id:"SEMANTIC_SCENE_GRAPH",status:["CONTRACT_SIGNATURE_SCENE_GRAPH_V5_SPATIAL_BOUND","CONTRACT_BOUND_SCENE_GRAPH_V9","ARCHETYPE_SEMANTIC_MANIFEST_V10","CONTRACT_SEMANTIC_PROJECTION_V11","STRUCTURED_VISUAL_ONTOLOGY_V12"].includes(clean(semantic.version))&&required.length>=3?"PASS":"FAIL"},
...
async function produceNextWaveBatch2Shot() {
  ...
  if (![WAVE_BATCH_2_ENGINE_VERSION, WAVE_BATCH_2_REPLACEMENT_ENGINE_VERSION, WAVE_BATCH_2_V10_ENGINE_VERSION, WAVE_BATCH_2_V11_ENGINE_VERSION, WAVE_BATCH_2_V12_ENGINE_VERSION].includes(engineVersion)) throw new Error(`BATCH_2_ENGINE_NOT_QUALIFIED · ${engineVersion}`);
  const scope = arr(JSON.parse(String(batch.scope_json || "[]"))).map(rec), index = Number(batch.current_index || 0), target = scope[index];
  ...
  const specification = { version: engineVersion === WAVE_BATCH_2_V12_ENGINE_VERSION ? "SHOT_PRODUCT_SPECIFICATION_V6_STRUCTURED_VISUAL_ONTOLOGY" : engineVersion === WAVE_BATCH_2_V11_ENGINE_VERSION ? "SHOT_PRODUCT_SPECIFICATION_V5_CONTRACT_SEMANTIC_PROJECTION" : engineVersion === WAVE_BATCH_2_V10_ENGINE_VERSION ? "SHOT_PRODUCT_SPECIFICATION_V4_ARCHETYPE_SEMANTIC" : engineVersion === WAVE_BATCH_2_REPLACEMENT_ENGINE_VERSION ? "SHOT_PRODUCT_SPECIFICATION_V3_CONTRACT_BOUND" : "SHOT_PRODUCT_SPECIFICATION_V2_CONTROLLED_SCALE", batchVersion: clean(batch.version), engineVersion, contract, manifest }, specificationJson = JSON.stringify(specification), specificationHash = await sha(specificationJson), frameIds: string[] = [], frameHashes: string[] = [];
  ...
  for (const [role, frame] of [["CERT_ENTRY", oracle.frames[0]], ["CERT_MIDPOINT", oracle.frames[1]], ["CERT_EXIT", oracle.frames[2]]] as const) {
    const replacement = engineVersion === WAVE_BATCH_2_REPLACEMENT_ENGINE_VERSION, v10 = engineVersion === WAVE_BATCH_2_V10_ENGINE_VERSION, v11 = engineVersion === WAVE_BATCH_2_V11_ENGINE_VERSION, v12 = engineVersion === WAVE_BATCH_2_V12_ENGINE_VERSION;
    const fileId = await storeMaterial(env, db, authorization, briefRow, { role, identity: `BATCH2-${clean(contract.briefId)}-${role}-${v12 ? "E12" : v11 ? "E11" : v10 ? "E10" : replacement ? "E9" : "E8"}`, bytes: frame.bytes, mimeType: "image/png", extension: "png", sourceType: engineVersion, provider: "FRAMEFLOW_OWNED", providerAssetId: specificationHash, sourceUrl: specificationHash, landingUrl: specificationHash, licenseCode: "CHANNEL_OWNED", width: frame.width, height: frame.height, runtimeScope: v12 ? "wave-09-batch-2-engine-v12" : v11 ? "wave-09-batch-2-engine-v11" : v10 ? "wave-09-batch-2-engine-v10" : replacement ? "wave-09-batch-2-engine-v9" : "wave-09-batch-2-engine-v8", archiveFolder: v12 ? "Wave 09 Batch 2 Engine V12" : v11 ? "Wave 09 Batch 2 Engine V11" : v10 ? "Wave 09 Batch 2 Engine V10" : replacement ? "Wave 09 Batch 2 Engine V9" : "Wave 09 Batch 2 Engine V8" });
    const stored = await db.prepare("SELECT content_hash,status FROM v7_material_files WHERE id=?").bind(fileId).first<Row>();
  ...
  await db.batch([
    ...
    db.prepare("UPDATE v7_production_batches SET status=?,completed_units=?,current_index=?,updated_at=?,completed_at=? WHERE id=?").bind(isFinal ? "PRODUCT_COMPLETE" : "PRODUCING", nextCompleted, nextCompleted, now, isFinal ? now : null, batch.id),
    db.prepare("UPDATE v7_material_runs SET status=? WHERE id=?").bind(isFinal ? "BATCH_2_PRODUCT_COMPLETE" : engineVersion === WAVE_BATCH_2_V12_ENGINE_VERSION ? "BATCH_2_V12_REPRODUCING" : engineVersion === WAVE_BATCH_2_V11_ENGINE_VERSION ? "BATCH_2_V11_REPRODUCING" : engineVersion === WAVE_BATCH_2_V10_ENGINE_VERSION ? "BATCH_2_V10_REPRODUCING" : engineVersion === WAVE_BATCH_2_REPLACEMENT_ENGINE_VERSION ? "BATCH_2_V9_REPRODUCING" : "BATCH_2_PRODUCING", run.id),
