...
const WAVE_BATCH_2_V19_PROCESS_REVISION = "V19_2_OBSTACLE_AVOIDING_CONNECTOR_SOLVER";
...
function systemicLayoutSlots(family: string, count: number) {
  const layouts: Record<string, Array<{ x: number; y: number; w: number; h: number }>> = {
    ...
    TABLE: [{ x: 62, y: 108, w: 260, h: 112 }, { x: 350, y: 108, w: 548, h: 112 }, { x: 62, y: 244, w: 260, h: 112 }, { x: 350, y: 244, w: 260, h: 112 }, { x: 638, y: 244, w: 260, h: 112 }, { x: 350, y: 372, w: 260, h: 60 }],
  };
...
function systemicLayoutMeasurement(spec: Row) {
  ...
  return { clippingCount, textOcclusionCount, connectorTextIntersectionCount, unsolvedConnectorCount, objectOverlapCount, mobileLegibilityFailures, motionPathObjectIntersectionCount, motionPathSafeAreaFailures, minimumGlyphScale: 2, allGeometryInsideSafeArea: clippingCount === 0 && motionPathSafeAreaFailures === 0, connectorCorridorsReserved: connectorTextIntersectionCount === 0 && unsolvedConnectorCount === 0, motionRailReserved: motionPathObjectIntersectionCount === 0, measuredFromSolvedGeometry: true };
}
...
function waveProductOracle(manifest: Row) {
  ...
  const checks=[
    ...
    {id:"SCENE_SPEC_LAYOUT_SAFETY",status:!contractNativeScene||Number(sceneLayoutProof.clippingCount)===0&&Number(sceneLayoutProof.textOcclusionCount)===0&&Number(sceneLayoutProof.connectorTextIntersectionCount)===0&&Number(sceneLayoutProof.unsolvedConnectorCount)===0&&Number(sceneLayoutProof.objectOverlapCount)===0&&Number(sceneLayoutProof.mobileLegibilityFailures)===0&&Number(sceneLayoutProof.motionPathObjectIntersectionCount)===0&&Number(sceneLayoutProof.motionPathSafeAreaFailures)===0&&sceneLayoutProof.allGeometryInsideSafeArea===true&&sceneLayoutProof.connectorCorridorsReserved===true&&sceneLayoutProof.motionRailReserved===true&&sceneLayoutProof.measuredFromSolvedGeometry===true?"PASS":"FAIL",evidence:"layout proof is recomputed from solved geometry, including full connector segments, mobile legibility and motion-path exclusion"},
...
async function qualifyWaveBatch2V13Activation(context: Awaited<ReturnType<typeof waveBatch2V13ActivationContext>>, progress: Row = {}) {
  ...
  for (const target of scope.slice(startIndex, endIndex)) {
    ...
    const measured = systemicLayoutMeasurement(spec), layoutSafe = Number(measured.clippingCount) === 0 && Number(measured.textOcclusionCount) === 0 && Number(measured.connectorTextIntersectionCount) === 0 && Number(measured.unsolvedConnectorCount) === 0 && Number(measured.objectOverlapCount) === 0 && Number(measured.mobileLegibilityFailures) === 0 && Number(measured.motionPathObjectIntersectionCount) === 0 && Number(measured.motionPathSafeAreaFailures) === 0 && measured.allGeometryInsideSafeArea === true && measured.connectorCorridorsReserved === true && measured.motionRailReserved === true && measured.measuredFromSolvedGeometry === true && JSON.stringify(measured) === JSON.stringify(layoutProof);
...
async function preflightWaveBatch2V13Activation() {
  const context = await waveBatch2V13ActivationContext(), now = new Date().toISOString(), preflightId = `${context.replacementId}-ZERO-SPEND-PREFLIGHT-V19-2`;
  const existing = await context.db.prepare("SELECT * FROM v7_batch_activation_preflights WHERE id=?").bind(preflightId).first<Row>();
  if (existing && clean(existing.input_hash) === context.inputHash && clean(existing.status) === "PASS") return snapshot();
  const prior = existing && clean(existing.input_hash) === context.inputHash && clean(existing.status) === "RUNNING" ? rec(JSON.parse(String(existing.result_json || "{}"))) : {};
...
async function adoptWaveBatch2V13EngineRootCorrection() {
  const context = await waveBatch2V13ActivationContext(), activationId = `${context.replacementId}-ACTIVATION-V1`, preflightId = `${context.replacementId}-ZERO-SPEND-PREFLIGHT-V19-2`;
  const existingActivation = await context.db.prepare("SELECT * FROM v7_batch_activations WHERE id=?").bind(activationId).first<Row>();
  if (existingActivation) {
