...
function findingText(finding: Finding) {
  ...
  return [finding.severity, finding.category, finding.summary].filter(Boolean).join(" · ") || "QA finding";
}
...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    if ((!data?.sequenceProof || !["RENDER_QUEUED", "RENDERING", "QA_RUNNING"].includes(data.sequenceProof.status)) && data?.sequenceProduct?.status !== "PRODUCING" || working) return;
    const timer = window.setTimeout(() => void (data.sequenceProof?.status === "QA_RUNNING" ? sequenceAction("RUN_SEQUENCE_QA", true) : load()), 2500);
    return () => window.clearTimeout(timer);
  ...
  async function sequenceAction(action: "PLAN_SEQUENCE_PROOF" | "RUN_SEQUENCE_QA" | "PRODUCE_INTEGRATED_SEQUENCE", quiet = false) {
    setWorking(action); if (!quiet) setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    <section className="sequenceProofPlane">
      ...
      {data.sequenceProduct && <section className="sequenceProduct">
        ...
        <div className={`productVerdict ${data.sequenceProduct.status === "PRODUCT_COMPLETE" ? "complete" : data.sequenceProduct.status === "PRODUCTION_BLOCKED" ? "blocked" : "running"}`}><strong>{data.sequenceProduct.status === "PRODUCT_COMPLETE" ? "Production Definition of Done passed" : data.sequenceProduct.status === "PRODUCTION_BLOCKED" ? "Composer stopped without declaring completion" : "Integrated production is running"}</strong><span>{data.sequenceProduct.status === "PRODUCT_COMPLETE" ? "Source lineage, continuity graph, mobile-safe contain, full-frame scan and technical master read-back are complete. Independent audit 105 has not been started." : "QA remains outside the production loop. Scale stays locked until the product is complete."}</span></div>
