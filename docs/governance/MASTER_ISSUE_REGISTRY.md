...
| X18 | P0 | Evaluation architecture makes the owner manually classify the entire primary corpus before the Factory has performed first-pass QA | `SOURCE_IMPLEMENTED__PRODUCTION_ACCEPTANCE_PENDING`: `FACTORY_FIRST_QA_POLICY_V1` uses two exact-byte owner anchors for fail-closed calibration, then runs bounded independent review; owner receipts and owner identity remain separate, temporal media stays Browser-required and no result can promote fixture/dataset/assurance/release state | Evaluation |
