...
export default function MaterialProductionPage() {
  ...
  async function canaryAction(action: "AUTHORIZE_CONTROLLED_CANARY" | "AUTHORIZE_CONTROLLED_CANARY_V3" | "AUTHORIZE_CONTROLLED_CANARY_V4" | "AUTHORIZE_CONTROLLED_CANARY_V5" | "RELEASE_CONTROLLED_CANARY_V5_UNIT" | "RELEASE_PRODUCTION_RECOVERY_PROBE" | "BUILD_RECOVERY_CONTRACT_ALIGNMENT" | "RELEASE_CONTRACT_ALIGNED_RECOVERY_PROBE" | "RECONCILE_CONTRACT_ALIGNED_RECOVERY_TERMINAL" | "BUILD_RELEASE_TRAIN_PREFLIGHT" | "BUILD_STABILIZATION_RELEASE" | "RELEASE_RELEASE_TRAIN_SEQUENCE_PROOF" | "START_RELEASE_TRAIN_BATCH" | "RELEASE_NEXT_RELEASE_TRAIN_BATCH_UNIT" | "START_CONTROLLED_CANARY_UNIT" | "RELEASE_NEXT_CONTROLLED_CANARY_UNIT") {
    setWorking(action); setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "SEQUENCE_PROOF_PASS_REVIEW" && data.canary.version === "STABILIZATION_RELEASE_V1_TYPED_STATE_MACHINE" && <div className="reliabilityStart"><p>MP-002 passed the stabilized shot-contract rubric. Open the remaining eight canonical pilot units on the locked renderer; every unit hard-stops at terminal QA, no retry is permitted, and the concurrency ceiling remains 2.</p><button onClick={() => void canaryAction("START_RELEASE_TRAIN_BATCH")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "START_RELEASE_TRAIN_BATCH" ? "Opening bounded batch…" : "Run remaining canonical pilot batch"}</button></div>}
