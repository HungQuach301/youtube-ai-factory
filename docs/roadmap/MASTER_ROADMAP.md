...
Phase 23 source is ready. Migration `0080` and `CONTROLLED_DEFECT_DERIVATION_V1` bind the eligible clean-audio control to `cfp-v1-02` and permit exactly one deterministic `RIGHTS_LINEAGE_MISSING` P0 derivative. The action re-reads the exact clean parent, emits canonical clean/mutated manifests, removes only `rightsReceiptId` from the mutated payload and verifies both manifest hashes through R2. A dedicated credential is hard-scoped to this action. It uses zero provider requests/spend and creates only `CONTROLLED_FIXTURE_GROUND_TRUTH_ONLY` authority. On success WP7 projects one controlled injection and 1/5 P0 coverage, but three owner-confirmed references and four uncovered P0 families keep readiness `INSUFFICIENT_GROUND_TRUTH`. Next deploy and execute the sole derivation; subsequent fixtures remain separately gated.
