...
export function NichePortfolioView() {
  ...
  return <FactoryShell active="niches">
    ...
    {data && <>
      ...
      <section className="npAuthority"><div><small>{data.contract} · NICHE-LEVEL IDENTITY ONLY</small><strong>{data.decisionState.replaceAll("_", " ")}</strong><p>Every comparison row is a typed niche opportunity. Expert assumptions remain separate from evidence; legacy video topics are preserved in Channel Studio and cannot enter this ranking.</p></div><div><StatusPill tone="good">ZERO SPEND</StatusPill><span>Only hypothesis intake is routed</span></div></section>
      <section className="npSummary" aria-label="Portfolio summary"><article><small>NICHE OPPORTUNITIES</small><strong>{data.summary.opportunities}</strong></article><article><small>COMPARABLE</small><strong>{data.summary.comparable}</strong></article><article><small>ELIGIBLE</small><strong>{data.summary.eligible}</strong></article><article><small>PREREQUISITE BLOCKED</small><strong>{data.summary.blockedByPrerequisite}</strong></article><article><small>RESEARCH REQUIRED</small><strong>{data.summary.researchRequired}</strong></article><article><small>EXPERT SEEDED</small><strong>{data.summary.expertSeeded}</strong></article><article><small>VIDEO TOPICS EXCLUDED</small><strong>{data.summary.excludedLegacyContentTopics}</strong></article></section>
