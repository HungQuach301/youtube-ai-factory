...
|---|---:|---|---|---|
| A1 | P0 | `FROZEN` conflates immutable bytes with downstream eligibility | `PARTIAL`: UI projection separates control and quality, stored lifecycle still requires independent fields | FP3.1 |
| A2 | P0 | M0 Safety Scope is not evaluated on financial content | `CONFIRMED_MANDATORY`: every owned M0/M1 gate must PASS; `NOT_EVALUATED` is ineligible | FP3.1 |
| A3 | P0 | No automatic supersede when capability/settings version changes | `CONFIRMED_MANDATORY` | FP3.1 |
| A4 | P1 | `NOT_EVALUATED` is presented with `FAIL` | `CONFIRMED_MANDATORY`: separate state and reporting semantics | FP3.1 |
| A5 | P1 | Stage 11 control can appear READY while upstream repair is required | `PARTIAL`: evidence precedence blocks operation; persist fail-closed control state | FP3.1 |
...
| B6 | P1 | No sealed performance prediction for Stage 16 comparison | `CONFIRMED_MANDATORY`: compose prediction in Stages 04/05/08 and seal at 11 | Contract Pack |
...
| ID | Priority | Finding | Reconciled disposition | Owning wave |
|---|---:|---|---|---|
| C1 | P0 | VP9+Opus distribution render is treated as master | `CONFIRMED_MANDATORY`: archival/mezzanine plus derived distribution render | Contract Pack/Technical media |
...
| D3 | P1 | Model aliases and updates can halt or silently change behavior | `CONFIRMED_MANDATORY`: use provider-supported immutable IDs where available and shadow qualification | FP3.1 |
...
| D7 | P1 | Failed provider requests are not classified | `CONFIRMED_MANDATORY`: taxonomy, trace and reconciliation | FP3.1 |
...
| F5 | P1 | Personalized financial advice is checked too late | `CONFIRMED_MANDATORY`: high-precision deterministic lint plus semantic/human safety path | FP3.1/Upstream quality |
...
|---|---:|---|---|---|
| X1 | P0 | Canonical hashing is not unified across material lineage paths | `CONFIRMED_MANDATORY`; JCS and input normalization must be defined separately | FP3.1 |
| X2 | P0 | Expiring leases do not prevent stale writers | `CONFIRMED_MANDATORY`: fencing, heartbeat and orphan reconciliation | FP3.1 |
| X3 | P0 | Spend is recorded after provider completion without atomic reservation | `CONFIRMED_MANDATORY` | FP3.1 |
| X4 | P1 | End-to-end trace/span and provider-failure evidence are incomplete | `CONFIRMED_MANDATORY` with redaction/encryption/retention | FP3.1 |
...
| X8 | P1 | Voice/visual/music identity is decided at video rather than channel level | `CONFIRMED_MANDATORY` before FP4/FP5 | Contract Pack |
...
| X10 | P1 | Platform compliance is not a separate channel-level risk plane | `CONFIRMED_MANDATORY` before first publish | Contract Pack/Video #1 |
