...
export async function GET(request: Request) {
  try {
    ...
    if (["owner-label-task", "owner-label-workflow", "provider-history-recovery", "commercial-clean-audio"].includes(view) || artifactCandidateId || fixtureArtifactId) {
      const { env } = await authorized(request, false);
      if (view === "commercial-clean-audio") {
        return Response.json(await commercialCleanAudioSnapshot(env.DB), { headers: NO_STORE });
      }
      if (view === "provider-history-recovery") {
        ...
        const [materialization, rightsEvidence, commercialAudio] = await Promise.all([controlledFixtureMaterializationSnapshot(env.DB), currentRightsEvidenceSnapshot(env.DB), commercialCleanAudioSnapshot(env.DB)]);
        ...
        const commercialAudioPanel = commercialAudio.policy ? `<section class="result"><small>PAID-PLAN REPLACEMENT · APPEND-ONLY</small><h2>5. Replacement clean audio</h2>${commercialArtifact ? `<dl><div><dt>Entitlement</dt><dd>${escapeHtml(commercialArtifact.subscriptionTier)} · ${escapeHtml(commercialArtifact.subscriptionStatus)}</dd></div><div><dt>Subscription R2</dt><dd>${commercialArtifact.subscriptionR2ReadbackVerified ? "PASS" : "FAIL"}</dd></div><div><dt>Provider request ID</dt><dd>${commercialArtifact.providerNativeRequestIdCaptured ? "CAPTURED" : "MISSING"}</dd></div><div><dt>Rights</dt><dd>${escapeHtml(commercialArtifact.rightsState)}</dd></div><div><dt>Factory Audio QA</dt><dd>${factoryAudioQa ? escapeHtml(factoryAudioQa.decisionState) : "PENDING"}</dd></div><div><dt>Owner ground truth</dt><dd>${escapeHtml(commercialArtifact.ownerGroundTruthState)}</dd></div></dl><audio controls preload="metadata" src="/api/factory/sequential-production/evaluation?fixtureArtifact=${encodeURIComponent(commercialArtifact.id)}"></audio>${factoryAudioQa ? `<p><strong>Factory score:</strong> ${factoryAudioQa.overallScore}/100 · P0 ${factoryAudioQa.p0Count} · P1 ${factoryAudioQa.p1Count} · ${escapeHtml(factoryAudioQa.ownerAttentionState)}</p><p>${escapeHtml(factoryAudioQa.rationale)}</p>` : `<p>Rights đã PASS. Factory sẽ nghe exact audio một lần bằng ${escapeHtml(commercialAudio.qaPolicy?.modelId)} với trần $${number(commercialAudio.qaPolicy?.reservedSpendCeilingUsd).toFixed(2)}.</p><button id="runFactoryAudioQa" type="button">Chạy Factory Audio QA</button><p id="factoryAudioQaProgress" class="progress" aria-live="polite"></p>`}` : `<p>Factory sẽ đọc đúng một subscription snapshot. Chỉ gói Starter/Creator/Pro/Scale/Business/Enterprise đang active mới cho phép đúng một TTS replacement; PAYG đơn lẻ vẫn bị chặn. Trần TTS dự phòng $${commercialAudio.policy.reservedSpendCeilingUsd.toFixed(2)}.</p><button id="regenerateCommercialAudio" type="button">Xác minh gói và tạo replacement</button><p id="commercialAudioProgress" class="progress" aria-live="polite"></p>`}</section>` : "";
        ...
        const commercialAudioScript = !commercialArtifact ? `<script>(()=>{const button=document.getElementById('regenerateCommercialAudio'),progress=document.getElementById('commercialAudioProgress');if(!button)return;button.addEventListener('click',async()=>{button.disabled=true;progress.textContent='Đang xác minh paid base plan, niêm phong subscription snapshot và tạo đúng một replacement…';try{const response=await fetch('/api/factory/sequential-production/evaluation',{method:'POST',headers:{'content-type':'application/json','idempotency-key':'commercial-clean-audio-regeneration-v1'},body:JSON.stringify({action:'REGENERATE_COMMERCIAL_CLEAN_AUDIO_CONTROL'})});const payload=await response.json();if(!response.ok)throw new Error(payload?.error?.message||'Không thể tạo replacement');location.assign('/api/factory/sequential-production/evaluation?view=provider-history-recovery&commercialized=1')}catch(error){progress.textContent=error instanceof Error?error.message:'Không thể tạo replacement';button.disabled=false}})})();</script>` : "";
        const factoryAudioQaScript = commercialArtifact && !factoryAudioQa ? `<script>(()=>{const button=document.getElementById('runFactoryAudioQa'),progress=document.getElementById('factoryAudioQaProgress');if(!button)return;button.addEventListener('click',async()=>{button.disabled=true;progress.textContent='Factory đang nghe exact audio từ đầu đến cuối và ghi independent receipt…';try{const response=await fetch('/api/factory/sequential-production/evaluation',{method:'POST',headers:{'content-type':'application/json','idempotency-key':'factory-clean-audio-qa-v1'},body:JSON.stringify({action:'RUN_FACTORY_CLEAN_AUDIO_QA'})});const payload=await response.json();if(!response.ok)throw new Error(payload?.error?.message||'Factory Audio QA chưa hoàn tất');location.assign('/api/factory/sequential-production/evaluation?view=provider-history-recovery&factoryAudioQa=1')}catch(error){progress.textContent=error instanceof Error?error.message:'Factory Audio QA chưa hoàn tất';button.disabled=false}})})();</script>` : "";
        return new Response(`<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Khôi phục evidence provider</title><style>*{box-sizing:border-box}body{margin:0;padding:24px;background:#101f19;color:#d9e8e1;font:15px Arial,sans-serif}main{max-width:900px;margin:4vh auto;padding:28px;border:1px solid #365349;border-radius:18px;background:#13271f}small{color:#a8e8ca;font-weight:800}h1,h2{font-family:Georgia,serif}p,li,dt{color:#a9bdb5;line-height:1.6}.scope,.result{margin:22px 0;padding:18px;border-radius:14px;background:#0b1712}.scope strong{color:#d9f7e9}.conclusion{border:1px solid #9c6a3e;background:#241d12}.conclusion h3{margin-top:22px;color:#f0d5b2}button{min-height:48px;padding:0 18px;border:0;border-radius:10px;background:#a8e8ca;color:#09291e;font-weight:800;cursor:pointer}button:disabled{opacity:.55}audio{display:block;width:100%;margin-top:16px}dl{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}dl div{padding:12px;border:1px solid #294238;border-radius:10px;background:#102019}dt{font-size:11px}dd{margin:6px 0 0;font-size:20px;font-weight:800;overflow-wrap:anywhere}.notice{padding:12px;border-radius:10px;background:#173c2c;color:#bcefd5}.progress{min-height:24px;color:#a8e8ca}@media(max-width:700px){body{padding:12px}main{padding:18px}dl{grid-template-columns:1fr 1fr}}</style></head><body><main><small>OWNER-AUTHENTICATED · FAIL-CLOSED</small><h1>Khôi phục evidence lịch sử ElevenLabs</h1>${recorded || audioHashed || url.searchParams.get("materialized") === "1" || url.searchParams.get("rightsCaptured") === "1" || url.searchParams.get("commercialized") === "1" || url.searchParams.get("factoryAudioQa") === "1" ? '<p class="notice">Đã hoàn tất lệnh và đọc lại snapshot bất biến.</p>' : ''}<section class="scope"><strong>Phạm vi cố định</strong><ul><li>Metadata discovery: 2 provider reads, không TTS, không spend.</li><li>Exact-audio recovery: tối đa 16 download/batch, hai lần/item và dừng ở 132 reads; không tự cấp rights.</li><li>Fixture cũ được giữ nguyên rights-pending; không sửa hoặc xóa evidence.</li><li>Replacement: đúng một subscription read + một TTS, chỉ explicit active paid base plan mới PASS.</li><li>Factory Audio QA: đúng một independent audio request, trần $0.20; không tự tạo owner ground truth.</li></ul></section>${metadataResult}${audioResult}${conclusion}${materializationPanel}${rightsEvidencePanel}${commercialAudioPanel}${historyScript}${materializationScript}${rightsEvidenceScript}${commercialAudioScript}${factoryAudioQaScript}<p>Rights PASS, Factory independent QA, owner ground truth, dataset, assurance và release là các quyền tách biệt.</p></main></body></html>`, { headers: { ...NO_STORE, "content-type": "text/html; charset=utf-8", "x-frame-options": "SAMEORIGIN" } });
      }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof EvaluationCommandError || error instanceof FixtureMaterializationError || error instanceof RightsEvidenceError || error instanceof CommercialCleanAudioError) return Response.json({ error: { code: error.code, message: error.message }, providerRequests: 0, spendUsd: 0 }, { status: error.status, headers: NO_STORE });
