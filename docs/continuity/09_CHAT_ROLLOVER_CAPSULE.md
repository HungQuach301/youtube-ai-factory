...
**Latest Video Excellence runtime:** Sites v378 is the deployed baseline. Golden revision 9's valid 80.252-second VP9/Opus master is `REPAIR_REQUIRED`: the exact production bytes are a one-treatment camera-only slideshow built from 33 flattened PNGs, and its exact audience mix failed perceptual audio assurance at 65 with P0/P1 `1/3`. Documents 36 and 37 are authoritative. Pan/zoom no longer counts as semantic motion; the failed audio is not reusable. The next renderer must prove mixed visual treatments, layered semantic animation and source-video/B-roll coverage, while the audio plane must provide production music/SFX and a perceptually qualified mix. `FIRST_PASS_QUALITY_V1` prohibits another routine QA-repair loop. Exact next action: FP1 truthful operator projection, then FP2 capability registry. Do not render Golden r10 before FP2–FP5 pass. Stage 11, videos 2–15 and auto-publish remain blocked.
