...
**Latest Video Excellence runtime:** Sites v385 is the deployed baseline. Golden revision 9's valid 80.252-second VP9/Opus master is `REPAIR_REQUIRED`: the exact production bytes are a one-treatment camera-only slideshow built from 33 flattened PNGs, and its exact audience mix failed perceptual audio assurance at 65 with P0/P1 `1/3`. Documents 36–38 are authoritative. FP1 projects `ROOT_REPAIR_REQUIRED` with Stage 07A/07B/08/09/10 as root owners; v382 preserves the three visual owners from unresolved pixel/motion evidence when the historical master scan lacks `motionProvenance`. v384 stabilizes the async D1 projection with a truthful Suspense loading state, and v385 keeps the server-rendered workspace tabs as accessible HTML navigation without an unnecessary client hydration boundary. FP2 installs nine versioned capabilities, 20 hardest archetypes, 20 designed fixtures and a shared fail-closed dispatch audit; all bindings are intentionally `QUALIFICATION_REQUIRED`, so no production request is eligible. The next action is zero-dispatch FP3 executable Stage 07B/08 contracts. Do not render Golden r10 before FP3–FP5 and every required capability qualification pass. Stage 11, videos 2–15 and auto-publish remain blocked.
