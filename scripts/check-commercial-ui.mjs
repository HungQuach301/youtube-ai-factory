...
const checks = [
  ...
  ["V2 read-only API boundary", files.nichePortfolioRoute.includes("export async function GET") && !/export async function (POST|PATCH|DELETE)/.test(files.nichePortfolioRoute)],
  ["V2 zero-spend authority", files.nichePortfolioProjection.includes('v2Commands: "DECLARED_NOT_ROUTED"') && files.nichePortfolioProjection.includes("providerRequests: 0") && files.nichePortfolioProjection.includes("productionDataMutation: false")],
