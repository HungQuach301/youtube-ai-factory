...
test("V20 through V21.3 rejection evidence require one Factory-candidate-span-bound request before V21.4", () => {
  for (const control of [
    "SHOT_PRODUCT_ENGINE_V21_AUTHORED_SEMANTIC_PLAN_COMPILER",
    "V21_4_FACTORY_ISSUED_OPAQUE_CANDIDATE_SPAN_BINDING_AND_INDEPENDENT_VALIDATION",
    "AUTHOR_WAVE_BATCH_2_V21_4_SEMANTIC_PLANS",
    "WAVE_BATCH_2_V21_4_SEMANTIC_PLAN_AUTHORING",
    ...
    "FACTORY_ISSUED_CANDIDATE_SPAN_EXISTENCE_CLAUSE_COVERAGE_AND_AMBIGUITY_GATE_V4",
    "FACTORY_RECONSTRUCTED_FROM_FACTORY_ISSUED_CANDIDATE_SPAN",
    "FACTORY_ISSUED_OPAQUE_CANDIDATE_SPANS_V1",
  ...
  assert.match(route, /Number\(v21_3Result\.plansValidated\) !== 44/);
  assert.match(route, /Number\(v21_3Result\.providerDispatches\) !== 1/);
  ...
  assert.doesNotMatch(route.match(/const semanticCitationSchema[^;]+;/)?.[0] || "", /quote|tokenIds/);
