...
| X22 | P0 | Eligible ElevenLabs classifier state did not match the canonical D1 entitlement evidence value, causing a production failure before TTS | `HANDLED__SOURCE_READY_PRODUCTION_PENDING`: migration `0076` preserves the failed run and conditionally authorizes one recovery only for one subscription read, zero TTS and no artifact; runtime normalizes the eligible state without weakening the paid-plan gate; 51/51 targeted and 181/181 full regressions pass | Technical media/WP7 |
| X23 | P0 | Factory Audio QA relied on free-form JSON from an audio model without Structured Outputs, and parsed before sealing response bytes/usage | `HANDLED__SOURCE_READY_PRODUCTION_PENDING`: migration `0077` preserves the one-request production failure, reserves its unknown spend at USD 0.20 and conditionally authorizes one forced-function recovery with capture-before-parse; cumulative reserved ceiling is USD 0.40; 52/52 targeted and 182/182 full regressions pass | Technical media/WP7 |
