...
async function runBenchmark(projectId: string) {
  ...
  if (autoPass) { await db.update(videoProjects).set({ nextAction: "Build differentiated publishing package", updatedAt: new Date().toISOString() }).where(eq(videoProjects.id, projectId)); await db.insert(workflowEvents).values({ projectId, fromStatus: renders.length ? "RENDER_READY" : "ASSEMBLY_READY", toStatus: "REFERENCE_PASSED", eventType: "REFERENCE_GATE_PASSED", summary: `Reference benchmark v${version} scored ${composite}/100 with ${decision}; publishing package unlocked` }); }
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (payload.action === "APPROVE_BENCHMARK") { const [latest] = await db.select().from(referenceBenchmarkRuns).where(eq(referenceBenchmarkRuns.projectId, id)).orderBy(desc(referenceBenchmarkRuns.version)).limit(1); if (!latest || latest.decision === "RECOMPOSE_REQUIRED") return Response.json({ error: "A critical production gap must be resolved first" }, { status: 409 }); await db.update(referenceBenchmarkRuns).set({ status: "PASSED" }).where(eq(referenceBenchmarkRuns.id, latest.id)); await db.update(videoProjects).set({ nextAction: "Build differentiated publishing package", updatedAt: new Date().toISOString() }).where(eq(videoProjects.id, id)); await db.insert(workflowEvents).values({ projectId: id, toStatus: "REFERENCE_PASSED", eventType: "REFERENCE_GATE_APPROVED", summary: `Human approved benchmark v${latest.version}; publishing package unlocked` }); return Response.json({ ok: true }); }
