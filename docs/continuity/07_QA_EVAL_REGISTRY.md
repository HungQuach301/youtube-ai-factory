...
| Hidden Systems production-scale treatment corpus (`0112`) | Ten distinct treatment/topology/motion cases; SOURCE/MAKE/HYBRID; 1920×1080/30fps VP9; thirty decoded entry/mutation/exit samples; mobile 48px+, contrast 4.5+, 96px+ safe margin; dataset and asset-preparation lineage; exact replay | Targeted tests 4/4 PASS; executor repeats exact output SHA-256 `e6423727…`; provider requests/spend zero | `INTERNAL_TREATMENT_QUALIFICATION_ONLY`; live receipt/deployment verification pending; R22/master/assurance/release/publication remain blocked |
