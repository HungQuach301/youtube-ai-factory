...
- Migration `0113` and `factory-provider-control-plane` source-qualify atomic exact-envelope cost reservation, append-only provider-native reconciliation, drift staleness and explicit one-time qualified fallback planning. Focused tests pass 3/3 and the repository passes 244/244. There is no provider client or live mutation path; dispatch, retry, spend, R22, master, assurance, release and publication authority remain zero.
