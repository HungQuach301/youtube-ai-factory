...
**Latest Video Excellence runtime:** Sites v380 is the deployed baseline. Golden revision 9's valid 80.252-second VP9/Opus master is `REPAIR_REQUIRED`: the exact production bytes are a one-treatment camera-only slideshow built from 33 flattened PNGs, and its exact audience mix failed perceptual audio assurance at 65 with P0/P1 `1/3`. Documents 36 and 37 are authoritative. FP1 now projects the effective state as `ROOT_REPAIR_REQUIRED`, names Stage 07A/07B/08/09/10 as root owners, exposes readiness, active requests and spend, and groups the 18 stages into five operator phases. The next action is FP2 Capability Registry. Do not render Golden r10 before FP2–FP5 pass. Stage 11, videos 2–15 and auto-publish remain blocked.
