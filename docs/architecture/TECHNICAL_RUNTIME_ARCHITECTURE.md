...
Migrations `0106`-`0111` implement the contract/timebase foundation, fenced canonical writer, append-only Provider Gateway/compiler records, deterministic render tape, canary compositor admission and a bounded live qualification receipt. The Gateway remains `ZERO_DISPATCH`. The compiler creates Blueprint, full-duration Shot Contracts and Scene Graph; the renderer writes exact semantic tapes to R2. Asset eligibility binds exact R2 read-back to current rights. The pixel/video compositor accepts only a qualified active worker, exact tape and eligible SOURCE/HYBRID assets; it verifies 60-90 second coverage, VP9 geometry/timebase/frame count, deterministic replay hash and entry/midpoint/exit decoded pixels before atomically recording `COMPOSED_FROM` lineage, job, artifact and canary receipt. The source-ready non-R22 runner additionally proves clean lease release, controlled orphan reconciliation and exact replay on both streams before one immutable qualification receipt. All actions remain separately disabled, zero-spend and R22-blocked; provider reconciliation, assurance and UI projections remain later work.
