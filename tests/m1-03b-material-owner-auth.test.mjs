...
test("registry and baseline move exactly one owner POST and preserve expected debt metrics", () => {
  ...
  assert.equal(baseline.uncoveredHandlers.length, 42);
  assert.equal(baseline.uncoveredHandlers.some((item) => item.identity === handlerIdentity), false);
  assert.deepEqual(Object.fromEntries(["GET", "POST", "HEAD"].map((method) => [method, baseline.uncoveredHandlers.filter((item) => item.method === method).length])), { GET: 32, POST: 9, HEAD: 1 });
