    ...
    {data && <>
      ...
      {mode === "intelligence" ? <>
        ...
        <section className={`pfWorkflowGate ${data.workflow.result?.downstreamGate.state === "READY_FOR_TYPED_HANDOFF" ? "ready" : "blocked"}`}>
          ...
          {data.workflow.result ? <><div className="pfWorkflowMetrics"><article><small>READINESS</small><strong>{data.workflow.result.readiness.replaceAll("_", " ")}</strong></article><article><small>POLICY</small><strong>{data.workflow.result.policyVersion}</strong></article><article><small>DECISION BINDING</small><strong>{data.workflow.decisionBinding.replaceAll("_", " ")}</strong></article><article><small>CHANNEL STRATEGY</small><strong>{data.workflow.result.downstreamGate.reason}</strong></article></div><section className="pfEvidenceAssessment" aria-labelledby="evidence-assessment-title"><header><div><small>EVIDENCE READINESS · AUTHORITATIVE</small><h3 id="evidence-assessment-title">Why this gate is {data.workflow.result.evidenceAssessment.ready ? "ready" : "blocked"}</h3></div><strong>{data.workflow.result.evidenceAssessment.passedCount}/{data.workflow.result.evidenceAssessment.total} passed</strong></header><ul>{data.workflow.result.evidenceAssessment.criteria.map((criterion) => <li key={criterion.id} className={criterion.passed ? "pass" : "fail"}><span aria-hidden="true">{criterion.passed ? "✓" : "!"}</span><div><b>{criterion.label}</b><small>{criterion.id.replaceAll("_", " ")}</small></div><strong>{criterion.actual}<small>Required: {criterion.required}</small></strong></li>)}</ul></section><footer><div><small>ALLOWED NEXT ACTIONS</small><span>{data.workflow.result.allowedNextActions.join(" · ")}</span></div><div><small>COMMAND AUTHORITY</small><span>Expert decision routed · SIWC + owner allowlist · zero provider requests · USD 0 · no automatic Channel Strategy activation</span></div></footer></> : <p>{data.workflow.blockers.join(" ")}</p>}
        </section>
