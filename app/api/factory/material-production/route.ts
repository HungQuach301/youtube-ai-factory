...
function waveProductionManifest(contract: Row, engineVersion = WAVE_PRODUCTION_ENGINE_VERSION, batchVersion = WAVE_BATCH_1_VERSION) {
  const semanticProjection = engineVersion === WAVE_BATCH_2_V11_ENGINE_VERSION;
...
function waveProductOracle(manifest: Row) {
  ...
  const semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11",archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),corpus=clean(semantic.corpusHashInput).toUpperCase(),generic=["$100 PURCHASE","REWARD ENTRY"],genericEscape=contractBound&&generic.some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy);
  const checks=[
    {id:"SEMANTIC_SCENE_GRAPH",status:["CONTRACT_SIGNATURE_SCENE_GRAPH_V5_SPATIAL_BOUND","CONTRACT_BOUND_SCENE_GRAPH_V9","ARCHETYPE_SEMANTIC_MANIFEST_V10","CONTRACT_SEMANTIC_PROJECTION_V11"].includes(clean(semantic.version))&&required.length>=3?"PASS":"FAIL",evidence:`${clean(semantic.kind)} · ${required.join(" · ")}`},
  ...
  return{version:semanticProjection?"WAVE_CONTRACT_SEMANTIC_PRODUCT_ORACLE_V11":archetypeBound?"WAVE_ARCHETYPE_SEMANTIC_PRODUCT_ORACLE_V10":contractBound?"WAVE_CONTRACT_BOUND_PRODUCT_ORACLE_V9":"WAVE_SPATIAL_RELATION_PRODUCT_ORACLE_V7",logicalId:clean(manifest.logicalId),frames,checks,passed:checks.every((item)=>item.status==="PASS")};
}

function waveManifestQualification(manifest: Row) {
  const states=arr(manifest.states).map(rec),semantic=rec(manifest.semanticModel),required=arr(semantic.requiredElements).map(clean).filter(Boolean),semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11",archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),corpus=clean(semantic.corpusHashInput).toUpperCase(),genericEscape=contractBound&&["$100 PURCHASE","REWARD ENTRY"].some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy),checks=[
    {id:"SEMANTIC_SCENE_GRAPH",status:["CONTRACT_SIGNATURE_SCENE_GRAPH_V5_SPATIAL_BOUND","CONTRACT_BOUND_SCENE_GRAPH_V9","ARCHETYPE_SEMANTIC_MANIFEST_V10","CONTRACT_SEMANTIC_PROJECTION_V11"].includes(clean(semantic.version))&&required.length>=3?"PASS":"FAIL"},
  ...
  return { version:semanticProjection?"WAVE_MANIFEST_QUALIFICATION_V11":archetypeBound?"WAVE_MANIFEST_QUALIFICATION_V10":"WAVE_MANIFEST_QUALIFICATION_V7", checks, passed:checks.every((item)=>item.status==="PASS"), signatureInput:{ kind:semantic.kind, actors:semantic.actors, requiredElements:required, corpusHashInput:semantic.corpusHashInput, states:states.map((item)=>({role:item.role,sourceEvidence:item.sourceEvidence})), projections:semantic.stateProjection, transitions:semantic.transitionModel, layoutPolicy:manifest.layoutPolicy } };
}
...
async function produceNextWaveBatch2Shot() {
  ...
  if (![WAVE_BATCH_2_ENGINE_VERSION, WAVE_BATCH_2_REPLACEMENT_ENGINE_VERSION, WAVE_BATCH_2_V10_ENGINE_VERSION, WAVE_BATCH_2_V11_ENGINE_VERSION].includes(engineVersion)) throw new Error(`BATCH_2_ENGINE_NOT_QUALIFIED · ${engineVersion}`);
  const scope = arr(JSON.parse(String(batch.scope_json || "[]"))).map(rec), index = Number(batch.current_index || 0), target = scope[index];
  ...
  const specification = { version: engineVersion === WAVE_BATCH_2_V11_ENGINE_VERSION ? "SHOT_PRODUCT_SPECIFICATION_V5_CONTRACT_SEMANTIC_PROJECTION" : engineVersion === WAVE_BATCH_2_V10_ENGINE_VERSION ? "SHOT_PRODUCT_SPECIFICATION_V4_ARCHETYPE_SEMANTIC" : engineVersion === WAVE_BATCH_2_REPLACEMENT_ENGINE_VERSION ? "SHOT_PRODUCT_SPECIFICATION_V3_CONTRACT_BOUND" : "SHOT_PRODUCT_SPECIFICATION_V2_CONTROLLED_SCALE", batchVersion: clean(batch.version), engineVersion, contract, manifest }, specificationJson = JSON.stringify(specification), specificationHash = await sha(specificationJson), frameIds: string[] = [], frameHashes: string[] = [];
  ...
  for (const [role, frame] of [["CERT_ENTRY", oracle.frames[0]], ["CERT_MIDPOINT", oracle.frames[1]], ["CERT_EXIT", oracle.frames[2]]] as const) {
    const replacement = engineVersion === WAVE_BATCH_2_REPLACEMENT_ENGINE_VERSION, v10 = engineVersion === WAVE_BATCH_2_V10_ENGINE_VERSION, v11 = engineVersion === WAVE_BATCH_2_V11_ENGINE_VERSION;
    const fileId = await storeMaterial(env, db, authorization, briefRow, { role, identity: `BATCH2-${clean(contract.briefId)}-${role}-${v11 ? "E11" : v10 ? "E10" : replacement ? "E9" : "E8"}`, bytes: frame.bytes, mimeType: "image/png", extension: "png", sourceType: engineVersion, provider: "FRAMEFLOW_OWNED", providerAssetId: specificationHash, sourceUrl: specificationHash, landingUrl: specificationHash, licenseCode: "CHANNEL_OWNED", width: frame.width, height: frame.height, runtimeScope: v11 ? "wave-09-batch-2-engine-v11" : v10 ? "wave-09-batch-2-engine-v10" : replacement ? "wave-09-batch-2-engine-v9" : "wave-09-batch-2-engine-v8", archiveFolder: v11 ? "Wave 09 Batch 2 Engine V11" : v10 ? "Wave 09 Batch 2 Engine V10" : replacement ? "Wave 09 Batch 2 Engine V9" : "Wave 09 Batch 2 Engine V8" });
    const stored = await db.prepare("SELECT content_hash,status FROM v7_material_files WHERE id=?").bind(fileId).first<Row>();
