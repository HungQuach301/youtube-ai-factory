...
Migration `0069_evaluation_provider_binding_diagnostics.sql` separately diagnoses the 46 historical ElevenLabs audio tasks. A unique match against the first 24 characters of an artifact SHA-256 is explicitly classified as `LEGACY_SYNTHETIC_RESPONSE_BINDING_DISCOVERED`, because the historical runtime wrote that derived value rather than the provider-native request ID. The diagnostic has zero rights, dataset, assurance or release authority. New ElevenLabs audio must capture the `request-id` response header, bind it to the internal request row and exact response-byte SHA-256, pass R2 checksum read-back and only then complete the provider ledger entry. Terms and paid-plan evidence remain separate time-bound requirements.
