...
export const v7EvaluationCurrentRightsEvidenceCapturePolicies = sqliteTable("v7_evaluation_current_rights_evidence_capture_policies", {
  id: text("id").primaryKey(), channelId: text("channel_id").notNull(), policyVersion: text("policy_version").notNull(), jurisdictionScope: text("jurisdiction_scope").notNull(), expectedOfficialSources: integer("expected_official_sources").notNull(), maximumPublicReads: integer("maximum_public_reads").notNull(), maximumSourceBytes: integer("maximum_source_bytes").notNull(),
  generationTimeSubscriptionBindingRequired: integer("generation_time_subscription_binding_required", { mode: "boolean" }).notNull(), basePlanEvidenceRequired: integer("base_plan_evidence_required", { mode: "boolean" }).notNull(), inputOwnershipEvidenceRequired: integer("input_ownership_evidence_required", { mode: "boolean" }).notNull(), betaModelForbidden: integer("beta_model_forbidden", { mode: "boolean" }).notNull(),
  rightsPassAuthority: integer("rights_pass_authority", { mode: "boolean" }).notNull().default(false), datasetSealingAuthority: integer("dataset_sealing_authority", { mode: "boolean" }).notNull().default(false), assuranceQualificationAuthority: integer("assurance_qualification_authority", { mode: "boolean" }).notNull().default(false), releaseAuthority: integer("release_authority", { mode: "boolean" }).notNull().default(false), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("v7_evaluation_current_rights_evidence_policy_channel_uq").on(table.channelId, table.policyVersion)]);

export const v7EvaluationCurrentRightsEvidenceCaptureRuns = sqliteTable("v7_evaluation_current_rights_evidence_capture_runs", {
  id: text("id").primaryKey(), channelId: text("channel_id").notNull(), policyVersion: text("policy_version").notNull(), fixtureArtifactId: text("fixture_artifact_id").notNull(), providerBindingReceiptId: text("provider_binding_receipt_id").notNull(), idempotencyKey: text("idempotency_key").notNull(), intentHash: text("intent_hash").notNull(), lifecycleState: text("lifecycle_state").notNull(), plannedSources: integer("planned_sources").notNull(), processedSources: integer("processed_sources").notNull().default(0), verifiedSources: integer("verified_sources").notNull().default(0), publicReads: integer("public_reads").notNull().default(0), accountReads: integer("account_reads").notNull().default(0), providerGenerationRequests: integer("provider_generation_requests").notNull().default(0), spendUsd: real("spend_usd").notNull().default(0), actor: text("actor").notNull(), errorCode: text("error_code"), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`), completedAt: text("completed_at"),
}, (table) => [uniqueIndex("v7_evaluation_current_rights_evidence_run_idempotency_uq").on(table.idempotencyKey), index("v7_evaluation_current_rights_evidence_run_channel_idx").on(table.channelId, table.createdAt)]);

export const v7EvaluationOfficialTermsSnapshotReceipts = sqliteTable("v7_evaluation_official_terms_snapshot_receipts", {
  id: text("id").primaryKey(), runId: text("run_id").notNull(), channelId: text("channel_id").notNull(), policyVersion: text("policy_version").notNull(), sourceKey: text("source_key").notNull(), sourceCategory: text("source_category").notNull(), sourceUrl: text("source_url").notNull(), retrievalState: text("retrieval_state").notNull(), httpStatus: integer("http_status"), contentType: text("content_type"), responseByteSize: integer("response_byte_size"), exactResponseHash: text("exact_response_hash"), r2StorageKey: text("r2_storage_key"), r2ReadbackHash: text("r2_readback_hash"), r2ReadbackVerified: integer("r2_readback_verified", { mode: "boolean" }).notNull().default(false), retrievedAt: text("retrieved_at").notNull(), errorCode: text("error_code"), evidenceHash: text("evidence_hash").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("v7_evaluation_official_terms_snapshot_run_source_uq").on(table.runId, table.sourceKey)]);
...
export const factoryProviderCostReservations = sqliteTable("factory_provider_cost_reservations", {
  id: text("id").primaryKey(), workRequestId: text("work_request_id").notNull().references(() => factoryProviderWorkRequests.id), routeDecisionId: text("route_decision_id").notNull().references(() => factoryProviderRouteDecisions.id),
  costEnvelopeId: text("cost_envelope_id").notNull().references(() => factoryCostEnvelopes.id), bindingId: text("binding_id").notNull().references(() => factoryProviderBindings.id),
  qualificationId: text("qualification_id").notNull().references(() => factoryCapabilityQualifications.id), idempotencyKey: text("idempotency_key").notNull(), reservationIntentHash: text("reservation_intent_hash").notNull(),
  reservedProviderRequests: integer("reserved_provider_requests").notNull(), reservedSpendMicros: integer("reserved_spend_micros").notNull(), reservationState: text("reservation_state").notNull(),
  dispatchAuthority: integer("dispatch_authority", { mode: "boolean" }).notNull(), r22Authority: integer("r22_authority", { mode: "boolean" }).notNull(), masterAuthority: integer("master_authority", { mode: "boolean" }).notNull(),
  ...
  uniqueIndex("factory_provider_cost_reservation_idempotency_uq").on(table.idempotencyKey),
  index("factory_provider_cost_reservation_envelope_idx").on(table.costEnvelopeId, table.createdAt),
]);

export const factoryProviderNativeRequestReceipts = sqliteTable("factory_provider_native_request_receipts", {
  id: text("id").primaryKey(), reservationId: text("reservation_id").notNull().references(() => factoryProviderCostReservations.id), bindingId: text("binding_id").notNull().references(() => factoryProviderBindings.id),
  providerId: text("provider_id").notNull().references(() => factoryProviders.id), nativeRequestId: text("native_request_id"), providerResponseId: text("provider_response_id"), requestFingerprint: text("request_fingerprint").notNull(),
  requestState: text("request_state").notNull(), rawResponseHash: text("raw_response_hash"), usageJson: text("usage_json").notNull(), actualProviderRequests: integer("actual_provider_requests").notNull(),
  actualSpendMicros: integer("actual_spend_micros").notNull(), retryAuthority: integer("retry_authority", { mode: "boolean" }).notNull(), evidenceHash: text("evidence_hash").notNull(), observedAt: text("observed_at").notNull(),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("factory_provider_native_request_reservation_fingerprint_uq").on(table.reservationId, table.requestFingerprint)]);

export const factoryProviderReconciliationReceipts = sqliteTable("factory_provider_reconciliation_receipts", {
  id: text("id").primaryKey(), reservationId: text("reservation_id").notNull().references(() => factoryProviderCostReservations.id), nativeRequestReceiptId: text("native_request_receipt_id").notNull().references(() => factoryProviderNativeRequestReceipts.id),
  reconciliationKey: text("reconciliation_key").notNull(), reconciliationSequence: integer("reconciliation_sequence").notNull(), outcome: text("outcome").notNull(), reservedProviderRequests: integer("reserved_provider_requests").notNull(),
  reservedSpendMicros: integer("reserved_spend_micros").notNull(), actualProviderRequests: integer("actual_provider_requests").notNull(), actualSpendMicros: integer("actual_spend_micros").notNull(),
  remainingReservedSpendMicros: integer("remaining_reserved_spend_micros").notNull(), retryAuthority: integer("retry_authority", { mode: "boolean" }).notNull(), dispatchAuthority: integer("dispatch_authority", { mode: "boolean" }).notNull(),
  reconciliationHash: text("reconciliation_hash").notNull(), evidenceHash: text("evidence_hash").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
  uniqueIndex("factory_provider_reconciliation_key_uq").on(table.reconciliationKey),
  uniqueIndex("factory_provider_reconciliation_sequence_uq").on(table.reservationId, table.reconciliationSequence),
]);

export const factoryProviderDriftReceipts = sqliteTable("factory_provider_drift_receipts", {
  id: text("id").primaryKey(), bindingId: text("binding_id").notNull().references(() => factoryProviderBindings.id), qualificationId: text("qualification_id").notNull().references(() => factoryCapabilityQualifications.id),
  observationKey: text("observation_key").notNull(), baselineJson: text("baseline_json").notNull(), observedJson: text("observed_json").notNull(), driftDimensionsJson: text("drift_dimensions_json").notNull(),
  driftState: text("drift_state").notNull(), invalidatesQualification: integer("invalidates_qualification", { mode: "boolean" }).notNull(), dispatchAuthority: integer("dispatch_authority", { mode: "boolean" }).notNull(),
  observationHash: text("observation_hash").notNull(), evidenceHash: text("evidence_hash").notNull(), observedAt: text("observed_at").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
  uniqueIndex("factory_provider_drift_observation_key_uq").on(table.observationKey),
  index("factory_provider_drift_binding_latest_idx").on(table.bindingId, table.observedAt, table.createdAt),
]);

export const factoryProviderFallbackAuthorizations = sqliteTable("factory_provider_fallback_authorizations", {
  id: text("id").primaryKey(), primaryBindingId: text("primary_binding_id").notNull().references(() => factoryProviderBindings.id), fallbackBindingId: text("fallback_binding_id").notNull().references(() => factoryProviderBindings.id),
  fallbackQualificationId: text("fallback_qualification_id").notNull().references(() => factoryCapabilityQualifications.id), authorizationKey: text("authorization_key").notNull(), reason: text("reason").notNull(),
  authorizedBy: text("authorized_by").notNull(), authorizationState: text("authorization_state").notNull(), maxProviderRequests: integer("max_provider_requests").notNull(), maxSpendMicros: integer("max_spend_micros").notNull(),
  oneTimePlan: integer("one_time_plan", { mode: "boolean" }).notNull(), dispatchAuthority: integer("dispatch_authority", { mode: "boolean" }).notNull(), r22Authority: integer("r22_authority", { mode: "boolean" }).notNull(),
  expiresAt: text("expires_at").notNull(), authorizationHash: text("authorization_hash").notNull(), evidenceHash: text("evidence_hash").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("factory_provider_fallback_authorization_key_uq").on(table.authorizationKey)]);
...
export const factoryAssuranceCalibrationCorpusSnapshots = sqliteTable("factory_assurance_calibration_corpus_snapshots", {
  id: text("id").primaryKey(), snapshotKey: text("snapshot_key").notNull(), channelId: text("channel_id").notNull(), formatKey: text("format_key").notNull(), policyVersion: text("policy_version").notNull(), thresholdVersion: text("threshold_version").notNull(),
  sourceSnapshotHash: text("source_snapshot_hash").notNull(), candidateCount: integer("candidate_count").notNull(), ownerConfirmedCount: integer("owner_confirmed_count").notNull(), sealedCleanCount: integer("sealed_clean_count").notNull(), sealedDefectCount: integer("sealed_defect_count").notNull(),
  distinctArtifactCount: integer("distinct_artifact_count").notNull(), distinctCorrelationGroupCount: integer("distinct_correlation_group_count").notNull(), partitionCountsJson: text("partition_counts_json").notNull(), layerReadinessJson: text("layer_readiness_json").notNull(), lifecycleState: text("lifecycle_state").notNull(),
  qualificationAuthority: integer("qualification_authority", { mode: "boolean" }).notNull(), passAuthority: integer("pass_authority", { mode: "boolean" }).notNull(), providerDispatchAuthority: integer("provider_dispatch_authority", { mode: "boolean" }).notNull(), r22Authority: integer("r22_authority", { mode: "boolean" }).notNull(), masterAuthority: integer("master_authority", { mode: "boolean" }).notNull(), releaseAuthority: integer("release_authority", { mode: "boolean" }).notNull(), publicationAuthority: integer("publication_authority", { mode: "boolean" }).notNull(),
  providerRequests: integer("provider_requests").notNull(), spendMicros: integer("spend_micros").notNull(), snapshotHash: text("snapshot_hash").notNull(), evidenceHash: text("evidence_hash").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("factory_assurance_calibration_corpus_snapshot_key_uq").on(table.snapshotKey), index("factory_assurance_calibration_corpus_snapshot_scope_idx").on(table.channelId, table.formatKey, table.lifecycleState, table.createdAt)]);

export const factoryAssuranceCalibrationCorpusItems = sqliteTable("factory_assurance_calibration_corpus_items", {
  id: text("id").primaryKey(), snapshotId: text("snapshot_id").notNull().references(() => factoryAssuranceCalibrationCorpusSnapshots.id), itemKey: text("item_key").notNull(), sourceFamily: text("source_family").notNull(), sourceId: text("source_id").notNull(), sourceReceiptId: text("source_receipt_id").notNull(),
  candidateKind: text("candidate_kind").notNull(), mimeType: text("mime_type").notNull(), storageKey: text("storage_key"), byteSize: integer("byte_size"), exactArtifactHash: text("exact_artifact_hash").notNull(), labelSource: text("label_source").notNull(), expectedOutcome: text("expected_outcome").notNull(), expectedSeverity: text("expected_severity").notNull(),
  defectFamiliesJson: text("defect_families_json").notNull(), applicableLayersJson: text("applicable_layers_json").notNull(), correlationGroup: text("correlation_group").notNull(), ownerLabelHash: text("owner_label_hash").notNull(), bytesState: text("bytes_state").notNull(), checksumState: text("checksum_state").notNull(), rightsState: text("rights_state").notNull(), partitionHint: text("partition_hint").notNull(), countEligible: integer("count_eligible", { mode: "boolean" }).notNull(), evidenceHash: text("evidence_hash").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("factory_assurance_calibration_corpus_item_key_uq").on(table.snapshotId, table.itemKey), index("factory_assurance_calibration_corpus_item_layer_idx").on(table.snapshotId, table.partitionHint, table.labelSource, table.correlationGroup)]);
...
export const factoryAssuranceCorpusRemediationEvidenceRuns = sqliteTable("factory_assurance_corpus_remediation_evidence_runs", {
  id: text("id").primaryKey(), remediationSnapshotId: text("remediation_snapshot_id").notNull().references(() => factoryAssuranceCorpusRemediationSnapshots.id), idempotencyKey: text("idempotency_key").notNull(), requestHash: text("request_hash").notNull(), policyVersion: text("policy_version").notNull(),
  batchLimit: integer("batch_limit").notNull(), plannedItems: integer("planned_items").notNull(), processedItems: integer("processed_items").notNull(), byteVerifiedItems: integer("byte_verified_items").notNull(), checksumPassItems: integer("checksum_pass_items").notNull(), provenancePassItems: integer("provenance_pass_items").notNull(), rightsPassItems: integer("rights_pass_items").notNull(), exactEvidenceReadyItems: integer("exact_evidence_ready_items").notNull(), remainingItems: integer("remaining_items").notNull(), bytesRead: integer("bytes_read").notNull(), lifecycleState: text("lifecycle_state").notNull(), actor: text("actor").notNull(),
  countEligible: integer("count_eligible", { mode: "boolean" }).notNull(), qualificationAuthority: integer("qualification_authority", { mode: "boolean" }).notNull(), passAuthority: integer("pass_authority", { mode: "boolean" }).notNull(), providerDispatchAuthority: integer("provider_dispatch_authority", { mode: "boolean" }).notNull(), r22Authority: integer("r22_authority", { mode: "boolean" }).notNull(), masterAuthority: integer("master_authority", { mode: "boolean" }).notNull(), releaseAuthority: integer("release_authority", { mode: "boolean" }).notNull(), publicationAuthority: integer("publication_authority", { mode: "boolean" }).notNull(), providerRequests: integer("provider_requests").notNull(), spendMicros: integer("spend_micros").notNull(), manifestHash: text("manifest_hash").notNull(), evidenceHash: text("evidence_hash").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("factory_assurance_corpus_remediation_evidence_run_idempotency_uq").on(table.remediationSnapshotId, table.idempotencyKey), index("factory_assurance_corpus_remediation_evidence_run_scope_idx").on(table.remediationSnapshotId, table.lifecycleState, table.createdAt)]);

export const factoryAssuranceCorpusRemediationEvidenceReceipts = sqliteTable("factory_assurance_corpus_remediation_evidence_receipts", {
  id: text("id").primaryKey(), runId: text("run_id").notNull().references(() => factoryAssuranceCorpusRemediationEvidenceRuns.id), remediationSnapshotId: text("remediation_snapshot_id").notNull().references(() => factoryAssuranceCorpusRemediationSnapshots.id), remediationItemId: text("remediation_item_id").notNull().references(() => factoryAssuranceCorpusRemediationItems.id), sourceCandidateId: text("source_candidate_id").notNull(), sourceArtifactId: text("source_artifact_id").notNull(), storageKey: text("storage_key").notNull(), mimeType: text("mime_type").notNull(), declaredHash: text("declared_hash").notNull(), computedHash: text("computed_hash"), declaredBytes: integer("declared_bytes"), actualBytes: integer("actual_bytes").notNull(), objectMetadataJson: text("object_metadata_json").notNull(), sourceProvenanceJson: text("source_provenance_json").notNull(), bytesState: text("bytes_state").notNull(), checksumState: text("checksum_state").notNull(), provenanceState: text("provenance_state").notNull(), rightsState: text("rights_state").notNull(), rightsReceiptKind: text("rights_receipt_kind").notNull(), rightsReceiptId: text("rights_receipt_id"), exactEvidenceState: text("exact_evidence_state").notNull(), reconciliationReasonsJson: text("reconciliation_reasons_json").notNull(), policyVersion: text("policy_version").notNull(),
  countEligible: integer("count_eligible", { mode: "boolean" }).notNull(), qualificationAuthority: integer("qualification_authority", { mode: "boolean" }).notNull(), passAuthority: integer("pass_authority", { mode: "boolean" }).notNull(), providerDispatchAuthority: integer("provider_dispatch_authority", { mode: "boolean" }).notNull(), r22Authority: integer("r22_authority", { mode: "boolean" }).notNull(), masterAuthority: integer("master_authority", { mode: "boolean" }).notNull(), releaseAuthority: integer("release_authority", { mode: "boolean" }).notNull(), publicationAuthority: integer("publication_authority", { mode: "boolean" }).notNull(), providerRequests: integer("provider_requests").notNull(), spendMicros: integer("spend_micros").notNull(), evidenceHash: text("evidence_hash").notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("factory_assurance_corpus_remediation_evidence_receipt_item_uq").on(table.remediationSnapshotId, table.remediationItemId, table.policyVersion), index("factory_assurance_corpus_remediation_evidence_receipt_scope_idx").on(table.remediationSnapshotId, table.exactEvidenceState, table.rightsState, table.createdAt)]);
