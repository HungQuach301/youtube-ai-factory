...
Live-canary runner deployment receipt: migration `0111`, clean lease release and the bounded non-R22 canary/orphan/replay runner built and deployed as Sites version 524 from exact commit `2e0d0df636dfc89a291db7b62a615fcf330301d1`. Live D1 exposes the new append-only qualification table with zero rows. An environment revision containing only the writer/runner flags was prepared but not deployed because Sites required explicit production-side-effect approval; the two pending keys were removed immediately. Therefore the active deployment remains disabled and created no R2 canary, runtime stream, provider request, spend or R21/R22 mutation.
