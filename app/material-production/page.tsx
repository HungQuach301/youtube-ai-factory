...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    if (!["STABILIZATION_RELEASE_V1_TYPED_STATE_MACHINE", "CANONICAL_UNIT_SCENES_V2"].includes(data?.canary?.version || "") || data?.canary?.status !== "BATCH_UNIT_PASS_REVIEW" || data.authorization?.modelPolicy.batchAuthorized !== true || data.authorization.modelPolicy.runToCompletion10Mp !== true || working || data.requestLedger.active > 0) return;
  ...
  useEffect(() => {
    if (data?.run?.status !== "CANONICAL_UNIT_SCENE_REBUILD" || data.canary?.version !== "CANONICAL_UNIT_SCENES_V2" || data.canary.status !== "SCENE_REBUILD_RUNNING" || working || data.requestLedger.active > 0) return;
  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "SCENE_REBUILD_RUNNING" && data.canary.version === "CANONICAL_UNIT_SCENES_V2" && <p className="stateBanner">Canonical contract scenes are being frozen and verified one unit at a time. Provider dispatch remains locked until all 24 frames pass.</p>}
