...
const WAVE_BATCH_2_V13_REPRODUCTION_VERSION = "WAVE_09_BATCH_2_V6_CONTRACT_NATIVE_SCENE_REPRODUCTION";
const WAVE_BATCH_2_V13_ENGINE_VERSION = "SHOT_PRODUCT_ENGINE_V13_CONTRACT_NATIVE_SCENE_SPECIFICATION";
...
async function snapshot() {
  ...
  const engine = clean(productionBatch?.engine_version), v13ControlVisible = [WAVE_BATCH_2_V12_ENGINE_VERSION, WAVE_BATCH_2_V13_ENGINE_VERSION].includes(engine), v12ControlVisible = engine === WAVE_BATCH_2_V11_ENGINE_VERSION, v11ControlVisible = engine === WAVE_BATCH_2_V10_ENGINE_VERSION;
  const expectedPreflightAction = v13ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V13_ACTIVATION" : v12ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V12_ACTIVATION" : v11ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" : "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION";
  const expectedActivationAction = v13ControlVisible ? "ADOPT_WAVE_BATCH_2_V13_ENGINE_ROOT_CORRECTION" : v12ControlVisible ? "ADOPT_WAVE_BATCH_2_V12_ENGINE_ROOT_CORRECTION" : v11ControlVisible ? "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" : "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION";
...
async function newRequest(db: DB, authorization: Row, briefId: string, phase: string, provider: string, modelId = "none", reasoning = "none", expected = 0, maximum = 0, stableRequestId?: string) {
  ...
  if (phase === "WAVE_BATCH_2_PRODUCT_AUDIT" && briefId === "WAVE-09-BATCH-2") {
    ...
    const v13QualifiedAudit = clean(batch?.engine_version) === WAVE_BATCH_2_V13_ENGINE_VERSION
      ...
      && rootPolicy.v8V9V10V11V12ProductsPreservedAsEvidence === true
      && rootPolicy.retryPriorAudits === false
      && clean(rejectedBatch?.status) === "ENGINE_ROOT_CAUSE_PRESERVED"
      && clean(rejectedAudit?.status) === "ENGINE_ROOT_CAUSE_REQUIRED"
      && currentDurableIntent;
    ...
    if (!batchAuditAuthorized) throw new Error("BATCH_2_PRODUCT_AUDIT_FIREWALL · qualified V8 initial audit or lineage-bound V9/V10/V11/V12/V13 durable audit intent is required");
  }
...
function contractNativeSceneSpecification(contract: Row, bindings: Row[], actors: string[], kind: string) {
  ...
  const labels = [...new Set([...actors, ...bindings.flatMap((binding) => arr(binding.nodes).map(clean))])].filter(Boolean).slice(0, 6);
  while (labels.length < 3) labels.push(["SOURCE RECORD", "MOVING EVIDENCE", "VERIFIED OUTCOME"][labels.length]);
  const slots = [
  ...
  const objects = labels.map((label, index) => {
    ...
    const type = actors.includes(label) ? "ACTOR" : /BOUNDARY|BARRIER|LIMIT|RAIL/.test(label) ? "BOUNDARY" : /RECORD|LEDGER|ROW|FIELD/.test(label) ? "RECORD" : /AMOUNT|FEE|REWARD|PURCHASE/.test(label) ? "VALUE" : "EVIDENCE";
  ...
  const movingObject = { id: "MOVING_OBJECT", type: "MOVING_EVIDENCE", label: labels[1], sourceClause: clauses[1] || clauses[0] || clean(contract.claim), audienceVisible: true };
  const path = [[190 + offset * 18, 300], [480, 286 - offset * 14], [770 - offset * 18, 300]];
  const relationships = bindings.map((binding, index) => ({ id: `RELATION_${index + 1}`, from: objects[index % objects.length].id, to: objects[(index + 1) % objects.length].id, relation: clean(binding.relation), polarity: clean(binding.polarity), sourceClause: clean(binding.clause), corridor: index % 2 ? [[296, 376], [664, 376]] : [[296, 236], [664, 236]] }));
  const states = bindings.map((binding, index) => ({
    role: ["ENTRY", "MIDPOINT", "EXIT"][index],
    visibleObjectIds: objects.slice(0, Math.min(objects.length, index + 3)).map((item) => item.id),
    activeRelationshipId: relationships[index].id,
    movingObjectPosition: path[index],
    action: clean(binding.relation),
    constraint: clean(binding.polarity) === "NEGATIVE_CONSTRAINT" ? clean(binding.clause) : "SOURCE CLAUSE PRESERVED",
    stateDelta: ["CONTEXT ESTABLISHED", "ACTION IN PROGRESS", "OUTCOME AND LIMIT VISIBLE"][index],
  }));
  return {
    version: "CONTRACT_NATIVE_SCENE_SPECIFICATION_V13", layoutFamily, canvas: { width: 960, height: 540, safeInset: 48 },
    ...
    rendererContract: { input: "SCENE_SPECIFICATION_ONLY", tokenFallbackForbidden: true, geometryAuthoritative: true, provenanceRequiredForEveryVisibleObject: true, provenanceRequiredForEveryRelationship: true },
  };
}
...
function waveProductionManifest(contract: Row, engineVersion = WAVE_PRODUCTION_ENGINE_VERSION, batchVersion = WAVE_BATCH_1_VERSION) {
  ...
  return {
    ...
    semanticModel: { version: contractNativeScene ? "CONTRACT_NATIVE_SCENE_SPECIFICATION_V13" : structuredOntology ? "STRUCTURED_VISUAL_ONTOLOGY_V12" : semanticProjection ? "CONTRACT_SEMANTIC_PROJECTION_V11" : archetypeBound ? "ARCHETYPE_SEMANTIC_MANIFEST_V10" : contractBound ? "CONTRACT_BOUND_SCENE_GRAPH_V9" : "CONTRACT_SIGNATURE_SCENE_GRAPH_V5_SPATIAL_BOUND", kind, archetype: clean(contract.archetype), visualFamily: clean(contract.visualFamily), claim: clean(contract.claim), actors: archetypeBound ? semanticActors : contractBound ? contractElements.slice(0, 6) : semanticActors, requiredElements: contractBound ? contractElements : requiredElements, stateBindings: contractBindings, stateProjection: semanticProjection ? stateProjection : undefined, visualOntology, sceneSpecification, corpusHashInput: `${clean(contract.briefId)} ${corpus}`, contractSignature: `${clean(contract.briefId)}:${corpus}`, relationPolicy: contractNativeScene ? "CONTRACT_NATIVE_TYPED_OBJECTS_RELATIONSHIPS_GEOMETRY_AND_TRANSITIONS" : structuredOntology ? "STRUCTURED_ACTOR_LANE_CONTAINER_BOUNDARY_MOTION_TOPOLOGY" : archetypeBound ? "ARCHETYPE_NATIVE_TOPOLOGY_WITH_DIRECTION_BOUNDARY_AND_STATE_DELTA" : "EXPLICIT_VIEWER_VISIBLE_NODES_AND_EDGES", uncertaintyPolicy: "UNRESOLVED_MUST_REMAIN_VISIBLE", elementProvenance: archetypeBound ? contractElements.map((element) => ({ element, source: "CONTRACT_CLAUSE" })) : undefined, transitionModel: archetypeBound ? contractBindings.map((binding, index) => ({ from: index === 0 ? "PRE_SCENE" : ["ENTRY", "MIDPOINT"][index - 1], to: ["ENTRY", "MIDPOINT", "EXIT"][index], relation: binding.relation, polarity: binding.polarity })) : undefined },
    batchVersion,
    productionRoute: contract.productionRoute,
    specificationCompiler: contractNativeScene ? "CONTRACT_NATIVE_SCENE_COMPILER_V13" : structuredOntology ? "STRUCTURED_VISUAL_ONTOLOGY_COMPILER_V12" : semanticProjection ? "CONTRACT_SEMANTIC_PROJECTION_COMPILER_V11" : archetypeBound ? "ARCHETYPE_SEMANTIC_COMPILER_V10" : contractBound ? "CONTRACT_BOUND_SCENE_GRAPH_COMPILER_V9" : scaleBaseline ? "CONTROLLED_SCALE_COMPILER_V8" : "SPATIAL_RELATION_BOUND_COMPILER_V7",
...
function renderWaveSemanticScene(manifest: Row, state: 0 | 1 | 2) {
  ...
  if(archetypeBound){fill(48,108,864,34,theme[0]);center(safe(semantic.claim,72),480,112,1,theme[2],72);center(safe(currentBinding.clause,72),480,126,1,accent,72);}
  if(contractNativeScene){
    const spec=rec(semantic.sceneSpecification),sceneState=rec(arr(spec.states)[state]),objects=arr(spec.objects).map(rec),relationships=arr(spec.relationships).map(rec),visibleIds=new Set(arr(sceneState.visibleObjectIds).map(clean)),activeRelation=relationships.find((item)=>clean(item.id)===clean(sceneState.activeRelationshipId))||relationships[state],position=arr(sceneState.movingObjectPosition).map(Number),mx=Number(position[0]||[190,480,770][state]),my=Number(position[1]||300),family=clean(spec.layoutFamily);
...
function waveProductOracle(manifest: Row) {
  ...
  const checks=[
    ...
    {id:"CONTRACT_NATIVE_SCENE_SPECIFICATION",status:!contractNativeScene||clean(sceneSpec.version)==="CONTRACT_NATIVE_SCENE_SPECIFICATION_V13"&&sceneObjects.length>=3&&sceneRelationships.length===3&&sceneStates.length===3&&sceneObjects.every((item)=>Boolean(clean(item.id))&&Boolean(clean(item.type))&&Boolean(clean(item.label))&&Boolean(clean(item.sourceClause))&&Object.keys(rec(item.geometry)).length===4)&&sceneRelationships.every((item)=>Boolean(clean(item.from))&&Boolean(clean(item.to))&&Boolean(clean(item.relation))&&Boolean(clean(item.sourceClause)))?"PASS":"FAIL",evidence:`${sceneObjects.length} typed objects · ${sceneRelationships.length} contract relationships · ${sceneStates.length} states`},
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&Boolean(clean(item.activeRelationshipId))&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="SCENE_SPECIFICATION_ONLY"&&rendererContract.tokenFallbackForbidden===true&&rendererContract.geometryAuthoritative===true?"PASS":"FAIL",evidence:"renderer consumes only typed scene objects, authoritative geometry and state transitions"},
...
function waveManifestQualification(manifest: Row) {
  const states=arr(manifest.states).map(rec),semantic=rec(manifest.semanticModel),required=arr(semantic.requiredElements).map(clean).filter(Boolean),contractNativeScene=clean(semantic.version)==="CONTRACT_NATIVE_SCENE_SPECIFICATION_V13",structuredOntology=clean(semantic.version)==="STRUCTURED_VISUAL_ONTOLOGY_V12",semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11"||structuredOntology||contractNativeScene,archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),ontology=rec(semantic.visualOntology),ontologyStates=arr(ontology.states).map(rec),layoutProof=rec(ontology.layoutProof),motionPolicy=rec(ontology.motionPolicy),sceneSpec=rec(semantic.sceneSpecification),sceneObjects=arr(sceneSpec.objects).map(rec),sceneRelationships=arr(sceneSpec.relationships).map(rec),sceneStates=arr(sceneSpec.states).map(rec),sceneLayoutProof=rec(sceneSpec.layoutProof),sceneMotionProof=rec(sceneSpec.motionProof),rendererContract=rec(sceneSpec.rendererContract),corpus=clean(semantic.corpusHashInput).toUpperCase(),genericEscape=contractBound&&["$100 PURCHASE","REWARD ENTRY"].some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy),checks=[
    ...
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="SCENE_SPECIFICATION_ONLY"&&rendererContract.tokenFallbackForbidden===true?"PASS":"FAIL"},
...
async function waveBatch2V13ActivationContext() {
  ...
  const rejected = await db.prepare("SELECT * FROM v7_production_batches WHERE run_id=? AND wave_key='BATCH_2' AND engine_version=? ORDER BY created_at DESC LIMIT 1").bind(run.id, WAVE_BATCH_2_V12_ENGINE_VERSION).first<Row>();
  const audit = rejected ? await db.prepare("SELECT * FROM v7_batch_product_audits WHERE batch_id=? AND status='ENGINE_ROOT_CAUSE_REQUIRED' ORDER BY created_at DESC LIMIT 1").bind(rejected.id).first<Row>() : null;
  if (!rejected || clean(rejected.status) !== "ENGINE_ROOT_CAUSE_REQUIRED" || Number(rejected.completed_units) !== 50 || !audit) throw new Error("BATCH_2_V12_ROOT_CAUSE_CHECKPOINT_REQUIRED");
  const products = await db.prepare("SELECT COUNT(*) AS total FROM v7_shot_products WHERE batch_id=? AND engine_version=? AND status='PRODUCT_COMPLETE'").bind(rejected.id, WAVE_BATCH_2_V12_ENGINE_VERSION).first<{ total: number }>();
  if (Number(products?.total || 0) !== 50) throw new Error("BATCH_2_V12_EVIDENCE_INCOMPLETE");
  const scope = arr(JSON.parse(String(rejected.scope_json || "[]"))).map(rec);
  if (scope.length !== 50) throw new Error(`BATCH_2_V13_SCOPE_INCOMPLETE · ${scope.length}/50`);
  const usage = await db.prepare("SELECT COUNT(*) AS total,COALESCE(SUM(actual_cost_usd),0) AS cost FROM v7_material_requests WHERE authorization_id=?").bind(authorization.id).first<{ total: number; cost: number }>();
  const inputHash = await sha(JSON.stringify({ action: "ADOPT_WAVE_BATCH_2_V13_ENGINE_ROOT_CORRECTION", runId: run.id, authorizationId: authorization.id, sourceBatchId: rejected.id, sourceBatchUpdatedAt: rejected.updated_at, sourceAuditId: audit.id, sourceAuditUpdatedAt: audit.updated_at, scope, targetVersion: WAVE_BATCH_2_V13_REPRODUCTION_VERSION, targetEngine: WAVE_BATCH_2_V13_ENGINE_VERSION }));
  return { env, db, run, authorization, replacementId, rejected, audit, scope, inputHash, requestsBefore: Number(usage?.total || 0), costBefore: Number(usage?.cost || 0) };
}
...
async function qualifyWaveBatch2V13Activation(context: Awaited<ReturnType<typeof waveBatch2V13ActivationContext>>, progress: Row = {}) {
  ...
  for (const target of scope.slice(startIndex, endIndex)) {
    ...
    const sceneSpecComplete = clean(spec.version) === "CONTRACT_NATIVE_SCENE_SPECIFICATION_V13" && objects.length >= 3 && relationships.length === 3 && states.length === 3 && objects.every((item) => Boolean(clean(item.type)) && Boolean(clean(item.sourceClause)) && Object.keys(rec(item.geometry)).length === 4) && relationships.every((item) => Boolean(clean(item.sourceClause)));
    const pixelTraceComplete = states.every((item) => arr(item.visibleObjectIds).length >= 3 && Boolean(clean(item.activeRelationshipId)) && arr(item.movingObjectPosition).length === 2) && rendererContract.input === "SCENE_SPECIFICATION_ONLY" && rendererContract.tokenFallbackForbidden === true && rendererContract.geometryAuthoritative === true;
...
async function preflightWaveBatch2V13Activation() {
  ...
  await context.db.prepare("INSERT INTO v7_batch_activation_preflights (id,program_id,run_id,authorization_id,action,source_batch_id,source_audit_id,target_batch_id,input_hash,status,result_json,requests_before,cost_before,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET source_batch_id=excluded.source_batch_id,source_audit_id=excluded.source_audit_id,input_hash=excluded.input_hash,status=excluded.status,result_json=excluded.result_json,requests_before=excluded.requests_before,cost_before=excluded.cost_before,updated_at=excluded.updated_at").bind(preflightId, PROGRAM_ID, context.run.id, context.authorization.id, "PREFLIGHT_WAVE_BATCH_2_V13_ACTIVATION", context.rejected.id, context.audit.id, context.replacementId, context.inputHash, result.status, JSON.stringify(result), context.requestsBefore, context.costBefore, now, now).run();
  const readBack = await context.db.prepare("SELECT * FROM v7_batch_activation_preflights WHERE id=? AND input_hash=?").bind(preflightId, context.inputHash).first<Row>();
...
async function verifyCommittedWaveBatch2V13Activation(db: DB, runId: string, authorizationId: string, replacementId: string, rejectedId: string, activationId: string) {
  const [activation, targetBatch, sourceBatch, sourceProducts, active] = await Promise.all([
    ...
    db.prepare("SELECT * FROM v7_production_batches WHERE id=? AND run_id=?").bind(rejectedId, runId).first<Row>(),
    db.prepare("SELECT COUNT(*) AS total FROM v7_shot_products WHERE batch_id=? AND engine_version=? AND status='PRODUCT_COMPLETE_REJECTED_ENGINE_EVIDENCE'").bind(rejectedId, WAVE_BATCH_2_V12_ENGINE_VERSION).first<{ total: number }>(),
    db.prepare("SELECT COUNT(*) AS total FROM v7_material_requests WHERE authorization_id=? AND status IN ('QUEUED','IN_PROGRESS')").bind(authorizationId).first<{ total: number }>(),
  ]);
  const targetStatus = clean(targetBatch?.status), committedTarget = ["PRODUCING", "PRODUCT_COMPLETE", "PREPARING", "DISPATCHING", "QA_RUNNING", "PASS", "ENGINE_ROOT_CAUSE_REQUIRED", "AUDIT_INCOMPLETE", "PRODUCTION_BLOCKED"].includes(targetStatus);
  return Boolean(activation && targetBatch && committedTarget && (targetStatus !== "PRODUCING" || Number(active?.total || 0) === 0) && clean(sourceBatch?.status) === "ENGINE_ROOT_CAUSE_PRESERVED" && Number(sourceProducts?.total || 0) === 50);
...
async function adoptWaveBatch2V13EngineRootCorrection() {
  ...
  const rootCausePolicy = { ...rec(JSON.parse(String(rejected.root_cause_policy_json || "{}"))), incident: "BATCH_2_V12_TOKEN_DERIVED_ONTOLOGY_RENDERER_FAILURE", rejectedEngineVersion: WAVE_BATCH_2_V12_ENGINE_VERSION, rejectedAuditId: audit.id, rejectedAuditScore: Number(audit.score), observedRootCause: priorRoot.rootProductionCause, correctedLayers: ["CONTRACT_NATIVE_SCENE_SPECIFICATION", "TYPED_VISUAL_OBJECTS", "AUTHORITATIVE_GEOMETRY", "SCENE_SPEC_PIXEL_TRACE", "PHYSICAL_TRANSITIONS", "PORTFOLIO_LANGUAGE_POLICY"], correction: "CONTRACT_TO_TYPED_OBJECTS_RELATIONSHIPS_GEOMETRY_TRANSITIONS_TO_PIXELS", replacementEngineVersion: WAVE_BATCH_2_V13_ENGINE_VERSION, fullScopeRegression: "50_SCENE_SPECS_50_PIXEL_TRACES_150_FRAMES_LAYOUT_SAFE", portfolioPolicy, reproduceScope: "ALL_50_PRODUCTS", outputRepair: false, v8V9V10V11V12ProductsPreservedAsEvidence: true, retryPriorAudits: false };
  const productionDoD = { ...rec(JSON.parse(String(rejected.production_dod_json || "{}"))), version: "SHOT_PRODUCT_DOD_V13", contractNativeSceneSpecification: true, typedObjectsAndRelationships: true, rendererInputSceneSpecificationOnly: true, semanticToPixelTraceRequired: true, physicalMotionNotLabelSwap: true, clippingConnectorAndObjectOverlapZero: true, portfolioPolicy, fullScopeRegression: "50_OF_50_PASS" };
  const modelPolicy = { ...rec(JSON.parse(String(authorization.model_policy_json || "{}"))), version: WAVE_BATCH_2_V13_REPRODUCTION_VERSION, productionEngine: WAVE_BATCH_2_V13_ENGINE_VERSION, scope: "REPRODUCE_ALL_50_AFTER_V12_ENGINE_REJECTION", deterministicProductionRequests: 0, independentAuditRequests: 1, autoRepair: false, priorAuditRetry: false, qaFailureRoute: "ROOT_PRODUCTION_PROCESS" };
  ...
  try {
    await db.batch([
      db.prepare("UPDATE v7_shot_products SET status='PRODUCT_COMPLETE_REJECTED_ENGINE_EVIDENCE',updated_at=? WHERE batch_id=? AND engine_version=? AND status='PRODUCT_COMPLETE'").bind(now, rejected.id, WAVE_BATCH_2_V12_ENGINE_VERSION),
      db.prepare("UPDATE v7_production_batches SET status='ENGINE_ROOT_CAUSE_PRESERVED',updated_at=? WHERE id=? AND status='ENGINE_ROOT_CAUSE_REQUIRED'").bind(now, rejected.id),
      ...
      db.prepare("UPDATE v7_material_authorizations SET scope='WAVE_09_BATCH_2_ENGINE_V13_REPRODUCTION',status='PAUSED',shot_count=50,max_remote_requests=?,max_actual_spend_usd=?,model_policy_json=?,completed_at=NULL,updated_at=? WHERE id=?").bind(requestsBefore + 1, costBefore + 10, JSON.stringify(modelPolicy), now, authorization.id),
      db.prepare("UPDATE v7_material_runs SET status='BATCH_2_V13_REPRODUCING',mode='CONTRACT_NATIVE_SCENE_REPRODUCTION' WHERE id=?").bind(run.id),
      db.prepare("UPDATE v7_stage_states SET status='BATCH_2_V13_REPRODUCING',blocker='INTEGRATED_PRODUCTION_TRANSACTION',evidence_summary='V8/V9/V10/V11/V12 evidence preserved · V13 50/50 contract-native scene specifications and pixel traces, 150/150 frames, physical transitions and 10-family portfolio gates PASS · no output repair · prior audits not retried',updated_at=? WHERE id=?").bind(now, STAGE_ID),
      db.prepare("INSERT INTO v7_batch_activations (id,program_id,run_id,authorization_id,action,idempotency_key,target_batch_id,input_hash,status,result_json,created_at,committed_at,updated_at) VALUES (?,?,?,?,?,?,?,?, 'COMMITTED',?,?,?,?)").bind(activationId, PROGRAM_ID, run.id, authorization.id, "ADOPT_WAVE_BATCH_2_V13_ENGINE_ROOT_CORRECTION", activationId, replacementId, context.inputHash, JSON.stringify(activationResult), now, now, now),
    ]);
  } catch (error) {
