...
const WAVE_BATCH_2_V13_REJECTED_ENGINE_VERSION = "SHOT_PRODUCT_ENGINE_V14_EXECUTABLE_STORYBOARD_COMPILER";
const WAVE_BATCH_2_V13_REPRODUCTION_VERSION = "WAVE_09_BATCH_2_V8_CONTRACT_NATIVE_COMPOSITION_REPRODUCTION";
const WAVE_BATCH_2_V13_ENGINE_VERSION = "SHOT_PRODUCT_ENGINE_V15_CONTRACT_NATIVE_COMPOSITION_COMPILER";
...
async function snapshot() {
  ...
  const expectedPreflightAction = v15ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V15_ACTIVATION" : v13ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V13_ACTIVATION" : v12ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V12_ACTIVATION" : v11ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" : "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION";
  const expectedActivationAction = v15ControlVisible ? "ADOPT_WAVE_BATCH_2_V15_ENGINE_ROOT_CORRECTION" : v13ControlVisible ? "ADOPT_WAVE_BATCH_2_V13_ENGINE_ROOT_CORRECTION" : v12ControlVisible ? "ADOPT_WAVE_BATCH_2_V12_ENGINE_ROOT_CORRECTION" : v11ControlVisible ? "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" : "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION";
...
function contractNativeSceneSpecification(contract: Row, bindings: Row[], actors: string[], kind: string) {
  ...
  const auditedCompositions: Record<string, { composition: string; labels: string[]; actions: string[]; persistentLabels: string[] }> = {
    ...
    "MP-040": { composition: "MERCHANT_TO_PROCESSOR_BOUNDARY", labels: ["MERCHANT TERMINAL", "PROCESSOR BOUNDARY", "BOUNDARY SHUTTER", "AUTHORIZATION REQUEST"], actions: ["REQUEST AT MERCHANT", "SHUTTER OPENS", "REQUEST DEPARTS TO PROCESSOR"], persistentLabels: ["ECONOMIC SHUTTER CLOSED"] },
  ...
  return {
    version: "CONTRACT_NATIVE_COMPOSITION_SPECIFICATION_V15", layoutFamily, composition: auditedBlueprint?.composition || `CONTRACT_NATIVE_${layoutFamily}`, persistentLabels: auditedBlueprint?.persistentLabels || [], canvas: { width: 960, height: 540, safeInset: 48 },
    ...
    rendererContract: { input: "CONTRACT_NATIVE_COMPOSITION_ONLY", tokenFallbackForbidden: true, geometryAuthoritative: true, audienceTextBudget: { titleGlyphs: 30, objectGlyphs: 22, actionGlyphs: 34, contractClausePixelsForbidden: true }, provenanceRequiredForEveryVisibleObject: true, provenanceRequiredForEveryRelationship: true },
  };
}
...
function renderWaveSemanticScene(manifest: Row, state: 0 | 1 | 2) {
  ...
  if(contractNativeScene){
    ...
    if(auditedComposition){
      fill(48,42,864,82,theme[1]);center(composition.replaceAll("_"," "),480,54,2,theme[2],42);center(["ENTRY","MIDPOINT","EXIT"][state],480,92,2,accent,12);
      fill(48,136,864,310,"#0b241f");fill(64,148,832,38,"#061b18");center(sceneState.action,480,158,2,accent,40);
      if(composition==="ISSUER_LEDGER_CONTAINMENT"||composition==="ISSUER_SIDE_NOT_NETWORK_REVENUE"||composition==="SEPARATE_VALUE_CONTAINERS"){
        const leftLabel=composition==="SEPARATE_VALUE_CONTAINERS"?"ISSUER LEDGER":composition==="ISSUER_SIDE_NOT_NETWORK_REVENUE"?"ISSUER SIDE / LEDGER":"ISSUER LEDGER",rightLabel=composition==="ISSUER_LEDGER_CONTAINMENT"?"NETWORK WORKSPACE":"NETWORK TRAY";
        fill(76,208,356,176,"#183d36");fill(528,208,356,176,"#183d36");center(leftLabel,254,224,2,theme[2],24);center(rightLabel,706,224,2,theme[2],24);
        const interchangeX=state===0?128:state===1?274:state===2?304:128;nativeCard(interchangeX,276,112,60,"INTERCHANGE",true);
        if(composition==="SEPARATE_VALUE_CONTAINERS"){if(state===0){center("EMPTY",706,298,3,"#9abbb0",8);}else{const feeX=state===1?486:666;nativeCard(feeX,276,126,60,"SERVICE FEES",true);}}
        else center("EMPTY",706,298,3,"#9abbb0",8);
        if(state>=1)nativeArrow(240,306,interchangeX+8,306,true);fill(82,394,796,34,"#342014");center(persistent[0],480,404,2,"#fff0df",40);
      }else if(composition==="FIVE_POSITION_ROLE_MAP"||composition==="FIVE_WORK_SURFACES"){
        const labels=objects.slice(0,5).map((item)=>clean(item.label)),verbs=["PRESENTS","ACCEPTS","ROUTES","SWITCHES","ISSUES"];
        labels.forEach((label,index)=>{const x=70+index*168;nativeCard(x,218,148,composition==="FIVE_WORK_SURFACES"?126:78,label,index<=state+2);if(composition==="FIVE_WORK_SURFACES")center(verbs[index],x+74,316,2,accent,14);if(index<4)nativeArrow(x+148,257,x+168,257,index<state+2);});
        if(state>=1){fill(96,360,280,36,"#102e55");center("EXAMPLE ROLE A",236,370,2,"#e3f0ff",18);fill(584,360,280,36,"#102e55");center("EXAMPLE ROLE B",724,370,2,"#e3f0ff",18);}
        if(state===2){line(60,410,900,410,8,"#e8b65d");line(60,388,60,424,8,"#e8b65d");line(900,388,900,424,8,"#e8b65d");center(persistent[0],480,414,2,"#fff0df",40);}
      }else if(composition==="CONTROLLED_SCENARIO_TABLE"){
        const cols=["PRODUCT","CATEGORY","CHANNEL","DATE"];cols.forEach((label,index)=>{const x=88+index*204;fill(x,210,184,44,"#102e55");center(label,x+92,222,2,"#e3f0ff",16);fill(x,258,184,88,state>=1?"#eef5f1":"#173d35");center(state>=1?["REWARDS CARD","PURCHASE","ONLINE","MAY 2026"][index]:"PIN",x+92,290,2,state>=1?"#173d35":"#9abbb0",18);});
        fill(76,194,808,10,accent);if(state===2){line(66,186,894,186,8,"#e8b65d");line(66,366,894,366,8,"#e8b65d");line(66,186,66,366,8,"#e8b65d");line(894,186,894,366,8,"#e8b65d");}
        fill(172,388,616,38,"#342014");center(persistent[0],480,399,2,"#fff0df",40);
      }else{
        const routeLabels=composition==="MERCHANT_TO_PROCESSOR_BOUNDARY"?["MERCHANT TERMINAL","PROCESSOR BOUNDARY"]:composition==="PROCESSOR_ACQUIRER_DESTINATION"?["MERCHANT","PROCESSOR OR ACQUIRER","NETWORK INACTIVE"]:composition==="INFORMATION_TO_NETWORK"?["PROCESSOR OR ACQUIRER","OUTBOUND LANE","NETWORK","ISSUER INACTIVE"]:["NETWORK","PROCESSOR OR ACQUIRER","MERCHANT","MERCHANT TERMINAL"];
        const count=routeLabels.length,step=count===2?420:count===3?280:210;routeLabels.forEach((label,index)=>{const x=70+index*step;nativeCard(x,252,count===2?300:count===3?230:184,84,label,index<=state+1);if(index<count-1)nativeArrow(x+(count===2?300:count===3?230:184),294,x+step,294,index<state+1);});
        if(composition==="MERCHANT_TO_PROCESSOR_BOUNDARY"){fill(450,202,18,194,"#e8b65d");for(let s=0;s<6;s++)fill(434,214+s*30,50,8,s%2?"#342014":"#fff0df");center("ECONOMIC SHUTTER CLOSED",480,410,2,"#e8b65d",30);}
        if(composition==="INFORMATION_TO_NETWORK")center("INFORMATION ONLY",480,214,3,"#83c5ff",20);
        if(composition==="RESPONSE_FOUR_BOUNDARIES")center("1             2             3",480,372,3,"#e8b65d",38);
        fill(112,396,736,32,"#342014");center(persistent[0],480,405,2,"#fff0df",46);
      }
    }else{
  ...
  if(!specialized.includes(kind))[0,1,2].forEach((index)=>fill(382+index*72,518,54,8,index<=state?accent:"#315447"));
  for(let y=0;y<height;y++){const row=y*(1+width*4);raw[row]=0;raw.set(pixels.subarray(y*width*4,(y+1)*width*4),row+1);}const ihdr=new Uint8Array(13);ihdr.set(u32(width),0);ihdr.set(u32(height),4);ihdr.set([8,6,0,0,0],8);const bytes=joinBytes([new Uint8Array([137,80,78,71,13,10,26,10]),pngChunk("IHDR",ihdr),pngChunk("IDAT",deflateStored(raw)),pngChunk("IEND",new Uint8Array())]);return{bytes,pixels,width,height};
}
...
function waveProductOracle(manifest: Row) {
  ...
  const checks=[
    ...
    {id:"CONTRACT_NATIVE_SCENE_SPECIFICATION",status:!contractNativeScene||clean(sceneSpec.version)==="CONTRACT_NATIVE_COMPOSITION_SPECIFICATION_V15"&&Boolean(clean(sceneSpec.composition))&&sceneObjects.length>=3&&sceneRelationships.length===3&&sceneStates.length===3&&sceneObjects.every((item)=>Boolean(clean(item.id))&&Boolean(clean(item.type))&&Boolean(clean(item.label))&&Boolean(clean(item.sourceClause))&&Object.keys(rec(item.geometry)).length===4)&&sceneRelationships.every((item)=>Boolean(clean(item.from))&&Boolean(clean(item.to))&&Boolean(clean(item.relation))&&Boolean(clean(item.sourceClause)))?"PASS":"FAIL",evidence:`${sceneObjects.length} domain primitives · ${sceneRelationships.length} actions · ${sceneStates.length} states`},
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&Boolean(clean(item.activeRelationshipId))&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="CONTRACT_NATIVE_COMPOSITION_ONLY"&&rendererContract.tokenFallbackForbidden===true&&rendererContract.geometryAuthoritative===true?"PASS":"FAIL",evidence:"renderer consumes only contract-native composition, authoritative geometry and action timeline"},
...
function waveManifestQualification(manifest: Row) {
  const states=arr(manifest.states).map(rec),semantic=rec(manifest.semanticModel),required=arr(semantic.requiredElements).map(clean).filter(Boolean),contractNativeScene=clean(semantic.version)==="CONTRACT_NATIVE_COMPOSITION_SPECIFICATION_V15",structuredOntology=clean(semantic.version)==="STRUCTURED_VISUAL_ONTOLOGY_V12",semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11"||structuredOntology||contractNativeScene,archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),ontology=rec(semantic.visualOntology),ontologyStates=arr(ontology.states).map(rec),layoutProof=rec(ontology.layoutProof),motionPolicy=rec(ontology.motionPolicy),sceneSpec=rec(semantic.sceneSpecification),sceneObjects=arr(sceneSpec.objects).map(rec),sceneRelationships=arr(sceneSpec.relationships).map(rec),sceneStates=arr(sceneSpec.states).map(rec),sceneLayoutProof=rec(sceneSpec.layoutProof),sceneMotionProof=rec(sceneSpec.motionProof),rendererContract=rec(sceneSpec.rendererContract),corpus=clean(semantic.corpusHashInput).toUpperCase(),genericEscape=contractBound&&["$100 PURCHASE","REWARD ENTRY"].some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy),checks=[
    {id:"SEMANTIC_SCENE_GRAPH",status:["CONTRACT_SIGNATURE_SCENE_GRAPH_V5_SPATIAL_BOUND","CONTRACT_BOUND_SCENE_GRAPH_V9","ARCHETYPE_SEMANTIC_MANIFEST_V10","CONTRACT_SEMANTIC_PROJECTION_V11","STRUCTURED_VISUAL_ONTOLOGY_V12","CONTRACT_NATIVE_COMPOSITION_SPECIFICATION_V15"].includes(clean(semantic.version))&&required.length>=3?"PASS":"FAIL"},
    ...
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="CONTRACT_NATIVE_COMPOSITION_ONLY"&&rendererContract.tokenFallbackForbidden===true?"PASS":"FAIL"},
...
async function qualifyWaveBatch2V13Activation(context: Awaited<ReturnType<typeof waveBatch2V13ActivationContext>>, progress: Row = {}) {
  ...
  for (const target of scope.slice(startIndex, endIndex)) {
    ...
    const sceneSpecComplete = clean(spec.version) === "CONTRACT_NATIVE_COMPOSITION_SPECIFICATION_V15" && Boolean(clean(spec.composition)) && objects.length >= 3 && relationships.length === 3 && states.length === 3 && objects.every((item) => Boolean(clean(item.type)) && Boolean(clean(item.sourceClause)) && Object.keys(rec(item.geometry)).length === 4) && relationships.every((item) => Boolean(clean(item.sourceClause)));
    const pixelTraceComplete = states.every((item) => arr(item.visibleObjectIds).length >= 3 && Boolean(clean(item.activeRelationshipId)) && arr(item.movingObjectPosition).length === 2) && rendererContract.input === "CONTRACT_NATIVE_COMPOSITION_ONLY" && rendererContract.tokenFallbackForbidden === true && rendererContract.geometryAuthoritative === true;
...
async function preflightWaveBatch2V13Activation() {
  ...
  await context.db.prepare("INSERT INTO v7_batch_activation_preflights (id,program_id,run_id,authorization_id,action,source_batch_id,source_audit_id,target_batch_id,input_hash,status,result_json,requests_before,cost_before,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET source_batch_id=excluded.source_batch_id,source_audit_id=excluded.source_audit_id,input_hash=excluded.input_hash,status=excluded.status,result_json=excluded.result_json,requests_before=excluded.requests_before,cost_before=excluded.cost_before,updated_at=excluded.updated_at").bind(preflightId, PROGRAM_ID, context.run.id, context.authorization.id, "PREFLIGHT_WAVE_BATCH_2_V15_ACTIVATION", context.rejected.id, context.audit.id, context.replacementId, context.inputHash, result.status, JSON.stringify(result), context.requestsBefore, context.costBefore, now, now).run();
  const readBack = await context.db.prepare("SELECT * FROM v7_batch_activation_preflights WHERE id=? AND input_hash=?").bind(preflightId, context.inputHash).first<Row>();
...
async function adoptWaveBatch2V13EngineRootCorrection() {
  ...
  const rootCausePolicy = { ...rec(JSON.parse(String(rejected.root_cause_policy_json || "{}"))), incident: "BATCH_2_V14_GENERIC_COMPOSITION_AND_CONTRACT_PLACEMENT_FAILURE", rejectedEngineVersion: WAVE_BATCH_2_V13_REJECTED_ENGINE_VERSION, rejectedAuditId: audit.id, rejectedAuditScore: Number(audit.score), observedRootCause: priorRoot.rootProductionCause, correctedLayers: ["CONTRACT_NATIVE_COMPOSITION_COMPILER", "CONTAINMENT_OWNERSHIP", "BOUNDARY_ORDER", "ROLE_MAP", "CONTROLLED_SCENARIO_TABLE", "PORTFOLIO_LANGUAGE_POLICY"], correction: "CONTRACT_CLAUSE_TO_NATIVE_COMPOSITION_TO_EXACT_PIXELS", replacementEngineVersion: WAVE_BATCH_2_V13_ENGINE_VERSION, fullScopeRegression: "50_SCENE_SPECS_50_PIXEL_TRACES_150_FRAMES_LAYOUT_SAFE", portfolioPolicy, reproduceScope: "ALL_50_PRODUCTS", outputRepair: false, v8V9V10V11V12V13V14ProductsPreservedAsEvidence: true, retryPriorAudits: false };
  const productionDoD = { ...rec(JSON.parse(String(rejected.production_dod_json || "{}"))), version: "SHOT_PRODUCT_DOD_V15", contractNativeCompositionSpecification: true, explicitContainmentAndOwnership: true, exactBoundaryOrder: true, rendererInputContractNativeCompositionOnly: true, semanticToPixelTraceRequired: true, physicalMotionNotLabelSwap: true, audienceTextBudgetEnforced: true, clippingConnectorAndObjectOverlapZero: true, portfolioPolicy, fullScopeRegression: "50_OF_50_PASS" };
  const modelPolicy = { ...rec(JSON.parse(String(authorization.model_policy_json || "{}"))), version: WAVE_BATCH_2_V13_REPRODUCTION_VERSION, productionEngine: WAVE_BATCH_2_V13_ENGINE_VERSION, scope: "REPRODUCE_ALL_50_AFTER_V14_ENGINE_REJECTION", deterministicProductionRequests: 0, independentAuditRequests: 1, autoRepair: false, priorAuditRetry: false, qaFailureRoute: "ROOT_PRODUCTION_PROCESS" };
  ...
  try {
    await db.batch([
      ...
      db.prepare("UPDATE v7_material_authorizations SET scope='WAVE_09_BATCH_2_ENGINE_V15_REPRODUCTION',status='PAUSED',shot_count=50,max_remote_requests=?,max_actual_spend_usd=?,model_policy_json=?,completed_at=NULL,updated_at=? WHERE id=?").bind(requestsBefore + 1, costBefore + 10, JSON.stringify(modelPolicy), now, authorization.id),
      db.prepare("UPDATE v7_material_runs SET status='BATCH_2_V15_REPRODUCING',mode='CONTRACT_NATIVE_COMPOSITION_REPRODUCTION' WHERE id=?").bind(run.id),
      db.prepare("UPDATE v7_stage_states SET status='BATCH_2_V15_REPRODUCING',blocker='INTEGRATED_PRODUCTION_TRANSACTION',evidence_summary='V8-V14 evidence preserved · V15 50/50 contract-native compositions and pixel traces, 150/150 frames, explicit containment/boundary actions and portfolio gates PASS · no output repair · prior audits not retried',updated_at=? WHERE id=?").bind(now, STAGE_ID),
      db.prepare("INSERT INTO v7_batch_activations (id,program_id,run_id,authorization_id,action,idempotency_key,target_batch_id,input_hash,status,result_json,created_at,committed_at,updated_at) VALUES (?,?,?,?,?,?,?,?, 'COMMITTED',?,?,?,?)").bind(activationId, PROGRAM_ID, run.id, authorization.id, "ADOPT_WAVE_BATCH_2_V15_ENGINE_ROOT_CORRECTION", activationId, replacementId, context.inputHash, JSON.stringify(activationResult), now, now, now),
    ]);
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "PREFLIGHT_WAVE_BATCH_2_V15_ACTIVATION") return Response.json(await preflightWaveBatch2V13Activation(), { status: 200 });
    if (body.action === "ADOPT_WAVE_BATCH_2_V15_ENGINE_ROOT_CORRECTION") return Response.json(await adoptWaveBatch2V13EngineRootCorrection(), { status: 201 });
