...
Compositor-admission deployment receipt: migration `0110`, exact asset-eligibility verification, qualified pixel/video compositor and integrated-canary receipt path built and deployed as Sites version 523; the live D1 overview verified 54 user tables in total and thirty-four `factory_*` tables, including all four new tables. The tracked 60-second SOURCE/MAKE/HYBRID fixture produced identical VP9 bytes on two executions, while live feature flags remained disabled and the new tables contained zero rows. No provider request, spend, R21/R22 mutation, master, release or publication occurred.

Live-canary qualification receipt: migration `0111`, clean lease release and the bounded non-R22 canary/orphan/replay runner first deployed disabled in Sites version 524. A qualification-action-only constant-time temporary credential and explicit `NON_R22` admission correction were added through forward-only exact commits `879b791…` and `1638326…`, with the complete source deployed as version 527. Exactly one bounded live run stored qualification receipt `factory-live-canary-qualification-6d527fcba653a020acfba9a6`, integrated canary receipt `factory-integrated-canary-cb7ff0c35a03a21f6dd5ddb6`, exact output hash `cb7ff0c35a03a21f6dd5ddb6b7c72c6056e35cfbf94e15559b32ceb5150adb21`, two PASS replay receipts, one released lease and one reconciled orphan. D1/R2 read-back verified zero dispatch and zero spend. The writer flag, runner flag and temporary token were removed, then the same source was redeployed at environment revision 46. No R21/R22, master, release or publication state changed.
