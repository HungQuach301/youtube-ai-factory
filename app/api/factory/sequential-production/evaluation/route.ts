  ...
  try {
    ...
    if (["owner-label-task", "owner-label-workflow", "provider-history-recovery"].includes(view) || artifactCandidateId) {
      ...
      if (view === "provider-history-recovery") {
        ...
        const result = snapshot ? `<section class="result"><h2>Kết quả đã niêm phong</h2><dl><div><dt>History items</dt><dd>${number(snapshot.history_items_received)}</dd></div><div><dt>Có request ID</dt><dd>${number(snapshot.history_items_with_native_request_id)}</dd></div><div><dt>Candidate đã chẩn đoán</dt><dd>${number(snapshot.candidates_diagnosed)}</dd></div><div><dt>Khớp metadata duy nhất</dt><dd>${number(snapshot.unique_metadata_matches)}</dd></div><div><dt>Không khớp</dt><dd>${number(snapshot.no_metadata_matches)}</dd></div><div><dt>Mơ hồ</dt><dd>${number(snapshot.ambiguous_metadata_matches)}</dd></div><div><dt>Gói hiện tại</dt><dd>${escapeHtml(snapshot.subscription_tier)} · ${escapeHtml(snapshot.subscription_status)}</dd></div><div><dt>Provider requests</dt><dd>${number(snapshot.provider_requests)}</dd></div></dl><p>Subscription chỉ là quan sát hiện tại; không chứng minh gói lịch sử. Kết quả này không cấp rights, không seal dataset, không qualify assurance và không mở release.</p></section>` : `<form method="post" action="/api/factory/sequential-production/evaluation"><input type="hidden" name="action" value="DISCOVER_ELEVENLABS_HISTORY_METADATA"><input type="hidden" name="idempotencyKey" value="provider-history-discovery-2026-08-22-v1"><button type="submit">Chạy khám phá có giới hạn</button></form>`;
        return new Response(`<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Khôi phục evidence provider</title><style>*{box-sizing:border-box}body{margin:0;padding:24px;background:#101f19;color:#d9e8e1;font:15px Arial,sans-serif}main{max-width:900px;margin:4vh auto;padding:28px;border:1px solid #365349;border-radius:18px;background:#13271f}small{color:#a8e8ca;font-weight:800}h1,h2{font-family:Georgia,serif}p,li,dt{color:#a9bdb5;line-height:1.6}.scope{margin:22px 0;padding:18px;border-radius:14px;background:#0b1712}.scope strong{color:#d9f7e9}button{min-height:48px;padding:0 18px;border:0;border-radius:10px;background:#a8e8ca;color:#09291e;font-weight:800;cursor:pointer}dl{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}dl div{padding:12px;border-radius:10px;background:#0b1712}dt{font-size:11px}dd{margin:6px 0 0;font-size:22px;font-weight:800}.notice{padding:12px;border-radius:10px;background:#173c2c;color:#bcefd5}@media(max-width:700px){body{padding:12px}main{padding:18px}dl{grid-template-columns:1fr 1fr}}</style></head><body><main><small>OWNER-AUTHENTICATED · FAIL-CLOSED</small><h1>Khôi phục evidence lịch sử ElevenLabs</h1>${recorded ? '<p class="notice">Đã hoàn tất lệnh và đọc lại snapshot bất biến.</p>' : ''}<section class="scope"><strong>Phạm vi cố định</strong><ul><li>Đọc tối đa 1.000 history items trong đúng cửa sổ thời gian của 46 candidate.</li><li>Đọc subscription hiện tại; không suy diễn quyền lịch sử.</li><li>Tối đa 2 provider requests, không TTS và không phát sinh spend.</li><li>Chỉ chẩn đoán metadata; muốn bind audio vẫn phải hash bytes chính xác.</li></ul></section>${result}</main></body></html>`, { headers: { ...NO_STORE, "content-type": "text/html; charset=utf-8", "x-frame-options": "SAMEORIGIN" } });
      }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!["RUN_CORPUS_VERIFICATION_BATCH", "RECORD_OWNER_LABEL_RECEIPT", "DISCOVER_ELEVENLABS_HISTORY_METADATA"].includes(action)) throw new EvaluationCommandError("EVALUATION_ACTION_INVALID", 400, "Use RUN_CORPUS_VERIFICATION_BATCH, RECORD_OWNER_LABEL_RECEIPT, or DISCOVER_ELEVENLABS_HISTORY_METADATA");
