...
- Audience Golden Sequence R1–R13 are preserved rejected evidence. Sites v494 sealed R13 exact master `bc62a2a2…`; audio QA passes 95, while visual QA fails 64 because approval remains ambiguous and accurate equation/netting panels persist too long as slides. Migration `0098` prepares R14 as 32 distinct causal states with one composition per evidence sample, explicit approval before hold, four-stage clearing/netting and one full-frame exception branch at a time. Browser/device, owner review and freeze remain blocked until their owning gates open. Document 78 is authoritative; V2 is untouched.
...
Phase 36 is active. Sites v494 materializes R13 master `bc62a2a23310dc81ddbbff1b5407c7e1e9019787aeac72f5508ea6012aa561ae`. Audio QA passes at 95 with no P0/P1. Visual QA fails at 64 with three P1 findings: authorization remains ambiguous, the explicit equation holds across six samples, and the labeled netting panel/translated token hold across six more. P2 adds mobile crowding and an abrupt all-at-once exception chart. Migration `0098` creates R14 only from the exact R13 visual FAIL and audio PASS receipts. R14 uses 32 distinct causal states/compositions, explicit approval before hold, four-stage clearing/netting and one full-frame branch per exception. The corrected 32-frame atlas has no adjacent repeated composition. Complete verification, commit/push, deploy owner-only and execute R14; Browser/device and owner gates remain closed until Factory PASS. V2 is untouched.
