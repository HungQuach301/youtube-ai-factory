...
type Env = { DB?: DB; BUCKET?: Bucket; ELEVENLABS_API_KEY?: string; OPENAI_API_KEY?: string; FACTORY_EXPERT_EMAILS?: string; FACTORY_AUTOMATION_ACTOR_EMAIL?: string; FACTORY_AUTOMATION_ACTOR_NAME?: string; SEQUENTIAL_EXECUTOR_TOKEN?: string; CLEAN_AUDIO_CONTROL_AUTOMATION_TOKEN?: string; CONTROLLED_DEFECT_AUTOMATION_TOKEN?: string };
...
async function authorized(request: Request, allowAutomation = true, allowCleanControlAutomation = false, allowControlledDefectAutomation = false) {
  ...
  if (!user && (generalAutomation || cleanControlAutomation || controlledDefectAutomation)) {
  ...
  try {
    ...
    if (["owner-label-task", "owner-label-workflow", "provider-history-recovery", "commercial-clean-audio"].includes(view) || artifactCandidateId || fixtureArtifactId) {
      ...
      if (view === "commercial-clean-audio") {
        const [commercialAudio, ownerGroundTruth, cleanControlEligibility, controlledDefectDerivation] = await Promise.all([commercialCleanAudioSnapshot(env.DB), cleanAudioOwnerGroundTruthSnapshot(env.DB), cleanAudioControlEligibilitySnapshot(env.DB), controlledDefectDerivationSnapshot(env.DB)]);
        return Response.json({ ...commercialAudio, ownerGroundTruth, cleanControlEligibility, controlledDefectDerivation }, { headers: NO_STORE });
      }
      if (view === "provider-history-recovery") {
        ...
        const [materialization, rightsEvidence, commercialAudio, ownerGroundTruth, cleanControlEligibility, controlledDefectDerivation] = await Promise.all([controlledFixtureMaterializationSnapshot(env.DB), currentRightsEvidenceSnapshot(env.DB), commercialCleanAudioSnapshot(env.DB), cleanAudioOwnerGroundTruthSnapshot(env.DB), cleanAudioControlEligibilitySnapshot(env.DB), controlledDefectDerivationSnapshot(env.DB)]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { env, actor } = await authorized(request, !["RECORD_OWNER_LABEL_RECEIPT", "RECORD_CLEAN_AUDIO_OWNER_GROUND_TRUTH"].includes(action), action === "EVALUATE_CLEAN_AUDIO_CONTROL_ELIGIBILITY", action === "DERIVE_RIGHTS_LINEAGE_MISSING_CONTROL");
    if (!["RUN_CORPUS_VERIFICATION_BATCH", "RECORD_OWNER_LABEL_RECEIPT", "DISCOVER_ELEVENLABS_HISTORY_METADATA", "HASH_ELEVENLABS_HISTORY_AUDIO", "MATERIALIZE_CLEAN_AUDIO_CONTROL", "CAPTURE_CURRENT_COMMERCIAL_RIGHTS_EVIDENCE", "REGENERATE_COMMERCIAL_CLEAN_AUDIO_CONTROL", "RUN_FACTORY_CLEAN_AUDIO_QA", "RECORD_CLEAN_AUDIO_OWNER_GROUND_TRUTH", "EVALUATE_CLEAN_AUDIO_CONTROL_ELIGIBILITY", "DERIVE_RIGHTS_LINEAGE_MISSING_CONTROL"].includes(action)) throw new EvaluationCommandError("EVALUATION_ACTION_INVALID", 400, "Use a supported evaluation action");
