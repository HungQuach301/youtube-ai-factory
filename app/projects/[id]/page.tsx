...
function ContinuousOptimizationSystem({ projectId, setProjectNotice }: { projectId: string; setProjectNotice: (message: string) => void }) {
  ...
  async function optimizationAction(action: "START_CYCLE" | "RUN_STAGE" | "RUN_WAVE" | "SET_MODE" | "APPROVE_HANDOFF", input: { mode?: "AUTOPILOT" | "EXCEPTIONS" | "MANUAL" } = {}) {
    setWorking(action); const labels = { START_CYCLE: "Opening a controlled rebuild cycle and invalidating weak downstream artifacts…", RUN_STAGE: "Running the active candidate tournament and independent micro-gates…", RUN_WAVE: "Autopilot is executing a bounded optimization wave…", SET_MODE: "Updating optimization governance…", APPROVE_HANDOFF: "Approving the selected champion and evidence packet…" }; setProjectNotice(labels[action]);
    try { const response = await fetch(`/api/projects/${projectId}/optimization`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, ...input }) }); const payload = await response.json().catch(() => ({})) as { error?: string; version?: number; stageLabel?: string; score?: number; threshold?: number; status?: string; winner?: string; count?: number }; if (!response.ok) throw new Error(payload.error || "Optimization action stopped safely"); await loadOptimization(); if (action === "START_CYCLE") setProjectNotice(`Optimized rebuild cycle v${payload.version} started · Creative Strategy is active`); else if (action === "RUN_STAGE") setProjectNotice(`${payload.stageLabel || "Stage"} tournament · ${payload.score}/${payload.threshold} · ${(payload.status || "").replaceAll("_", " ")}`); else if (action === "RUN_WAVE") setProjectNotice(`Bounded Autopilot wave completed · ${payload.count || 0} stage contracts processed`); else if (action === "APPROVE_HANDOFF") setProjectNotice("Champion handoff approved · next stage unlocked"); else setProjectNotice("Optimization governance updated"); }
  ...
  const cycle = data.cycles[0]; const activeContract = data.contracts.find((contract) => contract.key === cycle?.activeStage); const cycleRuns = cycle ? data.runs.filter((run) => run.cycleId === cycle.id) : []; const latestRun = cycleRuns[0]; const passed = cycle?.stages.filter((stage) => ["PASSED_REUSED", "HANDOFF_READY"].includes(stage.status)).length || 0; const openP0 = cycle?.issues.filter((issue) => issue.severity === "P0" && !["RESOLVED"].includes(issue.status)).length || 0; const attempts = cycleRuns.reduce((sum, run) => sum + run.attempt, 0); const activeState = cycle?.stages.find((stage) => stage.key === cycle.activeStage);
