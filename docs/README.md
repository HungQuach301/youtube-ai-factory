...
The private personal GitHub repository is connected and contains the complete Sites history. Exact dual-remote equality was restored at `3d752ad907989d8ad16f9ecce092975e66767657`; the bounded temporary workflow and bundle self-removed and trigger issue `#4` was closed. Migrations `0108`-`0111` implement the zero-dispatch Gateway/compiler, deterministic exact-byte render-tape worker, exact asset-eligibility receipts, fenced pixel/video canary admission and one append-only live qualification receipt spanning clean release, orphan recovery and replay. Migration `0112` adds the append-only production-scale Hidden Systems treatment/case qualification boundary; Sites v531 stores one live ten-case PASS package with exact D1/R2 read-back and idempotent replay. Migration `0113` adds plan-only cost reservation, provider-native reconciliation, drift invalidation and explicit fallback controls; Sites v534 deploys the D1-safe guards and all five tables read back empty. Migration `0114` live-schema-qualifies exact-artifact Evidence Lineage and L0-L7 `AI_SHADOW` orchestration with immutable dependency drift and no acceptance authority; Sites v536 exposes all seven tables with zero rows and no temporary writer/Assurance/R22 flags at env revision 50. Migration `0115` adds source-qualified blind-control/holdout calibration receipts and a canonical read-only QA Cockpit projection; every result remains a `QUALIFIED_CANDIDATE` or `ADVISORY` with zero qualification/PASS/dispatch/R22/master/release/publication authority until live schema/read-back and exact dependency registration complete. Synchronized checkpoints must preserve `HEAD = origin/main = github/main`; all later changes use the forward-only exact-object protocol and must not reuse the completed force authority. Every material checkpoint must re-prove equality.
