...
| X28 | P0 | Technical decode, contact-sheet and Browser pixel-motion evidence can be mistaken for competitive YouTube audience quality | `CONFIRMED_MANDATORY__OWNER_REJECTED_CURRENT_MASTER`: Document 77 reclassifies the current master as `TECHNICAL_CLEAN_CONTROL_ONLY`; output-form mismatch is P0 and visual richness, meaningful motion, mobile typography and retention risk are P1. Release requires a new 60–90 second Golden Sequence and the full audience-master QA stack before full-video expansion | Video #1/Assurance |
