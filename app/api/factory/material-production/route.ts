...
function productionSceneManifest(contract: Row, rendererVersion = PRODUCTION_SCENE_RENDERER_VERSION, manifestVersion = "EXECUTABLE_PRODUCTION_SCENE_MANIFEST_V2") {
  ...
  const states = logicalId === "MP-002" ? [
    { role: "ENTRY", sceneLabel: "CREDIT PURCHASE", primary: "$100.00", secondary: "INSERT OR TAP CARD", sourceEvidence: evidence[0] || clean(contract.claim), sceneDelta: "PURCHASE_READY" },
    ...
    { role: "EXIT", sceneLabel: "CREDIT PURCHASE", primary: "APPROVED", secondary: "$100.00", sourceEvidence: evidence[2] || evidence.at(-1) || clean(contract.claim), sceneDelta: "APPROVAL_CONFIRMED" },
  ] : fallbackStates;
  ...
  return {
    ...
    layoutPolicy: { safeArea: 36, minimumGlyphScale: 2, noOverlap: true, noCrop: true, mobileReadability: true },
  };
}
...
function productionScenePng(manifest: Row, state: 0 | 1 | 2) {
  ...
  text(fit(current.sceneLabel,30),64,55,3,"#d7eee5");text(fit(manifest.logicalId,12),820,58,2,"#83bca6");
  if(sceneType==="CHECKOUT_TERMINAL"){
    ...
    fill(158,325,198,62,state===2?"#1b7b5b":"#d8e2dc");centerAt(state===2?"APPROVED":state===1?"AUTHORIZING":"READY",257,344,3,state===2?"#ffffff":"#24483f",18);
    fill(510,150,310,238,"#f6efd9");fill(535,176,260,12,"#d7cba8");fill(535,211,180,9,"#d7cba8");fill(535,242,222,9,"#d7cba8");fill(535,286,260,2,"#9d957d");
    centerAt(current.secondary,665,318,current.secondary&&clean(current.secondary).length>18?2:3,"#173d34",28);
  ...
  center(current.secondary,468,2,"#a8cdbd",38);
...
async function buildProductionScenePreflight() {
  ...
  for (const briefRow of activeBriefs) {
    ...
    unitStatements.push(db.prepare("INSERT INTO v7_unit_materializations (id,program_id,baseline_id,run_id,authorization_id,canary_version,brief_id,logical_brief_id,archetype,certification_id,certified_renderer_version,unit_renderer_version,contract_hash,semantic_manifest_json,semantic_manifest_hash,frame_ids_json,frame_hashes_json,lint_json,status,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'FROZEN',?) ON CONFLICT(id) DO UPDATE SET certification_id=excluded.certification_id,certified_renderer_version=excluded.certified_renderer_version,unit_renderer_version=excluded.unit_renderer_version,contract_hash=excluded.contract_hash,semantic_manifest_json=excluded.semantic_manifest_json,semantic_manifest_hash=excluded.semantic_manifest_hash,frame_ids_json=excluded.frame_ids_json,frame_hashes_json=excluded.frame_hashes_json,lint_json=excluded.lint_json,status='FROZEN',created_at=excluded.created_at").bind(unitId, PROGRAM_ID, baseline.id, run.id, authorization.id, STABILIZATION_RELEASE_VERSION, briefRow.id, logicalId, unitContract.archetype, certification.id, certification.renderer_version, unitRenderer, contractHash, manifestJson, manifestHash, JSON.stringify(frameIds), JSON.stringify(frameHashes), JSON.stringify(lint), now));
    promotionStatements.push(db.prepare("INSERT INTO v7_artifact_promotions (id,program_id,baseline_id,regression_id,run_id,authorization_id,canary_version,brief_id,logical_brief_id,archetype,certification_id,renderer_version,contract_hash,frame_ids_json,frame_hashes_json,status,preflight_json,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'FROZEN',?,?) ON CONFLICT(id) DO UPDATE SET certification_id=excluded.certification_id,renderer_version=excluded.renderer_version,contract_hash=excluded.contract_hash,frame_ids_json=excluded.frame_ids_json,frame_hashes_json=excluded.frame_hashes_json,status='FROZEN',preflight_json=excluded.preflight_json,created_at=excluded.created_at").bind(promotionId, PROGRAM_ID, baseline.id, regressionId, run.id, authorization.id, STABILIZATION_RELEASE_VERSION, briefRow.id, logicalId, unitContract.archetype, certification.id, unitRenderer, contractHash, JSON.stringify(frameIds), JSON.stringify(frameHashes), JSON.stringify(preflight), now));
...
async function startReleaseTrainBatch() {
  ...
  const now = new Date().toISOString(), policy = { ...rec(JSON.parse(String(authorization.model_policy_json || "{}"))), batchAuthorized: true, nextUnitDispatch: "TERMINAL_PASS_ONLY", concurrency: 1, concurrencyCeiling: 2, remainingRequestBudget: 8, remainingCostBudgetUsd: 8 };
  await db.batch([
    ...
    db.prepare("UPDATE v7_architecture_baselines SET execution_state='CANARY_ONLY' WHERE id=?").bind(canary.baseline_id),
    db.prepare("UPDATE v7_material_authorizations SET scope='RELEASE_TRAIN_BATCH_MP003_MP010',status='AUTHORIZED',max_remote_requests=?,max_actual_spend_usd=?,model_policy_json=?,updated_at=? WHERE id=?").bind(Number(canary.requests_before) + 9, Number(canary.cost_before) + 9, JSON.stringify(policy), now, authorization.id),
