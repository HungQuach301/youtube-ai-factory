...
export default function MaterialProductionPage() {
  ...
  async function canaryAction(action: "AUTHORIZE_CONTROLLED_CANARY" | "AUTHORIZE_CONTROLLED_CANARY_V3" | "AUTHORIZE_CONTROLLED_CANARY_V4" | "AUTHORIZE_CONTROLLED_CANARY_V5" | "RELEASE_CONTROLLED_CANARY_V5_UNIT" | "RELEASE_PRODUCTION_RECOVERY_PROBE" | "BUILD_RECOVERY_CONTRACT_ALIGNMENT" | "RELEASE_CONTRACT_ALIGNED_RECOVERY_PROBE" | "START_CONTROLLED_CANARY_UNIT" | "RELEASE_NEXT_CONTROLLED_CANARY_UNIT") {
    setWorking(action); setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
