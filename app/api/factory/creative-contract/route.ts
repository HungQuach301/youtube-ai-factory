...
function evaluate(artifact: Record<string, unknown>) {
  ...
  const gates: Gate[] = [
    ...
    { id:"CHAMPION",label:"Champion is tournament winner",status:Boolean(champion)&&Number(champion?.totalScore||0)>=90?"PASS":"FAIL",evidence:`${String(champion?.name||"missing")} · ${Number(champion?.totalScore||0)}/90` },
...
export async function POST(request:Request){try{const body=await request.json() as {action?:string};if(body.action==="RUN")return Response.json(await start(),{status:202});if(body.action==="POLL")return Response.json(await poll());return Response.json({error:"Unsupported Creative Contract action"},{status:400});}catch(error){const message=error instanceof Error?error.message:"Creative Contract failed";return Response.json({error:message},{status:message.includes("not ready")||message.includes("must be frozen")?409:500});}}
