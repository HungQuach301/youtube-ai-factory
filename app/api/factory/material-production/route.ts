...
const WAVE_BATCH_2_V19_PROCESS_REVISION = "V19_1_CAPACITY_SAFE_MOTION_PATH_SOLVER";
...
function contractNativeSceneSpecification(contract: Row, bindings: Row[], actors: string[], kind: string) {
  ...
  const relationships = bindings.map((binding, index) => {
    const from = objects[index % objects.length], to = objects[(index + 1) % objects.length], a = rec(from.geometry), b = rec(to.geometry), ax = Number(a.x) + Number(a.w) / 2, bx = Number(b.x) + Number(b.w) / 2;
    return { id: `RELATION_${index + 1}`, from: from.id, to: to.id, relation: actions[index], polarity: clean(binding.polarity), sourceClause: clean(binding.clause), corridor: [[ax, Number(a.y) + Number(a.h)], [ax, railY], [bx, railY], [bx, Number(b.y) + Number(b.h)]] };
  });
...
function waveProductOracle(manifest: Row) {
  ...
  const checks=[
    ...
    {id:"SCENE_SPEC_LAYOUT_SAFETY",status:!contractNativeScene||Number(sceneLayoutProof.clippingCount)===0&&Number(sceneLayoutProof.textOcclusionCount)===0&&Number(sceneLayoutProof.connectorTextIntersectionCount)===0&&Number(sceneLayoutProof.objectOverlapCount)===0&&Number(sceneLayoutProof.mobileLegibilityFailures)===0&&Number(sceneLayoutProof.motionPathObjectIntersectionCount)===0&&Number(sceneLayoutProof.motionPathSafeAreaFailures)===0&&sceneLayoutProof.allGeometryInsideSafeArea===true&&sceneLayoutProof.motionRailReserved===true&&sceneLayoutProof.measuredFromSolvedGeometry===true?"PASS":"FAIL",evidence:"layout proof is recomputed from solved geometry, including mobile legibility and motion-path exclusion"},
...
function systemicRootCauseRecord(audit: Row) {
  ...
  return {
    ...
    systemicCorrections: ["TOPOLOGY_CLASSIFICATION_FROM_SOURCE_CLAUSES", "CONSTRAINT_SOLVED_GEOMETRY", "PRIMITIVE_ONLY_RENDERER", "RECOMPUTED_LAYOUT_AND_MOBILE_MEASUREMENTS", "TEN_STRUCTURAL_LAYOUT_LANGUAGES", "ADVERSARIAL_FIXTURE_QUALIFICATION", "CAPACITY_SAFE_SLOTS_AND_TOPOLOGY_DIRECTED_MOTION_PATH"],
...
async function qualifyWaveBatch2V13Activation(context: Awaited<ReturnType<typeof waveBatch2V13ActivationContext>>, progress: Row = {}) {
  ...
  for (const target of scope.slice(startIndex, endIndex)) {
    ...
    const measured = systemicLayoutMeasurement(spec), layoutSafe = Number(measured.clippingCount) === 0 && Number(measured.textOcclusionCount) === 0 && Number(measured.connectorTextIntersectionCount) === 0 && Number(measured.objectOverlapCount) === 0 && Number(measured.mobileLegibilityFailures) === 0 && Number(measured.motionPathObjectIntersectionCount) === 0 && Number(measured.motionPathSafeAreaFailures) === 0 && measured.allGeometryInsideSafeArea === true && measured.motionRailReserved === true && measured.measuredFromSolvedGeometry === true && JSON.stringify(measured) === JSON.stringify(layoutProof);
...
async function preflightWaveBatch2V13Activation() {
  const context = await waveBatch2V13ActivationContext(), now = new Date().toISOString(), preflightId = `${context.replacementId}-ZERO-SPEND-PREFLIGHT-V19-1`;
  const existing = await context.db.prepare("SELECT * FROM v7_batch_activation_preflights WHERE id=?").bind(preflightId).first<Row>();
  if (existing && clean(existing.input_hash) === context.inputHash && clean(existing.status) === "PASS") return snapshot();
  const prior = existing && clean(existing.input_hash) === context.inputHash && clean(existing.status) === "RUNNING" ? rec(JSON.parse(String(existing.result_json || "{}"))) : {};
...
async function adoptWaveBatch2V13EngineRootCorrection() {
  const context = await waveBatch2V13ActivationContext(), activationId = `${context.replacementId}-ACTIVATION-V1`, preflightId = `${context.replacementId}-ZERO-SPEND-PREFLIGHT-V19-1`;
  const existingActivation = await context.db.prepare("SELECT * FROM v7_batch_activations WHERE id=?").bind(activationId).first<Row>();
  if (existingActivation) {
