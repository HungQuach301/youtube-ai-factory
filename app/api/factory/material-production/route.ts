...
async function storeMaterial(env: Env, db: DB, authorization: Row, briefRow: Row, options: { role: "PRIMARY" | "OVERLAY" | "QA_PROXY" | "QA_ENTRY" | "QA_MIDPOINT" | "QA_EXIT" | "SOURCE_ENTRY" | "SOURCE_MIDPOINT" | "SOURCE_EXIT"; identity?: string; bytes: Uint8Array; mimeType: string; extension: string; sourceType: string; provider: string; providerAssetId?: string; sourceUrl?: string; landingUrl?: string; licenseCode: string; width: number; height: number; duration?: number; thumbnailUrl?: string }) {
...
async function sourceFrameQa() {
  ...
  const content: Row[] = [{ type: "input_text", text: `You are the source-footage gate for a premium documentary. Inspect the actual decoded ENTRY, MIDPOINT and EXIT frames in order. Judge the source layer only: it must provide unmistakable, composition-ready physical context, contain no prohibited or contradictory evidence, and materially differ across time. Topic similarity is not enough. Do not credit metadata, filenames or a future overlay. Reject ambiguous cash/card handoffs, unclear tender, generic checkout footage, logos, weak crops or near-duplicate frames. ${hybrid ? "This is a HYBRID shot. Score semantic specificity only against SOURCE MUST PROVE below. Do NOT require the stock footage to display the exact $100 value, explanatory labels, authorization state or settlement distinction; those are owned by the authored layer. The source must not contradict those later authored states." : "This is a SOURCE shot and the footage itself must carry the full visible meaning."} PASS requires every dimension >=86 and overall >=90. The replacement query and sourceLayerContract must describe source footage only—never ask stock footage to contain invented data or authored explanatory text.\n\nSOURCE MUST PROVE:\n${sourceContextJob(brief)}\n\nFULL FROZEN CONTRACT (contradiction checks only for HYBRID):\n${JSON.stringify({ narrationClause: brief.narrationClause, viewerMustUnderstand: brief.viewerMustUnderstand, requiredEvidence: brief.requiredEvidence, prohibitedEvidence: brief.prohibitedEvidence, acceptance: brief.acceptance })}` }];
