...
export async function createAudienceGoldenRepairRevisionAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string) {
  ...
  if (clean(latest?.id).endsWith(":r8")) return createAudienceGoldenRepairRevision9Authorized(env, actor, idempotencyKey, latest as Row);
...
async function createAudienceGoldenRepairRevision4Authorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string, rejected: Row) {
  const priorReceipt = await first(env.DB, "SELECT * FROM v7_youtube_golden_revision_4_receipts WHERE channel_id=? AND idempotency_key=? LIMIT 1", CHANNEL_ID, idempotencyKey); if (priorReceipt) return { outcome: "REPLAYED", snapshot: await audienceGoldenSnapshot(env.DB) };
  const materialization = await first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE blueprint_id=?", rejected.id); if (!materialization) throw new AudienceGoldenError("REJECTED_MATERIALIZATION_MISSING", 409, "Revision 3 materialization is required");
  const [visualQa, audioQa] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_VISUAL'", materialization.id), first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_AUDIO'", materialization.id)]);
  if (!visualQa || clean(visualQa.decision_state) !== "FAIL" || !audioQa || clean(audioQa.decision_state) !== "PASS") throw new AudienceGoldenError("REVISION_4_EVIDENCE_REQUIRED", 409, "Revision 4 requires failed visual QA and passing exact-audio QA");
  const replacementId = `audience-golden-blueprint:${CHANNEL_ID}:r4`, existing = await first(env.DB, "SELECT id FROM v7_youtube_golden_sequence_blueprints WHERE id=?", replacementId);
  if (!existing) {
    const narration = clean(rejected.narration_text), narrationHash = await sha256Hex(new TextEncoder().encode(narration));
    const assetManifest = { source: "OPENAI_IMAGEGEN_BUILTIN", assets: [{ path: "public/golden/payment-world-r4.jpg", sha256: "00354e385a041d3d1d854ba496612d404e3ca8e170cfab5754a73f7be05e123f", role: "CONTINUOUS_TRANSACTION_WORLD" }, { path: "public/golden/payment-exceptions-r4.jpg", sha256: "be3c8d80167d0b30eba4387534907c14085e000456ad8809fa838ca2a2a29911", role: "EXCEPTION_BRANCH_WORLD" }] };
    const repairContract = { revision: 4, rootOwners: ["OUTPUT_FORM", "VISUAL_STORYTELLING", "OBJECT_CONTINUITY", "COLLISION_SAFETY"], inheritedAudioPassReceiptId: audioQa.id, visual: { form: "CINEMATIC_ILLUSTRATED_WORLD", continuousHeroObject: "TRANSACTION_TOKEN", maximumAnnotationLines: 2, minimumTextPx1080: 60, collisionSafeZones: true, emptyFramesAllowed: false }, audio: { settingsInheritedFromPassingRevision: 3, musicBed: false, targetLufs: -14, truePeakDbtp: -2 } };
    await run(env.DB, `INSERT INTO v7_youtube_golden_sequence_blueprints (id,channel_id,policy_version,blueprint_version,episode_key,title_promise,narration_text,narration_hash,story_contract_json,visual_contract_json,audio_contract_json,lifecycle_state,actor) VALUES (?,?,?,?,?,?,?,?,?,?,?,'SEALED',?)`, replacementId, CHANNEL_ID, AUDIENCE_POLICY_VERSION, AUDIENCE_BLUEPRINT_VERSION, `${AUDIENCE_GOLDEN_STORY.episodeKey}-revision-4`, AUDIENCE_GOLDEN_STORY.titlePromise, narration, narrationHash, canonicalStringify({ ...AUDIENCE_GOLDEN_STORY, assetManifest, repairContract }), canonicalStringify(repairContract.visual), canonicalStringify(repairContract.audio), actor);
    ...
    await run(env.DB, "INSERT INTO v7_youtube_golden_revision_4_receipts (id,channel_id,rejected_blueprint_id,rejected_materialization_receipt_id,replacement_blueprint_id,revision_key,visual_failure_receipt_id,audio_pass_receipt_id,asset_manifest_json,repair_contract_json,actor,idempotency_key,evidence_hash) VALUES (?,?,?,?,?,'AUDIENCE_GOLDEN_REVISION_4',?,?,?,?,?,?,?)", makeId("audience-golden-revision-4"), CHANNEL_ID, rejected.id, materialization.id, replacementId, visualQa.id, audioQa.id, canonicalStringify(assetManifest), canonicalStringify(repairContract), actor, idempotencyKey, evidenceHash);
  }
  return { outcome: "REPAIR_REVISION_4_SEALED", snapshot: await audienceGoldenSnapshot(env.DB) };
}

async function createAudienceGoldenRepairRevision5Authorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string, rejected: Row) {
  const priorReceipt = await first(env.DB, "SELECT * FROM v7_youtube_golden_revision_5_receipts WHERE channel_id=? AND idempotency_key=? LIMIT 1", CHANNEL_ID, idempotencyKey); if (priorReceipt) return { outcome: "REPLAYED", snapshot: await audienceGoldenSnapshot(env.DB) };
  const materialization = await first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE blueprint_id=?", rejected.id); if (!materialization) throw new AudienceGoldenError("REJECTED_MATERIALIZATION_MISSING", 409, "Revision 4 materialization is required");
  const [visualQa, audioQa] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_VISUAL'", materialization.id), first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_AUDIO'", materialization.id)]);
  if (!visualQa || clean(visualQa.decision_state) !== "FAIL" || !audioQa || clean(audioQa.decision_state) !== "PASS") throw new AudienceGoldenError("REVISION_5_EVIDENCE_REQUIRED", 409, "Revision 5 requires failed visual QA and passing exact-audio QA");
  const replacementId = `audience-golden-blueprint:${CHANNEL_ID}:r5`, existing = await first(env.DB, "SELECT id FROM v7_youtube_golden_sequence_blueprints WHERE id=?", replacementId);
  if (!existing) {
    const narration = clean(rejected.narration_text), narrationHash = await sha256Hex(new TextEncoder().encode(narration));
    const assetManifest = { source: "OPENAI_IMAGEGEN_BUILTIN", assets: [
    ...
    const repairContract = { revision: 5, rootOwners: ["OBJECT_TRANSFORMATION", "SCENE_DIVERSITY", "STATE_MAPPING", "LOCALIZATION", "MOBILE_TYPOGRAPHY"], inheritedAudioPassReceiptId: audioQa.id, visual: { form: "CINEMATIC_TRANSFORMING_PROCESS", distinctVisualWorlds: 5, maximumWorldHoldSeconds: 14, visibleBalanceTransformation: true, convergingRecordPlates: true, physicalNettingTransformation: true, vietnameseOnlyEssentialLabels: true, persistentLowerThird: false, minimumTextPx1080: 72, emptyFramesAllowed: false }, audio: { settingsInheritedFromPassingRevision: 4, musicBed: false, targetLufs: -14, truePeakDbtp: -2 } };
    await run(env.DB, `INSERT INTO v7_youtube_golden_sequence_blueprints (id,channel_id,policy_version,blueprint_version,episode_key,title_promise,narration_text,narration_hash,story_contract_json,visual_contract_json,audio_contract_json,lifecycle_state,actor) VALUES (?,?,?,?,?,?,?,?,?,?,?,'SEALED',?)`, replacementId, CHANNEL_ID, AUDIENCE_POLICY_VERSION, AUDIENCE_BLUEPRINT_VERSION, `${AUDIENCE_GOLDEN_STORY.episodeKey}-revision-5`, AUDIENCE_GOLDEN_STORY.titlePromise, narration, narrationHash, canonicalStringify({ ...AUDIENCE_GOLDEN_STORY, assetManifest, repairContract }), canonicalStringify(repairContract.visual), canonicalStringify(repairContract.audio), actor);
    const evidenceHash = await canonicalHash({ rejectedBlueprintId: rejected.id, rejectedMaterializationId: materialization.id, replacementId, visualFailureReceiptId: visualQa.id, audioPassReceiptId: audioQa.id, assetManifest, repairContract });
    await run(env.DB, "INSERT INTO v7_youtube_golden_revision_5_receipts (id,channel_id,rejected_blueprint_id,rejected_materialization_receipt_id,replacement_blueprint_id,revision_key,visual_failure_receipt_id,audio_pass_receipt_id,asset_manifest_json,repair_contract_json,actor,idempotency_key,evidence_hash) VALUES (?,?,?,?,?,'AUDIENCE_GOLDEN_REVISION_5',?,?,?,?,?,?,?)", makeId("audience-golden-revision-5"), CHANNEL_ID, rejected.id, materialization.id, replacementId, visualQa.id, audioQa.id, canonicalStringify(assetManifest), canonicalStringify(repairContract), actor, idempotencyKey, evidenceHash);
  }
  return { outcome: "REPAIR_REVISION_5_SEALED", snapshot: await audienceGoldenSnapshot(env.DB) };
}

async function createAudienceGoldenRepairRevision6Authorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string, rejected: Row) {
  const priorReceipt = await first(env.DB, "SELECT * FROM v7_youtube_golden_revision_6_receipts WHERE channel_id=? AND idempotency_key=? LIMIT 1", CHANNEL_ID, idempotencyKey); if (priorReceipt) return { outcome: "REPLAYED", snapshot: await audienceGoldenSnapshot(env.DB) };
  const materialization = await first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE blueprint_id=?", rejected.id); if (!materialization) throw new AudienceGoldenError("REJECTED_MATERIALIZATION_MISSING", 409, "Revision 5 materialization is required");
  const [visualQa, audioQa] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_VISUAL'", materialization.id), first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_AUDIO'", materialization.id)]);
  if (!visualQa || clean(visualQa.decision_state) !== "FAIL" || !audioQa || clean(audioQa.decision_state) !== "PASS") throw new AudienceGoldenError("REVISION_6_EVIDENCE_REQUIRED", 409, "Revision 6 requires failed visual QA and passing exact-audio QA");
  const replacementId = `audience-golden-blueprint:${CHANNEL_ID}:r6`, existing = await first(env.DB, "SELECT id FROM v7_youtube_golden_sequence_blueprints WHERE id=?", replacementId);
  if (!existing) {
    const narration = clean(rejected.narration_text), narrationHash = await sha256Hex(new TextEncoder().encode(narration));
    const assetManifest = { source: "OPENAI_IMAGEGEN_BUILTIN", inheritedFromRevision: 5, assets: [
      { path: "public/golden/payment-world-r4.jpg", sha256: "00354e385a041d3d1d854ba496612d404e3ca8e170cfab5754a73f7be05e123f", role: "HOOK_AND_PAYOFF_WORLD" },
      { path: "public/golden/payment-bank-r5.jpg", sha256: "557f925178232be56df515aae9782509b881adce8b87723e9e63adb3bf3e1120", role: "AUTHORIZATION_HOLD_WORLD" },
      { path: "public/golden/payment-clearing-r5.jpg", sha256: "26c6ad2e9291419170c6e34734feb7aef4a3ba79491f61be34540bb6c43a634c", role: "TWO_RECORD_RECONCILIATION_WORLD" },
      { path: "public/golden/payment-settlement-r5.jpg", sha256: "1599ccbbafbc11f476b88f6b1ae0bede4c681f4d23701684706217ead96428bc", role: "NETTING_AND_MERCHANT_WORLD" },
    ...
    const repairContract = { revision: 6, rootOwners: ["COMPOSITION_CHANGE_DENSITY", "ARITHMETIC_CONSISTENCY", "CLEARING_COMPONENT_MAPPING", "STATE_LABEL_BINDING", "MOBILE_COLLISION_SAFETY"], inheritedAudioPassReceiptId: audioQa.id, visual: { form: "CINEMATIC_MULTI_COMPOSITION_PROCESS", maximumLockedCompositionSeconds: 3.5, phaseSpecificHardCrops: true, synchronizedHoldArithmetic: "10.00_MINUS_HOLD_EQUALS_AVAILABLE", clearingFormula: "2.00_PLUS_FEE_0.05_EQUALS_2.05", explicitStateLabels: ["KHOẢN GIỮ", "BẢN GHI KHỚP", "TIỀN ĐÃ CHUYỂN"], collidingStageLabelsAllowed: false, minimumTextPx1080: 72 }, audio: { settingsInheritedFromPassingRevision: 5, musicBed: false, targetLufs: -14, truePeakDbtp: -2 } };
    await run(env.DB, `INSERT INTO v7_youtube_golden_sequence_blueprints (id,channel_id,policy_version,blueprint_version,episode_key,title_promise,narration_text,narration_hash,story_contract_json,visual_contract_json,audio_contract_json,lifecycle_state,actor) VALUES (?,?,?,?,?,?,?,?,?,?,?,'SEALED',?)`, replacementId, CHANNEL_ID, AUDIENCE_POLICY_VERSION, AUDIENCE_BLUEPRINT_VERSION, `${AUDIENCE_GOLDEN_STORY.episodeKey}-revision-6`, AUDIENCE_GOLDEN_STORY.titlePromise, narration, narrationHash, canonicalStringify({ ...AUDIENCE_GOLDEN_STORY, assetManifest, repairContract }), canonicalStringify(repairContract.visual), canonicalStringify(repairContract.audio), actor);
    const evidenceHash = await canonicalHash({ rejectedBlueprintId: rejected.id, rejectedMaterializationId: materialization.id, replacementId, visualFailureReceiptId: visualQa.id, audioPassReceiptId: audioQa.id, assetManifest, repairContract });
...
async function createAudienceGoldenRepairRevision9Authorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string, rejected: Row) {
  const priorReceipt = await first(env.DB, "SELECT * FROM v7_youtube_golden_revision_9_receipts WHERE channel_id=? AND idempotency_key=? LIMIT 1", CHANNEL_ID, idempotencyKey); if (priorReceipt) return { outcome: "REPLAYED", snapshot: await audienceGoldenSnapshot(env.DB) };
  const materialization = await first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE blueprint_id=?", rejected.id); if (!materialization) throw new AudienceGoldenError("REJECTED_MATERIALIZATION_MISSING", 409, "Revision 8 materialization is required");
  const [visualQa, audioQa] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_VISUAL'", materialization.id), first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE materialization_receipt_id=? AND qa_layer='FACTORY_AUDIO'", materialization.id)]);
  if (!visualQa || clean(visualQa.decision_state) !== "FAIL" || !audioQa || clean(audioQa.decision_state) !== "PASS") throw new AudienceGoldenError("REVISION_9_EVIDENCE_REQUIRED", 409, "Revision 9 requires failed visual QA and passing exact-audio QA");
  const replacementId = `audience-golden-blueprint:${CHANNEL_ID}:r9`, existing = await first(env.DB, "SELECT id FROM v7_youtube_golden_sequence_blueprints WHERE id=?", replacementId);
  if (!existing) {
    const narration = clean(rejected.narration_text), narrationHash = await sha256Hex(new TextEncoder().encode(narration));
    const assetManifest = { source: "OPENAI_IMAGEGEN_BUILTIN", inheritedFromRevision: 8, assets: [
      { path: "public/golden/payment-world-r4.jpg", sha256: "00354e385a041d3d1d854ba496612d404e3ca8e170cfab5754a73f7be05e123f", role: "HOOK_AND_PAYOFF_WORLD" },
      { path: "public/golden/payment-bank-r5.jpg", sha256: "557f925178232be56df515aae9782509b881adce8b87723e9e63adb3bf3e1120", role: "AUTHORIZATION_HOLD_WORLD" },
      { path: "public/golden/payment-clearing-r5.jpg", sha256: "26c6ad2e9291419170c6e34734feb7aef4a3ba79491f61be34540bb6c43a634c", role: "TWO_RECORD_RECONCILIATION_WORLD" },
      { path: "public/golden/payment-settlement-r5.jpg", sha256: "1599ccbbafbc11f476b88f6b1ae0bede4c681f4d23701684706217ead96428bc", role: "NETTING_AND_MERCHANT_WORLD" },
    ...
    const repairContract = { revision: 9, rootOwners: ["SYSTEMIC_COMPOSITION_GRAMMAR", "MEANINGFUL_MOTION_DENSITY", "RETENTION_PACING", "NETTING_SEMANTIC_TRANSFORMATION", "CURRENT_VS_HISTORY_STATE"], inheritedAudioPassReceiptId: audioQa.id, visual: { form: "CINEMATIC_FORTY_BEAT_COMPOSITOR", microSceneCount: 40, maximumSameCompositionSeconds: 1.9, alternatingLayoutFamilies: 5, headlineBandPersistenceAllowed: false, cameraOnlyTransitionAllowed: false, nettingMustTransformManyToOne: true, stateRoles: ["ĐÃ QUA", "HIỆN TẠI", "TIẾP THEO"], minimumCriticalTextPx1080: 84 }, audio: { settingsInheritedFromPassingRevision: 8, regenerateWithSameQualifiedSettings: true, speechSpeed: .96, stability: .78, style: .04, targetLufs: -15, truePeakDbtp: -3, musicBed: false } };
    await run(env.DB, `INSERT INTO v7_youtube_golden_sequence_blueprints (id,channel_id,policy_version,blueprint_version,episode_key,title_promise,narration_text,narration_hash,story_contract_json,visual_contract_json,audio_contract_json,lifecycle_state,actor) VALUES (?,?,?,?,?,?,?,?,?,?,?,'SEALED',?)`, replacementId, CHANNEL_ID, AUDIENCE_POLICY_VERSION, AUDIENCE_BLUEPRINT_VERSION, `${AUDIENCE_GOLDEN_STORY.episodeKey}-revision-9`, AUDIENCE_GOLDEN_STORY.titlePromise, narration, narrationHash, canonicalStringify({ ...AUDIENCE_GOLDEN_STORY, assetManifest, repairContract }), canonicalStringify(repairContract.visual), canonicalStringify(repairContract.audio), actor);
    const evidenceHash = await canonicalHash({ rejectedBlueprintId: rejected.id, rejectedMaterializationId: materialization.id, replacementId, visualFailureReceiptId: visualQa.id, audioPassReceiptId: audioQa.id, assetManifest, repairContract });
    await run(env.DB, "INSERT INTO v7_youtube_golden_revision_9_receipts (id,channel_id,rejected_blueprint_id,rejected_materialization_receipt_id,replacement_blueprint_id,revision_key,visual_failure_receipt_id,audio_pass_receipt_id,asset_manifest_json,repair_contract_json,actor,idempotency_key,evidence_hash) VALUES (?,?,?,?,?,'AUDIENCE_GOLDEN_REVISION_9',?,?,?,?,?,?,?)", makeId("audience-golden-revision-9"), CHANNEL_ID, rejected.id, materialization.id, replacementId, visualQa.id, audioQa.id, canonicalStringify(assetManifest), canonicalStringify(repairContract), actor, idempotencyKey, evidenceHash);
  }
  return { outcome: "REPAIR_REVISION_9_SEALED", snapshot: await audienceGoldenSnapshot(env.DB) };
}
...
export async function generateAudienceGoldenAudioAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string) {
  ...
  const revisionTwo = clean(blueprint.id).endsWith(":r2"), saferVoiceRevision = clean(blueprint.id).endsWith(":r8") || clean(blueprint.id).endsWith(":r9");
  const requestBody = { text: clean(blueprint.narration_text), model_id: "eleven_multilingual_v2", voice_settings: saferVoiceRevision ? { stability: .78, similarity_boost: .78, style: .04, speed: .96, use_speaker_boost: true } : revisionTwo ? { stability: .56, similarity_boost: .78, style: .06, speed: .98, use_speaker_boost: true } : { stability: .66, similarity_boost: .8, style: .14, speed: 1.06, use_speaker_boost: true } };
