...
async function runArchetypeRegression() {
  ...
  const baseline=await db.prepare("SELECT * FROM v7_architecture_baselines WHERE program_id=? AND stage_key=? AND status='QUALIFIED_FOR_ARCHETYPE_CERTIFICATION' ORDER BY created_at DESC LIMIT 1").bind(PROGRAM_ID,STAGE).first<Row>();if(!baseline)throw new Error("RELIABILITY_BASELINE_PASS_REQUIRED");
  const regressionId=`${clean(baseline.id)}-REGRESSION-V1`,existing=await db.prepare("SELECT id FROM v7_archetype_regressions WHERE id=?").bind(regressionId).first<Row>();if(existing)return snapshot();
  const usageBefore=await db.prepare("SELECT COUNT(*) AS total,COUNT(DISTINCT idempotency_key) AS unique_keys,COALESCE(SUM(actual_cost_usd),0) AS cost,SUM(CASE WHEN status IN ('QUEUED','IN_PROGRESS') THEN 1 ELSE 0 END) AS active FROM v7_material_requests WHERE authorization_id=?").bind(authorization.id).first<Row>();
  ...
  const qualification=rec(JSON.parse(String(baseline.qualification_json||"{}"))),checks=[
    ...
    {id:"IDEMPOTENCY_UNIQUE",status:Number(usageBefore?.total||0)===Number(usageBefore?.unique_keys||0)?"PASS":"FAIL",evidence:`${Number(usageBefore?.unique_keys||0)}/${Number(usageBefore?.total||0)} unique request keys`},
