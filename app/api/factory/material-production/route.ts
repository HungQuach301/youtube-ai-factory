...
function systemicLayoutSlots(family: string, count: number) {
  const layouts: Record<string, Array<{ x: number; y: number; w: number; h: number }>> = {
    ...
    STACK: [{ x: 94, y: 112, w: 772, h: 70 }, { x: 94, y: 202, w: 772, h: 70 }, { x: 94, y: 292, w: 772, h: 70 }, { x: 94, y: 382, w: 238, h: 70 }, { x: 361, y: 382, w: 238, h: 70 }, { x: 628, y: 382, w: 238, h: 70 }],
    RADIAL: [{ x: 362, y: 110, w: 236, h: 82 }, { x: 72, y: 226, w: 236, h: 82 }, { x: 652, y: 226, w: 236, h: 82 }, { x: 362, y: 342, w: 236, h: 82 }, { x: 72, y: 342, w: 236, h: 82 }, { x: 652, y: 342, w: 236, h: 82 }],
    TIMELINE: [{ x: 56, y: 154, w: 156, h: 104 }, { x: 230, y: 250, w: 156, h: 104 }, { x: 404, y: 154, w: 156, h: 104 }, { x: 578, y: 250, w: 156, h: 104 }, { x: 752, y: 154, w: 156, h: 104 }, { x: 404, y: 354, w: 156, h: 76 }],
    ...
    LEDGER: [{ x: 58, y: 126, w: 384, h: 176 }, { x: 518, y: 126, w: 384, h: 176 }, { x: 58, y: 326, w: 240, h: 84 }, { x: 326, y: 326, w: 308, h: 84 }, { x: 662, y: 326, w: 240, h: 84 }],
    FIELD: [{ x: 70, y: 118, w: 820, h: 82 }, { x: 70, y: 226, w: 236, h: 158 }, { x: 362, y: 226, w: 236, h: 158 }, { x: 654, y: 226, w: 236, h: 158 }, { x: 70, y: 410, w: 382, h: 60 }, { x: 508, y: 410, w: 382, h: 60 }],
    TABLE: [{ x: 62, y: 118, w: 260, h: 122 }, { x: 350, y: 118, w: 548, h: 122 }, { x: 62, y: 266, w: 260, h: 122 }, { x: 350, y: 266, w: 260, h: 122 }, { x: 638, y: 266, w: 260, h: 122 }],
  };
...
function systemicLayoutMeasurement(spec: Row) {
  ...
  const connectorTextIntersectionCount = relationships.filter((relationship) => {
    ...
    return corridor.some((point) => boxes.some((box) => ![clean(relationship.from), clean(relationship.to)].includes(box.id) && point[0] > box.x && point[0] < box.x + box.w && point[1] > box.y && point[1] < box.y + box.h));
  }).length;
  const mobileLegibilityFailures = boxes.filter((box) => box.label.length > 22 || box.w < 150 || box.h < 60).length;
  return { clippingCount, textOcclusionCount, connectorTextIntersectionCount, objectOverlapCount, mobileLegibilityFailures, minimumGlyphScale: 2, allGeometryInsideSafeArea: clippingCount === 0, connectorCorridorsReserved: connectorTextIntersectionCount === 0, measuredFromSolvedGeometry: true };
}
...
function contractNativeSceneSpecification(contract: Row, bindings: Row[], actors: string[], kind: string) {
  ...
  const slots = systemicLayoutSlots(layoutFamily, labels.length), railY = 472;
  ...
  const path = [objects[0], objects[Math.floor(objects.length / 2)], objects[objects.length - 1]].map((item) => { const box = rec(item.geometry); return [Number(box.x) + Number(box.w) / 2, railY]; });
...
function waveProductOracle(manifest: Row) {
  ...
  const checks=[
    ...
    {id:"CONTRACT_NATIVE_SCENE_SPECIFICATION",status:!contractNativeScene||clean(sceneSpec.version)==="CONSTRAINT_SCENE_SPECIFICATION_V19"&&clean(sceneSpec.processVersion)==="ROOT_CAUSE_TO_PROCESS_V1"&&Boolean(clean(sceneSpec.semanticTopology))&&Boolean(clean(sceneSpec.composition))&&sceneObjects.length>=3&&sceneRelationships.length===3&&sceneStates.length===3&&sceneObjects.every((item)=>Boolean(clean(item.id))&&Boolean(clean(item.type))&&Boolean(clean(item.label))&&Boolean(clean(item.sourceClause))&&Object.keys(rec(item.geometry)).length===4)&&sceneRelationships.every((item)=>Boolean(clean(item.from))&&Boolean(clean(item.to))&&Boolean(clean(item.relation))&&Boolean(clean(item.sourceClause)))?"PASS":"FAIL",evidence:`${sceneObjects.length} domain primitives · ${sceneRelationships.length} actions · ${sceneStates.length} states`},
    ...
    {id:"SCENE_SPEC_LAYOUT_SAFETY",status:!contractNativeScene||Number(sceneLayoutProof.clippingCount)===0&&Number(sceneLayoutProof.textOcclusionCount)===0&&Number(sceneLayoutProof.connectorTextIntersectionCount)===0&&Number(sceneLayoutProof.objectOverlapCount)===0&&Number(sceneLayoutProof.mobileLegibilityFailures)===0&&sceneLayoutProof.allGeometryInsideSafeArea===true&&sceneLayoutProof.measuredFromSolvedGeometry===true?"PASS":"FAIL",evidence:"layout proof is recomputed from solved geometry, including mobile legibility"},
...
function waveManifestQualification(manifest: Row) {
  const states=arr(manifest.states).map(rec),semantic=rec(manifest.semanticModel),required=arr(semantic.requiredElements).map(clean).filter(Boolean),contractNativeScene=clean(semantic.version)==="CONSTRAINT_SCENE_SPECIFICATION_V19",structuredOntology=clean(semantic.version)==="STRUCTURED_VISUAL_ONTOLOGY_V12",semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11"||structuredOntology||contractNativeScene,archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),ontology=rec(semantic.visualOntology),ontologyStates=arr(ontology.states).map(rec),layoutProof=rec(ontology.layoutProof),motionPolicy=rec(ontology.motionPolicy),sceneSpec=rec(semantic.sceneSpecification),sceneObjects=arr(sceneSpec.objects).map(rec),sceneRelationships=arr(sceneSpec.relationships).map(rec),sceneStates=arr(sceneSpec.states).map(rec),sceneLayoutProof=rec(sceneSpec.layoutProof),sceneMotionProof=rec(sceneSpec.motionProof),rendererContract=rec(sceneSpec.rendererContract),corpus=clean(semantic.corpusHashInput).toUpperCase(),genericEscape=contractBound&&["$100 PURCHASE","REWARD ENTRY"].some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy),checks=[
    ...
    {id:"SCENE_SPEC_LAYOUT_SAFETY",status:!contractNativeScene||Number(sceneLayoutProof.clippingCount)===0&&Number(sceneLayoutProof.textOcclusionCount)===0&&Number(sceneLayoutProof.connectorTextIntersectionCount)===0&&Number(sceneLayoutProof.objectOverlapCount)===0&&Number(sceneLayoutProof.mobileLegibilityFailures)===0&&sceneLayoutProof.allGeometryInsideSafeArea===true&&sceneLayoutProof.measuredFromSolvedGeometry===true?"PASS":"FAIL"},
...
function systemicRootCauseRecord(audit: Row) {
  ...
  return {
    ...
    systemicCorrections: ["TOPOLOGY_CLASSIFICATION_FROM_SOURCE_CLAUSES", "CONSTRAINT_SOLVED_GEOMETRY", "PRIMITIVE_ONLY_RENDERER", "RECOMPUTED_LAYOUT_AND_MOBILE_MEASUREMENTS", "TEN_STRUCTURAL_LAYOUT_LANGUAGES", "ADVERSARIAL_FIXTURE_QUALIFICATION"],
    acceptance: { fullPortfolioProducts: 50, fullPortfolioFrames: 150, layoutFamilies: 10, adversarialFixtures: 10, measuredFailuresMaximum: 0, remoteRequests: 0 },
  };
}
...
async function qualifyWaveBatch2V13Activation(context: Awaited<ReturnType<typeof waveBatch2V13ActivationContext>>, progress: Row = {}) {
  ...
  for (const target of scope.slice(startIndex, endIndex)) {
    ...
    const measured = systemicLayoutMeasurement(spec), layoutSafe = Number(measured.clippingCount) === 0 && Number(measured.textOcclusionCount) === 0 && Number(measured.connectorTextIntersectionCount) === 0 && Number(measured.objectOverlapCount) === 0 && Number(measured.mobileLegibilityFailures) === 0 && measured.allGeometryInsideSafeArea === true && measured.measuredFromSolvedGeometry === true && JSON.stringify(measured) === JSON.stringify(layoutProof);
...
async function adoptWaveBatch2V13EngineRootCorrection() {
  ...
  const rootCausePolicy = { ...rec(JSON.parse(String(rejected.root_cause_policy_json || "{}"))), ...systemicRca, rejectedEngineVersion: WAVE_BATCH_2_V13_REJECTED_ENGINE_VERSION, rejectedAuditId: audit.id, rejectedAuditScore: Number(audit.score), observedRootCause: priorRoot.rootProductionCause, correctedLayers: systemicRca.affectedProcessSteps, correction: "REPLACE_SAMPLE_BRANCH_RENDERING_WITH_SEMANTIC_TOPOLOGY_CONSTRAINT_SOLVING_AND_MEASURED_PREFLIGHT", replacementEngineVersion: WAVE_BATCH_2_V13_ENGINE_VERSION, fullScopeRegression: "50_PRODUCTS_150_FRAMES_10_ADVERSARIAL_FIXTURES_MEASURED", portfolioPolicy, reproduceScope: "ALL_50_PRODUCTS", outputRepair: false, v8V9V10V11V12V13V14V15V16V17V18ProductsPreservedAsEvidence: true, retryPriorAudits: false };
  const productionDoD = { ...rec(JSON.parse(String(rejected.production_dod_json || "{}"))), version: "SHOT_PRODUCT_DOD_V19_SYSTEMIC", rootCauseVerifiedBeforeDesign: true, semanticTopologyClassification: true, constraintSolvedGeometry: true, primitiveOnlyRenderer: true, logicalIdBranchingForbidden: true, auditSampleBranchingForbidden: true, measuredLayoutProof: true, mobileLegibilityMeasured: true, adversarialFixtures: "10_OF_10_PASS", sharedDiagramShellForbidden: true, clippingConnectorTextAndObjectOverlapZero: true, portfolioPolicy, fullScopeRegression: "50_OF_50_PASS" };
