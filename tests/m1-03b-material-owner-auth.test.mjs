...
test("registry and baseline move exactly one owner POST and preserve expected debt metrics", () => {
  ...
  assert.equal(baseline.uncoveredHandlers.length, 39);
  assert.equal(baseline.uncoveredHandlers.some((item) => item.identity === handlerIdentity), false);
  assert.deepEqual(Object.fromEntries(["GET", "POST", "HEAD"].map((method) => [method, baseline.uncoveredHandlers.filter((item) => item.method === method).length])), { GET: 32, POST: 6, HEAD: 1 });
  assert.equal(registry.handlers.filter((item) => item.status === "GAP_UNAUTHENTICATED_WRITE").length, 19);
});
