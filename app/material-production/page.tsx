...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    if (data?.run?.status !== "CANONICAL_UNIT_SCENE_REBUILD" || !["CANONICAL_UNIT_SCENES_V2", "CANONICAL_UNIT_SCENES_V3"].includes(data.canary?.version || "") || data.canary.status !== "SCENE_REBUILD_RUNNING" || working || data.requestLedger.active > 0) return;
  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "SCENE_REBUILD_RUNNING" && ["CANONICAL_UNIT_SCENES_V2", "CANONICAL_UNIT_SCENES_V3"].includes(data.canary.version) && <p className="stateBanner">Canonical contract scenes are being frozen and verified one unit at a time. Provider dispatch remains locked until all 24 frames pass.</p>}
