...
| X31 | P0 | One judge/static atlas can miss temporal, audio, Browser and payoff defects | Append-only L0-L7 exact-artifact shadow orchestration, `0115` calibration schema and canonical QA Cockpit are live with zero PASS authority. Threshold V2 blocks repeated-byte pseudo-diversity, non-owner holdouts, missing receipts and unreconciled requests. Live `0118`-`0123` preserve 62 terminally quarantined candidates and replace them only through fresh-identity/fresh-correlation work orders. Live `0124` selects one bounded audio batch; `0125` adds its exact typed `PLAN_ONLY` contract plus active USD 0.08 / two-request envelope; `0126` certifies one exact audio binding/qualification/current-rights/CURRENT-drift chain; and live `0127` freezes one canonical work request, zero-dispatch route and exact reservation with no native request, reconciliation, generation or spend. Source-qualified `0128` adds a 15-minute one-dispatch authorization that requires fresh paid-plan, voice/model and official-rights R2 read-back while itself making zero synthesis requests and spending $0. Materialize only after the bounded authorization is live and a separately typed provider execution rechecks/consumes it; then continue correlation/owner labels and missing L1/L4, clean/P0 and holdout controls |
