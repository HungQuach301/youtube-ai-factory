...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => void batchAction("PRODUCE_NEXT_WAVE_BATCH_1_SHOT", true), 700);
    return () => window.clearTimeout(timer);
  }, [data?.productionBatch?.status, data?.productionBatch?.completedUnits, data?.productionBatch?.totalUnits, data?.requestLedger.active, working]);
  useEffect(() => {
    if (data?.productionBatch?.audit?.status !== "QA_RUNNING" || working) return;
    const timer = window.setTimeout(() => void batchAction("RUN_WAVE_BATCH_1_AUDIT", true), 2500);
    return () => window.clearTimeout(timer);
  }, [data?.productionBatch?.audit?.status, data?.requestLedger.active, working]);
  ...
  async function batchAction(action: "START_WAVE_BATCH_1" | "ADOPT_WAVE_BATCH_1_ENGINE_ROOT_CORRECTION" | "ADOPT_WAVE_BATCH_1_SEMANTIC_ENGINE_ROOT_CORRECTION" | "PRODUCE_NEXT_WAVE_BATCH_1_SHOT" | "RECONCILE_WAVE_BATCH_1_AUDIT_TRANSPORT" | "RUN_WAVE_BATCH_1_AUDIT", quiet = false) {
    setWorking(action); if (!quiet) setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    <section className="sequenceProofPlane">
      <header><div><p>WAVE 09 · BATCH 1</p><h2>26 new shots must be complete before independent QA.</h2><span>Production compiles, renders, measures and stores each shot as a complete three-state product. QA may release or reject the engine; it cannot repair outputs.</span></div><strong>{data.productionBatch?.status.replaceAll("_", " ") || "READY"}</strong></header>
      ...
      {data.productionBatch && <>
        ...
        {data.productionBatch.status === "PRODUCTION_BLOCKED" && <div className="productStart"><div><small>ROOT PRODUCTION CORRECTION REQUIRED</small><strong>Reject the engine version before any output exists</strong><p>The failed production DoD remains evidence. Apply the replacement compiler only after its full 26-shot layout regression passes; no output repair or QA request is allowed.</p></div><button onClick={() => void batchAction("ADOPT_WAVE_BATCH_1_ENGINE_ROOT_CORRECTION")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "ADOPT_WAVE_BATCH_1_ENGINE_ROOT_CORRECTION" ? "Qualifying replacement engine…" : "Qualify and adopt replacement engine"}</button></div>}
        {data.productionBatch.status === "PRODUCT_COMPLETE" && !data.productionBatch.audit && <div className="productStart"><div><small>INDEPENDENT AUDIT · 7/26 RISK-STRATIFIED SAMPLE</small><strong>Production is closed at 36/166</strong><p>One audit checks actual ENTRY/MIDPOINT/EXIT pixels across the sample. A failure rejects the production-engine version and routes to its root layer; output repair is prohibited.</p></div><button onClick={() => void batchAction("RUN_WAVE_BATCH_1_AUDIT")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "RUN_WAVE_BATCH_1_AUDIT" ? "Starting independent audit…" : "Audit completed Batch 1 · 1 request"}</button></div>}
        ...
        {data.productionBatch.status === "ENGINE_ROOT_CAUSE_REQUIRED" && <div className="productStart"><div><small>SEMANTIC ENGINE REPLACEMENT</small><strong>Preserve V2 evidence and reproduce from Engine V3</strong><p>V3 compiles explicit participants, records, identifiers, connectors, barriers, uncertainty and intervals into viewer-visible scene graphs. It must pass all 26 specifications and portfolio variety before reproduction begins.</p></div><button onClick={() => void batchAction("ADOPT_WAVE_BATCH_1_SEMANTIC_ENGINE_ROOT_CORRECTION")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "ADOPT_WAVE_BATCH_1_SEMANTIC_ENGINE_ROOT_CORRECTION" ? "Qualifying semantic Engine V3…" : "Qualify Engine V3 and reproduce 26"}</button></div>}
        ...
        {data.productionBatch.status === "PASS" && <p className="stateBanner">Batch 1 PASS · 26/26 new shots and 36/166 portfolio products are complete. Batch 2 remains unopened.</p>}
      </>}
    </section>
