...
export default function MaterialProductionPage() {
  ...
  async function canaryAction(action: "AUTHORIZE_CONTROLLED_CANARY" | "AUTHORIZE_CONTROLLED_CANARY_V3" | "AUTHORIZE_CONTROLLED_CANARY_V4" | "AUTHORIZE_CONTROLLED_CANARY_V5" | "START_CONTROLLED_CANARY_UNIT" | "RELEASE_NEXT_CONTROLLED_CANARY_UNIT") {
    setWorking(action); setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "READY_FOR_EXPLICIT_UNIT_RELEASE" && <div className="reliabilityStart"><p>V5 zero-spend preflight passed. MP-001 is bound to the frozen source-bound composite champion; provider dispatch remains disabled until a separate explicit release is confirmed.</p></div>}
