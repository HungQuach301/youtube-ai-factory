...
export async function GET(request: Request) {
  try {
    ...
    const projection = await channelStudioProjection(channel);
    return Response.json(projection, { headers: { "cache-control": "no-store" } });
  } catch (error) {
    const status = error instanceof ChannelNotFoundError ? 404 : 503;
    return Response.json({ error: error instanceof Error ? error.message : "Channel Studio projection is unavailable", fallback: false }, { status, headers: { "cache-control": "no-store" } });
  }
}
