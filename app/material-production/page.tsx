...
type Finding = string | { severity?: string; category?: string; summary?: string };
  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "SEQUENCE_PROOF_PASS_REVIEW" && ["STABILIZATION_RELEASE_V1_TYPED_STATE_MACHINE", "MP002_TARGETED_REPAIR_V1"].includes(data.canary.version) && <div className="reliabilityStart"><p>MP-002 passed the stabilized shot-contract rubric. Open the remaining eight canonical pilot units on the locked renderer; every unit hard-stops at terminal QA, no retry is permitted, and the concurrency ceiling remains 2.</p><button onClick={() => void canaryAction("START_RELEASE_TRAIN_BATCH")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "START_RELEASE_TRAIN_BATCH" ? "Opening bounded batch…" : "Run remaining canonical pilot batch"}</button></div>}
        ...
        {data.canary.status === "SEQUENCE_OR_BATCH_FAILED_PRESERVED" && !["RELEASE_TRAIN_V1_SHOT_CONTRACT_QA", "STABILIZATION_RELEASE_V1_TYPED_STATE_MACHINE"].includes(data.canary.version) && <p className="stateBanner errorState">MP-002 remains blocked under {data.releasePolicy.version}: 84 overall is internal-only and Semantic Fit 68 is below {data.releasePolicy.controlledSemanticFit}. Request 86, retry and batch release remain locked.</p>}
