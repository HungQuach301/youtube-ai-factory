...
Phase 24 is source-accepted and production-pending. Migration `0081` and `CLEAN_AV_MASTER_MATERIALIZATION_V1` bind exactly one `cfp-v1-13` task to the eligible clean-audio parent. The executor creates separate 1920×1080 archival and 1280×720 distribution VP9/Opus masters at 30 fps/48 kHz, then seals exact lineage, checksums, contact sheet, full-decode motion/black/freeze evidence and A/V sync at no more than 80 ms. One bounded `gpt-5.6` visual review and one exact-master Browser review remain independent; only both likely-clean outcomes open a non-delegable owner task. Every dataset, assurance, Golden r10, Stage 11 and release gate stays locked. Next deploy `0081`, materialize once, execute the two independent reviews and stop at owner ground truth.
