...
const WAVE_BATCH_2_V13_REJECTED_ENGINE_VERSION = "SHOT_PRODUCT_ENGINE_V15_CONTRACT_NATIVE_COMPOSITION_COMPILER";
const WAVE_BATCH_2_V13_REPRODUCTION_VERSION = "WAVE_09_BATCH_2_V9_EVIDENCE_NATIVE_VISUAL_LANGUAGE_REPRODUCTION";
const WAVE_BATCH_2_V13_ENGINE_VERSION = "SHOT_PRODUCT_ENGINE_V16_EVIDENCE_NATIVE_VISUAL_LANGUAGE_COMPILER";
...
async function snapshot() {
  ...
  const expectedPreflightAction = v15ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V16_ACTIVATION" : v13ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V13_ACTIVATION" : v12ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V12_ACTIVATION" : v11ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" : "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION";
  const expectedActivationAction = v15ControlVisible ? "ADOPT_WAVE_BATCH_2_V16_ENGINE_ROOT_CORRECTION" : v13ControlVisible ? "ADOPT_WAVE_BATCH_2_V13_ENGINE_ROOT_CORRECTION" : v12ControlVisible ? "ADOPT_WAVE_BATCH_2_V12_ENGINE_ROOT_CORRECTION" : v11ControlVisible ? "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" : "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION";
...
function contractNativeSceneSpecification(contract: Row, bindings: Row[], actors: string[], kind: string) {
  ...
  return {
    version: "EVIDENCE_NATIVE_VISUAL_LANGUAGE_SPECIFICATION_V16", layoutFamily, composition: auditedBlueprint?.composition || `CONTRACT_NATIVE_${layoutFamily}`, persistentLabels: auditedBlueprint?.persistentLabels || [], assumptionPins, canvas: { width: 960, height: 540, safeInset: 48 },
    ...
    rendererContract: { input: "EVIDENCE_NATIVE_VISUAL_LANGUAGE_ONLY", tokenFallbackForbidden: true, geometryAuthoritative: true, audienceTextBudget: { titleGlyphs: 30, objectGlyphs: 22, actionGlyphs: 34, contractClausePixelsForbidden: true }, provenanceRequiredForEveryVisibleObject: true, provenanceRequiredForEveryRelationship: true },
  };
}
...
function renderWaveSemanticScene(manifest: Row, state: 0 | 1 | 2) {
  ...
  if(contractNativeScene){
    ...
    if(auditedComposition){
      ...
      fill(28,30,904,474,backgrounds[ci]);fill(48,42,864,82,backgrounds[ci]);center(composition.replaceAll("_"," "),480,54,2,inks[ci],42);center(["ENTRY","MIDPOINT","EXIT"][state],480,92,2,accent,12);
      fill(48,136,864,310,backgrounds[ci]);fill(64,148,832,38,"#061b18");center(sceneState.action,480,158,2,accent,40);
      if(composition==="ISSUER_LEDGER_CONTAINMENT"){
        fill(62,202,216,156,"#e9eef8");center("ACQUIRER",170,220,3,"#102e55",16);fill(324,202,278,156,"#f7f1e4");center("ECONOMIC LANE",463,220,2,"#593823",22);fill(648,202,248,156,"#e4f5eb");center("ISSUER LEDGER",772,220,2,"#174448",22);
        fill(340,326,246,22,"#d6c9aa");center("EMPTY AT ENTRY",463,332,1,"#593823",20);fill(680,260,184,76,"#ffffff");center("LEDGER SLOT",772,282,2,"#174448",16);
        ...
        if(state>=1)nativeArrow(334,311,432,311,true);if(state===2){line(518,252,518,350,10,"#e8b65d");center("STOP",518,292,3,"#fff0df",8);}fill(156,382,648,46,"#7a2e22");center("NOT NETWORK REVENUE",480,396,3,"#fff1df",24);
        ...
        fill(560,202,330,172,"#e9eef8");center("NETWORK TRAY",725,220,3,"#102e55",18);if(state===0)center("EMPTY",725,292,4,"#52766b",8);else nativeCard(state===1?452:638,282,174,60,"NETWORK AND SERVICE FEES",true);
        fill(448,196,64,184,"#342014");center("IF",480,238,4,"#ffd166",4);center("CONDITION",480,292,2,"#fff0df",12);center(state===0?"CLOSED":"MET",480,330,2,state===0?"#e8b65d":"#72c8a3",8);if(state===2)nativeArrow(512,312,630,312,true);
        ...
        fill(664,202,232,184,"#25183a");center("INTEGRATED EXCEPTION",780,216,2,"#f3eaff",24);fill(686,270,86,64,"#f3eaff");center("ACQUIRER",729,290,1,"#25183a",12);fill(788,270,86,64,"#f3eaff");center("NETWORK",831,290,1,"#25183a",12);if(state===2){line(674,252,886,252,7,"#d6a6ff");line(674,252,674,350,7,"#d6a6ff");line(886,252,886,350,7,"#d6a6ff");}
        ...
        if(state===2){line(402,376,742,376,8,"#d6a6ff");line(402,352,402,392,8,"#d6a6ff");line(742,352,742,392,8,"#d6a6ff");center("INTEGRATED: ACQUIRER + NETWORK",572,398,2,"#f3eaff",34);}
      }else if(composition==="CONTROLLED_SCENARIO_TABLE"){
        const pins=rec(spec.assumptionPins),cols=["PRODUCT","CATEGORY","CHANNEL","DATE"];fill(56,194,848,46,"#43356b");center("HIGHLIGHTED SOURCE SCHEDULE ROW",480,208,2,"#f3eaff",36);cols.forEach((label,index)=>{const x=72+index*210;fill(x,254,194,38,"#25183a");center(label,x+97,264,2,"#f3eaff",16);fill(x,296,194,state===2?72:52,"#ffffff");center(state===0?"PIN REQUIRED":clean(pins[label]),x+97,state===2?320:310,1,"#25183a",26);});
        if(state===2){line(62,246,898,246,8,"#d6a6ff");line(62,378,898,378,8,"#d6a6ff");line(62,246,62,378,8,"#d6a6ff");line(898,246,898,378,8,"#d6a6ff");center("CONTROLLED SCENARIO ENCLOSED",480,390,2,"#d6a6ff",34);}
        ...
        if(composition==="MERCHANT_TO_PROCESSOR_BOUNDARY"){
          fill(64,204,278,170,"#fff4d8");center("MERCHANT TERMINAL",203,220,3,"#593823",22);fill(94,272,218,72,"#ffffff");center("REQUEST FIELDS",203,282,2,"#593823",18);center("AMOUNT · TENDER · ID",203,316,1,"#593823",24);
          ...
          circle(154,296,88,"#f2a65a");center("MERCHANT",154,290,2,"#342014",14);fill(388,192,34,212,"#102e55");center("BOUNDARY",405,414,1,"#e3f0ff",12);fill(568,206,318,180,"#e9eef8");center("PROCESSOR OR ACQUIRER",727,226,2,"#102e55",28);
          const pulseX=[206,388,686][state];circle(pulseX,302,26,"#83c5ff");center("REQUEST",pulseX,296,1,"#071b35",12);if(state>=1)nativeArrow(236,302,pulseX-30,302,true);if(state===2){fill(630,286,194,52,"#ffffff");center("REQUEST AT REST",727,300,2,"#102e55",20);}
          ...
          const nodes=[{x:64,y:210,l:"NETWORK"},{x:286,y:310,l:"PROCESSOR OR ACQUIRER"},{x:530,y:210,l:"MERCHANT"},{x:752,y:310,l:"MERCHANT TERMINAL"}];nodes.forEach((n,index)=>nativeCard(n.x,n.y,160,68,n.l,index<=state+1));for(let index=0;index<3;index++)nativeArrow(nodes[index].x+160,nodes[index].y+34,nodes[index+1].x,nodes[index+1].y+34,index<state+1);
          const pulsePos=[[184,244],[454,344],[832,344]][state];circle(pulsePos[0],pulsePos[1],28,"#ffb27a");center("RESPONSE",pulsePos[0],pulsePos[1]-5,1,"#28140d",14);center(state===0?"ORIGIN: NETWORK":state===1?"CROSSES 1 + 2":"CROSSES 3 · TERMINAL ARRIVAL",480,404,2,"#e8b65d",34);
        }
      }
    }else{
...
function waveProductOracle(manifest: Row) {
  ...
  const checks=[
    ...
    {id:"CONTRACT_NATIVE_SCENE_SPECIFICATION",status:!contractNativeScene||clean(sceneSpec.version)==="EVIDENCE_NATIVE_VISUAL_LANGUAGE_SPECIFICATION_V16"&&Boolean(clean(sceneSpec.composition))&&sceneObjects.length>=3&&sceneRelationships.length===3&&sceneStates.length===3&&sceneObjects.every((item)=>Boolean(clean(item.id))&&Boolean(clean(item.type))&&Boolean(clean(item.label))&&Boolean(clean(item.sourceClause))&&Object.keys(rec(item.geometry)).length===4)&&sceneRelationships.every((item)=>Boolean(clean(item.from))&&Boolean(clean(item.to))&&Boolean(clean(item.relation))&&Boolean(clean(item.sourceClause)))?"PASS":"FAIL",evidence:`${sceneObjects.length} domain primitives · ${sceneRelationships.length} actions · ${sceneStates.length} states`},
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&Boolean(clean(item.activeRelationshipId))&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="EVIDENCE_NATIVE_VISUAL_LANGUAGE_ONLY"&&rendererContract.tokenFallbackForbidden===true&&rendererContract.geometryAuthoritative===true?"PASS":"FAIL",evidence:"renderer consumes only contract-native composition, authoritative geometry and action timeline"},
...
function waveManifestQualification(manifest: Row) {
  const states=arr(manifest.states).map(rec),semantic=rec(manifest.semanticModel),required=arr(semantic.requiredElements).map(clean).filter(Boolean),contractNativeScene=clean(semantic.version)==="EVIDENCE_NATIVE_VISUAL_LANGUAGE_SPECIFICATION_V16",structuredOntology=clean(semantic.version)==="STRUCTURED_VISUAL_ONTOLOGY_V12",semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11"||structuredOntology||contractNativeScene,archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),ontology=rec(semantic.visualOntology),ontologyStates=arr(ontology.states).map(rec),layoutProof=rec(ontology.layoutProof),motionPolicy=rec(ontology.motionPolicy),sceneSpec=rec(semantic.sceneSpecification),sceneObjects=arr(sceneSpec.objects).map(rec),sceneRelationships=arr(sceneSpec.relationships).map(rec),sceneStates=arr(sceneSpec.states).map(rec),sceneLayoutProof=rec(sceneSpec.layoutProof),sceneMotionProof=rec(sceneSpec.motionProof),rendererContract=rec(sceneSpec.rendererContract),corpus=clean(semantic.corpusHashInput).toUpperCase(),genericEscape=contractBound&&["$100 PURCHASE","REWARD ENTRY"].some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy),checks=[
    ...
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="EVIDENCE_NATIVE_VISUAL_LANGUAGE_ONLY"&&rendererContract.tokenFallbackForbidden===true?"PASS":"FAIL"},
...
async function qualifyWaveBatch2V13Activation(context: Awaited<ReturnType<typeof waveBatch2V13ActivationContext>>, progress: Row = {}) {
  ...
  for (const target of scope.slice(startIndex, endIndex)) {
    ...
    const sceneSpecComplete = clean(spec.version) === "EVIDENCE_NATIVE_VISUAL_LANGUAGE_SPECIFICATION_V16" && Boolean(clean(spec.composition)) && objects.length >= 3 && relationships.length === 3 && states.length === 3 && objects.every((item) => Boolean(clean(item.type)) && Boolean(clean(item.sourceClause)) && Object.keys(rec(item.geometry)).length === 4) && relationships.every((item) => Boolean(clean(item.sourceClause)));
    const pixelTraceComplete = states.every((item) => arr(item.visibleObjectIds).length >= 3 && Boolean(clean(item.activeRelationshipId)) && arr(item.movingObjectPosition).length === 2) && rendererContract.input === "EVIDENCE_NATIVE_VISUAL_LANGUAGE_ONLY" && rendererContract.tokenFallbackForbidden === true && rendererContract.geometryAuthoritative === true;
...
async function preflightWaveBatch2V13Activation() {
  ...
  await context.db.prepare("INSERT INTO v7_batch_activation_preflights (id,program_id,run_id,authorization_id,action,source_batch_id,source_audit_id,target_batch_id,input_hash,status,result_json,requests_before,cost_before,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET source_batch_id=excluded.source_batch_id,source_audit_id=excluded.source_audit_id,input_hash=excluded.input_hash,status=excluded.status,result_json=excluded.result_json,requests_before=excluded.requests_before,cost_before=excluded.cost_before,updated_at=excluded.updated_at").bind(preflightId, PROGRAM_ID, context.run.id, context.authorization.id, "PREFLIGHT_WAVE_BATCH_2_V16_ACTIVATION", context.rejected.id, context.audit.id, context.replacementId, context.inputHash, result.status, JSON.stringify(result), context.requestsBefore, context.costBefore, now, now).run();
  const readBack = await context.db.prepare("SELECT * FROM v7_batch_activation_preflights WHERE id=? AND input_hash=?").bind(preflightId, context.inputHash).first<Row>();
...
async function adoptWaveBatch2V13EngineRootCorrection() {
  ...
  const rootCausePolicy = { ...rec(JSON.parse(String(rejected.root_cause_policy_json || "{}"))), incident: "BATCH_2_V15_OBJECT_MOTION_GATE_SCOPE_AND_PORTFOLIO_FAILURE", rejectedEngineVersion: WAVE_BATCH_2_V13_REJECTED_ENGINE_VERSION, rejectedAuditId: audit.id, rejectedAuditScore: Number(audit.score), observedRootCause: priorRoot.rootProductionCause, correctedLayers: ["EVIDENCE_NATIVE_VISUAL_LANGUAGE_COMPILER", "DISCRETE_OBJECT_MOTION", "CONDITIONAL_GATE", "EXCEPTION_SCOPE", "SOURCE_BOUND_ASSUMPTION_PINS", "PORTFOLIO_LANGUAGE_POLICY"], correction: "EXACT_EVIDENCE_CLAUSE_TO_DISTINCT_VISUAL_LANGUAGE_TO_PHYSICAL_PIXELS", replacementEngineVersion: WAVE_BATCH_2_V13_ENGINE_VERSION, fullScopeRegression: "50_SCENE_SPECS_50_PIXEL_TRACES_150_FRAMES_LAYOUT_SAFE", portfolioPolicy, reproduceScope: "ALL_50_PRODUCTS", outputRepair: false, v8V9V10V11V12V13V14V15ProductsPreservedAsEvidence: true, retryPriorAudits: false };
  const productionDoD = { ...rec(JSON.parse(String(rejected.production_dod_json || "{}"))), version: "SHOT_PRODUCT_DOD_V16", evidenceNativeVisualLanguages: true, discreteObjectMotionRequired: true, explicitContainmentAndOwnership: true, exactBoundaryOrder: true, sourceBoundAssumptionPins: true, rendererInputEvidenceNativeVisualLanguageOnly: true, semanticToPixelTraceRequired: true, physicalMotionNotLabelSwap: true, audienceTextBudgetEnforced: true, clippingConnectorAndObjectOverlapZero: true, portfolioPolicy, fullScopeRegression: "50_OF_50_PASS" };
  const modelPolicy = { ...rec(JSON.parse(String(authorization.model_policy_json || "{}"))), version: WAVE_BATCH_2_V13_REPRODUCTION_VERSION, productionEngine: WAVE_BATCH_2_V13_ENGINE_VERSION, scope: "REPRODUCE_ALL_50_AFTER_V15_ENGINE_REJECTION", deterministicProductionRequests: 0, independentAuditRequests: 1, autoRepair: false, priorAuditRetry: false, qaFailureRoute: "ROOT_PRODUCTION_PROCESS" };
  ...
  try {
    await db.batch([
      ...
      db.prepare("UPDATE v7_material_authorizations SET scope='WAVE_09_BATCH_2_ENGINE_V16_REPRODUCTION',status='PAUSED',shot_count=50,max_remote_requests=?,max_actual_spend_usd=?,model_policy_json=?,completed_at=NULL,updated_at=? WHERE id=?").bind(requestsBefore + 1, costBefore + 10, JSON.stringify(modelPolicy), now, authorization.id),
      db.prepare("UPDATE v7_material_runs SET status='BATCH_2_V16_REPRODUCING',mode='EVIDENCE_NATIVE_VISUAL_LANGUAGE_REPRODUCTION' WHERE id=?").bind(run.id),
      db.prepare("UPDATE v7_stage_states SET status='BATCH_2_V16_REPRODUCING',blocker='INTEGRATED_PRODUCTION_TRANSACTION',evidence_summary='V8-V15 evidence preserved · V16 50/50 evidence-native visual languages and pixel traces, 150/150 frames, discrete object motion, exact gates and source-bound pins PASS · no output repair · prior audits not retried',updated_at=? WHERE id=?").bind(now, STAGE_ID),
      db.prepare("INSERT INTO v7_batch_activations (id,program_id,run_id,authorization_id,action,idempotency_key,target_batch_id,input_hash,status,result_json,created_at,committed_at,updated_at) VALUES (?,?,?,?,?,?,?,?, 'COMMITTED',?,?,?,?)").bind(activationId, PROGRAM_ID, run.id, authorization.id, "ADOPT_WAVE_BATCH_2_V16_ENGINE_ROOT_CORRECTION", activationId, replacementId, context.inputHash, JSON.stringify(activationResult), now, now, now),
    ]);
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "PREFLIGHT_WAVE_BATCH_2_V16_ACTIVATION") return Response.json(await preflightWaveBatch2V13Activation(), { status: 200 });
    if (body.action === "ADOPT_WAVE_BATCH_2_V16_ENGINE_ROOT_CORRECTION") return Response.json(await adoptWaveBatch2V13EngineRootCorrection(), { status: 201 });
