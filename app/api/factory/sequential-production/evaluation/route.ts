  ...
  try {
    ...
    if (["owner-label-task", "owner-label-workflow", "provider-history-recovery"].includes(view) || artifactCandidateId || fixtureArtifactId) {
      ...
      if (view === "provider-history-recovery") {
        ...
        const materialization = await controlledFixtureMaterializationSnapshot(env.DB);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!["RUN_CORPUS_VERIFICATION_BATCH", "RECORD_OWNER_LABEL_RECEIPT", "DISCOVER_ELEVENLABS_HISTORY_METADATA", "HASH_ELEVENLABS_HISTORY_AUDIO", "MATERIALIZE_CLEAN_AUDIO_CONTROL"].includes(action)) throw new EvaluationCommandError("EVALUATION_ACTION_INVALID", 400, "Use a supported evaluation action");
    ...
    if (error instanceof EvaluationCommandError || error instanceof FixtureMaterializationError) return Response.json({ error: { code: error.code, message: error.message }, providerRequests: 0, spendUsd: 0 }, { status: error.status, headers: NO_STORE });
