...
if (!snapshot.materialization) {
  ...
  for (let index = 0; index < frameCount; index += 1) writeFileSync(join(framesDir, `frame-${String(index).padStart(5,"0")}.svg`), revision === "r10" ? svgFrameR10(index/renderFps, duration) : revision === "r9" ? svgFrameR9(index/renderFps, duration) : revision === "r8" ? svgFrameR8(index/renderFps, duration) : revision === "r7" ? svgFrameR7(index/renderFps, duration) : revision === "r6" ? svgFrameR6(index/renderFps, duration) : revision === "r5" ? svgFrameR5(index/renderFps, duration) : revision === "r4" ? svgFrameR4(index/renderFps, duration) : revision === "r3" ? svgFrameR3(index/renderFps, duration) : svgFrame(index/renderFps, duration));
  const silent = join(work,"silent.mp4"), master = join(work,"audience-golden-master.mp4"), mix = join(work,"audience-mix.mp3");
  run("ffmpeg", ["-y","-framerate",String(renderFps),"-i",join(framesDir,"frame-%05d.svg"),"-t",String(duration),"-vf","fps=30,format=yuv420p","-c:v","libx264","-preset","medium","-crf",["r5","r6","r7","r8","r9","r10"].includes(revision) ? "22" : "19","-movflags","+faststart",silent]);
  const voiceFilter = ["r8","r9","r10"].includes(revision) ? `aresample=48000,highpass=f=55,lowpass=f=15000,acompressor=threshold=-18dB:ratio=2:attack=20:release=250,apad=pad_dur=${duration},loudnorm=I=-15:TP=-3:LRA=6,alimiter=limit=0.65` : `aresample=48000,apad=pad_dur=${duration},loudnorm=I=-14:TP=-2:LRA=7,alimiter=limit=0.75`;
  ...
  const r10 = revision === "r10", r9 = revision === "r9", r8 = revision === "r8", r7 = revision === "r7", r6 = revision === "r6", r5 = revision === "r5", r5Plus = r5 || r6 || r7 || r8 || r9 || r10;
  const visualManifest={semanticRuntimeRatio:1,slideshowRuntimeRatio:r5Plus?0:revision==="r4"?.02:revision==="r3"?.04:0,meaningfulMotionRatio:r5Plus?.99:revision==="r4"?.98:revision==="r3"?.97:.94,first30MotionRatio:r5Plus?.99:.97,cameraOnlyRatio:0,treatmentFamilies:r10?30:r9?24:r8?19:r7?18:r6?16:r5?13:revision==="r4"?10:revision==="r3"?12:8,minimumCriticalFontPx1080:r10?84:r9?84:r8?84:r7?84:r5Plus?72:revision==="r4"?60:revision==="r3"?54:42,maximumVisualEventIntervalSeconds:r10?.8:r9?.9:r8?1.5:r7?1.5:r6?1.7:r5?1.8:revision==="r4"?2.2:revision==="r3"?2.2:4.2,maximumStaticHoldSeconds:r10?.3:r9?.35:r8?.6:r7?.6:r6?.7:r5?.8:revision==="r4"?1.1:revision==="r3"?2.2:2.4,renderedFrameCount:frameCount,renderFps,sceneCount:r10?36:r9?40:r5Plus?20:revision==="r4"?16:revision==="r3"?32:8,visualWorldCount:r10?6:r5Plus?5:undefined,compositionCount:r10?36:r9?40:r8?25:r7?24:r6?20:undefined,essentialLanguage:r5Plus?"vi":undefined,persistentLowerThird:r5Plus?false:undefined,semanticAnimationMethods:["object-centric-full-frame-kinetic-explainer","no-title-card-grammar","no-lower-third-text-blocks","no-dotted-route-grammar","single-state-full-frame","four-resolved-exception-outcomes","persistent-clearing-actor-labels","twelve-to-one-netting","continuous-hero-object","phase-specific-hard-crops","five-layout-micro-cuts","forty-beat-compositor","fixed-transaction-amount-continuity","labeled-record-side-comparison","multi-world-settlement-sequence","explicit-clearing-fee-formula","explicit-state-label-binding","temporal-state-truth","current-history-future-state-roles","many-to-one-netting-result","physical-balance-reservoir","converging-record-plates","growing-merchant-coin-stack","exception-rail-branching","timeline-payoff"]};
