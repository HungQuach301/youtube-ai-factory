...
| X31 | P0 | One judge/static atlas can miss temporal, audio, Browser and payoff defects | Append-only L0-L7 exact-artifact shadow orchestration, `0115` calibration schema and canonical QA Cockpit are live owner-only with zero PASS authority. Threshold V2 blocks repeated-byte pseudo-diversity, non-owner holdouts, missing receipts and unreconciled requests. Live `0118` reads all 66 remediation objects: 62 pass exact checksum/provenance and four prove historical R2-key overwrite. Live `0119` quarantines those four strict hash/size/metadata collisions. Live `0120`/`0121` inventory and materialize 46 provider plus 16 composite rights tasks with zero attachment or authority. Live `0122` reconciles those tasks with the prior immutable terminal evidence: 46 provider candidates have no exact retained provider-audio match and 16 composites lack exact source-manifest/parent lineage; all 62 are immutable failure-only quarantines requiring controlled-fixture replacement. Live `0123` stores one fresh-identity, fresh-correlation zero-dispatch plan per quarantine and forbids old-byte reuse. Live `0124` selects one bounded audio batch but keeps all 62 blocked; exact drift, cost envelope, reservation, typed request and paid-dispatch approval remain absent. Materialize independently rights-qualified replacements before correlation/owner labels and missing L1/L4, clean/P0 and holdout controls |
