...
| X30 | P0 | Revision-specific rendering can recreate slide grammar | Deterministic Blueprint/Shot/Scene Graph compiler and anti-slide gate implemented without revision branching; actual renderer/canary proof remain required |
...
| X34 | P1 | Hidden Systems Visual DNA is not executable independently of a renderer revision | Frozen Visual Profile/Format can now compile generically; Hidden Systems qualification, renderer and R22 canary proof remain required |
...
| X39 | P0 | Typed runtime/event/timebase/fencing architecture requires integration proof | Migrations `0106`-`0108`, atomic compiler effects, leases/fencing, stale projections and exact replay pass locally; deploy `0108`, integrate qualified workers and run bounded recovery exercises before closure |
