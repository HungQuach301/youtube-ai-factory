...
export async function nichePortfolioProjection(channelId?: string | null, databaseOverride?: DB): Promise<NichePortfolioProjection> {
  ...
  const [artifacts, sources, claims] = programIds.length ? await Promise.all([
  ...
  for (const channel of selectedChannels) {
    ...
    sortedCandidates.forEach(({ candidate }, index) => {
      ...
      comparison.push({
        ...
        expertPriorityBasis: expertPriority === null ? "NOT_RECORDED" : "RECORDED_IN_SOURCE", axes, legacyScore: numberOrNull(candidate.score), marketPotential: market,
  ...
  comparison.sort((a, b) => {
    ...
    return order[a.eligibility] - order[b.eligibility]
      ...
      || a.channel.name.localeCompare(b.channel.name) || a.systemRank - b.systemRank;
  });
  ...
  return {
    ...
    sourceState: "CANONICAL_V7_READ_ONLY_COMPATIBILITY_BRIDGE", scope: { mode: channelId ? "CHANNEL" : "PORTFOLIO", channelId: channelId || null },
    ...
    rankingPolicy: { systemRank: "CANONICAL_V1_CANDIDATE_ORDER", expertPriority: "SEPARATE_VERSIONED_FACT", totalScore: null, note: "The bridge preserves legacy research order but never treats the legacy score as a V2 axis or total score." },
    authority: { activation: "READ_ONLY", v2Commands: "DECLARED_NOT_ROUTED", providerRequests: 0, spendUsd: 0, productionDataMutation: false },
    downstreamGate: { consumer: "CHANNEL_STRATEGY", state: "BLOCKED", reason: "Slice 2 is comparison-only; expert priority, selection, commitment and strategy activation remain separate future commands." },
