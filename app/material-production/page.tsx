...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    if (!data?.sequenceProof || !["RENDER_QUEUED", "RENDERING", "QA_RUNNING"].includes(data.sequenceProof.status) || working) return;
    const timer = window.setTimeout(() => void (data.sequenceProof?.status === "QA_RUNNING" ? sequenceAction("RUN_SEQUENCE_QA", true) : load()), 2500);
    return () => window.clearTimeout(timer);
  }, [data?.sequenceProof?.status, data?.requestLedger.active, working, load]);
  ...
  async function sequenceAction(action: "PLAN_SEQUENCE_PROOF" | "RUN_SEQUENCE_QA", quiet = false) {
    setWorking(action); if (!quiet) setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    <section className="sequenceProofPlane">
      <header><div><p>30-SECOND SEQUENCE GATE</p><h2>Ten sealed MP units become one audience-facing playback.</h2><span>Exact promoted pixels only · 30 frames · 30 seconds · no regeneration · scale remains locked until sequence QA passes.</span></div><strong>{data.sequenceProof?.status.replaceAll("_", " ") || (data.canary?.passedUnits === 10 ? "READY" : "BLOCKED")}</strong></header>
      ...
      {data.sequenceProof && <>
        ...
        {data.sequenceProof.status === "QA_REQUIRED" && <div className="reliabilityStart"><p>Stored WebM and 10 unit samples passed byte/hash/duration read-back. Run one bounded sequence QA request.</p><button onClick={() => void sequenceAction("RUN_SEQUENCE_QA")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "RUN_SEQUENCE_QA" ? "Starting sequence QA…" : "Run sequence QA · 1 request"}</button></div>}
