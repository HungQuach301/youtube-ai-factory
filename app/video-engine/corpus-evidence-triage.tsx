...
export function CorpusEvidenceTriage({ blocked, excluded, metadataReview, metadataAccepted, rightsPending, rightsAccepted, rightsBases, rightsKinds, rightsProviders, rightsTasks = [], reasons, facts, states, kinds }: {
  ...
  return <section aria-label="Corpus evidence triage">
    ...
    <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "flex-end", gap: 14, margin: "18px 18px 0" }}>
      {providerTasks > 0 ? <a href="/api/factory/sequential-production/evaluation?view=provider-history-recovery" target="_blank" rel="noreferrer" style={{ color: "#f4cf78", fontSize: 12, fontWeight: 800 }}>Khôi phục evidence ElevenLabs ↗</a> : null}
      <a href="/api/factory/sequential-production/evaluation?view=owner-label-workflow" target="_blank" rel="noreferrer" style={{ color: "#9fe0c2", fontSize: 12, fontWeight: 800 }}>Mở màn hình đánh giá toàn trang ↗</a>
    </div>
    <iframe style={{ display: "block", width: "calc(100% - 36px)", minHeight: 1120, margin: 18, border: "1px solid #35604e", borderRadius: 14, background: "#101f19" }} src="/api/factory/sequential-production/evaluation?view=owner-label-workflow" title="Đánh giá mẫu sản xuất bằng mắt và tai" loading="eager" />
  </section>;
}
