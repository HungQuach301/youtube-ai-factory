...
async function projectSnapshot(projectId: string) {
  ...
  if (editArtifact && scriptArtifact && audioArtifact && profile) {
    ...
    const optimizedAssetAuditReady = optimizedMediaReady && uniquePhysicalAssets === 40 && optimizedClips.every((clip) => Number(clip.visualAsset?.semanticScore || 0) >= 85 && (clip.primaryFamily !== "MACRO_REALITY" || clip.visualAsset?.mimeType.startsWith("video/")));
