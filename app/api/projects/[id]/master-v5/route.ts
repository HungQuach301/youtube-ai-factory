...
async function snapshot(projectId: string) {
  ...
  const latestV5Evidence = records.filter((record) => record.entityType === "RENDER_EVIDENCE" && [5,51].includes(Number(jsonObject(record.settingsJson).renderPipelineVersion))).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))[0];
...
async function materializeSoundscape(projectId: string) {
  ...
  await db.insert(workflowEvents).values({ projectId, toStatus: "V51_MASTER_PREFLIGHT", eventType: "V51_SOUNDSCAPE_MATERIALIZED", summary: "V5.1 stored three full-length ElevenLabs music arcs, four ambience zones and eight narration-bound SFX cues" });
}
