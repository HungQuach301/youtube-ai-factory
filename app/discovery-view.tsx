    ...
    {data && <>
      ...
      {mode === "intelligence" ? <>
        ...
        <section className={`pfWorkflowGate ${data.workflow.result?.downstreamGate.state === "READY_FOR_TYPED_HANDOFF" ? "ready" : "blocked"}`}>
          ...
          {data.workflow.result ? <><div className="pfWorkflowMetrics"><article><small>READINESS</small><strong>{data.workflow.result.readiness.replaceAll("_", " ")}</strong></article><article><small>POLICY</small><strong>{data.workflow.result.policyVersion}</strong></article><article><small>DECISION BINDING</small><strong>{data.workflow.decisionBinding.replaceAll("_", " ")}</strong></article><article><small>CHANNEL STRATEGY</small><strong>{data.workflow.result.downstreamGate.reason}</strong></article></div><footer><div><small>ALLOWED NEXT ACTIONS</small><span>{data.workflow.result.allowedNextActions.join(" · ")}</span></div><div><small>COMMAND AUTHORITY</small><span>{data.workflow.result.commandContracts.length} declared · all not routed · provider requests 0 · spend USD 0</span></div></footer></> : <p>{data.workflow.blockers.join(" ")}</p>}
        </section>
