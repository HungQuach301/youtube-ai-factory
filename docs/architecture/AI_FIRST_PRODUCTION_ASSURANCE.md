...
The canonical `/qa-cockpit` projection reads immutable Factory calibration and Assurance receipts. It must show a truthful empty or blocked state when evidence is absent, distinguish measured `QUALIFIED_CANDIDATE` from a registered current qualification, expose dependency mismatch/drift and keep PASS authority closed. Its V13 audio-control view separately exposes exact binding, qualification, rights, drift, canonical route/reservation and the current short-lived paid-dispatch authorization. Read-only provider observations never count as generated audio or provider dispatch, and an authorization never counts as a provider request. Remediation work items remain count-ineligible until reviewed into a later immutable corpus snapshot. It cannot substitute legacy QA, demo rows or inferred completion.
...
Current live evidence supersedes the source-ready `0123` sentence later in this section. Owner-only Sites v557 stores one COMPLETE replacement plan with 62 planned / 0 materialized / 62 pending work orders: 46 provider-audio and 16 composite-master routes. D1 pagination verifies 62 unique replacement identities and correlation groups, valid exact hashes, the old-byte reuse/derivation ban, idempotent replay and zero downstream authority/provider requests/spend. Temporary runtime values are absent at environment revision 68. New controlled fixtures, corpus review, dataset execution and exact qualification registration remain pending.
...
This foundation deliberately has no Production acceptance authority. A fully passing run may compute candidate `AI_ACCEPTED`, but the persisted outcome remains `HUMAN_ESCALATION_REQUIRED` with `ADVISORY_ONLY` authority until judge calibration and a separately governed maturity transition succeed. Sites v539 verifies all four migration `0115` tables live and empty, the canonical QA Cockpit route deployed, owner-only access preserved and no temporary writer/Assurance/R22 flags at environment revision 50. Calibration threshold V2 hardens anti-correlation and execution-receipt admission before dataset execution. Migration `0118` reads all 66 current remediation objects: 62 pass exact checksum/provenance binding and four prove that historical mutable R2 keys now hold different artifact bytes. Migration `0119` and QA Cockpit V4 are live in Sites v547: one zero-provider run quarantines the four unrecoverable originals, references four already exact-verified survivors and narrows current-rights work to 62 items. Migration `0120` and QA Cockpit V5 are live in Sites v549. The fail-closed inventory stores one run and 62 receipts—46 provider terms/plan tasks and 16 composite parent-manifest tasks—while all four quarantines remain excluded. No exact current source receipt is attachable, so the result is 0 attached / 62 pending and replay is idempotent. Provider evidence must bind exact request/response/artifact bytes and current paid-plan coverage; composite evidence must cover the exact complete parent set; authorship evidence must be exact, territorial, commercial and current. The inventory grants neither rights PASS nor corpus-count authority. Migration `0121` and QA Cockpit V6 are live in Sites v552: every pending inventory receipt became one immutable `RECEIPT_REQUIRED` collection task with typed evidence requirements, while source-receipt fields and every authority/cost field remain structurally empty or zero. D1 verifies 46 provider and 16 composite tasks, idempotent replay and zero network/provider/spend behavior; the materializer cannot mutate candidate rights. Temporary values are absent at environment revision 63. Migration `0122` and QA Cockpit V7 are live in owner-only Sites v554 and reconcile this queue with the older immutable terminal evidence instead of requesting documents that cannot restore missing byte lineage. Candidate-specific provider diagnostics prove all 46 historical audio candidates have no exact retained provider-audio match, while lineage diagnostics prove all 16 composites lack exact source-manifest/parent binding. One COMPLETE run appends 62 failure-only quarantine and controlled-fixture replacement receipts; full D1 pagination and idempotent replay verify zero rights eligibility, provider requests, spend or downstream authority. Temporary runtime values are absent at environment revision 65. Migration `0123` and QA Cockpit V8 are source-qualified: each terminal quarantine maps to one immutable fresh-identity/fresh-correlation work order with route-specific generation-time rights and exact-lineage contracts, while historical bytes are forbidden as replacements or parents. Planning cannot materialize bytes, dispatch a provider, reserve spend or grant downstream authority. Live planning, new controlled fixtures, corpus review, dataset execution and exact qualification registration remain pending. No Browser session, R22, master, release or publication action is implemented by this slice.

Migration `0128` and QA Cockpit V13 add only the exact paid-dispatch authorization boundary. Fresh subscription, voice/model and official-rights observations plus R2 read-back may authorize one request for fifteen minutes against the immutable `0127` reservation. The authorization is not a generation receipt and creates no artifact, native request, reconciliation, actual cost, corpus-count, qualification/PASS, R22, master, release or publication authority.
