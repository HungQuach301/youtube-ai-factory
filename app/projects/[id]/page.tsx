...
function MediaStudio({ projectId, setProjectNotice }: { projectId: string; setProjectNotice: (message: string) => void }) {
  ...
  async function mediaAction(action: "GENERATE_DIAGRAMS" | "REGISTER_LINK" | "SELECT_DISCOVERY" | "SET_AUTOMATION_MODE" | "AUTO_SOURCE_ALL" | "VERIFY_RIGHTS" | "APPROVE_ASSET" | "BUILD_ASSEMBLY", input: { sceneId?: string; assetId?: string; sourceUrl?: string; candidate?: DiscoveryCandidate; verificationMode?: "AUTOPILOT" | "REVIEW" } = {}) {
    setWorking(input.assetId || input.sceneId || action);
    const labels = { GENERATE_DIAGRAMS: "Creating channel-owned diagrams…", REGISTER_LINK: "Registering media candidate…", SELECT_DISCOVERY: "Adding the selected candidate to rights review…", SET_AUTOMATION_MODE: "Updating the media verification policy…", AUTO_SOURCE_ALL: "Autopilot is searching, verifying and selecting media for every uncovered scene…", VERIFY_RIGHTS: "Recording human rights verification…", APPROVE_ASSET: "Approving asset for the timeline…", BUILD_ASSEMBLY: "Running assembly critics and building the timeline…" };
