...
| X21 | P0 | The immutable one-fixture materialization ceiling prevented a compliant replacement after paid-plan activation | `HANDLED__SOURCE_READY_PRODUCTION_PENDING`: migration `0075` adds exactly one append-only commercial replacement with generation-time plan, Terms, native request, exact-byte and R2 evidence; Factory perceptual QA is separately bounded and independent-only; 180/180 regressions pass with zero source-slice request/spend | Technical media/WP7 |
