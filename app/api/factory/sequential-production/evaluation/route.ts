  ...
  try {
    ...
    if (["owner-label-task", "owner-label-workflow"].includes(view) || artifactCandidateId) {
      ...
      if (view === "owner-label-workflow") {
        ...
        const primaryTotal = number(attention?.primary_representative_count), remaining = number(attention?.actionable_open), completed = Math.max(0, primaryTotal - remaining), itemNumber = Math.min(primaryTotal, completed + 1);
