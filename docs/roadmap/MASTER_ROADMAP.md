...
- Factory corpus Browser QA: source-ready under `FACTORY_BROWSER_QA_POLICY_V1` and migration `0065`; the no-authority Browser fixture passes and production activation/read-back is next.
...
Status: `PHASE_11_FACTORY_BROWSER_QA_SOURCE_READY__PRODUCTION_ACTIVATION_NEXT`.
...
Phase 11 adds `FACTORY_BROWSER_QA_POLICY_V1` and migration `0065` as a corpus-evaluation lane separate from `BROWSER_ASSURANCE_GATE_V1`. Each of the 47 preserved `BROWSER_REQUIRED` receipts produces one exact-hash task. The execution surface requires full visible playback, control/event ordering, audio/motion/focus/reflow/runtime checks and complete media-observable taxonomy labels before one immutable independent receipt can be stored. `UNCERTAIN` fails closed to owner attention. Source tests pass 22/22, the verified build passes and the no-authority Browser fixture completes. Production activation, exact task/modality read-back and bounded queue execution are next; rights-evidence collection and gold-set construction remain independent gates.
