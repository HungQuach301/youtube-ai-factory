CREATE TABLE `v7_youtube_golden_revision_4_receipts` (
  ...
  `revision_key` text NOT NULL CHECK (`revision_key` = 'AUDIENCE_GOLDEN_REVISION_4'),
...
CREATE TRIGGER `v7_youtube_golden_revision_4_no_update` BEFORE UPDATE ON `v7_youtube_golden_revision_4_receipts` BEGIN SELECT RAISE(ABORT, 'YOUTUBE_GOLDEN_REVISION_4_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `v7_youtube_golden_revision_4_no_delete` BEFORE DELETE ON `v7_youtube_golden_revision_4_receipts` BEGIN SELECT RAISE(ABORT, 'YOUTUBE_GOLDEN_REVISION_4_IMMUTABLE'); END;
