...
async function connectionCatalog() {
  ...
  const connections: FactoryConnection[] = [
    ...
    { id: "media_executor", name: "Media Execution Worker", group: "MEDIA_EXECUTION", status: configured(env.MEDIA_EXECUTOR_SHARED_SECRET) ? "CONNECTED" : "KEY_REQUIRED", capability: "Queue-backed ffprobe, ffmpeg frame extraction, source-hash verification and bounded media transforms.", requiredKeys: ["MEDIA_EXECUTOR_SHARED_SECRET"], securityModel: "Private-site transport auth + executor secret + per-job lease token · no AI authority", nextAction: configured(env.MEDIA_EXECUTOR_SHARED_SECRET) ? "Worker authentication ready · heartbeat must prove runtime health" : "Provision the protected executor secret" },
