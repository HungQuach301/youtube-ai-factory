...
| X31 | P0 | One judge/static atlas can miss temporal, audio, Browser and payoff defects | Append-only L0-L7 exact-artifact shadow orchestration, `0115` calibration schema and canonical QA Cockpit are live owner-only with zero PASS authority. Threshold V2 blocks repeated-byte pseudo-diversity, non-owner holdouts, missing receipts and unreconciled requests. Live `0118` reads all 66 remediation objects: 62 pass exact checksum/provenance and four prove historical R2-key overwrite. Live `0119` quarantines those four strict hash/size/metadata collisions. Live `0120`/`0121` inventory and materialize 46 provider plus 16 composite rights tasks with zero attachment or authority. Live `0122` reconciles those tasks with immutable terminal evidence; all 62 are failure-only quarantines requiring controlled-fixture replacement. Live `0123` stores fresh-identity/fresh-correlation zero-dispatch work orders and forbids old-byte reuse. Live `0124` selects one bounded audio batch, and `0125` adds its exact typed `PLAN_ONLY` contract plus active USD 0.08 / two-request envelope. Live `0126` preserves one fail-closed 403 rights observation and then certifies exactly one audio binding/qualification/current-rights/CURRENT-drift/route-ready chain through official-rights HTTP 200 plus exact R2 read-back. Canonical work request, route, reservation, provider generation and dispatch remain absent. Materialize only after the canonical plan-only route/reservation step and a separately authorized paid dispatch; then continue correlation/owner labels and missing L1/L4, clean/P0 and holdout controls |
...
An issue closes only through the named implementation, relevant tests/benchmarks, stored evidence, decision update and synchronized Git checkpoint. A document, UI label or plan alone cannot close an implementation issue.
