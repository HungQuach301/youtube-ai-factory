...
const SCENE_COPY: Record<GoldenSceneKind, [string, string]> = {
  ...
  OPENING_NET: ["$100 paid out ≠ $100 kept", "Contractual deduction determines merchant net"],
  ...
  COVERED_DEBIT: ["Covered debit · issuer ceiling", "$0.27 is not total merchant acceptance cost"],
};
...
function formulaStep(label: string, value: string, active: boolean, accent: string, width: number | string = 190) { return h("div", { style: { display: "flex", flexDirection: "column", justifyContent: "center", width, minHeight: 118, padding: "16px 18px", borderRadius: 14, opacity: active ? 1 : .3, background: active ? "#f7f1df" : "#173d35", color: active ? "#0b3029" : "#d7e4df", borderTop: `7px solid ${active ? accent : "#55766d"}` } }, h("div", { style: { display: "flex", color: active ? "#4b665e" : "#c1d2cc", fontSize: 14, fontWeight: 800, letterSpacing: ".04em", textTransform: "uppercase" } }, label), h("div", { style: { display: "flex", marginTop: 8, fontSize: 29, lineHeight: 1, fontWeight: 850 } }, value)); }
...
function sceneBody(scene: GoldenSceneKind, reveal: number, accent: string): ReactNode {
  if (scene === "OPENING_NET") return h("div", { style: { display: "flex", flexDirection: "column", gap: 18, width: "100%" } },
    h("div", { style: { display: "flex", alignItems: "center", width: "100%" } }, formulaStep("Gross sale", "$100.00", true, accent, 230), arrow(reveal >= 2, accent), formulaStep("Contractual deduction", "VARIABLE", reveal >= 2, accent, 290), arrow(reveal >= 3, accent), formulaStep("Merchant net", "NOT YET KNOWN", reveal >= 3, accent, 260)),
    h("div", { style: { display: "flex", justifyContent: "center", padding: 13, borderRadius: 12, background: "#3a3019", color: accent, fontSize: 19, fontWeight: 800, letterSpacing: ".03em", textTransform: "uppercase" } }, "$100 gross − contractual deduction = merchant net"));
  ...
  if (scene === "SQUARE_SCOPE") return h("div", { style: { display: "flex", flexDirection: "column", gap: 22, width: "100%" } }, scopeRibbon("SQUARE", "FREE", "IN PERSON", reveal >= 2, accent), h("div", { style: { display: "flex", justifyContent: "space-between", padding: "18px 22px", borderRadius: 14, background: reveal >= 3 ? "#f7f1df" : "#173d35", color: reveal >= 3 ? "#0b3029" : "#c1d2cc", fontSize: 21, fontWeight: 800, textTransform: "uppercase" } }, h("span", null, "Public US price"), h("span", null, "Checked for this production")));
  if (scene === "SQUARE_CALC") return h("div", { style: { display: "flex", flexDirection: "column", gap: 18, width: "100%" } }, h("div", { style: { display: "flex", alignItems: "center", width: "100%" } }, formulaStep("Gross", "$100.00", true, accent, 180), arrow(reveal >= 2, accent), formulaStep("2.6% of gross", "$2.60", reveal >= 2, accent, 200), h("div", { style: { display: "flex", width: 48, justifyContent: "center", color: accent, fontSize: 34, fontWeight: 800 } }, "+"), formulaStep("Fixed", "$0.15", reveal >= 2, accent, 170), h("div", { style: { display: "flex", width: 48, justifyContent: "center", color: accent, fontSize: 34, fontWeight: 800 } }, "="), formulaStep("Processing charge", "$2.75", reveal >= 3, accent, 220)), scopeRibbon("SQUARE", "FREE", "IN PERSON", true, accent));
  if (scene === "SQUARE_NET") return h("div", { style: { display: "flex", flexDirection: "column", gap: 18, width: "100%" } }, h("div", { style: { display: "flex", alignItems: "center", width: "100%" } }, formulaStep("Gross", "$100.00", true, accent, 210), h("div", { style: { display: "flex", width: 60, justifyContent: "center", color: accent, fontSize: 38, fontWeight: 850 } }, "−"), formulaStep("Square charge", "$2.75", reveal >= 2, accent, 230), h("div", { style: { display: "flex", width: 60, justifyContent: "center", color: accent, fontSize: 38, fontWeight: 850 } }, "="), formulaStep("Illustrative net", "$97.25", reveal >= 3, accent, 250)), h("div", { style: { display: "flex", justifyContent: "center", padding: 13, borderRadius: 12, background: "#3a3019", color: accent, fontSize: 18, fontWeight: 800, textTransform: "uppercase" } }, "Before unrelated fees · reserves · later reversals"));
  if (scene === "STRIPE_CALC") return h("div", { style: { display: "flex", flexDirection: "column", gap: 18, width: "100%" } }, scopeRibbon("STRIPE", "US STANDARD", "DOMESTIC CARD", true, accent), h("div", { style: { display: "flex", alignItems: "center", width: "100%" } }, formulaStep("Gross", "$100.00", true, accent, 170), arrow(reveal >= 2, accent), formulaStep("2.9%", "$2.90", reveal >= 2, accent, 170), h("div", { style: { display: "flex", width: 45, justifyContent: "center", color: accent, fontSize: 32, fontWeight: 850 } }, "+"), formulaStep("Fixed", "$0.30", reveal >= 2, accent, 170), h("div", { style: { display: "flex", width: 45, justifyContent: "center", color: accent, fontSize: 32, fontWeight: 850 } }, "="), formulaStep("Processing charge", "$3.20", reveal >= 3, accent, 220)));
  if (scene === "STRIPE_NET") return h("div", { style: { display: "flex", flexDirection: "column", gap: 18, width: "100%" } }, h("div", { style: { display: "flex", alignItems: "center", width: "100%" } }, formulaStep("Gross", "$100.00", true, accent, 210), h("div", { style: { display: "flex", width: 60, justifyContent: "center", color: accent, fontSize: 38, fontWeight: 850 } }, "−"), formulaStep("Stripe charge", "$3.20", reveal >= 2, accent, 230), h("div", { style: { display: "flex", width: 60, justifyContent: "center", color: accent, fontSize: 38, fontWeight: 850 } }, "="), formulaStep("Illustrative net", "$96.80", reveal >= 3, accent, 250)), h("div", { style: { display: "flex", justifyContent: "center", padding: 13, borderRadius: 12, background: "#3a3019", color: accent, fontSize: 18, fontWeight: 800, textTransform: "uppercase" } }, "Custom · international · manually entered pricing can differ"));
  if (scene === "COVERED_DEBIT") return h("div", { style: { display: "flex", flexDirection: "column", gap: 16, width: "100%" } }, h("div", { style: { display: "flex", alignItems: "center", gap: 18, width: "100%" } }, formulaStep("Covered debit", "UP TO $0.27", true, accent, 245), formulaStep("What it is", "ISSUER INTERCHANGE CEILING ONLY", reveal >= 2, accent, 315), formulaStep("What it is not", "TOTAL MERCHANT ACCEPTANCE COST", reveal >= 3, accent, 315)), h("div", { style: { display: "flex", justifyContent: "space-between", padding: "14px 20px", borderRadius: 12, background: "#3a3019", color: accent, fontSize: 18, fontWeight: 850, textTransform: "uppercase" } }, h("span", null, "Not total cost"), h("span", null, "Exempt debit treated differently")));
