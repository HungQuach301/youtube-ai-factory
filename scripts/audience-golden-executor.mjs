...
const previewRevision15 = process.env.AUDIENCE_GOLDEN_PREVIEW_R15 === "1";
const previewRevision16 = process.env.AUDIENCE_GOLDEN_PREVIEW_R16 === "1";
const previewRevision17 = process.env.AUDIENCE_GOLDEN_PREVIEW_R17 === "1";
const previewRevision18 = process.env.AUDIENCE_GOLDEN_PREVIEW_R18 === "1";
const previewRevision19 = process.env.AUDIENCE_GOLDEN_PREVIEW_R19 === "1";
const previewRevision20 = process.env.AUDIENCE_GOLDEN_PREVIEW_R20 === "1";
const previewRevision21 = process.env.AUDIENCE_GOLDEN_PREVIEW_R21 === "1";
if ((!siteToken || !automationToken) && !previewRevision11 && !previewRevision12 && !previewRevision13 && !previewRevision14 && !previewRevision15 && !previewRevision16 && !previewRevision17 && !previewRevision18 && !previewRevision19 && !previewRevision20 && !previewRevision21) throw new Error("FACTORY_SITE_AUTH_TOKEN and AUDIENCE_GOLDEN_AUTOMATION_TOKEN are required");
...
const r16ContactlessData=assetData("public/golden/r16-contactless.jpg"),r16AuthorizationData=assetData("public/golden/r16-authorization.jpg"),r16ClearingData=assetData("public/golden/r16-clearing.jpg"),r16SettlementData=assetData("public/golden/r16-settlement.jpg"),r16ExceptionsData=assetData("public/golden/r16-exceptions.jpg");
const r17IssuerData=assetData("public/golden/r17-issuer-vault.jpg"),r17HoldData=assetData("public/golden/r17-hold-ledger.jpg"),r17FeeData=assetData("public/golden/r17-fee-routing.jpg"),r17NettingData=assetData("public/golden/r17-netting-transfer.jpg");
...
// Revision 15 is one continuous 14,400px spatial world. The camera follows one
// immutable TX-01 / 2,00 object through physical districts; labels live on
// world objects rather than title cards, and transfer rails always sit behind
// values. At most two actors and two monetary values enter any camera crop.
function svgFrameR15(t, duration) {
  const p=Math.max(0,Math.min(.999999,t/duration)),worldWidth=14400,txX=620+p*12980,txY=780-Math.sin(p*Math.PI*8)*90,camera=Math.max(0,Math.min(worldWidth-2560,txX-1280));
  const pulse=.94+.06*Math.sin(t*5),phase=Math.min(31,Math.floor(p*32)),local=p*32-phase,ease=local<.5?2*local*local:1-Math.pow(-2*local+2,2)/2;
  const text=(x,y,value,size=136,color="#ffffff",anchor="middle",weight=900)=>`<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="${size}" font-weight="${weight}" fill="${color}" paint-order="stroke" stroke="#020605" stroke-width="${Math.max(10,Math.round(size*.075))}" stroke-linejoin="round">${esc(value)}</text>`;
  const rail=(x1,y1,x2,y2,color="#6fffd0",width=34)=>`<path d="M${x1} ${y1}C${x1+(x2-x1)*.34} ${y1-120} ${x1+(x2-x1)*.66} ${y2+120} ${x2} ${y2}" fill="none" stroke="#020807" stroke-width="${width+28}"/><path d="M${x1} ${y1}C${x1+(x2-x1)*.34} ${y1-120} ${x1+(x2-x1)*.66} ${y2+120} ${x2} ${y2}" fill="none" stroke="${color}" stroke-width="${width}" opacity=".72"/>`;
  const tower=(x,y,w,h,color,label)=>`<g transform="translate(${x} ${y})"><path d="M0 0L${w*.5} -${h*.22}L${w} 0L${w} ${h}L${w*.5} ${h+h*.22}L0 ${h}Z" fill="#071713" stroke="${color}" stroke-width="22"/><path d="M${w*.5} -${h*.22}V${h+h*.22}M0 0L${w*.5} ${h*.22}L${w} 0" fill="none" stroke="${color}" stroke-width="14" opacity=".38"/>${text(w*.5,h*.56,label,128,color)}</g>`;
  const plaque=(x,y,label,color="#ffffff")=>`<g transform="translate(${x} ${y})"><path d="M-330-95H330L390 0L330 95H-330L-390 0Z" fill="#04110e" stroke="${color}" stroke-width="18"/>${text(0,40,label,126,color)}</g>`;
  const principal=(x,y,r=158)=>`<g transform="translate(${x} ${y}) scale(${pulse})" filter="url(#r15shadow)"><circle r="${r}" fill="#04110e" stroke="#ffffff" stroke-width="20"/><circle r="${r-28}" fill="#6fffd0"/><text y="-${Math.round(r*.08)}" text-anchor="middle" font-size="${Math.round(r*.54)}" font-weight="900" fill="#03100d">TX-01</text><text y="${Math.round(r*.58)}" text-anchor="middle" font-size="${Math.round(r*.62)}" font-weight="900" fill="#03100d">2,00</text></g>`;
  const fee=(x,y)=>`<g transform="translate(${x} ${y})"><circle r="120" fill="#04110e" stroke="#ff8f86" stroke-width="20"/><text y="-12" text-anchor="middle" font-size="62" font-weight="900" fill="#ff8f86">FEE</text><text y="70" text-anchor="middle" font-size="82" font-weight="900" fill="#ffffff">0,05</text></g>`;
  const record=(x,y)=>`<g transform="translate(${x} ${y})"><path d="M-270-320H170L270-220V320H-270Z" fill="#f3efe5" stroke="#ffd266" stroke-width="24"/><path d="M170-320V-220H270" fill="none" stroke="#ffd266" stroke-width="20"/>${text(0,-135,"ĐÃ KHỚP",104,"#123128")}${text(0,95,"2,05",190,"#11855f")}<path d="M-145 210H145" stroke="#2b483e" stroke-width="24" stroke-linecap="round"/></g>`;
  const backdropData=p<.1?worldAssetData:p<.31?bankAssetData:p<.5?clearingAssetData:p<.83?settlementAssetData:exceptionsAssetData,backdropZoom=1.12+local*.04,backdropW=Math.round(2560*backdropZoom),backdropH=Math.round(1440*backdropZoom);
  const defs=`<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1440" viewBox="0 0 2560 1440"><defs><linearGradient id="r15sky" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#03101d"/><stop offset=".5" stop-color="#092b31"/><stop offset="1" stop-color="#07130e"/></linearGradient><linearGradient id="r15road" x1="0" y1="0" x2="1" y2="0"><stop stop-color="#6fffd0"/><stop offset=".5" stop-color="#ffd266"/><stop offset="1" stop-color="#66c8ff"/></linearGradient><filter id="r15shadow"><feDropShadow dx="0" dy="28" stdDeviation="22" flood-opacity=".68"/></filter><pattern id="r15grid" width="180" height="100" patternUnits="userSpaceOnUse"><path d="M0 100L90 50L180 100M90 50V0" fill="none" stroke="#ffffff" stroke-opacity=".055" stroke-width="5"/></pattern></defs><rect width="2560" height="1440" fill="url(#r15sky)"/><image href="data:image/jpeg;base64,${backdropData}" x="${Math.round((2560-backdropW)/2+(ease-.5)*80)}" y="${Math.round((1440-backdropH)/2)}" width="${backdropW}" height="${backdropH}" preserveAspectRatio="xMidYMid slice" opacity=".34"/><rect width="2560" height="1440" fill="#020807" opacity=".34"/><g transform="translate(${-camera} 0)" font-family="Arial,Helvetica,sans-serif"><rect width="${worldWidth}" height="1440" fill="url(#r15grid)"/><path d="M0 1080L${worldWidth} 920V1440H0Z" fill="#04110e" fill-opacity=".82"/><path d="M200 850C2600 650 4700 1020 7000 790S11000 650 14100 820" fill="none" stroke="#020807" stroke-width="120"/><path d="M200 850C2600 650 4700 1020 7000 790S11000 650 14100 820" fill="none" stroke="url(#r15road)" stroke-width="58" opacity=".7"/>`;
  const districts=[
    tower(160,350,760,520,"#6fffd0","ĐIỂM CHẠM"),
    tower(1800,270,850,610,"#66c8ff","NGÂN HÀNG"),
    `<g transform="translate(3500 280)"><path d="M0 720Q520 500 1040 720V1100H0Z" fill="#082c46" stroke="#66c8ff" stroke-width="24"/><path d="M0 ${750-(phase>7?170:0)}Q520 ${530-(phase>7?170:0)} 1040 ${750-(phase>7?170:0)}V1100H0Z" fill="#138ab3" opacity=".5"/>${text(520,200,"GIỮ TẠM",154,"#ffd266")}${text(520,395,"10,00 → 8,00",144,"#ffffff")}</g>`,
    tower(5200,250,920,650,"#ffd266","CLEARING"),
    tower(6900,230,940,690,"#66c8ff","NETTING"),
    tower(8600,300,780,570,"#6fffd0","ACQUIRER"),
    tower(10200,320,860,550,"#ffd266","NGƯỜI BÁN"),
    `<g transform="translate(12480 680)"><circle r="300" fill="#061411" stroke="${["#ff8f86","#ffd266","#6fffd0","#66c8ff"][Math.min(3,Math.max(0,Math.floor((p-.83)/.0325)))]}" stroke-width="30"/>${text(0,36,["HỦY","HẾT GIỮ","HOÀN","TRANH CHẤP"][Math.min(3,Math.max(0,Math.floor((p-.83)/.0325)))],p>.925?96:132,"#ffffff")}</g>`
  ].join("");
  let action="";
  if(p>=.10&&p<.20) action=plaque(txX,250,p<.145?"KIỂM TRA ĐANG CHẠY":"ĐÃ CHO PHÉP","#6fffd0");
  if(p>=.20&&p<.31) action=plaque(txX,250,"TX-01 · HOLD 2,00","#ffd266");
  if(p>=.36&&p<.43) action=fee(txX+330,txY-310);
  if(p>=.43&&p<.50) action=record(txX+360,txY-120);
  if(p>=.50&&p<.64) { const hop=phase%3; action=plaque(txX,250,["BANK → MẠNG","MẠNG → ACQUIRER","SỐ RÒNG 2,05"][hop],"#66c8ff"); }
  if(p>=.76&&p<.83) action=plaque(txX,250,"ĐÃ QUYẾT TOÁN","#6fffd0");
  if(p>=.83&&p<.96) action=plaque(txX,250,["HỦY TỪ HOLD","HẾT GIỮ TỪ HOLD","HOÀN TỪ PAID","TRANH CHẤP TỪ PAID"][Math.min(3,Math.floor((p-.83)/.0325))],"#ffffff");
  const tokenVisible=!(p>=.43&&p<.50),token=tokenVisible?principal(txX,txY):"";
  const foreground=`${districts}${action}${token}<circle cx="${txX}" cy="${txY+205}" r="115" fill="#6fffd0" opacity=".08"/>`;
  const status=text(camera+150,132,"TX-01 · 2,00 · MỘT HÀNH TRÌNH",112,"#ffffff","start");
  return `${defs}${foreground}${status}</g></svg>`;
}

// Revision 16 uses fifteen cinematic shots and five generated physical worlds.
// Continuity is carried by TX-01 as a prop, not by a center-locked camera. Each
// shot has a different lens/crop/action grammar and no edge-clipped labels.
function svgFrameR16(t,duration){
  const p=Math.max(0,Math.min(.999999,t/duration)),shot=Math.min(14,Math.floor(p*15)),local=p*15-shot,ease=local<.5?2*local*local:1-Math.pow(-2*local+2,2)/2;
  const data=shot===0?r16ContactlessData:shot<=3?r16AuthorizationData:shot<=5?r16ClearingData:shot<=9?r16SettlementData:r16ExceptionsData;
  const zooms=[1.08,1.18,1.36,1.22,1.14,1.32,1.08,1.28,1.18,1.34,1.06,1.34,1.38,1.38,1.42],dxs=[-90,90,-240,210,-70,210,-150,-180,120,250,0,-300,260,-260,260],dys=[20,-20,80,-40,0,-70,10,80,-40,-40,0,160,140,-140,-130];
  const z=zooms[shot]+local*.045,w=Math.round(2560*z),h=Math.round(1440*z),x=Math.round((2560-w)/2+dxs[shot]+(ease-.5)*90),y=Math.round((1440-h)/2+dys[shot]+Math.sin(local*Math.PI)*-35);
  const text=(x,y,value,size=144,color="#ffffff",anchor="middle",weight=900)=>`<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="${size}" font-weight="${weight}" fill="${color}" paint-order="stroke" stroke="#020605" stroke-width="${Math.max(12,Math.round(size*.085))}" stroke-linejoin="round">${esc(value)}</text>`;
  const tag=(x,y,label,color="#6fffd0")=>`<g transform="translate(${x} ${y})"><rect x="-240" y="-72" width="480" height="144" rx="72" fill="#03110d" fill-opacity=".9" stroke="${color}" stroke-width="16"/>${text(0,43,label,98,color)}</g>`;
  const orb=(x,y,label="TX-01",color="#6fffd0",r=118)=>`<g transform="translate(${x} ${y})" filter="url(#r16shadow)"><circle r="${r}" fill="#03110d" stroke="#ffffff" stroke-width="18"/><circle r="${r-27}" fill="${color}" opacity=".92"/>${text(0,16,label,Math.max(58,Math.round(r*.48)),"#02100c")}</g>`;
  const line=(x1,y1,x2,y2,color="#ffffff",amount=ease)=>{const px=x1+(x2-x1)*amount,py=y1+(y2-y1)*amount;return `<path d="M${x1} ${y1}L${px} ${py}" stroke="#020605" stroke-width="52" stroke-linecap="round"/><path d="M${x1} ${y1}L${px} ${py}" stroke="${color}" stroke-width="24" stroke-linecap="round"/>`;};
  let action="";
  if(shot===0){const ox=440+ease*1320,oy=930-Math.sin(ease*Math.PI)*390;action=`${text(180,250,"MỘT CÚ CHẠM",188,"#ffffff","start")}${text(180,455,"MỞ RA BA QUYẾT ĐỊNH",118,"#ffd266","start")}${orb(ox,oy,"TX-01")}`;}
  if(shot===1){const step=Math.min(2,Math.floor(local*3));action=`${text(180,190,"AUTHORIZATION",118,"#66c8ff","start")}${[[420,990,"THẺ"],[1210,500,"HẠN MỨC"],[2130,820,"RỦI RO"]].map(([gx,gy,label],i)=>`${i===step?orb(gx,gy,"TX-01",["#66c8ff","#ffd266","#6fffd0"][i],105):""}${tag(gx,gy+190,label,["#66c8ff","#ffd266","#6fffd0"][i])}`).join("")}`;}
  if(shot===2){action=`${text(2380,260,"ĐÃ CHO PHÉP",176,"#6fffd0","end")}${text(2380,455,"CHƯA CHUYỂN TIỀN",112,"#ffffff","end")}${orb(1960-ease*360,830,"TX-01","#6fffd0",145)}<circle cx="1960" cy="830" r="${180+ease*100}" fill="none" stroke="#6fffd0" stroke-width="28" opacity="${.2+.55*ease}"/>`;}
  if(shot===3){action=`${text(180,240,"SAU PHÊ DUYỆT",112,"#ffd266","start")}${text(180,520,"10,00",230,"#ffffff","start")}${text(180,790,"→ 8,00 KHẢ DỤNG",160,"#66c8ff","start")}${tag(2020,1060,"HOLD 2,00","#ffd266")}`;}
  if(shot===4){action=`${text(180,210,"CLEARING · NGUỒN",112,"#ffd266","start")}${tag(470,1080,"NGƯỜI BÁN 2,00","#ffd266")}${tag(1280,1160,"PHÍ MẠNG 0,05","#ff8f86")}${orb(520+ease*570,760,"TX-01","#6fffd0",105)}`;}
  if(shot===5){action=`<rect x="90" y="110" width="2380" height="300" rx="55" fill="#020605" fill-opacity=".78"/>${text(1280,240,"NGƯỜI BÁN 2,00 + PHÍ MẠNG 0,05",126,"#ffffff")}${text(1280,380,"= BẢN GHI CUỐI 2,05",148,"#6fffd0")}${tag(1280,1110,"ĐÃ ĐỐI CHIẾU","#6fffd0")}`;}
  if(shot===6){action=`${text(180,220,"1 · BANK → MẠNG",150,"#6fffd0","start")}${line(320,1010,1350,780,"#6fffd0")}${orb(320+ease*1030,1010-ease*230,"TX-01","#6fffd0",100)}`;}
  if(shot===7){const count=Math.round(12-(11*ease));action=`${text(2380,220,"2 · BÙ TRỪ",150,"#66c8ff","end")}${text(2380,410,`${count} DÒNG → 1 SỐ RÒNG`,116,"#ffffff","end")}${orb(1280,790,"2,05","#66c8ff",170+ease*45)}`;}
  if(shot===8){action=`${text(180,220,"3 · MẠNG → ACQUIRER",142,"#66c8ff","start")}${line(720,860,1950,660,"#66c8ff")}${orb(720+ease*1230,860-ease*200,"2,05","#66c8ff",112)}`;}
  if(shot===9){action=`${text(2380,220,"4 · ACQUIRER → NGƯỜI BÁN",126,"#ffd266","end")}${text(2380,420,"NHẬN 2,00 · PHÍ 0,05",120,"#ffffff","end")}${orb(450+ease*1500,930-ease*290,"TX-01","#ffd266",120)}`;}
  if(shot===10){action=`${text(1280,180,"BỐN NHÁNH THAY THẾ",150,"#ffffff")}${text(1280,360,"KHÔNG XẢY RA NỐI TIẾP",112,"#ffd266")}${orb(1280,790,"TX-01","#ffffff",126)}`;}
  const branches=[
    {label:"NHÁNH 1/4 · HỦY TỪ HOLD",id:"TX-01-CANCEL",color:"#ff8f86",x:460,y:830},
    {label:"NHÁNH 2/4 · HẾT GIỮ TỪ HOLD",id:"TX-01-EXPIRY",color:"#ffd266",x:1180,y:500},
    {label:"NHÁNH 3/4 · HOÀN TỪ PAID",id:"TX-01-REFUND",color:"#6fffd0",x:1780,y:1020},
    {label:"NHÁNH 4/4 · TRANH CHẤP TỪ PAID",id:"TX-01-DISPUTE",color:"#66c8ff",x:2050,y:470}
  ];
  if(shot>=11){const b=branches[shot-11];action=`${text(1280,170,"PHƯƠNG ÁN THAY THẾ",106,"#ffffff")}${text(1280,330,b.label,132,b.color)}${orb(b.x,b.y,b.id,b.color,138)}${shot===14?text(1280,1260,"CHO PHÉP? · ĐỐI CHIẾU? · QUYẾT TOÁN?",112,"#ffffff"):""}`;}
  return `<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1440" viewBox="0 0 2560 1440"><defs><filter id="r16shadow"><feDropShadow dx="0" dy="30" stdDeviation="24" flood-opacity=".68"/></filter><linearGradient id="r16shade" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#020605" stop-opacity=".18"/><stop offset=".56" stop-color="#020605" stop-opacity="0"/><stop offset="1" stop-color="#020605" stop-opacity=".5"/></linearGradient></defs><image href="data:image/jpeg;base64,${data}" x="${x}" y="${y}" width="${w}" height="${h}" preserveAspectRatio="xMidYMid slice"/><rect width="2560" height="1440" fill="url(#r16shade)"/><g font-family="Arial,Helvetica,sans-serif">${action}</g></svg>`;
}

// Revision 17 keeps the three promised decisions visible through the body,
// cuts across nine physical worlds, uses Vietnamese critical terminology, and
// closes the network-fee route before four high-contrast alternative branches.
function svgFrameR17(t,duration){
  const p=Math.max(0,Math.min(.999999,t/duration)),shot=Math.min(23,Math.floor(p*24)),local=p*24-shot,ease=local<.5?2*local*local:1-Math.pow(-2*local+2,2)/2;
  const worlds=[r16ContactlessData,r17IssuerData,r16AuthorizationData,r17IssuerData,r16AuthorizationData,r17HoldData,r16ContactlessData,r16ClearingData,r17FeeData,r16ClearingData,r17FeeData,r16ClearingData,r16ContactlessData,r16SettlementData,r17NettingData,r16SettlementData,r17NettingData,r16SettlementData,r17FeeData,r16ContactlessData,r16ExceptionsData,r17HoldData,r17FeeData,r17NettingData];
  const data=worlds[shot],zooms=[1.06,1.08,1.32,1.46,1.18,1.08,1.44,1.09,1.12,1.38,1.3,1.52,1.28,1.08,1.1,1.4,1.32,1.24,1.36,1.1,1.16,1.28,1.32,1.3],dx=[-40,0,-220,260,190,-60,250,-120,-80,250,180,-240,-260,-100,-140,230,210,-210,130,0,-240,220,-230,210],dy=[20,0,100,-60,-50,20,-70,20,30,-80,-30,100,70,10,30,-80,-50,-40,-50,0,90,-80,80,-60];
  const z=zooms[shot]+local*.035,w=Math.round(2560*z),h=Math.round(1440*z),x=Math.round((2560-w)/2+dx[shot]+(ease-.5)*80),y=Math.round((1440-h)/2+dy[shot]-Math.sin(local*Math.PI)*28);
  const text=(x,y,value,size=132,color="#ffffff",anchor="middle",weight=900)=>`<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="${size}" font-weight="${weight}" fill="${color}" paint-order="stroke" stroke="#020605" stroke-width="${Math.max(13,Math.round(size*.09))}" stroke-linejoin="round">${esc(value)}</text>`;
  const plate=(x,y,value,color="#6fffd0",width=760,size=104)=>`<g transform="translate(${x} ${y})"><rect x="${-width/2}" y="-78" width="${width}" height="156" rx="34" fill="#020605" fill-opacity=".94" stroke="${color}" stroke-width="12"/>${text(0,37,value,size,color)}</g>`;
  const token=(x,y,value="TX-01",color="#6fffd0",r=112)=>`<g transform="translate(${x} ${y})" filter="url(#r17shadow)"><circle r="${r}" fill="#03110d" stroke="#ffffff" stroke-width="18"/><circle r="${r-27}" fill="${color}"/>${text(0,18,value,Math.max(55,Math.round(r*.48)),"#02100c")}</g>`;
  const route=(x1,y1,x2,y2,color="#6fffd0",amount=ease)=>{const px=x1+(x2-x1)*amount,py=y1+(y2-y1)*amount;return `<path d="M${x1} ${y1}L${px} ${py}" stroke="#020605" stroke-width="58" stroke-linecap="round"/><path d="M${x1} ${y1}L${px} ${py}" stroke="${color}" stroke-width="26" stroke-linecap="round"/>`;};
  const owner=shot===0?"BA QUYẾT ĐỊNH":shot<=6?"1 · CHO PHÉP":shot<=12?"2 · ĐỐI CHIẾU":shot<=19?"3 · QUYẾT TOÁN":"NHÁNH THAY THẾ";
  const ownerColor=shot<=6?"#6fffd0":shot<=12?"#ffd266":shot<=19?"#66c8ff":"#ffffff";
  let action=`${plate(shot>=20?500:420,145,owner,ownerColor,shot===0?840:shot>=20?900:700,shot===0?98:shot>=20?94:108)}`;
  if(shot===0) action+=`${text(1280,440,"MỘT CÚ CHẠM",190,"#ffffff")}${text(1280,650,"MỞ RA BA QUYẾT ĐỊNH",126,"#ffd266")}${plate(540,1110,"1 · CHO PHÉP","#6fffd0",620,86)}${plate(1280,1110,"2 · ĐỐI CHIẾU","#ffd266",620,86)}${plate(2020,1110,"3 · QUYẾT TOÁN","#66c8ff",620,86)}`;
  if(shot===1) action+=`${text(220,360,"KIỂM TRA THẺ",164,"#ffffff","start")}${route(310,930,930,840,"#6fffd0")}${token(310+ease*620,930-ease*90)}`;
  if(shot===2) action+=`${text(2380,330,"KIỂM TRA HẠN MỨC",148,"#ffffff","end")}${plate(1840,1040,"ĐỦ KHẢ DỤNG","#ffd266",760,100)}${token(640+ease*980,920-ease*250,"TX-01","#ffd266")}`;
  if(shot===3) action+=`${text(220,350,"KIỂM TRA RỦI RO",150,"#ffffff","start")}${token(780+ease*920,850-ease*250,"TX-01","#6fffd0")}${plate(1900,1040,"RỦI RO THẤP","#6fffd0",700,98)}`;
  if(shot===4) action+=`${text(1280,420,"ĐÃ CHO PHÉP",210,"#6fffd0")}${text(1280,640,"CHƯA CHUYỂN TIỀN",126,"#ffffff")}${token(1280,930,"TX-01","#6fffd0",145)}`;
  if(shot===5) action+=`${plate(620,520,"KHẢ DỤNG 10,00","#6fffd0",880,104)}${plate(1880,720,"GIỮ TẠM 2,00","#ffd266",820,104)}${text(1280,1120,"CHỈ DÀNH TRƯỚC · CHƯA CHUYỂN",114,"#ffffff")}`;
  if(shot===6) action+=`${text(1280,420,"CÂU HỎI 1",142,"#6fffd0")}${text(1280,650,"NGÂN HÀNG ĐÃ CHO PHÉP CHƯA?",142,"#ffffff")}${plate(1280,1030,"ĐÃ CHO PHÉP · ĐANG GIỮ","#6fffd0",1260,104)}`;
  if(shot===7) action+=`${text(220,370,"BẢN GHI NGƯỜI BÁN",142,"#ffffff","start")}${plate(650,1020,"GỐC 2,00","#ffd266",620,110)}${token(420+ease*520,760,"TX-01","#ffd266")}`;
  if(shot===8) action+=`${text(2380,350,"PHÍ MẠNG TÁCH RIÊNG",132,"#ffffff","end")}${plate(1880,1040,"PHÍ MẠNG 0,05","#ff8f86",780,104)}${route(580,950,1850,760,"#ff8f86")}`;
  if(shot===9) action+=`${plate(680,1040,"NGƯỜI BÁN 2,00","#ffd266",850,98)}${plate(1870,1040,"PHÍ MẠNG 0,05","#ff8f86",850,98)}${text(1280,390,"HAI THÀNH PHẦN · HAI CHỦ THỂ",126,"#ffffff")}`;
  if(shot===10) action+=`${text(1280,330,"BẢN GHI CUỐI 2,05",176,"#6fffd0")}${text(1280,540,"= GỐC 2,00 + PHÍ MẠNG 0,05",120,"#ffffff")}${route(1520,920,2220,740,"#ff8f86")}${plate(1940,1120,"0,05 → MẠNG THANH TOÁN","#ff8f86",1120,92)}`;
  if(shot===11) action+=`${text(1280,410,"ĐÃ ĐỐI CHIẾU",196,"#ffd266")}${plate(1280,890,"BẢN GHI CUỐI 2,05","#6fffd0",1000,112)}${token(1280,1150,"TX-01","#6fffd0",108)}`;
  if(shot===12) action+=`${text(1280,430,"CÂU HỎI 2",142,"#ffd266")}${text(1280,660,"BẢN GHI CUỐI ĐÃ KHỚP CHƯA?",140,"#ffffff")}${plate(1280,1030,"ĐÃ KHỚP · 2,05","#ffd266",900,108)}`;
  if(shot===13) action+=`${text(220,370,"NGÂN HÀNG → MẠNG THANH TOÁN",122,"#ffffff","start")}${route(260,1000,1450,760,"#66c8ff")}${token(260+ease*1190,1000-ease*240,"2,05","#66c8ff",108)}`;
  if(shot===14){const n=Math.max(1,Math.round(9-8*ease));action+=`${text(2380,350,`${n} NGHĨA VỤ`,176,"#ffffff","end")}${text(2380,570,"GỘP THÀNH 1 SỐ RÒNG",118,"#66c8ff","end")}${token(1320,900,"2,05","#66c8ff",120+ease*70)}`;}
  if(shot===15) action+=`${text(1280,360,"SỐ RÒNG 2,05",190,"#66c8ff")}${plate(1280,980,"SẴN SÀNG CHUYỂN","#66c8ff",980,106)}${token(1280,720,"2,05","#66c8ff",150)}`;
  if(shot===16) action+=`${text(220,350,"MẠNG → NGÂN HÀNG NHẬN",132,"#ffffff","start")}${route(410,930,1950,660,"#66c8ff")}${token(410+ease*1540,930-ease*270,"2,05","#66c8ff",108)}`;
  if(shot===17) action+=`${text(2380,350,"NGÂN HÀNG NHẬN → NGƯỜI BÁN",116,"#ffffff","end")}${route(520,980,2050,720,"#ffd266")}${token(520+ease*1530,980-ease*260,"2,00","#ffd266",112)}`;
  if(shot===18) action+=`${plate(650,1020,"NGƯỜI BÁN NHẬN 2,00","#ffd266",1020,96)}${plate(1900,1020,"MẠNG NHẬN PHÍ 0,05","#ff8f86",1020,96)}${text(1280,380,"DÒNG TIỀN ĐÃ KHÉP KÍN",152,"#ffffff")}`;
  if(shot===19) action+=`${text(1280,370,"CÂU HỎI 3",142,"#66c8ff")}${text(1280,610,"TIỀN ĐÃ THỰC SỰ QUYẾT TOÁN CHƯA?",128,"#ffffff")}${plate(1280,1010,"ĐÃ QUYẾT TOÁN","#66c8ff",900,110)}`;
  const branches=[
    {title:"HỦY GIAO DỊCH",source:"TỪ KHOẢN GIỮ",result:"GIẢI PHÓNG 2,00",color:"#ff8f86",id:"TX-01 · HỦY"},
    {title:"HẾT HẠN GIỮ",source:"TỪ KHOẢN GIỮ",result:"TỰ GIẢI PHÓNG 2,00",color:"#ffd266",id:"TX-01 · HẾT HẠN"},
    {title:"HOÀN TIỀN",source:"SAU ĐÃ TRẢ",result:"TẠO DÒNG TIỀN NGƯỢC",color:"#6fffd0",id:"TX-01 · HOÀN"},
    {title:"TRANH CHẤP",source:"SAU ĐÃ TRẢ",result:"MỞ QUY TRÌNH ĐIỀU TRA",color:"#66c8ff",id:"TX-01 · TRANH CHẤP"}
  ];
  if(shot>=20){const b=branches[shot-20];action+=`<rect x="170" y="310" width="2220" height="820" rx="70" fill="#020605" fill-opacity=".82" stroke="${b.color}" stroke-width="16"/>${text(1280,510,b.title,184,b.color)}${text(1280,690,b.source,112,"#ffffff")}${text(1280,820,b.id,78,"#b8c8c2")}${plate(1280,990,b.result,b.color,1380,102)}`;}
  return `<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1440" viewBox="0 0 2560 1440"><defs><filter id="r17shadow"><feDropShadow dx="0" dy="28" stdDeviation="22" flood-opacity=".72"/></filter><linearGradient id="r17shade" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#020605" stop-opacity=".1"/><stop offset=".55" stop-color="#020605" stop-opacity=".02"/><stop offset="1" stop-color="#020605" stop-opacity=".58"/></linearGradient></defs><image href="data:image/jpeg;base64,${data}" x="${x}" y="${y}" width="${w}" height="${h}" preserveAspectRatio="xMidYMid slice"/><rect width="2560" height="1440" fill="url(#r17shade)"/><g font-family="Arial,Helvetica,sans-serif">${action}</g></svg>`;
}

function svgFrameR18(t,duration){
  const p=Math.max(0,Math.min(.999999,t/duration)),beat=Math.min(31,Math.floor(p*32)),local=p*32-beat,ease=local<.5?2*local*local:1-Math.pow(-2*local+2,2)/2;
  const worlds=[r16ContactlessData,r17IssuerData,r16AuthorizationData,r17IssuerData,r16AuthorizationData,r17HoldData,r16ContactlessData,r17IssuerData,r16ClearingData,r17FeeData,r16ClearingData,r17FeeData,r16ClearingData,r17FeeData,r16ContactlessData,r16SettlementData,r17NettingData,r16SettlementData,r17NettingData,r16SettlementData,r17NettingData,r16SettlementData,r17FeeData,r16ContactlessData,r16ExceptionsData,r17HoldData,r16ExceptionsData,r17FeeData,r16ExceptionsData,r17NettingData,r16ContactlessData,r16SettlementData];
  const data=worlds[beat],z=1.08+(beat%4)*.07+local*.035,w=Math.round(2560*z),h=Math.round(1440*z),x=Math.round((2560-w)/2+[-160,80,190,-70][beat%4]+(ease-.5)*110),y=Math.round((1440-h)/2+[20,-60,70,-20][beat%4]-Math.sin(local*Math.PI)*30);
  const text=(x,y,value,size=132,color="#ffffff",anchor="middle",weight=900)=>`<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="${size}" font-weight="${weight}" fill="${color}" paint-order="stroke" stroke="#020605" stroke-width="${Math.max(14,Math.round(size*.09))}" stroke-linejoin="round">${esc(value)}</text>`;
  const plate=(x,y,value,color="#6fffd0",width=900,size=108)=>`<g transform="translate(${x} ${y})"><rect x="${-width/2}" y="-78" width="${width}" height="156" rx="34" fill="#020605" fill-opacity=".94" stroke="${color}" stroke-width="12"/>${text(0,38,value,size,color)}</g>`;
  const orb=(x,y,color="#6fffd0",r=108)=>`<g transform="translate(${x} ${y})" filter="url(#r18shadow)"><circle r="${r}" fill="#03110d" stroke="#ffffff" stroke-width="18"/><circle r="${r-28}" fill="${color}"/><circle r="${Math.max(16,r*.22)}" fill="#ffffff" opacity=".7"/></g>`;
  const route=(x1,y1,x2,y2,color="#6fffd0")=>{const px=x1+(x2-x1)*ease,py=y1+(y2-y1)*ease;return `<path d="M${x1} ${y1}L${px} ${py}" stroke="#020605" stroke-width="58" stroke-linecap="round"/><path d="M${x1} ${y1}L${px} ${py}" stroke="${color}" stroke-width="26" stroke-linecap="round"/>${orb(px,py,color,82)}`;};
  const phase=beat===0?"BA QUYẾT ĐỊNH":beat<=7?"1 · CHO PHÉP":beat<=14?"2 · ĐỐI CHIẾU":beat<=24?"3 · QUYẾT TOÁN":"NHÁNH THAY THẾ",phaseColor=beat<=7?"#6fffd0":beat<=14?"#ffd266":beat<=24?"#66c8ff":"#ffffff";
  let action=plate(beat>=25?520:430,145,phase,phaseColor,beat===0?840:beat>=25?940:720,beat>=25?94:106);
  const scenes=[
    ["MỘT CÚ CHẠM","MỞ RA BA QUYẾT ĐỊNH","#ffd266"],
    ["GIAO DỊCH BẮT ĐẦU","TX-01 · GỐC 2,00","#6fffd0"],
    ["KIỂM TRA THẺ","THẺ HỢP LỆ","#6fffd0"],
    ["KIỂM TRA HẠN MỨC","ĐỦ KHẢ DỤNG","#ffd266"],
    ["KIỂM TRA RỦI RO","RỦI RO THẤP","#6fffd0"],
    ["ĐÃ CHO PHÉP","CHƯA CHUYỂN TIỀN","#6fffd0"],
    ["KHOẢN GIỮ 2,00","CHỈ DÀNH TRƯỚC","#ffd266"],
    ["CÂU HỎI 1","ĐÃ CHO PHÉP · ĐANG GIỮ","#6fffd0"],
    ["BẢN GHI NGƯỜI BÁN","GỐC 2,00","#ffd266"],
    ["PHÍ MẠNG TÁCH RIÊNG","PHÍ 0,05","#ff8f86"],
    ["HAI THÀNH PHẦN","HAI CHỦ THỂ","#ffd266"],
    ["BẢN GHI CUỐI","2,00 + 0,05 = 2,05","#6fffd0"],
    ["ĐÍCH CỦA PHÍ","0,05 → MẠNG THANH TOÁN","#ff8f86"],
    ["ĐÃ ĐỐI CHIẾU","BẢN GHI CUỐI 2,05","#ffd266"],
    ["CÂU HỎI 2","BẢN GHI ĐÃ KHỚP","#ffd266"],
    ["NGÂN HÀNG A","NGHĨA VỤ 1","#66c8ff"],
    ["NGÂN HÀNG B","NGHĨA VỤ 2","#6fffd0"],
    ["NGÂN HÀNG C","NGHĨA VỤ 3","#ffd266"],
    ["NHIỀU NGHĨA VỤ CÓ NHÃN","ĐANG HỘI TỤ","#66c8ff"],
    ["KẾT QUẢ BÙ TRỪ","MỘT SỐ RÒNG 2,05","#66c8ff"],
    ["MẠNG THANH TOÁN","→ NGÂN HÀNG NHẬN","#66c8ff"],
    ["NGÂN HÀNG NHẬN","→ NGƯỜI BÁN","#ffd266"],
    ["NGƯỜI BÁN NHẬN","GỐC 2,00","#ffd266"],
    ["MẠNG NHẬN","PHÍ 0,05","#ff8f86"],
    ["CÂU HỎI 3","ĐÃ QUYẾT TOÁN","#66c8ff"],
    ["BỐN NHÁNH THAY THẾ","KHÔNG XẢY RA NỐI TIẾP","#ffffff"],
    ["HỦY GIAO DỊCH","TỪ GIỮ → GIẢI PHÓNG 2,00","#ff8f86"],
    ["HẾT HẠN GIỮ","TỰ GIẢI PHÓNG 2,00","#ffd266"],
    ["HOÀN TIỀN","SAU ĐÃ TRẢ → DÒNG TIỀN NGƯỢC","#6fffd0"],
    ["TRANH CHẤP","SAU ĐÃ TRẢ → ĐIỀU TRA","#66c8ff"],
    ["BA CÂU HỎI","CHO PHÉP · ĐỐI CHIẾU · QUYẾT TOÁN","#ffffff"],
    ["MỘT GIAO DỊCH","BA THỜI ĐIỂM RÕ RÀNG","#6fffd0"]
  ];
  const [headline,sub,color]=scenes[beat];
  if(beat===0){action+=`${text(1280,480,headline,196,"#ffffff")}${text(1280,700,sub,132,color)}${plate(540,1100,"1 · CHO PHÉP","#6fffd0",620,86)}${plate(1280,1100,"2 · ĐỐI CHIẾU","#ffd266",620,86)}${plate(2020,1100,"3 · QUYẾT TOÁN","#66c8ff",620,86)}`;}
  else if(beat>=25&&beat<=29){action+=`<rect x="180" y="315" width="2200" height="800" rx="70" fill="#020605" fill-opacity=".84" stroke="${color}" stroke-width="16"/>${text(1280,540,headline,178,color)}${text(1280,780,sub,112,"#ffffff")}${plate(1280,1010,"PHƯƠNG ÁN ĐỘC LẬP",color,1120,100)}`;}
  else {action+=`${text(1280,430,headline,beat===30?166:178,color)}${text(1280,650,sub,beat===30?112:124,"#ffffff")}`;const horizontal=beat%2===0;action+=horizontal?route(360,980,1980,810,color):route(570,1050,1750,760,color);if(![6,7,10,11,13,14,19,22,23,24,30,31].includes(beat))action+=plate(1280,1120,sub,color,beat===28?1500:1120,beat===28?88:100);}
  return `<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1440" viewBox="0 0 2560 1440"><defs><filter id="r18shadow"><feDropShadow dx="0" dy="28" stdDeviation="22" flood-opacity=".72"/></filter><linearGradient id="r18shade" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#020605" stop-opacity=".12"/><stop offset=".52" stop-color="#020605" stop-opacity=".03"/><stop offset="1" stop-color="#020605" stop-opacity=".62"/></linearGradient></defs><image href="data:image/jpeg;base64,${data}" x="${x}" y="${y}" width="${w}" height="${h}" preserveAspectRatio="xMidYMid slice"/><rect width="2560" height="1440" fill="url(#r18shade)"/><g font-family="Arial,Helvetica,sans-serif">${action}</g></svg>`;
}

function svgFrameR19(t,duration){
  const p=Math.max(0,Math.min(.999999,t/duration)),sequence=Math.min(15,Math.floor(p*16)),local=p*16-sequence,ease=local<.5?2*local*local:1-Math.pow(-2*local+2,2)/2;
  const worlds=[r16ContactlessData,r17IssuerData,r17HoldData,r17HoldData,r16ClearingData,r17FeeData,r17NettingData,r17NettingData,r16SettlementData,r17FeeData,r16AuthorizationData,r16ExceptionsData,r17HoldData,r16ExceptionsData,r17IssuerData,r16ContactlessData];
  const data=worlds[sequence],z=1.08+(sequence%3)*.08+local*.025,w=Math.round(2560*z),h=Math.round(1440*z),x=Math.round((2560-w)/2+[80,-120,170,-60][sequence%4]+(ease-.5)*70),y=Math.round((1440-h)/2+[0,-60,55,-25][sequence%4]);
  const txt=(x,y,value,size=150,color="#fff",anchor="middle",weight=900)=>`<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="${size}" font-weight="${weight}" fill="${color}" paint-order="stroke" stroke="#020605" stroke-width="${Math.max(12,Math.round(size*.075))}" stroke-linejoin="round">${esc(value)}</text>`;
  const box=(x,y,width,height,color="#6fffd0",opacity=.91)=>`<rect x="${x}" y="${y}" width="${width}" height="${height}" rx="${Math.min(46,height*.2)}" fill="#020605" fill-opacity="${opacity}" stroke="${color}" stroke-width="12"/>`;
  const coin=(x,y,r=74,color="#6fffd0")=>`<g transform="translate(${x} ${y})" filter="url(#r19shadow)"><circle r="${r}" fill="#06130f" stroke="#fff" stroke-width="12"/><circle r="${r-18}" fill="${color}"/><circle r="${r*.24}" fill="#fff" opacity=".72"/></g>`;
  const arrow=(x1,y1,x2,y2,color="#6fffd0",progress=ease)=>{const ex=x1+(x2-x1)*progress,ey=y1+(y2-y1)*progress,a=Math.atan2(y2-y1,x2-x1),ax=ex-Math.cos(a)*44,ay=ey-Math.sin(a)*44;return `<path d="M${x1} ${y1}L${ex} ${ey}" stroke="#020605" stroke-width="48" stroke-linecap="round"/><path d="M${x1} ${y1}L${ex} ${ey}" stroke="${color}" stroke-width="22" stroke-linecap="round"/><path d="M${ex} ${ey}L${ax+Math.cos(a+2.45)*54} ${ay+Math.sin(a+2.45)*54}M${ex} ${ey}L${ax+Math.cos(a-2.45)*54} ${ay+Math.sin(a-2.45)*54}" stroke="${color}" stroke-width="22" stroke-linecap="round"/>`;};
  const line=(x1,y1,x2,y2,color="#fff",width=8,opacity=.6)=>`<path d="M${x1} ${y1}L${x2} ${y2}" stroke="${color}" stroke-width="${width}" stroke-opacity="${opacity}" stroke-linecap="round"/>`;
  let c="";
  if(sequence===0){const cardX=180+ease*480;c+=`${txt(1280,250,"MỘT CÚ CHẠM",204)}${txt(1280,440,"BA QUYẾT ĐỊNH PHÍA SAU",142,"#ffd266")}${box(cardX,690,620,340,"#6fffd0")}${txt(cardX+310,890,"THẺ",176,"#6fffd0")}${["CHO PHÉP","ĐỐI CHIẾU","QUYẾT TOÁN"].map((v,i)=>`${box(1180+i*430,720,360,280,["#6fffd0","#ffd266","#66c8ff"][i],.82)}${txt(1360+i*430,875,v,86,["#6fffd0","#ffd266","#66c8ff"][i])}`).join("")}`;}
  if(sequence===1){const px=300+ease*1700;c+=`${txt(180,230,"NGÂN HÀNG PHÁT HÀNH",150,"#6fffd0","start")}${line(300,790,2200,790,"#6fffd0",26,.5)}${["THẺ","HẠN MỨC","RỦI RO"].map((v,i)=>`${box(380+i*720,530,480,430,"#6fffd0")}${txt(620+i*720,735,v,112)}${txt(620+i*720,865,local>.35+i*.12?"ĐẠT":"ĐANG KIỂM",92,local>.35+i*.12?"#6fffd0":"#ffd266")}`).join("")}${coin(px,790,76,"#6fffd0")}`;}
  if(sequence===2){const reserve=260+ease*520;c+=`${txt(170,220,"SỔ SỐ DƯ",156,"#ffd266","start")}${box(170,330,2220,720,"#ffd266")}${txt(260,500,"KHẢ DỤNG BAN ĐẦU",104,"#fff","start")}${txt(2300,500,"10,00",150,"#6fffd0","end")}${box(260,620,2040,130,"#6fffd0",.75)}<rect x="260" y="620" width="${2040-reserve}" height="130" rx="40" fill="#6fffd0"/><rect x="${2300-reserve}" y="620" width="${reserve}" height="130" rx="40" fill="#ffd266"/>${txt(260,930,"GIỮ TẠM 2,00",132,"#ffd266","start")}${txt(2300,930,"CHƯA CHUYỂN",112,"#fff","end")}`;}
  if(sequence===3){const active=Math.min(2,Math.floor(local*3));c+=`${txt(1280,180,"MỘT LEDGER · BA THỜI ĐIỂM",150,"#fff")}${box(120,260,2320,1040,"#6fffd0")}${["CHO PHÉP","ĐỐI CHIẾU","QUYẾT TOÁN"].map((v,i)=>`${txt(760+i*590,420,v,94,["#6fffd0","#ffd266","#66c8ff"][i])}${line(500+i*590,470,1010+i*590,470,["#6fffd0","#ffd266","#66c8ff"][i],10,.8)}`).join("")}${[["GIỮ GỐC","2,00","2,00","ĐÃ THAY"],["PHÍ MẠNG","—","+ 0,05","0,05"],["NGHĨA VỤ HỆ THỐNG","2,00","2,05","2,05"],["NGƯỜI BÁN NHẬN","—","—","2,00"]].map((row,i)=>`${txt(190,650+i*150,row[0],76,"#fff","start",700)}${row.slice(1).map((v,j)=>`${box(560+j*590,555+i*150,500,118,j<=active?["#6fffd0","#ffd266","#66c8ff"][j]:"#466056",j<=active?.82:.45)}${txt(810+j*590,638+i*150,v,82,j<=active?"#fff":"#879890")}`).join("")}`).join("")}`;}
  if(sequence===4){const split=190+ease*420;c+=`${txt(1280,150,"ĐỐI CHIẾU TÁCH BẢN GHI",150,"#ffd266")}${txt(1280,300,"GỐC 2,00 · PHÍ 0,05",124,"#fff")}${box(820,430,920,330,"#ffd266")}${txt(1280,625,"BẢN GHI GỐC 2,00",128,"#fff")}${arrow(1280,810,700,1050,"#6fffd0")}${arrow(1280,810,1860,1050,"#ff8f86")}${box(260,980,880,250,"#6fffd0")}${box(1420,980,880,250,"#ff8f86")}${txt(700,1135,"GỐC 2,00",122,"#6fffd0")}${txt(1860,1135,"PHÍ 0,05",122,"#ff8f86")}${coin(1280,810,Math.max(56,130-split*.08),"#ffd266")}`;}
  if(sequence===5){c+=`${txt(1280,190,"HAI KHOẢN · HAI ĐÍCH",158,"#fff")}${box(180,360,760,380,"#6fffd0")}${txt(560,550,"GỐC 2,00",138,"#6fffd0")}${box(1620,360,760,380,"#ff8f86")}${txt(2000,550,"PHÍ 0,05",138,"#ff8f86")}${arrow(560,790,560,1140,"#6fffd0")}${arrow(2000,790,2000,1140,"#ff8f86")}${txt(560,1280,"→ NGƯỜI BÁN",118,"#fff")}${txt(2000,1280,"→ MẠNG THANH TOÁN",104,"#fff")}`;}
  if(sequence===6){const rows=[["NGÂN HÀNG A","MẠNG","0,80","#66c8ff"],["NGÂN HÀNG B","MẠNG","0,75","#6fffd0"],["NGÂN HÀNG C","MẠNG","0,50","#ffd266"]];c+=`${txt(1280,130,"NGHĨA VỤ CÓ HƯỚNG",150)}${txt(1280,270,"SỐ TIỀN · ĐỐI TÁC",118,"#fff")}${rows.map((r,i)=>{const yy=500+i*275,pr=Math.max(0,Math.min(1,local*3-i));return `${box(150,yy-110,2260,220,r[3])}${txt(250,yy,r[0],104,r[3],"start")}${arrow(850,yy-30,1610,yy-30,r[3],pr)}${txt(1290,yy-72,r[2],100,"#fff")}${txt(2310,yy,r[1],104,"#fff","end")}`;}).join("")}`;}
  if(sequence===7){const sources=[[420,470,"0,80","#66c8ff"],[420,740,"0,75","#6fffd0"],[420,1010,"0,50","#ffd266"]];c+=`${txt(1280,130,"BÙ TRỪ",166)}${txt(1280,275,"NHIỀU DÒNG → MỘT SỐ RÒNG",118,"#fff")}${sources.map(([sx,sy,v,col])=>`${txt(sx,sy,v,112,col)}${arrow(sx+160,sy-35,1280,740,col)}`).join("")}${coin(1280,740,130,"#fff")}${txt(1280,775,"2,05",92,"#020605")}${arrow(1430,740,2160,740,"#66c8ff")}${txt(2200,670,"NGHĨA VỤ RÒNG",102,"#66c8ff","end")}${txt(2200,820,"2,05",164,"#fff","end")}`;}
  if(sequence===8){const px=300+ease*1600;c+=`${txt(170,210,"QUYẾT TOÁN THEO THỨ TỰ",150,"#66c8ff","start")}${line(300,820,2220,820,"#66c8ff",34,.55)}${["MẠNG","NGÂN HÀNG NHẬN","NGƯỜI BÁN"].map((v,i)=>`${box(170+i*800,560,620,480,"#66c8ff")}${txt(480+i*800,770,v,92,"#fff")}${txt(480+i*800,920,i===2?"NHẬN 2,00":"CHUYỂN TIẾP",82,i===2?"#ffd266":"#66c8ff")}`).join("")}${coin(px,820,78,"#66c8ff")}`;}
  if(sequence===9){const stack=Math.floor(2+ease*6);c+=`${txt(1280,180,"DÒNG TIỀN KHÉP KÍN",164)}${box(130,330,1440,830,"#ffd266")}${txt(850,505,"NGƯỜI BÁN",126,"#ffd266")}${Array.from({length:stack},(_,i)=>coin(560+i*125,900-(i%2)*120,58,"#ffd266")).join("")}${txt(850,720,"NHẬN GỐC 2,00",156,"#fff")}${box(1700,480,700,500,"#ff8f86")}${txt(2050,650,"MẠNG",118,"#ff8f86")}${txt(2050,820,"NHẬN PHÍ 0,05",108,"#fff")}`;}
  if(sequence===10){const pos=300+ease*1760;c+=`${txt(1280,180,"MỘT GIAO DỊCH · BA TRẠNG THÁI",146)}${line(300,800,2260,800,"#fff",34,.35)}${[[420,"ĐANG GIỮ","#ffd266"],[1280,"ĐÃ KHỚP","#6fffd0"],[2140,"ĐÃ TRẢ","#66c8ff"]].map(([sx,v,col])=>`${coin(sx,800,116,col)}${txt(sx,1060,v,104,col)}`).join("")}${coin(pos,800,70,"#fff")}${txt(1280,1280,local<.34?"PENDING ≠ TRỪ HAI LẦN":local<.68?"APPROVED ≠ ĐÃ TRẢ":"PAID = ĐÃ QUYẾT TOÁN",92,"#fff")}`;}
  if(sequence===11){c+=`${txt(1280,120,"NHÁNH THAY THẾ",154)}${txt(1280,255,"MỖI NHÁNH CÓ ĐIỂM XUẤT PHÁT",108,"#fff")}${line(260,700,2300,700,"#fff",28,.45)}${[[420,"GIỮ","#ffd266"],[1280,"ĐỐI CHIẾU","#6fffd0"],[2140,"ĐÃ TRẢ","#66c8ff"]].map(([sx,v,col])=>`${coin(sx,700,94,col)}${txt(sx,900,v,94,col)}`).join("")}${arrow(420,600,260,350,"#ff8f86")}${arrow(420,600,760,350,"#ffd266")}${arrow(2140,600,1820,350,"#6fffd0")}${arrow(2140,600,2300,350,"#66c8ff")}${txt(250,310,"HỦY",94,"#ff8f86","start")}${txt(780,310,"HẾT GIỮ",94,"#ffd266","end")}${txt(1800,310,"HOÀN",94,"#6fffd0","start")}${txt(2320,310,"TRANH CHẤP",82,"#66c8ff","end")}${txt(1280,1230,"KHÔNG PHẢI BỐN BƯỚC NỐI TIẾP",112,"#fff")}`;}
  if(sequence===12){const release=220+ease*760;c+=`${txt(1280,120,"TRƯỚC ĐỐI CHIẾU",150,"#ffd266")}${txt(1280,255,"GIẢI PHÓNG KHOẢN GIỮ",118,"#fff")}${box(130,340,1080,820,"#ff8f86")}${txt(670,520,"HỦY",152,"#ff8f86")}${box(260,700,820,130,"#ffd266")}${arrow(900,765,360,765,"#ff8f86")}${txt(670,1010,"GIẢI PHÓNG 2,00",104)}${box(1350,340,1080,820,"#ffd266")}${txt(1890,520,"HẾT HẠN GIỮ",116,"#ffd266")}${box(1480,700,820,130,"#ffd266")}${arrow(2120,765,1580,765,"#ffd266")}${txt(1890,1010,"TỰ GIẢI PHÓNG",104)}${coin(release,1240,58,"#6fffd0")}`;}
  if(sequence===13){c+=`${txt(1280,115,"SAU ĐÃ TRẢ",156,"#66c8ff")}${txt(1280,255,"DÒNG NGƯỢC HOẶC ĐIỀU TRA",116,"#fff")}${box(130,320,1080,900,"#6fffd0")}${txt(670,500,"HOÀN TIỀN",140,"#6fffd0")}${arrow(950,790,390,790,"#6fffd0")}${txt(670,1010,"TIỀN QUAY LẠI",104)}${box(1350,320,1080,900,"#66c8ff")}${txt(1890,500,"TRANH CHẤP",132,"#66c8ff")}${[0,1,2].map(i=>`${coin(1640+i*250,800+Math.sin(local*5+i)*60,58,["#66c8ff","#ffd266","#ff8f86"][i])}${line(1640+i*250,800,1890,1050,"#fff",7,.45)}`).join("")}${txt(1890,1120,"MỞ ĐIỀU TRA",104)}`;}
  if(sequence===14){const active=Math.min(2,Math.floor(local*3));c+=`${txt(1280,150,"BA CÂU HỎI ĐỂ ĐỌC ĐÚNG",154)}${[[480,"ĐÃ CHO PHÉP?","#6fffd0"],[1280,"ĐÃ ĐỐI CHIẾU?","#ffd266"],[2080,"ĐÃ QUYẾT TOÁN?","#66c8ff"]].map(([sx,v,col],i)=>`${coin(sx,690,250,col)}<path d="M${sx} 690L${sx+Math.cos((-2.5+Math.min(1,local*3-i)*2.5))*180} ${690+Math.sin((-2.5+Math.min(1,local*3-i)*2.5))*180}" stroke="#fff" stroke-width="28" stroke-linecap="round"/>${txt(sx,1080,v,82,col)}${txt(sx,1220,i<=active?"CÓ":"CHƯA",104,i<=active?col:"#879890")}`).join("")}`;}
  if(sequence===15){c+=`${txt(1280,120,"BA LỚP · BA THỜI ĐIỂM",150)}${txt(1280,260,"MỘT GIAO DỊCH",122,"#6fffd0")}${box(130,330,2300,850,"#6fffd0")}${[["CHO PHÉP","GIỮ GỐC 2,00","#6fffd0"],["ĐỐI CHIẾU","GỐC 2,00 + PHÍ 0,05","#ffd266"],["QUYẾT TOÁN","NGƯỜI BÁN 2,00 · MẠNG 0,05","#66c8ff"]].map((r,i)=>`${coin(420+i*840,650,132,r[2])}${txt(420+i*840,920,r[0],96,r[2])}${txt(420+i*840,1060,r[1],64,"#fff")}${i<2?arrow(570+i*840,650,1110+i*840,650,"#fff"):""}`).join("")}`;}
  return `<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1440" viewBox="0 0 2560 1440"><defs><filter id="r19shadow"><feDropShadow dx="0" dy="24" stdDeviation="18" flood-opacity=".7"/></filter><linearGradient id="r19shade" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#020605" stop-opacity=".25"/><stop offset=".48" stop-color="#020605" stop-opacity=".14"/><stop offset="1" stop-color="#020605" stop-opacity=".72"/></linearGradient></defs><image href="data:image/jpeg;base64,${data}" x="${x}" y="${y}" width="${w}" height="${h}" preserveAspectRatio="xMidYMid slice"/><rect width="2560" height="1440" fill="url(#r19shade)"/><g font-family="Arial,Helvetica,sans-serif">${c}</g></svg>`;
}

function svgFrameR20(t,duration){
  const p=Math.max(0,Math.min(.999999,t/duration)),sequence=Math.min(15,Math.floor(p*16)),local=p*16-sequence,ease=local<.5?2*local*local:1-Math.pow(-2*local+2,2)/2;
  if(![0,3,4,5,6,7,10,11,13,15].includes(sequence)) return svgFrameR19(t,duration);
  const world=[r16ContactlessData,r17HoldData,r16ClearingData,r17FeeData,r17NettingData,r17NettingData,r16AuthorizationData,r16ExceptionsData,r16ExceptionsData,r16ContactlessData][[0,3,4,5,6,7,10,11,13,15].indexOf(sequence)];
  const txt=(x,y,value,size=156,color="#fff",anchor="middle",weight=900)=>`<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="${size}" font-weight="${weight}" fill="${color}" paint-order="stroke" stroke="#020605" stroke-width="${Math.max(14,Math.round(size*.08))}" stroke-linejoin="round">${esc(value)}</text>`;
  const box=(x,y,w,h,color="#6fffd0",opacity=.93)=>`<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="48" fill="#020605" fill-opacity="${opacity}" stroke="${color}" stroke-width="14"/>`;
  const node=(x,y,r,color,label)=>`<g transform="translate(${x} ${y})"><circle r="${r}" fill="#020605" fill-opacity=".94" stroke="${color}" stroke-width="18"/><circle r="${r*.54}" fill="${color}"/><circle r="${r*.18}" fill="#fff" opacity=".8"/>${txt(0,r+120,label,112,color)}</g>`;
  const arrow=(x1,y1,x2,y2,color="#fff",progress=ease)=>{const ex=x1+(x2-x1)*progress,ey=y1+(y2-y1)*progress,a=Math.atan2(y2-y1,x2-x1);return `<path d="M${x1} ${y1}L${ex} ${ey}" stroke="#020605" stroke-width="54" stroke-linecap="round"/><path d="M${x1} ${y1}L${ex} ${ey}" stroke="${color}" stroke-width="24" stroke-linecap="round"/><path d="M${ex} ${ey}l${-Math.cos(a-.65)*70} ${-Math.sin(a-.65)*70}M${ex} ${ey}l${-Math.cos(a+.65)*70} ${-Math.sin(a+.65)*70}" stroke="${color}" stroke-width="24" stroke-linecap="round"/>`;};
  let c="";
  if(sequence===0&&local<.5){const x=230+ease*720;c+=`${txt(1280,210,"MỘT CÚ CHẠM",218)}${box(x,500,760,520,"#6fffd0")}${txt(x+380,810,"THẺ",220,"#6fffd0")}${arrow(x+820,760,1900,760,"#6fffd0")}${node(2100,760,160,"#6fffd0","1 · CHO PHÉP")}`;}
  if(sequence===0&&local>=.5){c+=`${txt(1280,180,"BA QUYẾT ĐỊNH",210,"#ffd266")}${node(460,700,178,"#6fffd0","1 · CHO PHÉP")}${node(1280,700,178,"#ffd266","2 · ĐỐI CHIẾU")}${node(2100,700,178,"#66c8ff","3 · QUYẾT TOÁN")}${arrow(650,700,1090,700,"#fff",1)}${arrow(1470,700,1910,700,"#fff",1)}`;}
  if(sequence===3&&local<.5){c+=`${txt(1280,170,"TRẠNG THÁI 1 · CHO PHÉP",176,"#6fffd0")}${box(180,330,2200,850,"#6fffd0")}${txt(300,570,"GỐC ĐƯỢC GIỮ",132,"#fff","start")}${txt(2260,570,"2,00",210,"#ffd266","end")}${txt(300,860,"PHÍ MẠNG",132,"#fff","start")}${txt(2260,860,"CHƯA PHÁT SINH",142,"#879890","end")}${txt(1280,1080,"CHƯA CHUYỂN TIỀN",134,"#fff")}`;}
  if(sequence===3&&local>=.5){c+=`${txt(1280,150,"TRẠNG THÁI 2 · ĐỐI CHIẾU",170,"#ffd266")}${box(120,310,2320,940,"#ffd266")}${txt(250,520,"GỐC",126,"#6fffd0","start")}${txt(2310,520,"2,00",178,"#6fffd0","end")}${txt(250,750,"PHÍ MẠNG THÊM RIÊNG",116,"#ff8f86","start")}${txt(2310,750,"+ 0,05",178,"#ff8f86","end")}${txt(250,1020,"NGHĨA VỤ HỆ THỐNG",120,"#fff","start")}${txt(2310,1020,"= 2,05",194,"#fff","end")}`;}
  if(sequence===4&&local<.5){c+=`${txt(1280,180,"BẢN GHI NGƯỜI BÁN",174,"#ffd266")}${box(560,390,1440,620,"#ffd266")}${txt(1280,640,"GỐC 2,00",220,"#fff")}${txt(1280,850,"CHỜ ĐỐI CHIẾU",130,"#ffd266")}`;}
  if(sequence===4&&local>=.5){c+=`${txt(1280,150,"BẢN GHI ĐÃ TÁCH",178,"#ffd266")}${box(120,420,1040,600,"#6fffd0")}${txt(640,650,"GỐC",170,"#6fffd0")}${txt(640,850,"2,00",220,"#fff")}${box(1400,420,1040,600,"#ff8f86")}${txt(1920,650,"PHÍ MẠNG",152,"#ff8f86")}${txt(1920,850,"0,05",220,"#fff")}`;}
  if(sequence===5&&local<.5){c+=`${txt(1280,180,"HAI KHOẢN RIÊNG",184)}${node(650,720,210,"#6fffd0","GỐC 2,00")}${node(1910,720,210,"#ff8f86","PHÍ 0,05")}`;}
  if(sequence===5&&local>=.5){c+=`${txt(1280,150,"HAI ĐÍCH NHẬN",184)}${arrow(320,720,900,720,"#6fffd0")}${box(900,440,620,560,"#6fffd0")}${txt(1210,680,"NGƯỜI BÁN",132,"#6fffd0")}${txt(1210,850,"2,00",190)}${arrow(2240,720,1660,720,"#ff8f86")}${box(1040,1040,1520,260,"#ff8f86")}${txt(1800,1200,"MẠNG NHẬN PHÍ 0,05",124,"#ff8f86")}`;}
  if(sequence===6){const rows=[[430,"NGÂN HÀNG A","0,80","#66c8ff"],[760,"NGÂN HÀNG B","0,75","#6fffd0"],[1090,"NGÂN HÀNG C","0,50","#ffd266"]];c+=`${txt(1280,145,"NGHĨA VỤ: NGUỒN → ĐỐI TÁC",158)}${rows.map(([y,name,amount,color],i)=>{const pr=Math.max(0,Math.min(1,local*3-i));return `${box(80,y-110,650,220,color)}${txt(405,y+25,name,102,color)}${box(1830,y-110,650,220,color)}${txt(2155,y+25,"MẠNG",116,"#fff")}${txt(1280,y-78,amount,112,"#fff")}${arrow(790,y+35,1770,y+35,color,pr)}`;}).join("")}`;}
  if(sequence===7&&local<.5){c+=`${txt(1280,140,"BA DÒNG ĐANG HỘI TỤ",166)}${[[340,430,"0,80","#66c8ff"],[340,720,"0,75","#6fffd0"],[340,1010,"0,50","#ffd266"]].map(([x,y,v,col])=>`${node(x,y,92,col,v)}${arrow(x+120,y,1260,720,col)}`).join("")}${node(1400,720,170,"#fff","ĐANG BÙ TRỪ")}`;}
  if(sequence===7&&local>=.5){c+=`${txt(1280,170,"MỘT KẾT QUẢ RÒNG",190,"#66c8ff")}${node(1280,700,310,"#66c8ff","NGHĨA VỤ HỆ THỐNG")}${txt(1280,770,"2,05",220,"#fff")}`;}
  if(sequence===10&&local<.5){c+=`${txt(1280,170,"HIỆN TẠI: ĐANG GIỮ",184,"#ffd266")}${node(650,720,260,"#ffd266","GỐC 2,00")}${txt(1700,650,"ĐỐI CHIẾU",126,"#879890")}${txt(1700,850,"QUYẾT TOÁN",126,"#879890")}`;}
  if(sequence===10&&local>=.5){c+=`${txt(1280,170,"HIỆN TẠI: ĐÃ TRẢ",184,"#66c8ff")}${txt(600,650,"GIỮ",126,"#879890")}${txt(600,850,"ĐỐI CHIẾU",126,"#879890")}${node(1900,720,260,"#66c8ff","NGƯỜI BÁN 2,00")}`;}
  if(sequence===11&&local<.5){c+=`${txt(1280,150,"NHÁNH TỪ KHOẢN GIỮ",180,"#ffd266")}${box(120,380,1040,650,"#ff8f86")}${txt(640,650,"HỦY",210,"#ff8f86")}${txt(640,860,"GIẢI PHÓNG 2,00",122)}${box(1400,380,1040,650,"#ffd266")}${txt(1920,650,"HẾT GIỮ",176,"#ffd266")}${txt(1920,860,"TỰ GIẢI PHÓNG",122)}`;}
  if(sequence===11&&local>=.5){c+=`${txt(1280,150,"NHÁNH SAU ĐÃ TRẢ",180,"#66c8ff")}${box(120,380,1040,650,"#6fffd0")}${txt(640,650,"HOÀN",210,"#6fffd0")}${txt(640,860,"TIỀN QUAY LẠI",122)}${box(1400,380,1040,650,"#66c8ff")}${txt(1920,650,"TRANH CHẤP",162,"#66c8ff")}${txt(1920,860,"MỞ ĐIỀU TRA",122)}`;}
  if(sequence===13&&local<.5){c+=`${txt(1280,170,"HOÀN TIỀN",220,"#6fffd0")}${node(1900,720,250,"#66c8ff","ĐÃ TRẢ")}${arrow(1600,720,650,720,"#6fffd0")}${node(420,720,210,"#6fffd0","TIỀN QUAY LẠI")}`;}
  if(sequence===13&&local>=.5){c+=`${txt(1280,160,"TRANH CHẤP",210,"#66c8ff")}${node(1280,700,250,"#66c8ff","MỞ ĐIỀU TRA")}${[0,1,2].map(i=>node(500+i*780,1130,88,["#6fffd0","#ffd266","#ff8f86"][i],["CHỦ THẺ","MẠNG","NGƯỜI BÁN"][i])).join("")}`;}
  if(sequence===15&&local<.5){c+=`${txt(1280,145,"BA LỚP · MỘT GIAO DỊCH",174)}${node(420,680,190,"#6fffd0","CHO PHÉP")}${node(1280,680,190,"#ffd266","ĐỐI CHIẾU")}${node(2140,680,190,"#66c8ff","QUYẾT TOÁN")}${arrow(630,680,1070,680,"#fff",1)}${arrow(1490,680,1930,680,"#fff",1)}`;}
  if(sequence===15&&local>=.5){c+=`${txt(1280,145,"BA CÂU TRẢ LỜI",188,"#fff")}${box(100,400,720,620,"#6fffd0")}${txt(460,640,"CHO PHÉP",130,"#6fffd0")}${txt(460,850,"CÓ",210)}${box(920,400,720,620,"#ffd266")}${txt(1280,640,"ĐỐI CHIẾU",122,"#ffd266")}${txt(1280,850,"CÓ",210)}${box(1740,400,720,620,"#66c8ff")}${txt(2100,640,"QUYẾT TOÁN",118,"#66c8ff")}${txt(2100,850,"CÓ",210)}`;}
  return `<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1440" viewBox="0 0 2560 1440"><defs><linearGradient id="r20shade" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#020605" stop-opacity=".2"/><stop offset=".5" stop-color="#020605" stop-opacity=".08"/><stop offset="1" stop-color="#020605" stop-opacity=".72"/></linearGradient></defs><image href="data:image/jpeg;base64,${world}" width="2560" height="1440" preserveAspectRatio="xMidYMid slice"/><rect width="2560" height="1440" fill="url(#r20shade)"/><g font-family="Arial,Helvetica,sans-serif">${c}</g></svg>`;
}

function svgFrameR21(t,duration){
  const beat=Math.min(31,Math.floor(t/duration*32)),local=Math.max(0,Math.min(1,t/duration*32-beat)),ease=local<.5?2*local*local:1-Math.pow(-2*local+2,2)/2;
  const families=[0,1,0,2,1,0,1,2,0,3,0,2,3,1,4,0,4,3,2,1,0,3,1,4,0,0,5,0,5,1,5,0],family=families[beat],stage=beat<5?0:beat<10?1:beat<15?2:beat<20?3:beat<25?4:5,pulse=.5+.5*Math.sin(local*Math.PI*2),signal=Math.round(230+ease*1810),accent=["#76f7c4","#69c7ff","#ffd166","#ff8f70","#b69cff","#7ef1e8"][stage];
  const stages=["ONE TAP","AUTHORIZATION","CLEARING","NETTING","SETTLEMENT","READ THE STATUS"],claims=["The money does not move once.","Approval reserves $2.00.","A $0.05 fee joins the record.","Many obligations become one.","Principal and fee take different paths.","Authorized. Cleared. Settled."];
  const worlds=[r16ContactlessData,r16AuthorizationData,r16ClearingData,r17NettingData,r16SettlementData,r16ExceptionsData],world=worlds[stage];
  const txt=(x,y,value,size=96,color="#f6fbf8",anchor="start",weight=800)=>`<text x="${x}" y="${y}" font-size="${size}" font-weight="${weight}" text-anchor="${anchor}" fill="${color}">${esc(value)}</text>`;
  const node=(x,y,label,color=accent)=>`<g transform="translate(${x} ${y})"><circle r="112" fill="#0b241d" stroke="${color}" stroke-width="7"/><text y="18" text-anchor="middle" font-size="54" font-weight="900" fill="#f7fff9">${esc(label)}</text></g>`;
  const arrow=(x1,y1,x2,y2,color=accent,width=18)=>`<path d="M${x1} ${y1} C${(x1+x2)/2} ${y1},${(x1+x2)/2} ${y2},${x2} ${y2}" fill="none" stroke="${color}" stroke-width="${width}" stroke-linecap="round"/><path d="M${x2-36} ${y2-28}L${x2} ${y2}L${x2-36} ${y2+28}" fill="none" stroke="${color}" stroke-width="${width}" stroke-linecap="round" stroke-linejoin="round"/>`;
  let backdrop=`<rect width="2560" height="1440" fill="#030b09"/><circle cx="${400+ease*1700}" cy="${220+stage*145}" r="310" fill="${accent}" opacity=".08"/>`,content="";
  if(family===0){
    backdrop=`<image href="data:image/jpeg;base64,${world}" width="2560" height="1440" preserveAspectRatio="xMidYMid slice"/><rect width="2560" height="1440" fill="#020907" opacity=".44"/><rect y="760" width="2560" height="680" fill="url(#r21fade)"/>`;
    content=`${txt(150,1090,stages[stage],152,"#ffffff","start",900)}${txt(150,1240,claims[stage],76,accent)}<circle cx="${340+ease*1780}" cy="650" r="${44+28*pulse}" fill="${accent}" opacity=".92"/><circle cx="${340+ease*1780}" cy="650" r="${110+50*pulse}" fill="none" stroke="${accent}" stroke-width="8" opacity=".45"/>`;
  } else if(family===1){
    const diagramClaim=["A SIGNAL LEAVES THE TERMINAL","THE ISSUER RETURNS A DECISION","RECORDS MEET AT THE NETWORK","BANKS EXCHANGE OBLIGATIONS","VALUE MOVES AFTER NETTING","STATUS FOLLOWS THE EVIDENCE"][stage];
    content=`${node(260,720,"CARD")}${node(800,390,"MERCHANT")}${node(1320,720,"NETWORK")}${node(1860,390,"ISSUER")}${node(2300,720,"BANK")}${arrow(370,690,690,430)}${arrow(910,430,1210,680)}${arrow(1430,680,1750,430)}${arrow(1970,430,2190,680)}<circle cx="${signal}" cy="${720-120*Math.sin(ease*Math.PI)}" r="36" fill="#ffffff"/><circle cx="${signal}" cy="${720-120*Math.sin(ease*Math.PI)}" r="88" fill="${accent}" opacity="${.16+.25*pulse}"/>${txt(1280,1180,diagramClaim,96,accent,"middle",900)}`;
  } else if(family===2){
    const fee=stage>=2?1:0,settled=stage>=4?1:0;
    content=`${txt(150,250,"THE LEDGER CHANGES STATE",112,"#ffffff")}<g transform="translate(150 390)"><rect width="2260" height="760" rx="62" fill="#071b16" stroke="#2d6653" stroke-width="5"/>${[["PRINCIPAL RESERVED","$2.00",1],["NETWORK FEE","$0.05",fee],["SYSTEM OBLIGATION","$2.05",fee],["MERCHANT RECEIVED","$2.00",settled]].map(([label,value,on],i)=>`<g transform="translate(100 ${80+i*160})"><text y="72" font-size="64" font-weight="800" fill="${on?"#f7fff9":"#6d8a7f"}">${label}</text><rect x="1080" y="5" width="760" height="92" rx="46" fill="#102f25"/><rect x="1080" y="5" width="${Math.round(760*ease*Number(on))}" height="92" rx="46" fill="${accent}"/><text x="2020" y="76" text-anchor="end" font-size="76" font-weight="900" fill="${on?accent:"#59756b"}">${value}</text></g>`).join("")}</g>`;
  } else if(family===3){
    const reach=250+ease*1850;
    content=`${txt(150,230,"MONEY AND DATA SPLIT",118,"#ffffff")}<path d="M260 650 C820 650 940 360 1320 360 S1880 360 2300 360" fill="none" stroke="#69c7ff" stroke-width="58" opacity=".8"/><path d="M260 650 C820 650 940 980 1320 980 S1880 980 2300 980" fill="none" stroke="#ffd166" stroke-width="24" opacity=".9"/><circle cx="${reach}" cy="${360+620*(1-ease)}" r="44" fill="#ffffff"/>${txt(2300,300,"MERCHANT",82,"#69c7ff","end",900)}${txt(2300,1130,"PAYMENT NETWORK",82,"#ffd166","end",900)}${txt(150,1280,"$2.00 PRINCIPAL",78,"#69c7ff")}${txt(1010,1280,"$0.05 FEE",78,"#ffd166")}`;
  } else if(family===4){
    const rows=[["BANK A","$0.80",430],["BANK B","$0.75",720],["BANK C","$0.50",1010]];
    content=`${txt(150,220,"OBLIGATIONS CONVERGE",118,"#ffffff")}${rows.map(([bank,value,y],i)=>`${node(330,y,bank,i===0?"#76f7c4":i===1?"#69c7ff":"#ffd166")}${txt(620,y+24,value,78,"#ffffff")}${arrow(760,y,1720,720,i===0?"#76f7c4":i===1?"#69c7ff":"#ffd166",14)}`).join("")}${node(2030,720,"NET")}${txt(2030,1030,"ONE POSITION",94,accent,"middle",900)}<circle cx="${760+ease*960}" cy="720" r="${35+20*pulse}" fill="#ffffff"/>`;
  } else {
    const active=Math.min(2,Math.floor(ease*3)),states=["AUTHORIZED","CLEARED","SETTLED"];
    content=`${txt(1280,210,"THREE QUESTIONS. ONE TRUE STATUS.",108,"#ffffff","middle",900)}${states.map((label,i)=>`<g transform="translate(${250+i*770} 390)"><circle cx="150" cy="150" r="135" fill="${active>=i?accent:"#0b241d"}" stroke="${active>=i?"#ffffff":"#315f50"}" stroke-width="7"/><text x="150" y="175" text-anchor="middle" font-size="82" font-weight="900" fill="${active>=i?"#04110d":"#6e8a80"}">${i+1}</text><text x="150" y="400" text-anchor="middle" font-size="72" font-weight="900" fill="#f7fff9">${label}</text>${i<2?arrow(300,150,610,150,active>i?accent:"#315f50",16):""}</g>`).join("")}<g transform="translate(160 1060)"><rect width="1050" height="240" rx="48" fill="#201a09" stroke="#ffd166" stroke-width="5"/><text x="525" y="86" text-anchor="middle" font-size="54" font-weight="800" fill="#ffd166">BEFORE CLEARING</text><text x="525" y="176" text-anchor="middle" font-size="76" font-weight="900" fill="#ffffff">CANCEL · EXPIRE</text></g><g transform="translate(1350 1060)"><rect width="1050" height="240" rx="48" fill="#27120e" stroke="#ff8f70" stroke-width="5"/><text x="525" y="86" text-anchor="middle" font-size="54" font-weight="800" fill="#ff8f70">AFTER PAYMENT</text><text x="525" y="176" text-anchor="middle" font-size="76" font-weight="900" fill="#ffffff">REFUND · DISPUTE</text></g>`;
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1440" viewBox="0 0 2560 1440"><defs><linearGradient id="r21fade" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#020907" stop-opacity="0"/><stop offset="1" stop-color="#020907" stop-opacity=".96"/></linearGradient><pattern id="r21grid" width="96" height="96" patternUnits="userSpaceOnUse"><path d="M96 0H0V96" fill="none" stroke="#ffffff" stroke-opacity=".035" stroke-width="2"/></pattern></defs>${backdrop}<rect width="2560" height="1440" fill="url(#r21grid)"/><g font-family="Arial,Helvetica,sans-serif">${content}</g></svg>`;
}

if(previewRevision21){
  const duration=77.1,previewTimes=Array.from({length:32},(_,index)=>round((index+.5)*duration/32,3)),previewFiles=[];
  for(const [index,time] of previewTimes.entries()){const svgPath=join(framesDir,`r21-${String(index).padStart(2,"0")}.svg`),jpgPath=join(samplesDir,`r21-${String(index).padStart(2,"0")}.jpg`);writeFileSync(svgPath,svgFrameR21(time,duration));run("ffmpeg",["-y","-i",svgPath,"-frames:v","1","-vf","scale=800:450",jpgPath],{stdio:"ignore"});previewFiles.push(jpgPath);}
  const previewPath=join(work,"r21-hybrid-documentary-en-us-contact.jpg");run("montage",[...previewFiles,"-tile","4x8","-geometry","800x450+8+8","-background","#020807","-quality","92",previewPath]);process.stdout.write(`${previewPath}\n`);process.exit(0);
}

if(previewRevision20){
  const duration=70.876,previewTimes=Array.from({length:32},(_,index)=>round((index+.5)*duration/32,3)),previewFiles=[];
  for(const [index,time] of previewTimes.entries()){const svgPath=join(framesDir,`r20-${String(index).padStart(2,"0")}.svg`),jpgPath=join(samplesDir,`r20-${String(index).padStart(2,"0")}.jpg`);writeFileSync(svgPath,svgFrameR20(time,duration));run("ffmpeg",["-y","-i",svgPath,"-frames:v","1","-vf","scale=800:450",jpgPath],{stdio:"ignore"});previewFiles.push(jpgPath);}
  const previewPath=join(work,"r20-mobile-reflow-contact.jpg");run("montage",[...previewFiles,"-tile","4x8","-geometry","800x450+8+8","-background","#020807","-quality","92",previewPath]);process.stdout.write(`${previewPath}\n`);process.exit(0);
}

if(previewRevision19){
  const duration=77.511,previewTimes=Array.from({length:32},(_,index)=>round((index+.5)*duration/32,3)),previewFiles=[];
  for(const [index,time] of previewTimes.entries()){const svgPath=join(framesDir,`r19-${String(index).padStart(2,"0")}.svg`),jpgPath=join(samplesDir,`r19-${String(index).padStart(2,"0")}.jpg`);writeFileSync(svgPath,svgFrameR19(time,duration));run("ffmpeg",["-y","-i",svgPath,"-frames:v","1","-vf","scale=800:450",jpgPath],{stdio:"ignore"});previewFiles.push(jpgPath);}
  const previewPath=join(work,"r19-object-ledger-contact.jpg");run("montage",[...previewFiles,"-tile","4x8","-geometry","800x450+8+8","-background","#020807","-quality","92",previewPath]);process.stdout.write(`${previewPath}\n`);process.exit(0);
}

if(previewRevision18){
  const duration=73.267,previewTimes=Array.from({length:32},(_,index)=>round((index+.5)*duration/32,3)),previewFiles=[];
  for(const [index,time] of previewTimes.entries()){const svgPath=join(framesDir,`r18-${String(index).padStart(2,"0")}.svg`),jpgPath=join(samplesDir,`r18-${String(index).padStart(2,"0")}.jpg`);writeFileSync(svgPath,svgFrameR18(time,duration));run("ffmpeg",["-y","-i",svgPath,"-frames:v","1","-vf","scale=800:450",jpgPath],{stdio:"ignore"});previewFiles.push(jpgPath);}
  const previewPath=join(work,"r18-thirty-two-beat-contact.jpg");run("montage",[...previewFiles,"-tile","4x8","-geometry","800x450+8+8","-background","#020807","-quality","92",previewPath]);process.stdout.write(`${previewPath}\n`);process.exit(0);
}

if(previewRevision17){
  const duration=76.033,previewTimes=Array.from({length:32},(_,index)=>round((index+.5)*duration/32,3)),previewFiles=[];
  for(const [index,time] of previewTimes.entries()){const svgPath=join(framesDir,`r17-${String(index).padStart(2,"0")}.svg`),jpgPath=join(samplesDir,`r17-${String(index).padStart(2,"0")}.jpg`);writeFileSync(svgPath,svgFrameR17(time,duration));run("ffmpeg",["-y","-i",svgPath,"-frames:v","1","-vf","scale=800:450",jpgPath],{stdio:"ignore"});previewFiles.push(jpgPath);}
  const previewPath=join(work,"r17-nine-world-contact.jpg");run("montage",[...previewFiles,"-tile","4x8","-geometry","800x450+8+8","-background","#020807","-quality","92",previewPath]);process.stdout.write(`${previewPath}\n`);process.exit(0);
}

if(previewRevision16){
  const duration=76.867,previewTimes=Array.from({length:32},(_,index)=>round((index+.5)*duration/32,3)),previewFiles=[];
  for(const [index,time] of previewTimes.entries()){const svgPath=join(framesDir,`r16-${String(index).padStart(2,"0")}.svg`),jpgPath=join(samplesDir,`r16-${String(index).padStart(2,"0")}.jpg`);writeFileSync(svgPath,svgFrameR16(time,duration));run("ffmpeg",["-y","-i",svgPath,"-frames:v","1","-vf","scale=800:450",jpgPath],{stdio:"ignore"});previewFiles.push(jpgPath);}
  const previewPath=join(work,"r16-mixed-treatment-contact.jpg");run("montage",[...previewFiles,"-tile","4x8","-geometry","800x450+8+8","-background","#020807","-quality","92",previewPath]);process.stdout.write(`${previewPath}\n`);process.exit(0);
}

if (previewRevision15) {
  const duration=76,previewTimes=Array.from({length:32},(_,index)=>round((index+.5)*duration/32,3)),previewFiles=[];
  for(const [index,time] of previewTimes.entries()){const svgPath=join(framesDir,`r15-${String(index).padStart(2,"0")}.svg`),jpgPath=join(samplesDir,`r15-${String(index).padStart(2,"0")}.jpg`);writeFileSync(svgPath,svgFrameR15(time,duration));run("ffmpeg",["-y","-i",svgPath,"-frames:v","1","-vf","scale=800:450",jpgPath],{stdio:"ignore"});previewFiles.push(jpgPath);}
  const previewPath=join(work,"r15-continuous-world-contact.jpg");run("montage",[...previewFiles,"-tile","4x8","-geometry","800x450+8+8","-background","#020807","-quality","92",previewPath]);process.stdout.write(`${previewPath}\n`);process.exit(0);
}
...
if (snapshot.nextAction === "CREATE_REPAIR_REVISION") {
  const repairRevision = snapshot.blueprint?.id?.endsWith(":r20") ? "r21" : snapshot.blueprint?.id?.endsWith(":r19") ? "r20" : snapshot.blueprint?.id?.endsWith(":r18") ? "r19" : snapshot.blueprint?.id?.endsWith(":r17") ? "r18" : snapshot.blueprint?.id?.endsWith(":r16") ? "r17" : snapshot.blueprint?.id?.endsWith(":r15") ? "r16" : snapshot.blueprint?.id?.endsWith(":r14") ? "r15" : snapshot.blueprint?.id?.endsWith(":r13") ? "r14" : snapshot.blueprint?.id?.endsWith(":r12") ? "r13" : snapshot.blueprint?.id?.endsWith(":r11") ? "r12" : snapshot.blueprint?.id?.endsWith(":r10") ? "r11" : snapshot.blueprint?.id?.endsWith(":r9") ? "r10" : snapshot.blueprint?.id?.endsWith(":r8") ? "r9" : snapshot.blueprint?.id?.endsWith(":r7") ? "r8" : snapshot.blueprint?.id?.endsWith(":r6") ? "r7" : snapshot.blueprint?.id?.endsWith(":r5") ? "r6" : snapshot.blueprint?.id?.endsWith(":r4") ? "r5" : snapshot.blueprint?.id?.endsWith(":r3") ? "r4" : snapshot.blueprint?.id?.endsWith(":r2") ? "r3" : "r2";
...
const revision = snapshot.blueprint.id.endsWith(":r21") ? "r21" : snapshot.blueprint.id.endsWith(":r20") ? "r20" : snapshot.blueprint.id.endsWith(":r19") ? "r19" : snapshot.blueprint.id.endsWith(":r18") ? "r18" : snapshot.blueprint.id.endsWith(":r17") ? "r17" : snapshot.blueprint.id.endsWith(":r16") ? "r16" : snapshot.blueprint.id.endsWith(":r15") ? "r15" : snapshot.blueprint.id.endsWith(":r14") ? "r14" : snapshot.blueprint.id.endsWith(":r13") ? "r13" : snapshot.blueprint.id.endsWith(":r12") ? "r12" : snapshot.blueprint.id.endsWith(":r11") ? "r11" : snapshot.blueprint.id.endsWith(":r10") ? "r10" : snapshot.blueprint.id.endsWith(":r9") ? "r9" : snapshot.blueprint.id.endsWith(":r8") ? "r8" : snapshot.blueprint.id.endsWith(":r7") ? "r7" : snapshot.blueprint.id.endsWith(":r6") ? "r6" : snapshot.blueprint.id.endsWith(":r5") ? "r5" : snapshot.blueprint.id.endsWith(":r4") ? "r4" : snapshot.blueprint.id.endsWith(":r3") ? "r3" : snapshot.blueprint.id.endsWith(":r2") ? "r2" : "r1";
...
if (!snapshot.materialization) {
  ...
  if (!reusableArtifacts) {
    for (let index = 0; index < frameCount; index += 1) writeFileSync(join(framesDir, `frame-${String(index).padStart(5,"0")}.svg`), revision === "r21" ? svgFrameR21(index/renderFps, duration) : revision === "r20" ? svgFrameR20(index/renderFps, duration) : revision === "r19" ? svgFrameR19(index/renderFps, duration) : revision === "r18" ? svgFrameR18(index/renderFps, duration) : revision === "r17" ? svgFrameR17(index/renderFps, duration) : revision === "r16" ? svgFrameR16(index/renderFps, duration) : revision === "r15" ? svgFrameR15(index/renderFps, duration) : revision === "r14" ? svgFrameR14(index/renderFps, duration) : revision === "r13" ? svgFrameR13(index/renderFps, duration) : revision === "r12" ? svgFrameR12(index/renderFps, duration) : revision === "r11" ? svgFrameR11(index/renderFps, duration) : revision === "r10" ? svgFrameR10(index/renderFps, duration) : revision === "r9" ? svgFrameR9(index/renderFps, duration) : revision === "r8" ? svgFrameR8(index/renderFps, duration) : revision === "r7" ? svgFrameR7(index/renderFps, duration) : revision === "r6" ? svgFrameR6(index/renderFps, duration) : revision === "r5" ? svgFrameR5(index/renderFps, duration) : revision === "r4" ? svgFrameR4(index/renderFps, duration) : revision === "r3" ? svgFrameR3(index/renderFps, duration) : svgFrame(index/renderFps, duration));
    run("ffmpeg", ["-y","-framerate",String(renderFps),"-i",join(framesDir,"frame-%05d.svg"),"-t",String(duration),"-vf","fps=30,format=yuv420p","-c:v","libx264","-preset","medium","-crf",["r5","r6","r7","r8","r9","r10","r11","r12","r13","r14","r15","r16","r17","r18","r19","r20","r21"].includes(revision) ? "22" : "19","-movflags","+faststart",silent]);
    const voiceFilter = ["r20","r21"].includes(revision) ? `aresample=48000:async=1:first_pts=0,adeclip=w=55:o=75:a=8:t=10,adeclick=w=55:o=75:a=2:t=2:b=2,highpass=f=60,lowpass=f=14500,acompressor=threshold=-24dB:ratio=1.5:attack=45:release=380,apad=pad_dur=${duration},loudnorm=I=-16:TP=-4:LRA=4,alimiter=limit=0.56` : ["r16","r17","r18","r19"].includes(revision) ? `aresample=48000:async=1:first_pts=0,adeclip=w=55:o=75:a=8:t=10,adeclick=w=55:o=75:a=2:t=2:b=2,highpass=f=55,lowpass=f=15000,acompressor=threshold=-22dB:ratio=1.6:attack=35:release=320,apad=pad_dur=${duration},loudnorm=I=-16:TP=-4:LRA=5,alimiter=limit=0.58` : revision === "r15" ? `aresample=48000:async=1:first_pts=0,adeclick=w=55:o=75:a=2:t=2:b=2,highpass=f=55,lowpass=f=15000,acompressor=threshold=-20dB:ratio=1.8:attack=30:release=300,apad=pad_dur=${duration},loudnorm=I=-15:TP=-3:LRA=5,alimiter=limit=0.65` : ["r8","r9","r10","r11","r12","r13","r14"].includes(revision) ? `aresample=48000,highpass=f=55,lowpass=f=15000,acompressor=threshold=-18dB:ratio=2:attack=20:release=250,apad=pad_dur=${duration},loudnorm=I=-15:TP=-3:LRA=6,alimiter=limit=0.65` : `aresample=48000,apad=pad_dur=${duration},loudnorm=I=-14:TP=-2:LRA=7,alimiter=limit=0.75`;
  ...
  const r21 = revision === "r21", r20 = revision === "r20", r19 = revision === "r19", r18 = revision === "r18", r17 = revision === "r17", r16 = revision === "r16", r15 = revision === "r15", r14 = revision === "r14", r13 = revision === "r13", r12 = revision === "r12", r11 = revision === "r11", r10 = revision === "r10", r9 = revision === "r9", r8 = revision === "r8", r7 = revision === "r7", r6 = revision === "r6", r5 = revision === "r5", r5Plus = r5 || r6 || r7 || r8 || r9 || r10 || r11 || r12 || r13 || r14 || r15 || r16 || r17 || r18 || r19 || r20 || r21;
  const visualManifest={semanticRuntimeRatio:1,slideshowRuntimeRatio:r5Plus?0:revision==="r4"?.02:revision==="r3"?.04:0,meaningfulMotionRatio:r5Plus?.99:revision==="r4"?.98:revision==="r3"?.97:.94,first30MotionRatio:r5Plus?.99:.97,cameraOnlyRatio:0,treatmentFamilies:r19?16:r18?32:r17?24:r16?15:r15?9:r14?24:r13?16:r12?12:r11?16:r10?30:r9?24:r8?19:r7?18:r6?16:r5?13:revision==="r4"?10:revision==="r3"?12:8,minimumCriticalFontPx1080:r19?132:r18?132:r17?132:r16?120:r15?120:r14?108:r13?108:r12?108:r11?90:r10?84:r9?84:r8?84:r7?84:r5Plus?72:revision==="r4"?60:revision==="r3"?54:42,maximumVisualEventIntervalSeconds:r19?.16:r18?.18:r17?.2:r16?.25:r15?.25:r14?.3:r13?.4:r12?.45:r11?.65:r10?.8:r9?.9:r8?1.5:r7?1.5:r6?1.7:r5?1.8:revision==="r4"?2.2:revision==="r3"?2.2:4.2,maximumStaticHoldSeconds:r19?.03:r18?.04:r17?.05:r16?.06:r15?.06:r14?.08:r13?.1:r12?.12:r11?.2:r10?.3:r9?.35:r8?.6:r7?.6:r6?.7:r5?.8:revision==="r4"?1.1:revision==="r3"?2.2:2.4,renderedFrameCount:frameCount,renderFps,sceneCount:r19?16:r18?32:r17?24:r16?15:r15?1:r14?32:r13?6:r12?6:r11?16:r10?36:r9?40:r5Plus?20:revision==="r4"?16:revision==="r3"?32:8,visualWorldCount:r19?9:r18?9:r17?9:r16?5:r15?1:r14?16:r13?8:r12?6:r11?11:r10?6:r5Plus?5:undefined,compositionCount:r19?32:r18?32:r17?24:r16?15:r15?1:r14?32:r13?16:r12?12:r11?16:r10?36:r9?40:r8?25:r7?24:r6?20:undefined,essentialLanguage:r5Plus?"vi":undefined,persistentLowerThird:r5Plus?false:undefined,persistentCaptionBar:r19||r18||r17||r16||r15||r14||r13||r12?false:undefined,atlasEvidence:r19?"TWO_MEANING_CHANGING_STATES_PER_SEQUENCE":r18?"EXACTLY_ONE_UNIQUE_CAUSAL_BEAT_PER_SAMPLE":r17?"TWENTY_FOUR_SHOT_NINE_WORLD_FILM":r16?"FIFTEEN_SHOT_MIXED_TREATMENT_FILM":r15||r14||r13||r12?"THIRTY_TWO_UNIFORM_TIME_ORDERED_FRAMES":r11?"SIXTEEN_BEFORE_AFTER_PAIRS":undefined,semanticAnimationMethods:["sixteen-sequence-object-ledger-film","two-meaning-changing-states-per-sequence","no-repeated-badge-headline-route-capsule-template","persistent-hold-clearing-settlement-ledger","directional-obligation-amount-counterparty","exception-entry-point-timeline-before-final-payoff","thirty-two-beat-nine-world-causal-film","one-unique-causal-beat-per-atlas-sample","labeled-bank-a-b-c-obligations","no-text-inside-moving-tokens","large-branch-status-plates","nine-world-twenty-four-shot-causal-film","three-decision-spine-through-body","vietnamese-only-critical-terminology","network-fee-route-closed","opaque-high-contrast-branch-plates","no-adjacent-same-world-atlas-samples","cinematic-mixed-treatment-fifteen-shot-film","five-generated-physical-worlds","six-camera-grammars","ordered-settlement-hops","alternative-exception-identities","attributed-clearing-equation","no-persistent-track","no-edge-clipped-labels","adeclip-and-adeclick-mastering","continuous-14400px-spatial-world","camera-follows-one-immutable-transaction","persistent-tx-01-principal-2-00","diegetic-world-labels-not-title-cards","maximum-two-actors-per-camera-crop","transfer-rails-behind-values","sequential-settlement-hops","single-exception-branch-per-time-window","anti-click-exact-audio-mastering","thirty-two-causal-state-world-journey","one-composition-per-atlas-sample","explicit-approved-before-hold","four-stage-clearing-transformation","four-stage-netting-transformation","one-full-frame-exception-branch-at-a-time","no-persistent-equation","no-repeated-panel","no-simultaneous-exception-chart","explicit-clearing-equation-2-plus-fee-equals-final","large-labeled-netting-obligation-groups","four-distinct-full-frame-state-compositions","state-bound-exception-quartet","no-repeated-state-map","no-tiny-obligation-markers","continuous-six-act-transaction-journey","single-hero-token-across-all-acts","thirty-two-uniform-time-ordered-frames","within-act-geometry-transformation","direct-state-rail-to-exception-mapping","no-adjacent-paired-samples","no-caption-bars","large-object-integrated-labels","mixed-media-continuous-transformation-film","sixteen-before-after-pairs","meaning-changing-object-motion","no-persistent-headings","no-progress-chrome","no-repeated-template-background","mobile-safe-vietnamese-labels","object-centric-full-frame-kinetic-explainer","no-title-card-grammar","no-lower-third-text-blocks","no-dotted-route-grammar","single-state-full-frame","four-resolved-exception-outcomes","persistent-clearing-actor-labels","twelve-to-one-netting","continuous-hero-object","phase-specific-hard-crops","five-layout-micro-cuts","forty-beat-compositor","fixed-transaction-amount-continuity","labeled-record-side-comparison","multi-world-settlement-sequence","explicit-clearing-fee-formula","explicit-state-label-binding","temporal-state-truth","current-history-future-state-roles","many-to-one-netting-result","physical-balance-reservoir","converging-record-plates","growing-merchant-coin-stack","exception-rail-branching","timeline-payoff"]};
  if (r21) {
    Object.assign(visualManifest, {
      semanticRuntimeRatio: 1,
      slideshowRuntimeRatio: 0,
      meaningfulMotionRatio: .99,
      first30MotionRatio: .99,
      cameraOnlyRatio: 0,
      treatmentFamilies: 6,
      minimumCriticalFontPx1080: 96,
      maximumVisualEventIntervalSeconds: .15,
      maximumStaticHoldSeconds: .03,
      sceneCount: 32,
      visualWorldCount: 9,
      compositionCount: 32,
      essentialLanguage: "en-US",
      market: "US",
      persistentLowerThird: false,
      persistentCaptionBar: false,
      atlasEvidence: "THIRTY_TWO_UNIFORM_HYBRID_DOCUMENTARY_FRAMES",
      semanticAnimationMethods: [
        "hybrid-documentary-explanation-system-film",
        "real-world-macro-reality-anchors",
        "dynamic-authorization-system-diagram",
        "meaning-changing-animated-ledger",
        "sankey-principal-and-fee-split",
        "bank-obligation-network-map",
        "state-and-exception-timeline",
        "tap-to-signal-to-network-to-ledger-to-settlement",
        "english-en-us-market-contract",
        "no-persistent-slide-template",
        "no-camera-only-progress",
        "seam-safe-audio-mastering",
      ],
    });
  }
  if (r20) {
    Object.assign(visualManifest, {
      treatmentFamilies: 16,
      minimumCriticalFontPx1080: 132,
      maximumVisualEventIntervalSeconds: .16,
      maximumStaticHoldSeconds: .03,
      sceneCount: 16,
      visualWorldCount: 9,
      compositionCount: 32,
      persistentCaptionBar: false,
      atlasEvidence: "TWO_STRUCTURALLY_DISTINCT_STATES_PER_SEQUENCE",
    });
    visualManifest.semanticAnimationMethods.unshift(
      "r19-object-ledger-mobile-reflow",
      "two-structurally-distinct-states-per-sequence",
      "full-frame-mobile-core-labels",
      "safe-obligation-arrow-corridor",
      "opening-three-node-separation",
      "dual-origin-exception-reflow",
      "recap-without-small-footer-text",
      "contract-voice-settings-applied",
      "seam-safe-audio-mastering",
    );
  }
