...
| X28 | P0 | Technical decode, contact-sheet and Browser pixel-motion evidence can be mistaken for competitive YouTube audience quality | `ACTIVE_REPAIR__R14_SOURCE_READY`: Document 77 reclassifies the old master as `TECHNICAL_CLEAN_CONTROL_ONLY`. R1–R13 are immutable rejected Golden evidence. R13 exact-audio PASS 95 is preserved, while visual FAIL 64 proves that accurate equations and labels still fail when compositions persist as slides. Migration `0098` maps 32 evidence positions to 32 causal states, makes authorization explicit before hold, separates clearing/netting transformations and shows one exception branch per frame. Production Factory, Browser, owner and freeze evidence remain required; source readiness grants no release authority | Video #1/Assurance |
