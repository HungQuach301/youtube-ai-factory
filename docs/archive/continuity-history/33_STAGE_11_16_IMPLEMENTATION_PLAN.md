...
| E — repair router | Map every Stage 12/14 failure to the owning root stage and enforce one bounded append-only root revision before escalation | only `REOPEN_ROOT_STAGE` may reopen; downstream becomes blocked; revisions append | root-cause reason, superseded revision, new attempt namespace, no overwritten bytes, re-run evidence |
