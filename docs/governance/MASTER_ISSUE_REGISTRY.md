...
| X31 | P0 | One judge/static atlas can miss temporal, audio, Browser and payoff defects | Append-only L0-L7 exact-artifact shadow orchestration, `0115` calibration schema and canonical QA Cockpit are live owner-only with zero PASS authority. Threshold V2 blocks repeated-byte pseudo-diversity, non-owner holdouts, missing receipts and unreconciled requests. Live `0118` reads all 66 remediation objects: 62 pass exact checksum/provenance and four prove historical R2-key overwrite. Live `0119` quarantines those four strict hash/size/metadata collisions, preserves failed receipts, references exact survivors and narrows current-rights work to 62 eligible items. Live `0120` inventories 46 provider and 16 composite rights requirements with 0 exact current receipts attached and all 62 still pending, without mutating rights PASS. Live `0121` materializes those 62 pending receipts into append-only typed collection tasks with zero external lookup or downstream authority. Attach exact receipts, complete independent-correlation/owner-label review and close missing L1/L4, clean/P0 and holdout controls before calibration or AI acceptance |
