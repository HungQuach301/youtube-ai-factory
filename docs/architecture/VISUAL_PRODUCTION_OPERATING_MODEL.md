...
`factory-scene-graph-renderer` converts eligible Scene Graph nodes into a canonical per-frame semantic render tape, stores exact content-addressed bytes in R2 and records fenced lineage. `factory-pixel-video-compositor` requires SOURCE/HYBRID exact-byte read-back and current rights, covers one contiguous 60-90 second range, preserves the tape canvas/timebase, produces exact-repeat VP9 bytes and seals entry/midpoint/exit decoded pixels. The generic live non-R22 canary has passed. `factory-hidden-systems-treatment-qualification` now adds a revision-independent 1920×1080/30fps corpus for all ten Channel treatment families, exact asset-preparation manifests, distinct topology/motion and decoded entry/mutation/exit states. Its authority is qualification-only until the append-only live package is stored and exact R22 dependencies are separately admitted.
