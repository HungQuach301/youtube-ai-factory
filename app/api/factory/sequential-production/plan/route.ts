...
export async function POST(request: Request) {
  try {
    ...
    const providerPlan = { OPENAI: { stages: ["08"], maximumRequests: 2, maximumSpendUsd: 4.5 }, PEXELS: { stages: ["09"], maximumRequests: Math.max(1, maxRequests - 6), maximumSpendUsd: 0 }, PIXABAY: { stages: ["09"], fallbackOnly: true, maximumSpendUsd: 0 }, ELEVENLABS: { stages: ["10"], maximumRequests: 4, maximumSpendUsd: 5 }, contingencyUsd: Math.max(0, requested - 9.5), forbidden: ["LEGACY_ASSET_REUSE", "UNLICENSED_DOWNLOAD", "AUTO_PUBLISH"] };
