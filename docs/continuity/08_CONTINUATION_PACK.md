...
The former `MOTION_PROOF_REQUIRED` champion-C handoff is historical only. Documents 34–38 are authoritative for the current Video Excellence boundary. Golden r9 visual/audio are immutable rejected evidence; Stage 11, videos 2–15 and auto-publish remain blocked. FP1 truthful operator projection and FP2 Capability Registry are implemented. The registry contains nine mechanisms, 20 hardest archetypes and 20 designed fixtures; all qualification bindings remain `QUALIFICATION_REQUIRED`, and the shared dispatch firewall is enforced before new production/provider work. Continue with FP3 executable Stage 07B/08 contracts using zero provider dispatch. Do not render Golden r10 until FP3–FP5 and all capability qualifications pass, and do not restore routine independent-QA repair loops, generic fallback or placeholder eligibility.
