...
type Snapshot = {
  ...
  batchActivationControl: { preflight: null | { id: string; status: string; inputHash: string; result: { nextIndex?: number; total?: number; products?: number; frames?: number; grammars?: number; semanticProjectionCoverage?: number; families?: number; maximumFamilyShare?: number; requestsDelta?: number; costDelta?: number; failures?: Array<{ logicalId?: string; checks?: string[]; duplicateOf?: string }> }; requestsBefore: number; costBefore: number; updatedAt: string }; activation: null | { id: string; status: string; idempotencyKey: string; targetBatchId: string; inputHash: string; result: Record<string, unknown>; committedAt: string | null } };
...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    if (data?.batchActivationControl.preflight?.status !== "RUNNING" || working || data.requestLedger.active > 0) return;
    const action = data?.productionBatch?.engineVersion === "SHOT_PRODUCT_ENGINE_V10_ARCHETYPE_SEMANTIC_COMPILER" ? "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" : "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION";
    const timer = window.setTimeout(() => void batchAction(action, true), 700);
    return () => window.clearTimeout(timer);
  }, [data?.batchActivationControl.preflight?.status, data?.batchActivationControl.preflight?.result.nextIndex, data?.productionBatch?.engineVersion, data?.requestLedger.active, working]);
  useEffect(() => {
    if (!["DISPATCHING", "QA_RUNNING"].includes(data?.productionBatch?.audit?.status || "") || working) return;
  ...
  async function batchAction(action: "START_WAVE_BATCH_1" | "ADOPT_WAVE_BATCH_1_ENGINE_ROOT_CORRECTION" | "ADOPT_WAVE_BATCH_1_SEMANTIC_ENGINE_ROOT_CORRECTION" | "PRODUCE_NEXT_WAVE_BATCH_1_SHOT" | "RECONCILE_WAVE_BATCH_1_AUDIT_TRANSPORT" | "RUN_WAVE_BATCH_1_AUDIT" | "START_WAVE_BATCH_2" | "ADOPT_WAVE_BATCH_2_ENGINE_ROOT_CORRECTION" | "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION" | "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION" | "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" | "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" | "PRODUCE_NEXT_WAVE_BATCH_2_SHOT" | "RUN_WAVE_BATCH_2_AUDIT", quiet = false) {
    setWorking(action); if (!quiet) setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    <section className="sequenceProofPlane">
      ...
      {data.productionBatch && <>
        ...
        {data.productionBatch.status === "ENGINE_ROOT_CAUSE_REQUIRED" && data.productionBatch.waveKey === "BATCH_2" && data.productionBatch.engineVersion === "SHOT_PRODUCT_ENGINE_V8_SCALE_BASELINE" && <div className="productStart"><div><small>CONTRACT-BOUND ENGINE V9</small><strong>Preserve V8 evidence and reproduce all 50 from a qualified engine</strong><p>V9 binds every state to contract-derived nodes, relations and negative constraints, blocks unsupported generic labels, and requires 50/50 manifests plus 150/150 unique frame hashes before reproduction can begin.</p></div><button onClick={() => void batchAction("ADOPT_WAVE_BATCH_2_ENGINE_ROOT_CORRECTION")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "ADOPT_WAVE_BATCH_2_ENGINE_ROOT_CORRECTION" ? "Qualifying contract-bound engine V9…" : "Qualify V9 and reproduce all 50"}</button></div>}
        {data.productionBatch.status === "ENGINE_ROOT_CAUSE_REQUIRED" && data.productionBatch.waveKey === "BATCH_2" && data.productionBatch.engineVersion === "SHOT_PRODUCT_ENGINE_V9_CONTRACT_BOUND_SCENE_GRAPH" && <div className="productStart"><div><small>ARCHETYPE SEMANTIC ENGINE V10</small><strong>Preserve V8/V9 evidence and replace the shared graph grammar</strong><p>V10 compiles a native visual grammar per archetype, binds every visible element to contract provenance, verifies directed ENTRY → MIDPOINT → EXIT transitions, and enforces portfolio family quotas beyond pixel-hash uniqueness.</p>{data.batchActivationControl.preflight?.status === "RUNNING" && <p>ZERO-SPEND PREFLIGHT · {data.batchActivationControl.preflight.result.nextIndex || 0}/50 contracts checkpointed · resumable · 0 provider requests</p>}{data.batchActivationControl.preflight?.status === "FAIL" && <p>ZERO-SPEND PREFLIGHT BLOCKED · {data.batchActivationControl.preflight.result.products}/50 products · {data.batchActivationControl.preflight.result.frames}/150 frames · {(data.batchActivationControl.preflight.result.failures || []).slice(0, 3).map((item) => `${item.logicalId} ${item.checks?.join("/")} ${item.duplicateOf ? `duplicates ${item.duplicateOf}` : ""}`).join(" · ")}</p>}{data.batchActivationControl.preflight?.status === "PASS" && <p>ZERO-SPEND PREFLIGHT PASS · {data.batchActivationControl.preflight.result.products}/50 products · {data.batchActivationControl.preflight.result.grammars}/50 grammars · {data.batchActivationControl.preflight.result.frames}/150 frames · {data.batchActivationControl.preflight.result.requestsDelta} requests · ${Number(data.batchActivationControl.preflight.result.costDelta || 0).toFixed(2)} delta</p>}</div><button onClick={() => void batchAction(data.batchActivationControl.preflight?.status === "PASS" ? "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION" : "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION" ? `Running zero-spend V10 QA · ${data.batchActivationControl.preflight?.result.nextIndex || 0}/50…` : working === "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION" ? "Committing V10 activation…" : data.batchActivationControl.preflight?.status === "PASS" ? "Activate V10 exactly once" : data.batchActivationControl.preflight?.status === "RUNNING" ? `Resume V10 zero-spend QA · ${data.batchActivationControl.preflight.result.nextIndex || 0}/50` : data.batchActivationControl.preflight?.status === "FAIL" ? "Re-run V10 QA after engine update" : "Run V10 zero-spend activation QA"}</button></div>}
        {data.productionBatch.status === "ENGINE_ROOT_CAUSE_REQUIRED" && data.productionBatch.waveKey === "BATCH_2" && data.productionBatch.engineVersion === "SHOT_PRODUCT_ENGINE_V10_ARCHETYPE_SEMANTIC_COMPILER" && <div className="productStart"><div><small>CONTRACT SEMANTIC PROJECTION ENGINE V11</small><strong>Preserve V8/V9/V10 evidence and reconnect meaning to pixels</strong><p>V11 compiles each state into a visible entity → action → constraint projection. The renderer consumes that projection directly, so claim, evidence clause and temporal delta are composition inputs rather than metadata labels.</p>{data.batchActivationControl.preflight?.status === "RUNNING" && <p>ZERO-SPEND PREFLIGHT · {data.batchActivationControl.preflight.result.nextIndex || 0}/50 contracts · {data.batchActivationControl.preflight.result.semanticProjectionCoverage || 0}/50 semantic projections · resumable</p>}{data.batchActivationControl.preflight?.status === "FAIL" && <p>V11 PREFLIGHT BLOCKED · {data.batchActivationControl.preflight.result.semanticProjectionCoverage || 0}/50 projections · {data.batchActivationControl.preflight.result.frames || 0}/150 frames · {(data.batchActivationControl.preflight.result.failures || []).slice(0, 3).map((item) => `${item.logicalId} ${item.checks?.join("/")}`).join(" · ")}</p>}{data.batchActivationControl.preflight?.status === "PASS" && <p>V11 PREFLIGHT PASS · 50/50 semantic projections · {data.batchActivationControl.preflight.result.grammars}/50 grammars · {data.batchActivationControl.preflight.result.frames}/150 frames · 0 provider requests · $0 delta</p>}</div><button onClick={() => void batchAction(data.batchActivationControl.preflight?.status === "PASS" ? "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" : "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" ? `Running V11 zero-spend QA · ${data.batchActivationControl.preflight?.result.nextIndex || 0}/50…` : working === "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" ? "Committing V11 activation…" : data.batchActivationControl.preflight?.status === "PASS" ? "Activate V11 exactly once" : data.batchActivationControl.preflight?.status === "RUNNING" ? `Resume V11 QA · ${data.batchActivationControl.preflight.result.nextIndex || 0}/50` : data.batchActivationControl.preflight?.status === "FAIL" ? "Re-run V11 QA after engine update" : "Run V11 zero-spend activation QA"}</button></div>}
