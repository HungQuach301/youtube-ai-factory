...
Migrations `0106`-`0110` implement the contract/timebase foundation, fenced canonical writer, append-only Provider Gateway/compiler records, a deterministic render-tape worker and canary compositor admission. The Gateway remains `ZERO_DISPATCH`. The compiler creates Blueprint, full-duration Shot Contracts and Scene Graph; the renderer writes exact semantic tapes to R2. Asset eligibility then binds exact R2 read-back to a current commercial-rights receipt. The pixel/video compositor accepts only a qualified active worker, exact tape and eligible SOURCE/HYBRID assets; it verifies 60-90 second coverage, VP9 geometry/timebase/frame count, deterministic replay hash and entry/midpoint/exit decoded pixel artifacts before atomically recording `COMPOSED_FROM` lineage, job, artifact and canary receipt. The controlled FFmpeg executor rejects hash or coverage mismatch and repeated adjacent motion profiles. Its tracked fixture produces identical 60-second, 1,800-frame WebM bytes on two executions. All runtime actions have separate disabled-by-default flags, remain zero-spend and R22-blocked. The fixture is not a live canary; provider dispatch/reconciliation, assurance and UI projections remain later work.
