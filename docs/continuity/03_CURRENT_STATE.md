...
PHASE_45 = IN_PROGRESS__CORPUS_REMEDIATION_V1_LIVE__66_EXACT_EVIDENCE_WORK_ITEMS__31_EXACT_GAPS_OPEN
PHASE_46 = GATED__ASSURANCE_CALIBRATION_AND_EXACT_DEPENDENCY_ADMISSION_REQUIRED__R22_BLOCKED
SOURCE_MIGRATION = 0117_FACTORY_ASSURANCE_CORPUS_REMEDIATION_INVENTORY__LIVE_VERIFIED
SOURCE_MIGRATION_LIVE_D1 = VERIFIED__0117_ONE_SNAPSHOT__66_ITEMS__48_AUDIO__18_MASTER__REMEDIATION_REQUIRED
...
LAST_RECONCILED_SITE_VERSION = 543__CORPUS_REMEDIATION_V1__ENV_REVISION_55__OWNER_ONLY__TEMPORARY_FLAGS_ABSENT
...
```text
1. Resolve exact R2 byte read-back, checksum, provenance and current-rights evidence for the 66 remediation items; do not label or count them before exact evidence passes.
2. Resolve correlation and owner-label work; review eligible independent inputs into a new immutable threshold-V2 corpus snapshot.
3. Calibrate exact L0-L7 judge/sampler dependencies in `AI_SHADOW`; keep every result advisory until exact qualification registration passes.
4. Bind and qualify exact R22 dependencies plus canary admission, then exercise recovery/incident controls; compile R22 only after explicit Production authority exists.
```
