...
| Blind-control calibration and QA Cockpit projection (`0115`) | Independently labeled clean/defect controls, Production holdouts, correlation groups, repeated unchanged-byte observations, exact judge/model/prompt/rubric/schema/sampler identity, per-layer metrics and read-only operator projection | 4/4 focused calibration/Cockpit tests and 252/252 full tests PASS; verified application build; false-clean and P0/P1 repeat flips stay `ADVISORY`; all eight measured layers remain candidates until exact qualification registration | `SOURCE_QUALIFIED_AI_SHADOW_ONLY`; zero qualification/PASS/provider/R22/master/release/publication authority; live schema/read-back pending |
