...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    if (data?.canary?.status !== "AUTHORIZED" || data.canary.version === "CONTROLLED_CANARY_V5_SOURCE_BOUND_MATERIALIZATION" || working || data.requestLedger.active > 0) return;
    const timer = window.setTimeout(() => void canaryAction("START_CONTROLLED_CANARY_UNIT"), 900);
    return () => window.clearTimeout(timer);
  }, [data?.canary?.status, data?.canary?.version, data?.canary?.currentIndex, data?.requestLedger.active, working]);
  useEffect(() => {
  ...
  async function canaryAction(action: "AUTHORIZE_CONTROLLED_CANARY" | "AUTHORIZE_CONTROLLED_CANARY_V3" | "AUTHORIZE_CONTROLLED_CANARY_V4" | "AUTHORIZE_CONTROLLED_CANARY_V5" | "RELEASE_CONTROLLED_CANARY_V5_UNIT" | "RELEASE_PRODUCTION_RECOVERY_PROBE" | "BUILD_RECOVERY_CONTRACT_ALIGNMENT" | "RELEASE_CONTRACT_ALIGNED_RECOVERY_PROBE" | "RECONCILE_CONTRACT_ALIGNED_RECOVERY_TERMINAL" | "START_CONTROLLED_CANARY_UNIT" | "RELEASE_NEXT_CONTROLLED_CANARY_UNIT") {
    setWorking(action); setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "FAILED" && <div className="certificationResult"><b>TERMINAL CANARY GATES</b>{data.canary.gates.map((gate) => <small key={gate.id}>{gate.status} · {gate.id.replaceAll("_", " ")}{gate.evidence ? ` · ${gate.evidence}` : ""}</small>)}</div>}
        ...
        {data.canary.status === "RECOVERY_PASS_REVIEW" && <p className="stateBanner">MP-001 recovery PASS is sealed at 94/100. Authorization is paused; MP-002, sequence proof and scale remain locked until a full-queue production binding is built.</p>}
