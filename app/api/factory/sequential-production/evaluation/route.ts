...
export async function GET(request: Request) {
  try {
    ...
    if (["owner-label-task", "owner-label-workflow", "provider-history-recovery"].includes(view) || artifactCandidateId || fixtureArtifactId) {
      ...
      if (view === "provider-history-recovery") {
        ...
        const [materialization, rightsEvidence] = await Promise.all([controlledFixtureMaterializationSnapshot(env.DB), currentRightsEvidenceSnapshot(env.DB)]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof EvaluationCommandError || error instanceof FixtureMaterializationError || error instanceof RightsEvidenceError) return Response.json({ error: { code: error.code, message: error.message }, providerRequests: 0, spendUsd: 0 }, { status: error.status, headers: NO_STORE });
