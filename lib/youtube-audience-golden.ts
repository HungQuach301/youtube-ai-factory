...
export async function audienceGoldenSnapshot(db: AudienceGoldenDB) {
  const [policy, disposition, blueprint, audio, materialization, visualQa, audioQa, browserQa, ownerTask, ownerReceipt, freeze] = await Promise.all([
    ...
    first(db, "SELECT * FROM v7_youtube_golden_sequence_blueprints WHERE channel_id=? AND blueprint_version=? LIMIT 1", CHANNEL_ID, AUDIENCE_BLUEPRINT_VERSION),
    first(db, "SELECT * FROM v7_youtube_golden_audio_artifacts WHERE channel_id=? LIMIT 1", CHANNEL_ID),
    first(db, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE channel_id=? LIMIT 1", CHANNEL_ID),
    first(db, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='FACTORY_VISUAL' LIMIT 1", CHANNEL_ID),
    first(db, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='FACTORY_AUDIO' LIMIT 1", CHANNEL_ID),
    first(db, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='BROWSER_DEVICE' LIMIT 1", CHANNEL_ID),
    first(db, "SELECT * FROM v7_youtube_golden_owner_tasks WHERE channel_id=? LIMIT 1", CHANNEL_ID),
    first(db, "SELECT * FROM v7_youtube_golden_owner_receipts WHERE channel_id=? LIMIT 1", CHANNEL_ID),
    first(db, "SELECT * FROM v7_youtube_golden_freeze_receipts WHERE channel_id=? LIMIT 1", CHANNEL_ID),
  ]);
  ...
  return {
    ...
    nextAction: freeze ? "FROZEN" : ownerTask && !ownerReceipt ? "OWNER_FULL_PLAYBACK_REQUIRED" : materialization && !visualQa ? "RUN_FACTORY_VISUAL_QA" : materialization && !audioQa ? "RUN_FACTORY_AUDIO_QA" : materialization && !browserQa ? "RUN_BROWSER_QA" : audio && !materialization ? "MATERIALIZE_MASTER" : blueprint && !audio ? "GENERATE_AUDIO" : "SEAL_BLUEPRINT",
  };
}
...
export async function generateAudienceGoldenAudioAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string) {
  ...
  if (prior) { if (clean(prior.lifecycle_state) === "COMPLETE") return { outcome: "REPLAYED", snapshot: await audienceGoldenSnapshot(env.DB) }; throw new AudienceGoldenError("GOLDEN_AUDIO_ALREADY_ATTEMPTED", 409, `The immutable audio attempt is ${clean(prior.lifecycle_state)}`); }
  ...
  const runId = makeId("audience-golden-audio-run");
  await run(env.DB, "INSERT INTO v7_youtube_golden_audio_runs (id,channel_id,blueprint_id,policy_version,idempotency_key,intent_hash,lifecycle_state,reserved_spend_usd,actor) VALUES (?,?,?,?,?,?,'PLANNED',0.20,?)", runId, CHANNEL_ID, blueprint.id, AUDIENCE_POLICY_VERSION, idempotencyKey, intentHash, actor);
  try {
    await run(env.DB, "UPDATE v7_youtube_golden_audio_runs SET lifecycle_state='RUNNING' WHERE id=?", runId);
    ...
    await run(env.DB, "UPDATE v7_youtube_golden_audio_runs SET subscription_reads=1 WHERE id=?", runId);
    ...
    const response = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(clean(priorVoice.voice_id))}?output_format=mp3_44100_128`, { method: "POST", headers: { "xi-api-key": env.ELEVENLABS_API_KEY, "content-type": "application/json" }, body: JSON.stringify(requestBody), signal: AbortSignal.timeout(180_000) });
    await run(env.DB, "UPDATE v7_youtube_golden_audio_runs SET tts_requests=1 WHERE id=?", runId);
    ...
    await run(env.DB, "UPDATE v7_youtube_golden_audio_runs SET lifecycle_state='COMPLETE',completed_at=? WHERE id=?", now(), runId);
    ...
    await run(env.DB, "UPDATE v7_youtube_golden_audio_runs SET lifecycle_state='FAILED',error_code=?,completed_at=? WHERE id=?", known.code, now(), runId).catch(() => undefined);
    throw known;
  }
}
...
export async function commitAudienceGoldenMaterializationAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string, body: Row) {
  const [blueprint, audio, existing, policy] = await Promise.all([
    ...
    first(env.DB, "SELECT id FROM v7_youtube_golden_materialization_receipts WHERE channel_id=?", CHANNEL_ID),
...
export async function runAudienceGoldenVisualQaAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string) {
  ...
  const [materialization, existing] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE channel_id=?", CHANNEL_ID), first(env.DB, "SELECT id FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='FACTORY_VISUAL'", CHANNEL_ID)]); if (!materialization || existing) throw new AudienceGoldenError(existing ? "VISUAL_QA_CEILING_REACHED" : "MATERIALIZATION_REQUIRED", 409, existing ? "The visual QA request ceiling was reached" : "Materialization is required");
...
export async function runAudienceGoldenAudioQaAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string) {
  ...
  const [materialization, existing] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE channel_id=?", CHANNEL_ID), first(env.DB, "SELECT id FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='FACTORY_AUDIO'", CHANNEL_ID)]); if (!materialization || existing) throw new AudienceGoldenError(existing ? "AUDIO_QA_CEILING_REACHED" : "MATERIALIZATION_REQUIRED", 409, existing ? "The audio QA request ceiling was reached" : "Materialization is required");
  ...
  const prompt = "Listen to the entire exact 78-second Vietnamese audience mix as an independent adversarial YouTube QA reviewer. Judge naturalness, pronunciation, pacing/prosody, continuity/seams, narration-vs-music clarity, and listener comfort. Reject clicks, clipping, bad joins, excessive bed, robotic cadence, fatigue, or semantic delivery confusion. PASS requires overall >=92, every dimension >=90, P0=0, P1=0, P2<=2. Use the function exactly once.";
...
export async function recordAudienceGoldenBrowserQaAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string, evidence: Row) {
  const [materialization, existing] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE channel_id=?", CHANNEL_ID), first(env.DB, "SELECT id FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='BROWSER_DEVICE'", CHANNEL_ID)]); if (!materialization || existing) throw new AudienceGoldenError(existing ? "BROWSER_QA_CEILING_REACHED" : "MATERIALIZATION_REQUIRED", 409, existing ? "Browser QA is append-only" : "Materialization required");
...
async function maybeCreateOwnerTask(db: AudienceGoldenDB) {
  const [materialization, visual, audio, browser, existing] = await Promise.all([first(db, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE channel_id=?", CHANNEL_ID), first(db, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='FACTORY_VISUAL'", CHANNEL_ID), first(db, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='FACTORY_AUDIO'", CHANNEL_ID), first(db, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer='BROWSER_DEVICE'", CHANNEL_ID), first(db, "SELECT id FROM v7_youtube_golden_owner_tasks WHERE channel_id=?", CHANNEL_ID)]);
...
export async function recordAudienceGoldenOwnerDecisionAuthorized(env: AudienceGoldenEnv, actor: string, idempotencyKey: string, body: Row) {
  const [task, materialization, existing] = await Promise.all([first(env.DB, "SELECT * FROM v7_youtube_golden_owner_tasks WHERE channel_id=?", CHANNEL_ID), first(env.DB, "SELECT * FROM v7_youtube_golden_materialization_receipts WHERE channel_id=?", CHANNEL_ID), first(env.DB, "SELECT id FROM v7_youtube_golden_owner_receipts WHERE channel_id=?", CHANNEL_ID)]); if (!task || !materialization || existing) throw new AudienceGoldenError(existing ? "OWNER_DECISION_ALREADY_RECORDED" : "OWNER_TASK_NOT_READY", 409, existing ? "Owner ground truth is immutable" : "All independent QA gates must pass first");
  ...
  if (decision === "CLEAN_CONFIRMED") {
    const qaRows = await Promise.all(["FACTORY_VISUAL", "FACTORY_AUDIO", "BROWSER_DEVICE"].map((layer) => first(env.DB, "SELECT * FROM v7_youtube_golden_qa_receipts WHERE channel_id=? AND qa_layer=?", CHANNEL_ID, layer))); if (qaRows.some((row) => !row || clean(row.decision_state) !== "PASS")) throw new AudienceGoldenError("FREEZE_QA_GATE_FAILED", 409, "All QA layers must remain PASS");
...
export async function readAudienceGoldenMediaAuthorized(env: AudienceGoldenEnv, role: string) {
  ...
  const row = normalized === "source-audio" ? await first(env.DB, "SELECT storage_key,sha256,mime_type FROM v7_youtube_golden_audio_artifacts WHERE channel_id=?", CHANNEL_ID) : normalized === "master" ? await first(env.DB, "SELECT master_storage_key storage_key,master_hash sha256,'video/mp4' mime_type FROM v7_youtube_golden_materialization_receipts WHERE channel_id=?", CHANNEL_ID) : null;
