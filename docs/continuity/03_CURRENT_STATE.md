...
PHASE_45 = IN_PROGRESS__ASSURANCE_CALIBRATION_SCHEMA_AND_QA_COCKPIT_LIVE__CORPUS_ADMISSION_V1_LIVE__31_EXACT_GAPS_OPEN
PHASE_46 = GATED__ASSURANCE_CALIBRATION_AND_EXACT_DEPENDENCY_ADMISSION_REQUIRED__R22_BLOCKED
SOURCE_MIGRATION = 0116_FACTORY_ASSURANCE_CALIBRATION_CORPUS_ADMISSION__LIVE_SCHEMA_QUALIFIED
...
```text
1. Materialize the threshold-V2 calibration corpus with distinct exact bytes, independent evidence, owner-confirmed Production holdouts and receipt-bound repeat observations.
2. Calibrate exact L0-L7 judge/sampler dependencies in `AI_SHADOW`; keep every result advisory until exact qualification registration passes.
...
4. Exercise retention/recovery/incident controls and qualify any separately authorized provider-dispatch path; compile R22 only after explicit Production authority exists.
```
