CREATE TABLE `v7_youtube_golden_revision_21_receipts` (
  ...
  `revision_key` text NOT NULL CHECK (`revision_key` = 'AUDIENCE_GOLDEN_REVISION_21'),
  `visual_failure_receipt_id` text NOT NULL UNIQUE,
  `audio_pass_receipt_id` text NOT NULL UNIQUE,
  `owner_directive_key` text NOT NULL CHECK (`owner_directive_key` = 'OWNER_US_ENGLISH_HYBRID_DOCUMENTARY_2026_08_24'),
...
CREATE TRIGGER `v7_youtube_golden_revision_21_no_update` BEFORE UPDATE ON `v7_youtube_golden_revision_21_receipts` BEGIN SELECT RAISE(ABORT, 'YOUTUBE_GOLDEN_REVISION_21_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `v7_youtube_golden_revision_21_no_delete` BEFORE DELETE ON `v7_youtube_golden_revision_21_receipts` BEGIN SELECT RAISE(ABORT, 'YOUTUBE_GOLDEN_REVISION_21_IMMUTABLE'); END;
