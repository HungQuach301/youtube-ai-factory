...
6. Evidence/Assurance orchestration and QA Cockpit. `PARTIAL: EXACT-LINEAGE L0-L7 AI_SHADOW AND 0115 CALIBRATION SCHEMA LIVE; QA COCKPIT DEPLOYED; ANTI-CORRELATION/RECEIPT ADMISSION V2 SOURCE-QUALIFIED; DATASET EXECUTION PENDING`
...
Migrations `0106`-`0115` implement the contract/timebase foundation, fenced canonical writer, append-only Provider Gateway/compiler/control records, deterministic render tape, canary compositor admission, bounded live runtime/treatment qualification receipts, the live-schema Evidence/Assurance shadow foundation and the live calibration/Cockpit layer. The Gateway remains `ZERO_DISPATCH`. The compiler creates Blueprint, full-duration Shot Contracts and Scene Graph; the renderer writes exact semantic tapes to R2. Asset eligibility binds exact R2 read-back to current rights. The pixel/video compositor accepts only a qualified active worker, exact tape and eligible SOURCE/HYBRID assets; it verifies 60-90 second coverage, VP9 geometry/timebase/frame count, deterministic replay hash and entry/midpoint/exit decoded pixels before atomically recording `COMPOSED_FROM` lineage, job, artifact and canary receipt. The Assurance foundation binds one frozen artifact to canonical frame/audio evidence, exact source/deployment/runtime identity, L0-L7 qualifications and immutable receipts; producer self-PASS, stale judge dependencies and infrastructure-as-content verdicts fail closed. Calibration threshold V2 rejects reused artifact/evidence bytes, non-owner-confirmed Production holdouts, conflicting labels, missing repeat receipts and unreconciled request/cost state. It runs only as `AI_SHADOW`: candidate qualification/acceptance remains non-authoritative and every dispatch, AI acceptance, R22, master, release and publication authority is zero. Sites v539 verifies all four `0115` tables live and empty, `/qa-cockpit` deployed owner-only and environment revision 50 unchanged; actual calibration evidence remains pending.

Migration `0127` uses the same runtime writer as the single mutation boundary for controlled-audio route planning. `reserveFactoryRuntimeWork` issues a monotonic fence; one `FREEZE_STAGE` command atomically inserts the canonical work request, zero-dispatch route, exact cost reservation and assurance receipt, then releases the lease. The command payload and cost scope both record zero actual provider requests and zero actual spend. Any expired qualification/rights record, non-current latest drift, wrong binding or exhausted envelope fails before canonical effects commit.

Migration `0128` adds a distinct `ASSURANCE_AUDIO_PAID_DISPATCH_AUTHORIZATION` stream. Internal reservation, qualification, rights, drift and provider-activity gates run before external observation. Exact current subscription and official-rights responses are content-addressed in R2 and read back before a fenced `FREEZE_STAGE` may persist one 15-minute, one-request authorization. The resulting scope is not a provider event: `ProviderDispatched`, native request, generation, reconciliation and actual spend remain absent until a later typed executor consumes it.
