...
## ADR-116 — One panorama is also one repeated composition; R16 uses bounded mixed treatment

**Decision:** `AUDIENCE_GOLDEN_REVISION_16` may be created exactly once only when the same R15 exact master has visual FAIL and audio FAIL receipts. R16 must use 15 authored shots across five generated cinematic worlds and multiple camera grammars; neither poster succession nor one persistent horizontal track is allowed. Clearing must name the merchant principal, network-record fee and final record in one attributed equation. Settlement must appear in the causal order Bank→Network, net result, Network→Acquirer, Acquirer→Merchant. Cancel, expiry, refund and dispute are alternative `TX-01` identities rather than sequential states. Text may not clip the safe frame. Audio must be regenerated from the exact narration and mastered with de-clip, de-click, −16 LUFS and −4 dBTP targets. R15 bytes and receipts remain immutable; source preview, Factory, Browser, owner, freeze and release remain separate authorities.

## ADR-117 — Visual richness requires continuous promise structure, terminology closure and short world dwell

**Decision:** `AUDIENCE_GOLDEN_REVISION_17` may be created exactly once only when the same R16 exact master has visual FAIL and exact-audio PASS receipts. R17 must carry `1 · CHO PHÉP / 2 · ĐỐI CHIẾU / 3 · QUYẾT TOÁN` through the explanatory body, use 24 shots across nine physical worlds, avoid adjacent same-world atlas evidence where possible, and use Vietnamese critical terminology. The fee route must close explicitly: the merchant receives principal `2,00` and the payment network receives fee `0,05`. Every exception must use an opaque high-contrast plate clear of its marker, name the source state and resolve the outcome. R16 audio settings remain qualified but R17 still binds new exact bytes. Historical evidence and Factory, Browser, owner, freeze and release authorities remain separate.
...
## ADR-118 — A rich world still becomes a slide when adjacent evidence holds the same composition

**Decision:** `AUDIENCE_GOLDEN_REVISION_18` may be created exactly once only when the same R17 exact master has visual FAIL and exact-audio PASS receipts. R18 maps each of 32 uniformly sampled positions to one unique causal beat across nine alternating worlds; adjacent same-world samples are forbidden and every beat must contain meaning-changing motion. Netting obligations must identify `NGÂN HÀNG A`, `NGÂN HÀNG B` and `NGÂN HÀNG C` before converging to one net result. Moving tokens carry no embedded text, while exception identifiers and status plates remain at least 112 px at 1080p. R17 audio settings remain qualified but R18 still binds newly generated exact bytes. Historical evidence and Factory, Browser, owner, freeze and release authorities remain separate.

**Reason:** R17 repaired the body-wide promise, terminology, fee route, world richness and branch contrast, improving visual QA from 68 to 87. Its remaining P1 is localized temporal evidence: five adjacent sample pairs hold nearly unchanged compositions. The bounded R18 repair increases causal-beat density and relocates small labels without changing already-passing narrative or audio systems.

## ADR-119 — More beats do not repair a repeated template; R19 makes the ledger and objects carry causality

**Decision:** `AUDIENCE_GOLDEN_REVISION_19` may be created exactly once only when the same R18 exact master has visual FAIL and exact-audio PASS receipts. R19 uses sixteen object-ledger sequences sampled at two meaning-changing states each and forbids the repeated badge/headline/diagonal-route/status-capsule template. A persistent ledger must reconcile principal `2,00` reserved at authorization, network fee `0,05` added separately at clearing, gross system obligation `2,05`, merchant receipt `2,00` and payment-network receipt `0,05`. Directional obligations identify source, amount, counterparty and direction. Cancellation and hold expiry branch from the hold before clearing; refund and dispute branch after paid; the final settlement payoff follows that alternative map. R18 audio settings remain qualified but R19 binds new exact bytes. Factory, Browser, owner, freeze and release authorities remain separate.

**Reason:** R18 increased the nominal beat count and fixed label size, but independent QA scored visual motion 54 and retention pacing 68 because nearly every sample still shared one infographic template. The exact failure proves that metadata counts and text replacement cannot substitute for visible state transformation. R19 changes the explanatory mechanism itself while preserving the passing narrative and audio systems.

## ADR-120 — A stronger object ledger still fails when mobile hierarchy collides and sealed voice settings are bypassed

**Decision:** `AUDIENCE_GOLDEN_REVISION_20` may be created exactly once only when the same R19 exact master has both visual FAIL and exact-audio FAIL receipts. R20 keeps the sixteen-sequence object ledger but reflows the affected ledger, branch and recap moments as full-frame mobile compositions; every paired sample must change geometry, not only text or status. The A/B/C obligation rows reserve an empty center corridor so arrows never cross source labels, amounts or counterparties. The opening separates the three promised decisions and the recap removes small footer text. Audio is regenerated from the same narration with the sealed `.92` speed, `.88` stability, `.76` similarity and `0` style contract, continuous resampling, de-clip/de-click processing, −16 LUFS and −4 dBTP targets. R19 bytes and receipts remain immutable; Factory, Browser, owner, freeze and release authorities remain separate.

**Reason:** R19 materially improved visual QA to 87, but independent evidence still found two P1 mobile-layout defects and two P2 pacing/composition defects. Audio fell to 88 because the executor's contract-selection regex ended at R18, so R19 silently used default synthesis settings; a discontinuity near 00:25 became P1. R20 repairs both root causes without weakening thresholds or treating the source contact sheet as Production evidence.

## ADR-121 — The US audience contract overrides a technically passing Vietnamese audio layer

**Decision:** `AUDIENCE_GOLDEN_REVISION_21` may be created exactly once only when the same R20 exact master has visual FAIL and exact-audio PASS receipts and it binds owner directive `OWNER_US_ENGLISH_HYBRID_DOCUMENTARY_2026_08_24`. R20's audio PASS remains valid evidence for its exact Vietnamese bytes but is ineligible for reuse in the US-market artifact. R21 must generate new English/en-US narration and voice bytes under the existing paid-plan, provider-native, rights and R2 controls. Its visual form is `HYBRID_DOCUMENTARY_EXPLANATION_SYSTEM_FILM`: at least eleven reality-anchor beats interleave with dynamic system diagrams, animated ledgers, Sankey money/data flows, obligation network maps and exception timelines. Thirty-two sampled positions map to thirty-two causal beats; no treatment may persist longer than 3.2 seconds; persistent slide templates, headline bands and camera-only progress are forbidden. R20 bytes and receipts remain immutable, and Factory, Browser, owner, freeze, release and publication authorities remain separate.

**Reason:** R20 corrected its inherited mobile and audio defects but Production Factory visual QA still failed, while the owner separately identified that a Vietnamese Golden Sequence cannot qualify an English US channel. Treating a language-incompatible audio PASS as reusable would confuse technical cleanliness with market eligibility. Repeating another infographic or image-only repair would also ignore the requested production medium. R21 therefore changes the market-language and visual-system contracts together, while preserving the exact R20 evidence and all downstream gates.

## ADR-122 — Source-atlas variety is not proof of phase-distinct visual causality
