...
Sites v399 confirmed that all 12 blocked candidates have R2 metadata conflicts; seven also have byte-size/checksum failure, while five have checksum PASS and provenance FAIL. Deploy the v2 field-fact diagnostic to distinguish metadata-only failures from source-to-object byte divergence. Select a repair, incident or exclusion lane only from that production evidence, without rewriting prior receipts. Then collect explicit evidence for the 63 rights-pending candidates and bind owner-confirmed defect labels before correlation control. No dataset may be sealed and no assurance provider may be called until those gates pass.
