...
const checks = [
  ...
  ["Sequential production owner-first navigation", files.shell.includes('production: "/video-engine"') && files.productionV2Page.includes("Làm thật tốt video hiện tại trước khi mở video tiếp theo")],
  ["Sequential production five-view workspace", ["Video đang sản xuất", "18 bước sản xuất", "Thiết kế & dữ liệu", "Danh sách 15 video", "Bằng chứng bị loại"].every((label) => files.productionV2Page.includes(label))],
  ["Sequential production defaults to current video", files.productionV2Page.includes('requested as TabKey : "current"')],
  ["Sequential production exact exclusive lease", files.sequentialProductionContract.includes("ONE_VIDEO_AT_A_TIME") && files.sequentialProductionProjection.includes("Chỉ một video đang được phép chạy")],
  ["Sequential production blocks video N plus one", files.sequentialProductionProjection.includes("Video N+1 không được chạy trước") && files.sequentialProductionMigration.includes("BLOCKED_PREVIOUS_VIDEO")],
  ...
  ["V7 V23.4 V281 quality firewall", files.productionV2Page.includes("V7 → V23.4 → V281") && files.productionV2Page.includes("Tám vai trò đánh giá độc lập") && files.sequentialProductionProjection.includes("P0=0 và không còn P1 trọng yếu")],
  ["Sequential production separate publishing authority", files.productionV2Page.includes("autoPublish") && files.productionV2Page.includes("Tự động đăng YouTube") && files.sequentialProductionProjection.includes("quyền xuất bản là bước riêng")],
  ["Stage history is separate from current completion", files.productionV2Page.includes("Không dùng dữ liệu cũ để đánh dấu hoàn tất") && files.sequentialProductionContract.includes("PriorWorkClass")],
