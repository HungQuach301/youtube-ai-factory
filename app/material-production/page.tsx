...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => void batchAction("PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION", true), 700);
    return () => window.clearTimeout(timer);
  }, [data?.batchActivationControl.preflight?.status, data?.batchActivationControl.preflight?.result.nextIndex, data?.requestLedger.active, working]);
  useEffect(() => {
