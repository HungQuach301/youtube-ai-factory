...
| X28 | P0 | Technical decode, contact-sheet and Browser pixel-motion evidence can be mistaken for competitive YouTube audience quality | `ACTIVE_REPAIR__R20_SOURCE_VERIFIED`: Document 77 reclassifies the old master as `TECHNICAL_CLEAN_CONTROL_ONLY`. R1–R19 are immutable rejected Golden evidence. R19 improves visual QA to 87 but still fails mobile hierarchy, collision and paired-composition gates; audio fails 88 after the sealed voice contract was bypassed. Migration `0104` performs full-frame mobile reflow, collision-free obligation lanes, structurally distinct pairs and explicit seam-safe voice-contract selection. Production Factory, Browser, owner and freeze evidence remain required; source readiness grants no release authority | Video #1/Assurance |
| X29 | P0 | Canonical and compatibility frontends projected different production tables, hiding current materials and presenting stale state as authoritative | `HANDLED__SOURCE_VERIFIED_DEPLOYMENT_PENDING`: Video Engine now reconciles the current Audience Golden snapshot; canonical navigation targets `/video-engine`; Golden materials are directly discoverable; eight V7/V23 routes disclose compatibility-only authority; the mandatory Projection Integrity Gate and pure tests block route/source regression. R20 remains visual FAIL and release-ineligible | Projection integrity |
