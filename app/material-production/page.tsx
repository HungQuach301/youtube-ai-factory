...
export default function MaterialProductionPage() {
  ...
  useEffect(() => {
    if (data?.canary?.version !== "RELEASE_TRAIN_V1_SHOT_CONTRACT_QA" || data.canary.status !== "BATCH_UNIT_PASS_REVIEW" || data.authorization?.modelPolicy.batchAuthorized !== true || working || data.requestLedger.active > 0) return;
  ...
  async function canaryAction(action: "AUTHORIZE_CONTROLLED_CANARY" | "AUTHORIZE_CONTROLLED_CANARY_V3" | "AUTHORIZE_CONTROLLED_CANARY_V4" | "AUTHORIZE_CONTROLLED_CANARY_V5" | "RELEASE_CONTROLLED_CANARY_V5_UNIT" | "RELEASE_PRODUCTION_RECOVERY_PROBE" | "BUILD_RECOVERY_CONTRACT_ALIGNMENT" | "RELEASE_CONTRACT_ALIGNED_RECOVERY_PROBE" | "RECONCILE_CONTRACT_ALIGNED_RECOVERY_TERMINAL" | "BUILD_RELEASE_TRAIN_PREFLIGHT" | "RELEASE_RELEASE_TRAIN_SEQUENCE_PROOF" | "START_RELEASE_TRAIN_BATCH" | "RELEASE_NEXT_RELEASE_TRAIN_BATCH_UNIT" | "START_CONTROLLED_CANARY_UNIT" | "RELEASE_NEXT_CONTROLLED_CANARY_UNIT") {
    setWorking(action); setError(null);
    try {
      const response = await fetch("/api/factory/material-production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
      const payload = await response.json() as Snapshot & { error?: string };
  ...
  return <main className="shotShell">
    ...
    {data.reliability?.regressions[0]?.status === "PASS" && data.reliability.regressions[0].score === 100 && <section className="reliabilityBaseline">
      ...
      {data.canary && <>
        ...
        {data.canary.status === "SEQUENCE_OR_BATCH_FAILED_PRESERVED" && data.canary.version === "RELEASE_TRAIN_V1_SHOT_CONTRACT_QA" && <div className="reliabilityStart"><p>Request 83 and MP-002’s 42/100 audit remain immutable. Replace the proof-card adapter once with Production Scene Renderer v2, compile executable scene specs for MP-002–MP-010, render 27 new production frames and run every free contract, OCR/evidence, forbidden-label, layout, lineage and hash gate before another paid request.</p><button onClick={() => void canaryAction("BUILD_PRODUCTION_SCENE_PREFLIGHT")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "BUILD_PRODUCTION_SCENE_PREFLIGHT" ? "Rendering and validating 27 production frames…" : "Build Production Scene Renderer v2 · $0"}</button></div>}
        {data.canary.status === "READY_FOR_SEQUENCE_PROOF" && data.canary.version === "RELEASE_TRAIN_V2_PRODUCTION_SCENE_RENDERER" && <div className="reliabilityStart"><p>Production Scene Renderer v2 passed all zero-spend gates for MP-002–MP-010. MP-002 EXIT visibly co-locates APPROVED and $100.00; proof cards, fixtures, placeholders and QA metadata are absent. Release exactly request 84 with no retry.</p><button onClick={() => void canaryAction("RELEASE_RELEASE_TRAIN_SEQUENCE_PROOF")} disabled={Boolean(working) || data.requestLedger.active > 0 || data.canary.currentIndex !== 1}>{working === "RELEASE_RELEASE_TRAIN_SEQUENCE_PROOF" ? "Releasing repaired MP-002 proof…" : "Run repaired MP-002 Sequence Proof · max $1"}</button></div>}
        {data.canary.status === "SEQUENCE_PROOF_PASS_REVIEW" && data.canary.version === "RELEASE_TRAIN_V2_PRODUCTION_SCENE_RENDERER" && <div className="reliabilityStart"><p>MP-002 passed the strict production-scene rubric. Open the bounded MP-003–MP-010 batch on the locked renderer; every unit hard-stops at terminal QA, no retry is permitted, and the concurrency ceiling remains 2.</p><button onClick={() => void canaryAction("START_RELEASE_TRAIN_BATCH")} disabled={Boolean(working) || data.requestLedger.active > 0}>{working === "START_RELEASE_TRAIN_BATCH" ? "Opening bounded batch…" : "Run MP-003–MP-010 bounded batch"}</button></div>}
        {data.canary.status === "BATCH_UNIT_PASS_REVIEW" && data.canary.version === "RELEASE_TRAIN_V2_PRODUCTION_SCENE_RENDERER" && <p className="stateBanner">The current production-scene unit passed and reached a hard-stop. The next frozen unit is released under the same bounded authorization; no repair retry is allowed.</p>}
        {data.canary.status === "SEQUENCE_OR_BATCH_FAILED_PRESERVED" && data.canary.version !== "RELEASE_TRAIN_V1_SHOT_CONTRACT_QA" && <p className="stateBanner errorState">The failing unit and exact terminal gates are preserved. No retry or later-unit release occurred.</p>}
