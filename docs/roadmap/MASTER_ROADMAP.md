...
Phase 25 is source-ready for autonomous exact-master Browser QA. Migration `0082` and `CLEAN_AV_AUTONOMOUS_BROWSER_QA_V1` add at most three append-only runs, one active-run fence, same-origin mobile Chromium execution, real-time played-range coverage, required pause/resume/seek/end event order, decoded audio RMS, decoded-frame motion, four hash/read-back cue JPEGs, focus/reflow and zero-error gates. Infrastructure or incomplete evidence records a failed run but leaves Browser QA `PENDING`; only a complete canonical evidence bundle may consume the existing one-receipt ceiling. Source build and the 36-test evaluation suite pass. Next deploy `0082`, run one real cloud-browser attempt against the exact production hash and read back the receipt/evidence link. Owner truth remains a separate non-delegable action; all downstream locks and V2 remain untouched.
