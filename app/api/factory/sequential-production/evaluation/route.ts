  ...
  try {
    ...
    if (["owner-label-task", "owner-label-workflow", "provider-history-recovery", "commercial-clean-audio"].includes(view) || artifactCandidateId || fixtureArtifactId) {
      ...
      if (view === "commercial-clean-audio") {
        const [commercialAudio, ownerGroundTruth] = await Promise.all([commercialCleanAudioSnapshot(env.DB), cleanAudioOwnerGroundTruthSnapshot(env.DB)]);
        return Response.json({ ...commercialAudio, ownerGroundTruth }, { headers: NO_STORE });
      }
      if (view === "provider-history-recovery") {
        ...
        const [materialization, rightsEvidence, commercialAudio, ownerGroundTruth] = await Promise.all([controlledFixtureMaterializationSnapshot(env.DB), currentRightsEvidenceSnapshot(env.DB), commercialCleanAudioSnapshot(env.DB), cleanAudioOwnerGroundTruthSnapshot(env.DB)]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof EvaluationCommandError || error instanceof FixtureMaterializationError || error instanceof RightsEvidenceError || error instanceof CommercialCleanAudioError || error instanceof CleanAudioOwnerGroundTruthError) return Response.json({ error: { code: error.code, message: error.message }, providerRequests: 0, spendUsd: 0 }, { status: error.status, headers: NO_STORE });
