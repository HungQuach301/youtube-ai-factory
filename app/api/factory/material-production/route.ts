...
const WAVE_BATCH_2_V13_REJECTED_ENGINE_VERSION = "SHOT_PRODUCT_ENGINE_V16_EVIDENCE_NATIVE_VISUAL_LANGUAGE_COMPILER";
const WAVE_BATCH_2_V13_REPRODUCTION_VERSION = "WAVE_09_BATCH_2_V10_EXACT_CLAUSE_FRAME_REPRODUCTION";
const WAVE_BATCH_2_V13_ENGINE_VERSION = "SHOT_PRODUCT_ENGINE_V17_EXACT_CLAUSE_FRAME_COMPILER";
...
async function snapshot() {
  ...
  const expectedPreflightAction = v15ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V17_ACTIVATION" : v13ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V13_ACTIVATION" : v12ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V12_ACTIVATION" : v11ControlVisible ? "PREFLIGHT_WAVE_BATCH_2_V11_ACTIVATION" : "PREFLIGHT_WAVE_BATCH_2_V10_ACTIVATION";
  const expectedActivationAction = v15ControlVisible ? "ADOPT_WAVE_BATCH_2_V17_ENGINE_ROOT_CORRECTION" : v13ControlVisible ? "ADOPT_WAVE_BATCH_2_V13_ENGINE_ROOT_CORRECTION" : v12ControlVisible ? "ADOPT_WAVE_BATCH_2_V12_ENGINE_ROOT_CORRECTION" : v11ControlVisible ? "ADOPT_WAVE_BATCH_2_V11_ENGINE_ROOT_CORRECTION" : "ADOPT_WAVE_BATCH_2_V10_ENGINE_ROOT_CORRECTION";
...
function contractNativeSceneSpecification(contract: Row, bindings: Row[], actors: string[], kind: string) {
  ...
  return {
    version: "EXACT_CLAUSE_FRAME_SPECIFICATION_V17", layoutFamily, composition: auditedBlueprint?.composition || `CONTRACT_NATIVE_${layoutFamily}`, persistentLabels: auditedBlueprint?.persistentLabels || [], assumptionPins, canvas: { width: 960, height: 540, safeInset: 48 },
    ...
    rendererContract: { input: "EXACT_CLAUSE_FRAME_ONLY", tokenFallbackForbidden: true, geometryAuthoritative: true, audienceTextBudget: { titleGlyphs: 30, objectGlyphs: 22, actionGlyphs: 34, contractClausePixelsForbidden: true }, provenanceRequiredForEveryVisibleObject: true, provenanceRequiredForEveryRelationship: true },
  };
}
...
function renderWaveSemanticScene(manifest: Row, state: 0 | 1 | 2) {
  ...
  if(contractNativeScene){
    ...
    if(auditedComposition){
      ...
      fill(28,30,904,474,backgrounds[ci]);fill(48,136,864,310,backgrounds[ci]);
      if(composition==="ISSUER_LEDGER_CONTAINMENT"){
        ...
        if(state>=1)nativeArrow(598,260,492,260,true);if(state===2){line(518,252,518,350,10,"#e8b65d");center("STOP",518,292,3,"#fff0df",8);}fill(156,382,648,46,"#7a2e22");center("NOT NETWORK REVENUE",480,396,3,"#fff1df",24);
        ...
        fill(560,202,330,172,"#e9eef8");center("NETWORK TRAY",725,220,3,"#102e55",18);if(state===0)center("EMPTY",725,292,4,"#52766b",8);else{fill(state===1?430:604,276,242,72,"#ffffff");center("NETWORK AND",state===1?551:725,286,2,"#102e55",20);center("SERVICE FEES",state===1?551:725,320,2,"#102e55",18);}
        ...
        fill(54,188,392,190,"#ffffff");fill(54,188,392,44,"#43356b");center("HIGHLIGHTED SCHEDULE ROW",250,200,2,"#f3eaff",30);cols.forEach((label,index)=>{const y=244+index*30;text(label,76,y,2,"#25183a");text(clean(pins[label]),246,y,1,"#43356b");});
        ...
        if(composition==="MERCHANT_TO_PROCESSOR_BOUNDARY"){
          text("INFORMATION PACKET",62,54,4,"#fff0df");center(sceneState.action,480,112,2,accent,40);fill(64,204,278,170,"#fff4d8");center("MERCHANT TERMINAL",203,220,3,"#593823",22);fill(94,272,218,72,"#ffffff");center("SOURCE-BOUND",203,282,2,"#593823",18);center("REQUEST",203,316,2,"#593823",12);
...
function waveProductOracle(manifest: Row) {
  ...
  const checks=[
    ...
    {id:"CONTRACT_NATIVE_SCENE_SPECIFICATION",status:!contractNativeScene||clean(sceneSpec.version)==="EXACT_CLAUSE_FRAME_SPECIFICATION_V17"&&Boolean(clean(sceneSpec.composition))&&sceneObjects.length>=3&&sceneRelationships.length===3&&sceneStates.length===3&&sceneObjects.every((item)=>Boolean(clean(item.id))&&Boolean(clean(item.type))&&Boolean(clean(item.label))&&Boolean(clean(item.sourceClause))&&Object.keys(rec(item.geometry)).length===4)&&sceneRelationships.every((item)=>Boolean(clean(item.from))&&Boolean(clean(item.to))&&Boolean(clean(item.relation))&&Boolean(clean(item.sourceClause)))?"PASS":"FAIL",evidence:`${sceneObjects.length} domain primitives · ${sceneRelationships.length} actions · ${sceneStates.length} states`},
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&Boolean(clean(item.activeRelationshipId))&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="EXACT_CLAUSE_FRAME_ONLY"&&rendererContract.tokenFallbackForbidden===true&&rendererContract.geometryAuthoritative===true?"PASS":"FAIL",evidence:"renderer consumes only contract-native composition, authoritative geometry and action timeline"},
...
function waveManifestQualification(manifest: Row) {
  const states=arr(manifest.states).map(rec),semantic=rec(manifest.semanticModel),required=arr(semantic.requiredElements).map(clean).filter(Boolean),contractNativeScene=clean(semantic.version)==="EXACT_CLAUSE_FRAME_SPECIFICATION_V17",structuredOntology=clean(semantic.version)==="STRUCTURED_VISUAL_ONTOLOGY_V12",semanticProjection=clean(semantic.version)==="CONTRACT_SEMANTIC_PROJECTION_V11"||structuredOntology||contractNativeScene,archetypeBound=clean(semantic.version)==="ARCHETYPE_SEMANTIC_MANIFEST_V10"||semanticProjection,contractBound=clean(semantic.version)==="CONTRACT_BOUND_SCENE_GRAPH_V9"||archetypeBound,bindings=arr(semantic.stateBindings).map(rec),projections=arr(semantic.stateProjection).map(rec),ontology=rec(semantic.visualOntology),ontologyStates=arr(ontology.states).map(rec),layoutProof=rec(ontology.layoutProof),motionPolicy=rec(ontology.motionPolicy),sceneSpec=rec(semantic.sceneSpecification),sceneObjects=arr(sceneSpec.objects).map(rec),sceneRelationships=arr(sceneSpec.relationships).map(rec),sceneStates=arr(sceneSpec.states).map(rec),sceneLayoutProof=rec(sceneSpec.layoutProof),sceneMotionProof=rec(sceneSpec.motionProof),rendererContract=rec(sceneSpec.rendererContract),corpus=clean(semantic.corpusHashInput).toUpperCase(),genericEscape=contractBound&&["$100 PURCHASE","REWARD ENTRY"].some((term)=>required.includes(term)&&!corpus.includes(term)),provenance=arr(semantic.elementProvenance).map(rec),transitions=arr(semantic.transitionModel).map(rec),layout=rec(manifest.layoutPolicy),checks=[
    ...
    {id:"SCENE_SPEC_TO_PIXEL_TRACE",status:!contractNativeScene||sceneStates.every((item)=>arr(item.visibleObjectIds).length>=3&&arr(item.movingObjectPosition).length===2)&&rendererContract.input==="EXACT_CLAUSE_FRAME_ONLY"&&rendererContract.tokenFallbackForbidden===true?"PASS":"FAIL"},
...
async function qualifyWaveBatch2V13Activation(context: Awaited<ReturnType<typeof waveBatch2V13ActivationContext>>, progress: Row = {}) {
  ...
  for (const target of scope.slice(startIndex, endIndex)) {
    ...
    const sceneSpecComplete = clean(spec.version) === "EXACT_CLAUSE_FRAME_SPECIFICATION_V17" && Boolean(clean(spec.composition)) && objects.length >= 3 && relationships.length === 3 && states.length === 3 && objects.every((item) => Boolean(clean(item.type)) && Boolean(clean(item.sourceClause)) && Object.keys(rec(item.geometry)).length === 4) && relationships.every((item) => Boolean(clean(item.sourceClause)));
    const pixelTraceComplete = states.every((item) => arr(item.visibleObjectIds).length >= 3 && Boolean(clean(item.activeRelationshipId)) && arr(item.movingObjectPosition).length === 2) && rendererContract.input === "EXACT_CLAUSE_FRAME_ONLY" && rendererContract.tokenFallbackForbidden === true && rendererContract.geometryAuthoritative === true;
...
async function preflightWaveBatch2V13Activation() {
  ...
  await context.db.prepare("INSERT INTO v7_batch_activation_preflights (id,program_id,run_id,authorization_id,action,source_batch_id,source_audit_id,target_batch_id,input_hash,status,result_json,requests_before,cost_before,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET source_batch_id=excluded.source_batch_id,source_audit_id=excluded.source_audit_id,input_hash=excluded.input_hash,status=excluded.status,result_json=excluded.result_json,requests_before=excluded.requests_before,cost_before=excluded.cost_before,updated_at=excluded.updated_at").bind(preflightId, PROGRAM_ID, context.run.id, context.authorization.id, "PREFLIGHT_WAVE_BATCH_2_V17_ACTIVATION", context.rejected.id, context.audit.id, context.replacementId, context.inputHash, result.status, JSON.stringify(result), context.requestsBefore, context.costBefore, now, now).run();
  const readBack = await context.db.prepare("SELECT * FROM v7_batch_activation_preflights WHERE id=? AND input_hash=?").bind(preflightId, context.inputHash).first<Row>();
...
async function adoptWaveBatch2V13EngineRootCorrection() {
  ...
  const rootCausePolicy = { ...rec(JSON.parse(String(rejected.root_cause_policy_json || "{}"))), incident: "BATCH_2_V16_FRAME_TIMING_LABEL_FIT_AND_SHARED_CHROME_FAILURE", rejectedEngineVersion: WAVE_BATCH_2_V13_REJECTED_ENGINE_VERSION, rejectedAuditId: audit.id, rejectedAuditScore: Number(audit.score), observedRootCause: priorRoot.rootProductionCause, correctedLayers: ["EXACT_CLAUSE_FRAME_COMPILER", "FRAME_TIMING", "FULL_LABEL_FIT", "SCHEDULE_TO_EMPTY_SCENARIO_COMPOSITION", "BOUNDARY_OPENING", "PORTFOLIO_CHROME_REMOVAL"], correction: "EXACT_CLAUSE_TO_EXACT_IMMUTABLE_FRAME_WITHOUT_SHARED_CHROME", replacementEngineVersion: WAVE_BATCH_2_V13_ENGINE_VERSION, fullScopeRegression: "50_SCENE_SPECS_50_PIXEL_TRACES_150_FRAMES_LAYOUT_SAFE", portfolioPolicy, reproduceScope: "ALL_50_PRODUCTS", outputRepair: false, v8V9V10V11V12V13V14V15V16ProductsPreservedAsEvidence: true, retryPriorAudits: false };
  const productionDoD = { ...rec(JSON.parse(String(rejected.production_dod_json || "{}"))), version: "SHOT_PRODUCT_DOD_V17", exactClauseFrameSpecification: true, immutableFrameTimingExact: true, fullLabelFitRequired: true, sharedPortfolioChromeForbidden: true, discreteObjectMotionRequired: true, explicitContainmentAndOwnership: true, exactBoundaryOrder: true, sourceBoundAssumptionPins: true, rendererInputExactClauseFrameOnly: true, semanticToPixelTraceRequired: true, physicalMotionNotLabelSwap: true, clippingConnectorAndObjectOverlapZero: true, portfolioPolicy, fullScopeRegression: "50_OF_50_PASS" };
  const modelPolicy = { ...rec(JSON.parse(String(authorization.model_policy_json || "{}"))), version: WAVE_BATCH_2_V13_REPRODUCTION_VERSION, productionEngine: WAVE_BATCH_2_V13_ENGINE_VERSION, scope: "REPRODUCE_ALL_50_AFTER_V16_ENGINE_REJECTION", deterministicProductionRequests: 0, independentAuditRequests: 1, autoRepair: false, priorAuditRetry: false, qaFailureRoute: "ROOT_PRODUCTION_PROCESS" };
  ...
  try {
    await db.batch([
      ...
      db.prepare("UPDATE v7_material_authorizations SET scope='WAVE_09_BATCH_2_ENGINE_V17_REPRODUCTION',status='PAUSED',shot_count=50,max_remote_requests=?,max_actual_spend_usd=?,model_policy_json=?,completed_at=NULL,updated_at=? WHERE id=?").bind(requestsBefore + 1, costBefore + 10, JSON.stringify(modelPolicy), now, authorization.id),
      db.prepare("UPDATE v7_material_runs SET status='BATCH_2_V17_REPRODUCING',mode='EXACT_CLAUSE_FRAME_REPRODUCTION' WHERE id=?").bind(run.id),
      db.prepare("UPDATE v7_stage_states SET status='BATCH_2_V17_REPRODUCING',blocker='INTEGRATED_PRODUCTION_TRANSACTION',evidence_summary='V8-V16 evidence preserved · V17 exact clause frames, full label fit, source schedule facing empty scenario card, correct boundary timing and no shared portfolio chrome PASS · no output repair · prior audits not retried',updated_at=? WHERE id=?").bind(now, STAGE_ID),
      db.prepare("INSERT INTO v7_batch_activations (id,program_id,run_id,authorization_id,action,idempotency_key,target_batch_id,input_hash,status,result_json,created_at,committed_at,updated_at) VALUES (?,?,?,?,?,?,?,?, 'COMMITTED',?,?,?,?)").bind(activationId, PROGRAM_ID, run.id, authorization.id, "ADOPT_WAVE_BATCH_2_V17_ENGINE_ROOT_CORRECTION", activationId, replacementId, context.inputHash, JSON.stringify(activationResult), now, now, now),
    ]);
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "PREFLIGHT_WAVE_BATCH_2_V17_ACTIVATION") return Response.json(await preflightWaveBatch2V13Activation(), { status: 200 });
    if (body.action === "ADOPT_WAVE_BATCH_2_V17_ENGINE_ROOT_CORRECTION") return Response.json(await adoptWaveBatch2V13EngineRootCorrection(), { status: 201 });
